"""Data Layer — flow orchestrator.

Pipeline:
  data_collection → data_enrichment → behavioral_data → event_tracking → user_tracking

- data_collection is REQUIRED; all others are optional (continue on failure).
- Each engine enriches the accumulated data dict.
"""
...
from .config import DATA_LAYER_CONFIG

logger = logging.getLogger(__name__)


class DataLayerFlow:
    """Data Layer — orchestrates data-collection engines sequentially."""

    LAYER_NAME = "data_layer"

    def run(self, input_payload: dict[str, Any]) -> dict[str, Any]:
        """Run the full data-layer pipeline.
        ...
        required = set(DATA_LAYER_CONFIG["required_engines"])
        engine_names = DATA_LAYER_CONFIG["engines"]
