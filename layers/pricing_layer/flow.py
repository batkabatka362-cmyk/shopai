"""Pricing Layer — flow orchestrator.

Pipeline:
  pricing → price_elasticity → dynamic_pricing → discount_strategy →
  profitability_calculator → profit_optimization → payment_optimization

- Fully sequential pipeline.
- pricing and profitability_calculator are required; all others optional.
"""
...
from .config import PRICING_LAYER_CONFIG

logger = logging.getLogger(__name__)


class PricingLayerFlow:
    """Pricing Layer — orchestrates pricing-pipeline engines."""

    LAYER_NAME = "pricing_layer"

    def run(self, input_payload: dict[str, Any]) -> dict[str, Any]:
        """Run the full pricing-layer pipeline.
        ...
        required = set(PRICING_LAYER_CONFIG["required_engines"])
        engine_names = PRICING_LAYER_CONFIG["engines"]
        ...
        for name in engine_names:
            ...
            try:
                result = engine.run(enriched_input)
            except Exception as exc:
