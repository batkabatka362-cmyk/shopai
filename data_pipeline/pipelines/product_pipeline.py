"""Product Pipeline — Raw -> Clean -> Normalize -> Feature for product data."""
...
logger = get_logger("pipeline.product")


class ProductPipeline:
    """End-to-end data pipeline for product data."""

    def __init__(self) -> None:
        self._cleaner = DataCleaner()
        self._normalizer = DataNormalizer()
        self._validator = DataValidator(required_fields=["name", "price"])
        self._feature_builder = FeatureBuilder()

    def run(self, raw_data: list[dict[str, Any]]) -> dict[str, Any]:
        logger.info("Running product pipeline (%d records)", len(raw_data))
        ...
        logger.info("Pipeline done: %d valid, %d invalid", len(valid), len(invalid))
        return {"features": features, "valid_count": len(valid), "invalid_count": len(invalid), "invalid": invalid}
