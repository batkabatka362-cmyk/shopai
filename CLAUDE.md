...
| Store builder Phase 2a — Brand + SEO metafields | (this commit) | `_setup_brand` writes two `shopai.brand` + `shopai.seo_defaults` metafields via REST. Niche-aware voice / tagline / colours / typography / schema-org-type. Canonical source every AI engine reads so brand stays consistent across content / email / ad copy. 10 new tests. |
