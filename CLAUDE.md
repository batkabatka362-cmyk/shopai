...
| Deliberation → world-model calibration wire | (this commit) | Order webhook back-fill now also emits an EngineOutcome with `predicted=deliberation.outcome.value_usd` + `actual=revenue` so `world_model_calibration.record_outcome` fires. Without this wire `predict_outcome` MCP returned uncalibrated estimates forever. Value-0 predictions skip the feed (don't bias the calibrator to zero). Soft-fail isolated. 5 new tests. |
