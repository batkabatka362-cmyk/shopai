...
| Knowledge ingest CLI + MCP | (this commit) | Audit batch 3 final — `core/memory/knowledge_ingest.py` (orphan) gets owner-facing handles: `shopai ingest-knowledge --url|--file [--subject S]` CLI + `ingest_knowledge` MCP tool (write-gated). Owner pushes brand docs / changelogs / supplier FAQs into the KB manually. 6 tests. |
