...
| Store builder Phase 2c — Blog welcome article | (this commit) | `_setup_blog` seeds Shopify's default `news` blog with one niche-aware welcome article so a fresh store's blog never shows "no posts yet". Idempotent by article handle (`welcome-to-{store-slug}`). Skip-on-missing-blog (no error). REST. 9 tests. |
