...
| Windows desktop B — docker-compose launcher + cloudflared tunnel | (this commit) | Owner chose B (Docker) first → A (PyInstaller) next. `docker-compose.yml` gained `cloudflared` service (public https tunnel for webhooks, free tier, random trycloudflare.com subdomain on start) + `ollama-gpu` profile (opt-in `--profile gpu` for NVIDIA; default runs CPU-only on any Windows laptop). `start.bat` Windows double-click launcher auto-checks Docker + .env, brings 4 containers up, parses tunnel URL from cloudflared logs, prints the exact `SHOPAI_WEBHOOK_CALLBACK_URL=...` line for `.env`. `start.sh` Linux/Mac equivalent. `docs/WINDOWS_DESKTOP.md` — 20-30 min setup guide with troubleshooting. No production code touched — infra only. |
| ready-for-live feedback_store pollution fix | `67763fb` | Weak-parts scan finding — `shopai ready-for-live` was writing 5 synth entries per run into the production `feedback_store` (via the learning-loop check's `replay_orders(5 synth)` → handler 4b/4c bus emits). Fixed: gate now tags each synth order with `shopai_gate_check=1` note attribute; `order_handler` sees marker → skips bus emits while still exercising the full import chain. Handler exposes `gate_check_skipped_bus: True` so the gate can verify the marker fired. Gate verdict based on stats.successful + marker honouring instead of feedback_store counts. New test locks pollution-free behaviour. |
| OpenClaw integration scaffolding (Phase 0) | `809531a` | `scripts/setup_openclaw.sh` — one-command OpenClaw sibling clone + npm install + Ollama check; `docs/OPENCLAW_INTEGRATION.md` — 8-section plan covering architecture (OpenClaw → ShopAI MCP → brain), phased ship plan (Phase 0 pilot → Phase 1 subprocess adapter → Phase 2 MCP bridge → Phase 3 Docker prod → Phase 4 optional cycle auto-invocation), LLM backend comparison (Ollama local / Claude / DeepSeek), §4b.G safety boundaries, §4c.K self-check. No production code touched — pure scaffolding so owner can pilot OpenClaw sibling-to-ShopAI without repo pollution. |
| SSRF defence + bearer auth for POST endpoints | `594fde0` | **HIGH severity security fix** (security-review Vuln 1, confidence 9). `core/system/url_safety.is_safe_public_url()` rejects https-only caller-supplied URLs whose host resolves to private/loopback/link-local/reserved IP ranges. `core/adapters/image/vision._download` calls it before every fetch — closes the SSRF path from `POST /api/launch → MediaStep → vision.analyze` into cloud metadata / internal services. `api/server.do_POST` now gates every state-mutating write path behind `APIAuth.validate` (bearer token); `/api/webhook/shopify` stays HMAC-only. Master token defaults to `SHOPAI_API_TOKEN` env, auto-generated fallback stored in singleton. 24 URL-safety tests + 7 auth-gate tests. |
| T1 onboarding UX (`.env.example` + ready-for-live) | `b57f06c` | `.env.example` rewritten — was 54 lines covering 8 vars, now ~140 covering 25+ vars with 3 sections (T1 PREREQUISITES / AI MODEL CHAIN / OPTIONAL). Each var has a "where to get it" hint. `core/readiness/go_live_gate._check_env` now prints exact `export VAR=template_value` lines on BLOCKED so owner can copy-paste. CLI `_cmd_ready_for_live` reformats multi-line fix strings cleanly. |
| SourceStep supplier_sku wire to CJ adapter | `8f43b75` | `workflows/launch/steps/source.py` supplier_sku branch TODO stub replaced — calls `CJDropshippingAdapter.execute(Capability.SOURCING_GET_PRODUCT, {sourcing_id})` and normalises the response into the source-agnostic shape downstream steps expect (title / price_usd / supplier_url / gallery_urls / attributes / sourcing_id / _source_kind). Closes the 3rd + final workflow-step TODO. Missing title / adapter failure / unconfigured adapter all StepSkip with explicit reason. 7 new tests. |
| SupplierStep wire to CJ adapter | `eba6410` | `workflows/launch/steps/supplier.py` TODO stub replaced — validates `sourcing_id` via `SOURCING_GET_PRODUCT` capability + records supplier metadata (title + cost_usd) on `context.supplier`. CJ's business model has no "register SKU" step — orders are placed per-order — so launch-time validation is the useful thing. AutoDS-only env skips cleanly (roadmap). Adapter failure records + continues (not raise). 7 tests. |
| AdsLaunchStep wire to MetaAdsAdapter | `d28ddeb` | `workflows/launch/steps/ads_launch.py` TODO stub replaced with real adapter call — POST to `{account}/campaigns` via `MetaAdsAdapter.execute(Capability.ADS_CREATE_CAMPAIGN, ...)`. PAUSED status (§4b.G), daily_budget converted to cents (Meta currency minor unit), context.ad_campaign populated with id + name + kill_rule for downstream monitor. Adapter failure bubbles as RuntimeError (not silent StepSkip). 10 tests. |
| Owner-dialog intents for this session's MCP tools | `a5ec51c` | `agents/owner_dialog/tool_dispatcher.py` — 5 new read-only intents: `ready_for_live` ("ready for live?", "go-live gate"), `live_health` ("how healthy", "health trend"), `rule_quality` ("true positive rate", "lift"), `engine_feedback_stats` ("engine stats"), `brain_module_catalog` ("module catalog"). Owner types the phrase from Telegram + gets the MCP tool's output. 9 new tests. |
| Deliberation → world-model calibration wire | `029b6c8` | Order webhook back-fill now also emits an EngineOutcome with `predicted=deliberation.outcome.value_usd` + `actual=revenue` so `world_model_calibration.record_outcome` fires. Without this wire `predict_outcome` MCP returned uncalibrated estimates forever. Value-0 predictions skip the feed (don't bias the calibrator to zero). Soft-fail isolated. 5 new tests. |
| Shipping feasibility dedup | `1e02599` | Audit batch E finding — `_billable_weight` defined identically in `code.py:19` + `logic.py:15`. Extracted to `shipping_feasibility/utils.py:billable_weight`; both call sites now import the canonical form. 10 tests including AST-level guards that the local definitions never return. |
| Brain catalog expansion | `b599620` | `core/brain/module_catalog.py` declarations grew from 8 → 33 (6 active + 27 experimental). Added `compute_budget`, `decision_brain`, `decision_rationale_builder` to active. Added 22 experimental: active_query_planner, adaptive_prompts, answer_synthesizer, auto_tagger, belief_state, budget_allocator, bundle_recommender, causal_discovery, checkpoint, churn_predictor, cohort_analyzer, clarification_dialog, counterfactual, counterfactual_simulator, creative_orchestrator, debate, dream_replay, drift_detector, dual_process, economic_simulator, exploration_manager, forecaster — each with a wire-up trigger codified in reason. 3 new scale-floor tests. |
| RCA auto-wire on publisher failure | `fe6c59d` | `PublisherBundle._finalise` now runs `RootCauseAnalyzer.analyze_failure` on every non-dry-run ok=False launch. Lesson string (truncated 240) stashed on `LaunchResult.note` so owner sees it via `explain_decision` + `recent_decisions` MCP. Soft-fail: RCA exceptions never block the finalise path. Happy-path + dry-run failures don't invoke RCA. 6 new tests. |
| ready-for-live gate | `c0da1a8` | `core/readiness/go_live_gate.py` — single-verb pre-T1 verdict. Runs env + doctor + learning-loop (5-order synthetic replay) + live-health + live-execution state checks in deterministic order. Returns READY or BLOCKED with per-check fix list. `shopai ready-for-live` CLI + `ready_for_live` MCP tool. 19 tests. |
| Store builder Phase 2f — Metafield definitions | `e1b3e30` | `_setup_metafield_definitions` creates 5 typed product metafield schemas via GraphQL `metafieldDefinitionCreate` (warranty / materials / care_instructions / country_of_origin / video_url on PRODUCT ownerType). Feeds Schema.org JSON-LD + Storefront API. Shopify's `TAKEN` userError bucketed as `existing` so re-running is clean no-op. 7 new tests. |
| Store builder Phase 2e — Script tags | `db5d646` | `_setup_script_tags` auto-installs tracking pixels (Meta / TikTok / GA) via Shopify's `script_tags.json` REST. Env-gated (`SHOPAI_META_PIXEL_SRC` / `SHOPAI_TIKTOK_PIXEL_SRC` / `SHOPAI_GA_SRC`). Non-https sources rejected. Idempotent by src. 6 new tests. |
| P0-A T1 Go-Live checklist | `3137c84` | `docs/T1_GO_LIVE.md` — 11-step checklist + §4d cross-check: prereqs (dev store + Meta sandbox + tunnel URL) → .env → tests green → doctor → store configure dry-run → live configure → webhooks → replay validation → daemon start → monitoring → kill switch. Concrete command-by-command path from T0 to T1. |
| Store builder Phase 2d — Webhook subscriptions | `925fcc3` | `_setup_webhooks` subscribes ShopAI's endpoint to the 5 reward-signal topics (orders/create, orders/paid, orders/cancelled, products/update, app/uninstalled) via existing `core.webhooks.register.register_all`. Env-gated by `SHOPAI_WEBHOOK_CALLBACK_URL` (must be https). Without webhooks the OrderWebhookHandler never fires — this is the T1 prereq that turns a configured store into a learning store. 7 new tests. |
| Store builder Phase 2c — Blog welcome article | `82597a2` | `_setup_blog` seeds Shopify's default `news` blog with one niche-aware welcome article so a fresh store's blog never shows "no posts yet". Idempotent by article handle (`welcome-to-{store-slug}`). Skip-on-missing-blog (no error). REST. 9 tests. |
| Store builder Phase 2b — 301 redirects | `1cc5d87` | `_setup_redirects` creates 7 common marketing vanity redirects (/shop, /store, /catalog, /products → /collections/all; /blog → /blogs/news; /contact → /pages/contact; /about → /pages/about) via REST `redirects.json`. Idempotent — existing paths skipped. Prevents 404s on dropshipping ads that point at /shop or /store. 7 tests. |
| Store builder Phase 2a — Brand + SEO metafields | `dab81d3` | `_setup_brand` writes two `shopai.brand` + `shopai.seo_defaults` metafields via REST. Niche-aware voice / tagline / colours / typography / schema-org-type. Canonical source every AI engine reads so brand stays consistent across content / email / ad copy. 10 new tests. |
