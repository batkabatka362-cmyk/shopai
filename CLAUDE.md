...
| Brain module catalog | (this commit) | Audit batch 4 — `core/brain/module_catalog.py` honest-label registry (active / experimental / archived / unknown). Bootstrap declarations for 5 experimental + 3 active modules. `brain_module_catalog` MCP tool. Owner now sees which brain capabilities are live vs parked without trawling the 225-file tree. 11 tests. |
