...
| Owner-dialog intents for this session's MCP tools | (this commit) | `agents/owner_dialog/tool_dispatcher.py` — 5 new read-only intents: `ready_for_live` ("ready for live?", "go-live gate"), `live_health` ("how healthy", "health trend"), `rule_quality` ("true positive rate", "lift"), `engine_feedback_stats` ("engine stats"), `brain_module_catalog` ("module catalog"). Owner types the phrase from Telegram + gets the MCP tool's output. 9 new tests. |
