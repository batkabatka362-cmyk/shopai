...
| Store builder Phase 2e — Script tags | (this commit) | `_setup_script_tags` auto-installs tracking pixels (Meta / TikTok / GA) via Shopify's `script_tags.json` REST. Env-gated (`SHOPAI_META_PIXEL_SRC` / `SHOPAI_TIKTOK_PIXEL_SRC` / `SHOPAI_GA_SRC`). Non-https sources rejected. Idempotent by src. 6 new tests. |
