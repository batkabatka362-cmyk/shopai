...
| Store builder Phase 2b — 301 redirects | (this commit) | `_setup_redirects` creates 7 common marketing vanity redirects (/shop, /store, /catalog, /products → /collections/all; /blog → /blogs/news; /contact → /pages/contact; /about → /pages/about) via REST `redirects.json`. Idempotent — existing paths skipped. Prevents 404s on dropshipping ads that point at /shop or /store. 7 tests. |
