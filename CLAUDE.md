...
| Replay synthesize | (this commit) | `agents/replay/synthesize.py` generates deterministic Shopify-shaped order payloads; `shopai replay-orders --synthesize N [--seed S] [--attach-decision-rate R]` unblocks T1 validation without needing a live store export. 17 new tests lock Shopify shape + determinism. |
