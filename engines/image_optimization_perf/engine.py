...
class ImageOptimizationPerfEngine(BaseEngine):
    engine_name = "image_optimization_perf"
    required_input_fields = ['image_data', 'optimization_config']
    required_output_fields = ['optimized_images', 'format_selection']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} image optimization perf", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} image optimization perf: {data}"
