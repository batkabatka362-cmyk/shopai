...
class DataPipelineMonitorEngine(BaseEngine):
    engine_name = "data_pipeline_monitor"
    required_input_fields = ['pipeline_data', 'monitor_config']
    required_output_fields = ['pipeline_health', 'data_quality']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} data pipeline monitor", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} data pipeline monitor: {data}"
