...
class Order360Engine(BaseEngine):
    engine_name = "order_360"
    required_input_fields = ['order_data', 'integration_config']
    required_output_fields = ['unified_order', 'lifecycle_status']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} order 360", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} order 360: {data}"
