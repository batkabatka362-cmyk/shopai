...
class CollectionTrackerEngine(BaseEngine):
    engine_name = "collection_tracker"
    required_input_fields = ['collection_data', 'tracker_config']
    required_output_fields = ['collection_progress', 'completion_rewards']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} collection tracker", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} collection tracker: {data}"
