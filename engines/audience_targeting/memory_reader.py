"""Audience Targeting Engine — memory reader.

Reads past audience targeting records from memory storage.
Used to inform current targeting with historical context.
"""
...
def read_past_audiences(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past audience targeting records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_audiences(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past audience targeting records."""
    if not records:
        return {
            "avg_audience_count": 0.0,
            "avg_total_reachable": 0.0,
            "total_runs": 0,
        }

    audience_counts = [len(r.get("audiences", [])) for r in records]
    reachable_totals = [r.get("total_reachable", 0) for r in records]

    n = len(records)
    return {
        "avg_audience_count": round(sum(audience_counts) / n, 1) if n else 0.0,
        "avg_total_reachable": round(sum(reachable_totals) / n, 1) if n else 0.0,
        "total_runs": n,
    }
