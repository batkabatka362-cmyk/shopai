...
class PluginMarketplaceEngine(BaseEngine):
    engine_name = "plugin_marketplace"
    required_input_fields = ['plugin_data', 'marketplace_config']
    required_output_fields = ['marketplace_setup', 'review_process']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} plugin marketplace", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} plugin marketplace: {data}"
