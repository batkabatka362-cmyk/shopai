...
class CateringManagementEngine(BaseEngine):
    engine_name = "catering_management"
    required_input_fields = ['event_data', 'catering_config']
    required_output_fields = ['catering_plan', 'dietary_accommodations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} catering management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} catering management: {data}"
