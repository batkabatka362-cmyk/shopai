...
class NftCommerceEngine(BaseEngine):
    engine_name = "nft_commerce"
    required_input_fields = ['digital_asset', 'nft_config']
    required_output_fields = ['nft_listing', 'smart_contract_spec']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} nft commerce", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} nft commerce: {data}"
