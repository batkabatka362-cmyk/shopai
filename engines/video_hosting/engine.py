...
class VideoHostingEngine(BaseEngine):
    engine_name = "video_hosting"
    required_input_fields = ['video_data', 'hosting_config']
    required_output_fields = ['hosting_plan', 'encoding_specs']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} video hosting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} video hosting: {data}"
