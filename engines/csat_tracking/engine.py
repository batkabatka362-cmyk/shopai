...
class CsatTrackingEngine(BaseEngine):
    engine_name = "csat_tracking"
    required_input_fields = ['survey_data', 'csat_config']
    required_output_fields = ['csat_scores', 'driver_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} csat tracking", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} csat tracking: {data}"
