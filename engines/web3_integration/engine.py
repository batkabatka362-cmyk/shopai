...
class Web3IntegrationEngine(BaseEngine):
    engine_name = "web3_integration"
    required_input_fields = ['web3_data', 'integration_config']
    required_output_fields = ['web3_setup', 'wallet_connection']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} web3 integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} web3 integration: {data}"
