...
class ExciseTaxEngine(BaseEngine):
    engine_name = "excise_tax"
    required_input_fields = ['product_data', 'excise_config']
    required_output_fields = ['excise_calc', 'filing_requirements']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} excise tax", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} excise tax: {data}"
