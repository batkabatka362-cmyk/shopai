...
class ProfitPerCustomerEngine(BaseEngine):
    engine_name = "profit_per_customer"
    required_input_fields = ['customer_data', 'profit_config']
    required_output_fields = ['profit_breakdown', 'top_customers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} profit per customer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} profit per customer: {data}"
