...
class LevelSystemEngine(BaseEngine):
    engine_name = "level_system"
    required_input_fields = ['user_data', 'level_config']
    required_output_fields = ['level_setup', 'xp_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} level system", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} level system: {data}"
