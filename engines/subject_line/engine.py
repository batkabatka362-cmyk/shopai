"""
SubjectLine Engine — Subject Line
...
class SubjectLineEngine(BaseEngine):
    engine_name = "subject_line"
    required_input_fields = ['campaign_data', 'audience_data']
    required_output_fields = ['subject_lines', 'predicted_open_rates']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} subject line", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} subject line: {data}"
