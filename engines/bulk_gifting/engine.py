...
class BulkGiftingEngine(BaseEngine):
    engine_name = "bulk_gifting"
    required_input_fields = ['recipient_list', 'gift_config']
    required_output_fields = ['bulk_order', 'personalization_map']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} bulk gifting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} bulk gifting: {data}"
