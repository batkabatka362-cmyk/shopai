...
class CustomerPersonaEngine(BaseEngine):
    engine_name = "customer_persona"
    required_input_fields = ['customer_data', 'research_data']
    required_output_fields = ['persona_profiles', 'behavior_patterns']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} customer persona", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} customer persona: {data}"
