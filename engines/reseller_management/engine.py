...
class ResellerManagementEngine(BaseEngine):
    engine_name = "reseller_management"
    required_input_fields = ['reseller_data', 'program_config']
    required_output_fields = ['reseller_tiers', 'performance_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} reseller management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} reseller management: {data}"
