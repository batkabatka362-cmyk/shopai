...
class CompressionAnalysisEngine(BaseEngine):
    engine_name = "compression_analysis"
    required_input_fields = ['response_data', 'analysis_config']
    required_output_fields = ['compression_report', 'savings_estimate']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} compression analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} compression analysis: {data}"
