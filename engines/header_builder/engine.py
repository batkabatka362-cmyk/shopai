...
class HeaderBuilderEngine(BaseEngine):
    engine_name = "header_builder"
    required_input_fields = ['brand_data', 'header_config']
    required_output_fields = ['header_layout', 'sticky_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} header builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} header builder: {data}"
