...
class ImpairmentTestEngine(BaseEngine):
    engine_name = "impairment_test"
    required_input_fields = ['asset_data', 'test_config']
    required_output_fields = ['impairment_assessment', 'write_down_amount']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} impairment test", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} impairment test: {data}"
