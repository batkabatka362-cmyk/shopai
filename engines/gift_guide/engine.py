...
class GiftGuideEngine(BaseEngine):
    engine_name = "gift_guide"
    required_input_fields = ['occasion_data', 'product_catalog']
    required_output_fields = ['guide_content', 'category_mapping']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} gift guide", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} gift guide: {data}"
