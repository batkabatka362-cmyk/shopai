...
class StoreComparisonEngine(BaseEngine):
    engine_name = "store_comparison"
    required_input_fields = ['store_data', 'comparison_config']
    required_output_fields = ['comparison_report', 'best_practices']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} store comparison", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} store comparison: {data}"
