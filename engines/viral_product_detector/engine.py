...
class ViralProductDetectorEngine(BaseEngine):
    engine_name = "viral_product_detector"
    required_input_fields = ['social_data', 'sales_data']
    required_output_fields = ['viral_candidates', 'amplification_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} viral product detector", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} viral product detector: {data}"
