...
class NonCompeteEngine(BaseEngine):
    engine_name = "non_compete"
    required_input_fields = ['employee_data', 'non_compete_config']
    required_output_fields = ['agreement_draft', 'enforceability_assessment']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} non compete", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} non compete: {data}"
