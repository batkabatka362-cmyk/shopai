...
class PestleAnalysisEngine(BaseEngine):
    engine_name = "pestle_analysis"
    required_input_fields = ['market_data', 'analysis_config']
    required_output_fields = ['pestle_factors', 'impact_assessment']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pestle analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pestle analysis: {data}"
