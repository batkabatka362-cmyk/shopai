...
class CommercialInvoiceEngine(BaseEngine):
    engine_name = "commercial_invoice"
    required_input_fields = ['shipment_data', 'invoice_config']
    required_output_fields = ['invoice_content', 'trade_terms']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} commercial invoice", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} commercial invoice: {data}"
