...
class LtlFreightEngine(BaseEngine):
    engine_name = "ltl_freight"
    required_input_fields = ['shipment_data', 'freight_config']
    required_output_fields = ['freight_quotes', 'carrier_options']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ltl freight", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ltl freight: {data}"
