...
class ReviewClusteringEngine(BaseEngine):
    engine_name = "review_clustering"
    required_input_fields = ['review_data', 'clustering_config']
    required_output_fields = ['review_clusters', 'theme_labels']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} review clustering", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} review clustering: {data}"
