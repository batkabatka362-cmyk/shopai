...
class RecordLinkageEngine(BaseEngine):
    engine_name = "record_linkage"
    required_input_fields = ['dataset_a', 'dataset_b']
    required_output_fields = ['linked_records', 'match_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} record linkage", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} record linkage: {data}"
