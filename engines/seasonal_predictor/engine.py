...
class SeasonalPredictorEngine(BaseEngine):
    engine_name = "seasonal_predictor"
    required_input_fields = ['historical_data', 'calendar_data']
    required_output_fields = ['seasonal_forecast', 'preparation_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} seasonal predictor", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} seasonal predictor: {data}"
