...
class LogisticsOptimizerEngine(BaseEngine):
    engine_name = "logistics_optimizer"
    required_input_fields = ['logistics_data', 'constraint_config']
    required_output_fields = ['optimized_routes', 'cost_savings']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} logistics optimizer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} logistics optimizer: {data}"
