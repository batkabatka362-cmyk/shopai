...
class NextBestTimeEngine(BaseEngine):
    engine_name = "next_best_time"
    required_input_fields = ['customer_data', 'timing_config']
    required_output_fields = ['optimal_times', 'engagement_prediction']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} next best time", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} next best time: {data}"
