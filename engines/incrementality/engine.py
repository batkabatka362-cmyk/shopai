...
class IncrementalityEngine(BaseEngine):
    engine_name = "incrementality"
    required_input_fields = ['experiment_data', 'test_config']
    required_output_fields = ['incrementality_lift', 'channel_contribution']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} incrementality", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} incrementality: {data}"
