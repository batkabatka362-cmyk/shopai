...
class SoftwareSchemaEngine(BaseEngine):
    engine_name = "software_schema"
    required_input_fields = ['software_data', 'schema_config']
    required_output_fields = ['software_markup', 'feature_list']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} software schema", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} software schema: {data}"
