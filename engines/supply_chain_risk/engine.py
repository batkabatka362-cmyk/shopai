...
class SupplyChainRiskEngine(BaseEngine):
    engine_name = "supply_chain_risk"
    required_input_fields = ['supply_data', 'risk_config']
    required_output_fields = ['risk_map', 'contingency_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} supply chain risk", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} supply chain risk: {data}"
