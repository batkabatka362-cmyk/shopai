...
class QueueOptimizationEngine(BaseEngine):
    engine_name = "queue_optimization"
    required_input_fields = ['queue_data', 'optimization_config']
    required_output_fields = ['queue_config', 'wait_time_reduction']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} queue optimization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} queue optimization: {data}"
