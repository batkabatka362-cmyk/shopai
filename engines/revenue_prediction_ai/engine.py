...
class RevenuePredictionAiEngine(BaseEngine):
    engine_name = "revenue_prediction_ai"
    required_input_fields = ['revenue_data', 'growth_signals']
    required_output_fields = ['revenue_forecast', 'risk_factors']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} revenue prediction ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} revenue prediction ai: {data}"
