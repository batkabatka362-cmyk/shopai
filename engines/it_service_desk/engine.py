...
class ItServiceDeskEngine(BaseEngine):
    engine_name = "it_service_desk"
    required_input_fields = ['ticket_data', 'service_config']
    required_output_fields = ['ticket_routing', 'resolution_tracking']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} it service desk", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} it service desk: {data}"
