...
class TaskAssignmentEngine(BaseEngine):
    engine_name = "task_assignment"
    required_input_fields = ['task_data', 'assignment_config']
    required_output_fields = ['assignments', 'workload_balance']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} task assignment", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} task assignment: {data}"
