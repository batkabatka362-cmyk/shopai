...
class InterestCalcEngine(BaseEngine):
    engine_name = "interest_calc"
    required_input_fields = ['principal_data', 'interest_config']
    required_output_fields = ['interest_amount', 'accrual_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} interest calc", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} interest calc: {data}"
