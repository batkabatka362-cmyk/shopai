...
class AiAdWriterEngine(BaseEngine):
    engine_name = "ai_ad_writer"
    required_input_fields = ['product_data', 'campaign_goals']
    required_output_fields = ['ad_copies', 'cta_variants']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ai ad writer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ai ad writer: {data}"
