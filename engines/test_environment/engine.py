...
class TestEnvironmentEngine(BaseEngine):
    engine_name = "test_environment"
    required_input_fields = ['env_config', 'infrastructure_config']
    required_output_fields = ['environment_setup', 'teardown_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} test environment", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} test environment: {data}"
