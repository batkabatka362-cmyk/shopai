...
class PageBuilderEngine(BaseEngine):
    engine_name = "page_builder"
    required_input_fields = ['content_data', 'page_config']
    required_output_fields = ['page_layout', 'component_tree']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} page builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} page builder: {data}"
