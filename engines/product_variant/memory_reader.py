"""Product Variant Engine — memory reader.

Reads past variant generation records from memory storage.
Used to inform current variant generation with historical context.
"""
...
def read_variant_history(
    product_id: str | None = None,
    limit: int = 10,
) -> dict[str, Any]:
    """Read past variant generation records.

    Args:
        product_id: Optional filter — only return records for this product.
...
def compute_input_hash(input_data: dict[str, Any]) -> str:
    ...
    Used to detect duplicate / similar variant requests.
    """
    try:
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_variant_history(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over a list of past variant records."""
    if not records:
        return {
            "avg_variant_count": 0.0,
            "avg_excluded_count": 0.0,
            "total_runs": 0,
        }

    variant_counts = [r.get("total_variants", 0) for r in records]
    excluded_counts = [r.get("excluded_count", 0) for r in records]

    n = len(records)
    return {
        "avg_variant_count": round(sum(variant_counts) / n, 1) if n else 0.0,
        "avg_excluded_count": round(sum(excluded_counts) / n, 1) if n else 0.0,
        "total_runs": n,
    }
