...
class LimitedEditionEngine(BaseEngine):
    engine_name = "limited_edition"
    required_input_fields = ['product_data', 'edition_config']
    required_output_fields = ['edition_plan', 'scarcity_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} limited edition", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} limited edition: {data}"
