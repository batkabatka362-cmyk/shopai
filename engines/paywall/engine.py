...
class PaywallEngine(BaseEngine):
    engine_name = "paywall"
    required_input_fields = ['content_data', 'paywall_config']
    required_output_fields = ['paywall_setup', 'access_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} paywall", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} paywall: {data}"
