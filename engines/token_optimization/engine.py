...
class TokenOptimizationEngine(BaseEngine):
    engine_name = "token_optimization"
    required_input_fields = ['prompt_data', 'optimization_config']
    required_output_fields = ['optimized_prompt', 'token_savings']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} token optimization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} token optimization: {data}"
