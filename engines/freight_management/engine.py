...
class FreightManagementEngine(BaseEngine):
    engine_name = "freight_management"
    required_input_fields = ['shipment_data', 'freight_config']
    required_output_fields = ['freight_plan', 'carrier_selection']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} freight management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} freight management: {data}"
