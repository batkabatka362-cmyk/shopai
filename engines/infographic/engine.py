...
class InfographicEngine(BaseEngine):
    engine_name = "infographic"
    required_input_fields = ['data_points', 'design_config']
    required_output_fields = ['infographic_layout', 'data_visualizations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} infographic", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} infographic: {data}"
