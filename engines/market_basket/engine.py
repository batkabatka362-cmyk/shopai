...
class MarketBasketEngine(BaseEngine):
    engine_name = "market_basket"
    required_input_fields = ['transaction_data', 'basket_config']
    required_output_fields = ['association_rules', 'lift_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} market basket", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} market basket: {data}"
