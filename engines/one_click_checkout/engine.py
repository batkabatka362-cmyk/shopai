...
class OneClickCheckoutEngine(BaseEngine):
    engine_name = "one_click_checkout"
    required_input_fields = ['customer_data', 'checkout_config']
    required_output_fields = ['checkout_setup', 'token_management']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} one click checkout", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} one click checkout: {data}"
