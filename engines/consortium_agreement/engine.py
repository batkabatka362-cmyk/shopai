...
class ConsortiumAgreementEngine(BaseEngine):
    engine_name = "consortium_agreement"
    required_input_fields = ['consortium_data', 'agreement_config']
    required_output_fields = ['consortium_terms', 'work_allocation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} consortium agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} consortium agreement: {data}"
