...
class SitemapPageEngine(BaseEngine):
    engine_name = "sitemap_page"
    required_input_fields = ['site_data', 'page_config']
    required_output_fields = ['sitemap_layout', 'link_hierarchy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sitemap page", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sitemap page: {data}"
