...
class ClimateRiskEngine(BaseEngine):
    engine_name = "climate_risk"
    required_input_fields = ['climate_data', 'risk_config']
    required_output_fields = ['risk_assessment', 'adaptation_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} climate risk", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} climate risk: {data}"
