...
class SubjectLineTestingEngine(BaseEngine):
    engine_name = "subject_line_testing"
    required_input_fields = ['subject_variants', 'test_config']
    required_output_fields = ['test_results', 'winner_selection']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} subject line testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} subject line testing: {data}"
