...
class EmailPersonalizationEngine(BaseEngine):
    engine_name = "email_personalization"
    required_input_fields = ['subscriber_data', 'personalization_config']
    required_output_fields = ['personalized_blocks', 'dynamic_content']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} email personalization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} email personalization: {data}"
