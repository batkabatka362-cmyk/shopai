...
class VirtualShowroomEngine(BaseEngine):
    engine_name = "virtual_showroom"
    required_input_fields = ['product_data', 'showroom_config']
    required_output_fields = ['showroom_layout', 'navigation_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} virtual showroom", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} virtual showroom: {data}"
