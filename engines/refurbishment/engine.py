...
class RefurbishmentEngine(BaseEngine):
    engine_name = "refurbishment"
    required_input_fields = ['product_data', 'refurb_config']
    required_output_fields = ['refurb_plan', 'quality_grades']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} refurbishment", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} refurbishment: {data}"
