...
class BcgMatrixEngine(BaseEngine):
    engine_name = "bcg_matrix"
    required_input_fields = ['product_portfolio', 'market_data']
    required_output_fields = ['bcg_classification', 'investment_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} bcg matrix", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} bcg matrix: {data}"
