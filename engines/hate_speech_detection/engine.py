...
class HateSpeechDetectionEngine(BaseEngine):
    engine_name = "hate_speech_detection"
    required_input_fields = ['text_data', 'detection_config']
    required_output_fields = ['hate_scores', 'category_labels']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} hate speech detection", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} hate speech detection: {data}"
