...
class ReplenishmentEngine(BaseEngine):
    engine_name = "replenishment"
    required_input_fields = ['purchase_history', 'replenishment_config']
    required_output_fields = ['replenishment_schedule', 'reminder_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} replenishment", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} replenishment: {data}"
