...
class RatingSystemEngine(BaseEngine):
    engine_name = "rating_system"
    required_input_fields = ['item_data', 'rating_config']
    required_output_fields = ['rating_setup', 'aggregation_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} rating system", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} rating system: {data}"
