...
class LogFileAnalysisEngine(BaseEngine):
    engine_name = "log_file_analysis"
    required_input_fields = ['server_logs', 'analysis_config']
    required_output_fields = ['crawl_report', 'bot_behavior']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} log file analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} log file analysis: {data}"
