"""
ShareholderCommunication Engine
...
class ShareholderCommunicationEngine(BaseEngine):
    engine_name = "shareholder_communication"
    required_input_fields = ['shareholder_data', 'communication_config']
    required_output_fields = ['communication_plan', 'report_drafts']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shareholder communication", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shareholder communication: {data}"
