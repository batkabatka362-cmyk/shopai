...
class ShadowDeploymentEngine(BaseEngine):
    engine_name = "shadow_deployment"
    required_input_fields = ['model_data', 'shadow_config']
    required_output_fields = ['shadow_results', 'comparison_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shadow deployment", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shadow deployment: {data}"
