...
class NamedEntityEngine(BaseEngine):
    engine_name = "named_entity"
    required_input_fields = ['text_data', 'ner_config']
    required_output_fields = ['entities', 'entity_types']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} named entity", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} named entity: {data}"
