...
class DataGovernanceEngine(BaseEngine):
    engine_name = "data_governance"
    required_input_fields = ['data_catalog', 'governance_config']
    required_output_fields = ['governance_report', 'policy_compliance']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} data governance", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} data governance: {data}"
