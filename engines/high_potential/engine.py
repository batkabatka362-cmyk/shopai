...
class HighPotentialEngine(BaseEngine):
    engine_name = "high_potential"
    required_input_fields = ['employee_data', 'hipo_config']
    required_output_fields = ['hipo_identification', 'development_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} high potential", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} high potential: {data}"
