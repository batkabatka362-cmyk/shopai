...
class CodeQualityEngine(BaseEngine):
    engine_name = "code_quality"
    required_input_fields = ['code_data', 'quality_config']
    required_output_fields = ['quality_report', 'improvement_actions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} code quality", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} code quality: {data}"
