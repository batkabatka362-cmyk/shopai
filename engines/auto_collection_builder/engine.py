...
class AutoCollectionBuilderEngine(BaseEngine):
    engine_name = "auto_collection_builder"
    required_input_fields = ['product_data', 'rule_config']
    required_output_fields = ['collection_definitions', 'sort_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} auto collection builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} auto collection builder: {data}"
