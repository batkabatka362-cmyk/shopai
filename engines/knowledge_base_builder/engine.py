...
class KnowledgeBaseBuilderEngine(BaseEngine):
    engine_name = "knowledge_base_builder"
    required_input_fields = ['content_data', 'kb_config']
    required_output_fields = ['kb_articles', 'category_structure']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} knowledge base builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} knowledge base builder: {data}"
