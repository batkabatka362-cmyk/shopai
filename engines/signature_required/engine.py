...
class SignatureRequiredEngine(BaseEngine):
    engine_name = "signature_required"
    required_input_fields = ['order_data', 'signature_config']
    required_output_fields = ['signature_rules', 'exception_handling']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} signature required", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} signature required: {data}"
