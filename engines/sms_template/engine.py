...
class SmsTemplateEngine(BaseEngine):
    engine_name = "sms_template"
    required_input_fields = ['message_config', 'compliance_rules']
    required_output_fields = ['sms_content', 'character_count']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sms template", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sms template: {data}"
