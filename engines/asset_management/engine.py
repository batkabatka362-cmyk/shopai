...
class AssetManagementEngine(BaseEngine):
    engine_name = "asset_management"
    required_input_fields = ['asset_data', 'management_config']
    required_output_fields = ['asset_inventory', 'depreciation_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} asset management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} asset management: {data}"
