...
class SmartContentEngineEngine(BaseEngine):
    engine_name = "smart_content_engine"
    required_input_fields = ['content_data', 'seo_data']
    required_output_fields = ['content_actions', 'publishing_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} smart content engine", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} smart content engine: {data}"
