...
class ManagementReportingEngine(BaseEngine):
    engine_name = "management_reporting"
    required_input_fields = ['business_data', 'reporting_config']
    required_output_fields = ['management_reports', 'kpi_dashboard']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} management reporting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} management reporting: {data}"
