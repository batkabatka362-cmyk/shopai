...
class ProductTransitionEngine(BaseEngine):
    engine_name = "product_transition"
    required_input_fields = ['old_product', 'new_product']
    required_output_fields = ['transition_plan', 'customer_communication']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} product transition", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} product transition: {data}"
