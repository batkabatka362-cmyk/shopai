...
class SchemaMarkupEngine(BaseEngine):
    engine_name = "schema_markup"
    required_input_fields = ['page_data', 'schema_config']
    required_output_fields = ['markup_code', 'validation_results']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} schema markup", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} schema markup: {data}"
