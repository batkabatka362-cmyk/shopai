...
class ContractAnalysisEngine(BaseEngine):
    engine_name = "contract_analysis"
    required_input_fields = ['contract_text', 'analysis_config']
    required_output_fields = ['clause_analysis', 'risk_flags']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} contract analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} contract analysis: {data}"
