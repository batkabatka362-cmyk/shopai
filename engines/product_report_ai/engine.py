...
class ProductReportAiEngine(BaseEngine):
    engine_name = "product_report_ai"
    required_input_fields = ['product_data', 'report_config']
    required_output_fields = ['product_report', 'performance_rankings']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} product report ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} product report ai: {data}"
