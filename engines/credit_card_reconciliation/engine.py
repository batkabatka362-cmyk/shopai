"""
CreditCardReconciliation Engine
...
class CreditCardReconciliationEngine(BaseEngine):
    engine_name = "credit_card_reconciliation"
    required_input_fields = ['cc_data', 'transaction_data']
    required_output_fields = ['reconciliation_report', 'unmatched_items']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} credit card reconciliation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} credit card reconciliation: {data}"
