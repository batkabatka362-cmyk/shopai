...
class BannerDesignerEngine(BaseEngine):
    engine_name = "banner_designer"
    required_input_fields = ['design_brief', 'size_config']
    required_output_fields = ['banner_specs', 'copy_overlay']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} banner designer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} banner designer: {data}"
