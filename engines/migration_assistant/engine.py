...
class MigrationAssistantEngine(BaseEngine):
    engine_name = "migration_assistant"
    required_input_fields = ['source_code', 'target_config']
    required_output_fields = ['migration_plan', 'converted_code']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} migration assistant", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} migration assistant: {data}"
