...
class ChatbotTrainingEngine(BaseEngine):
    engine_name = "chatbot_training"
    required_input_fields = ['conversation_data', 'training_config']
    required_output_fields = ['training_dataset', 'intent_mapping']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} chatbot training", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} chatbot training: {data}"
