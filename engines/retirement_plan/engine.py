...
class RetirementPlanEngine(BaseEngine):
    engine_name = "retirement_plan"
    required_input_fields = ['employee_data', 'retirement_config']
    required_output_fields = ['plan_options', 'contribution_matching']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} retirement plan", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} retirement plan: {data}"
