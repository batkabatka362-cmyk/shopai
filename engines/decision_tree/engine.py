...
class DecisionTreeEngine(BaseEngine):
    engine_name = "decision_tree"
    required_input_fields = ['decision_data', 'tree_config']
    required_output_fields = ['decision_tree', 'outcome_mapping']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} decision tree", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} decision tree: {data}"
