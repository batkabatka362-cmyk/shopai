...
class InstacartAdsEngine(BaseEngine):
    engine_name = "instacart_ads"
    required_input_fields = ['campaign_data', 'instacart_config']
    required_output_fields = ['campaign_setup', 'product_targeting']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} instacart ads", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} instacart ads: {data}"
