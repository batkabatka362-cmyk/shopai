...
class GoingConcernEngine(BaseEngine):
    engine_name = "going_concern"
    required_input_fields = ['financial_data', 'assessment_config']
    required_output_fields = ['concern_assessment', 'disclosure_draft']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} going concern", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} going concern: {data}"
