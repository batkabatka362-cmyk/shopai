...
class BreadcrumbSchemaEngine(BaseEngine):
    engine_name = "breadcrumb_schema"
    required_input_fields = ['navigation_data', 'schema_config']
    required_output_fields = ['breadcrumb_markup', 'hierarchy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} breadcrumb schema", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} breadcrumb schema: {data}"
