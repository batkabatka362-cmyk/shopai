...
class YearEndEngine(BaseEngine):
    engine_name = "year_end"
    required_input_fields = ['financial_data', 'year_end_config']
    required_output_fields = ['year_end_checklist', 'adjustment_entries']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} year end", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} year end: {data}"
