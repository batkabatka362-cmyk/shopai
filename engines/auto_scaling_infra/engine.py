...
class AutoScalingInfraEngine(BaseEngine):
    engine_name = "auto_scaling_infra"
    required_input_fields = ['metrics_data', 'scaling_config']
    required_output_fields = ['scaling_policy', 'trigger_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} auto scaling infra", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} auto scaling infra: {data}"
