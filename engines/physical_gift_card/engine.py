...
class PhysicalGiftCardEngine(BaseEngine):
    engine_name = "physical_gift_card"
    required_input_fields = ['card_config', 'print_config']
    required_output_fields = ['card_design', 'fulfillment_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} physical gift card", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} physical gift card: {data}"
