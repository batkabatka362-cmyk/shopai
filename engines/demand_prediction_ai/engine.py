...
class DemandPredictionAiEngine(BaseEngine):
    engine_name = "demand_prediction_ai"
    required_input_fields = ['historical_data', 'external_signals']
    required_output_fields = ['demand_forecast', 'accuracy_metrics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} demand prediction ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} demand prediction ai: {data}"
