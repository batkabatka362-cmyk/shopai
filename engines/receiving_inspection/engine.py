...
class ReceivingInspectionEngine(BaseEngine):
    engine_name = "receiving_inspection"
    required_input_fields = ['shipment_data', 'inspection_config']
    required_output_fields = ['inspection_report', 'acceptance_decision']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} receiving inspection", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} receiving inspection: {data}"
