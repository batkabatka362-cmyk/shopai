...
class JobCostingEngine(BaseEngine):
    engine_name = "job_costing"
    required_input_fields = ['job_data', 'costing_config']
    required_output_fields = ['job_costs', 'profitability_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} job costing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} job costing: {data}"
