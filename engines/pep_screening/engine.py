...
class PepScreeningEngine(BaseEngine):
    engine_name = "pep_screening"
    required_input_fields = ['person_data', 'pep_config']
    required_output_fields = ['pep_status', 'risk_classification']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pep screening", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pep screening: {data}"
