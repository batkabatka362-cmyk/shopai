...
class DashboardBuilderCustomEngine(BaseEngine):
    engine_name = "dashboard_builder_custom"
    required_input_fields = ['widget_data', 'dashboard_config']
    required_output_fields = ['dashboard_layout', 'data_connections']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dashboard builder custom", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dashboard builder custom: {data}"
