...
class MediaMixEngine(BaseEngine):
    engine_name = "media_mix"
    required_input_fields = ['channel_data', 'budget_data']
    required_output_fields = ['media_plan', 'allocation_matrix']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} media mix", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} media mix: {data}"
