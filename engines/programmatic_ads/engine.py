...
class ProgrammaticAdsEngine(BaseEngine):
    engine_name = "programmatic_ads"
    required_input_fields = ['campaign_data', 'dsp_config']
    required_output_fields = ['media_plan', 'bid_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} programmatic ads", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} programmatic ads: {data}"
