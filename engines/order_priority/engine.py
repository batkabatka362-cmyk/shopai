...
class OrderPriorityEngine(BaseEngine):
    engine_name = "order_priority"
    required_input_fields = ['order_data', 'priority_config']
    required_output_fields = ['priority_score', 'processing_queue']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} order priority", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} order priority: {data}"
