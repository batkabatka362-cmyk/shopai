...
class TagManagerAuditEngine(BaseEngine):
    engine_name = "tag_manager_audit"
    required_input_fields = ['tag_data', 'audit_config']
    required_output_fields = ['tag_report', 'cleanup_recommendations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} tag manager audit", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} tag manager audit: {data}"
