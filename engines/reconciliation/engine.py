...
class ReconciliationEngine(BaseEngine):
    engine_name = "reconciliation"
    required_input_fields = ['transaction_data', 'account_data']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} reconciliation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} reconciliation: {data}"
