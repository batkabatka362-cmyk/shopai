...
class GeopoliticalRiskEngine(BaseEngine):
    engine_name = "geopolitical_risk"
    required_input_fields = ['country_data', 'risk_config']
    required_output_fields = ['risk_assessment', 'impact_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} geopolitical risk", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} geopolitical risk: {data}"
