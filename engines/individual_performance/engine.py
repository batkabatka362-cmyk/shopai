...
class IndividualPerformanceEngine(BaseEngine):
    engine_name = "individual_performance"
    required_input_fields = ['employee_data', 'performance_config']
    required_output_fields = ['performance_report', 'development_areas']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} individual performance", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} individual performance: {data}"
