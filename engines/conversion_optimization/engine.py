...
class ConversionOptimizationEngine(BaseEngine):
    engine_name = "conversion_optimization"
    required_input_fields = ['funnel_data', 'optimization_config']
    required_output_fields = ['optimization_plan', 'expected_lift']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} conversion optimization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} conversion optimization: {data}"
