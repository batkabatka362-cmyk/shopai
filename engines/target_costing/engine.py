...
class TargetCostingEngine(BaseEngine):
    engine_name = "target_costing"
    required_input_fields = ['product_data', 'target_config']
    required_output_fields = ['target_costs', 'gap_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} target costing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} target costing: {data}"
