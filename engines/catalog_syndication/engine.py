...
class CatalogSyndicationEngine(BaseEngine):
    engine_name = "catalog_syndication"
    required_input_fields = ['catalog_data', 'syndication_config']
    required_output_fields = ['syndication_plan', 'channel_mapping']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} catalog syndication", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} catalog syndication: {data}"
