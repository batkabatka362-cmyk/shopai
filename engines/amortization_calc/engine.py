...
class AmortizationCalcEngine(BaseEngine):
    engine_name = "amortization_calc"
    required_input_fields = ['intangible_data', 'amortization_config']
    required_output_fields = ['amortization_schedule', 'book_value']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} amortization calc", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} amortization calc: {data}"
