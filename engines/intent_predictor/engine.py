...
class IntentPredictorEngine(BaseEngine):
    engine_name = "intent_predictor"
    required_input_fields = ['behavior_data', 'context_data']
    required_output_fields = ['intent_predictions', 'action_recommendations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} intent predictor", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} intent predictor: {data}"
