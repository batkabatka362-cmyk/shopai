...
class TerminationNoticeEngine(BaseEngine):
    engine_name = "termination_notice"
    required_input_fields = ['contract_data', 'termination_config']
    required_output_fields = ['notice_draft', 'obligation_checklist']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} termination notice", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} termination notice: {data}"
