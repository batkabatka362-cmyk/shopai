...
class TimeBasedPricingEngine(BaseEngine):
    engine_name = "time_based_pricing"
    required_input_fields = ['product_data', 'time_config']
    required_output_fields = ['time_prices', 'schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} time based pricing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} time based pricing: {data}"
