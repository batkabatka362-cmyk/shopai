"""
MicroserviceOrchestration Engine
...
class MicroserviceOrchestrationEngine(BaseEngine):
    engine_name = "microservice_orchestration"
    required_input_fields = ['service_data', 'orchestration_config']
    required_output_fields = ['orchestration_plan', 'dependency_map']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} microservice orchestration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} microservice orchestration: {data}"
