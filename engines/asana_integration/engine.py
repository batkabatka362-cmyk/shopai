...
class AsanaIntegrationEngine(BaseEngine):
    engine_name = "asana_integration"
    required_input_fields = ['project_data', 'integration_config']
    required_output_fields = ['asana_sync', 'task_mapping']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} asana integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} asana integration: {data}"
