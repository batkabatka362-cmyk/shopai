...
class ContextWindowEngine(BaseEngine):
    engine_name = "context_window"
    required_input_fields = ['context_data', 'window_config']
    required_output_fields = ['optimized_context', 'truncation_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} context window", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} context window: {data}"
