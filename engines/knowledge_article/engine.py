...
class KnowledgeArticleEngine(BaseEngine):
    engine_name = "knowledge_article"
    required_input_fields = ['topic_data', 'article_config']
    required_output_fields = ['article_content', 'category_tags']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} knowledge article", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} knowledge article: {data}"
