...
class FsaHsaEngine(BaseEngine):
    engine_name = "fsa_hsa"
    required_input_fields = ['employee_data', 'account_config']
    required_output_fields = ['account_options', 'contribution_limits']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} fsa hsa", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} fsa hsa: {data}"
