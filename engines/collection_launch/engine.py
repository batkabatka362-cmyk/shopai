...
class CollectionLaunchEngine(BaseEngine):
    engine_name = "collection_launch"
    required_input_fields = ['collection_data', 'launch_config']
    required_output_fields = ['launch_campaign', 'teaser_content']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} collection launch", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} collection launch: {data}"
