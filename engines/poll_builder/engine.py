...
class PollBuilderEngine(BaseEngine):
    engine_name = "poll_builder"
    required_input_fields = ['question_data', 'poll_config']
    required_output_fields = ['poll_setup', 'result_display']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} poll builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} poll builder: {data}"
