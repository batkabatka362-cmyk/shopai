...
class ScenarioModelingEngine(BaseEngine):
    engine_name = "scenario_modeling"
    required_input_fields = ['base_data', 'scenario_config']
    required_output_fields = ['scenario_results', 'sensitivity_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} scenario modeling", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} scenario modeling: {data}"
