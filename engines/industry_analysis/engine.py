...
class IndustryAnalysisEngine(BaseEngine):
    engine_name = "industry_analysis"
    required_input_fields = ['industry_data', 'analysis_config']
    required_output_fields = ['industry_report', 'trend_outlook']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} industry analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} industry analysis: {data}"
