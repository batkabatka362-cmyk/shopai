...
class ChurnPreventionAiEngine(BaseEngine):
    engine_name = "churn_prevention_ai"
    required_input_fields = ['customer_data', 'behavior_data']
    required_output_fields = ['churn_scores', 'prevention_actions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} churn prevention ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} churn prevention ai: {data}"
