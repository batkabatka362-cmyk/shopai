...
class RepairServiceEngine(BaseEngine):
    engine_name = "repair_service"
    required_input_fields = ['product_data', 'service_config']
    required_output_fields = ['repair_options', 'cost_estimate']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} repair service", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} repair service: {data}"
