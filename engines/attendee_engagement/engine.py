...
class AttendeeEngagementEngine(BaseEngine):
    engine_name = "attendee_engagement"
    required_input_fields = ['attendee_data', 'engagement_config']
    required_output_fields = ['engagement_plan', 'gamification_setup']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} attendee engagement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} attendee engagement: {data}"
