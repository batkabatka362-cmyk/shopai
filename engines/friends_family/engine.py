...
class FriendsFamilyEngine(BaseEngine):
    engine_name = "friends_family"
    required_input_fields = ['customer_data', 'ff_config']
    required_output_fields = ['ff_program', 'discount_codes']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} friends family", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} friends family: {data}"
