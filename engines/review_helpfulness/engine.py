...
class ReviewHelpfulnessEngine(BaseEngine):
    engine_name = "review_helpfulness"
    required_input_fields = ['review_data', 'scoring_config']
    required_output_fields = ['helpfulness_scores', 'ranking']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} review helpfulness", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} review helpfulness: {data}"
