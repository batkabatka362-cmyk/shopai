...
class DeviceFingerprintEngine(BaseEngine):
    engine_name = "device_fingerprint"
    required_input_fields = ['device_data', 'fingerprint_config']
    required_output_fields = ['device_id', 'trust_score']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} device fingerprint", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} device fingerprint: {data}"
