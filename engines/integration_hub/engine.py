...
class IntegrationHubEngine(BaseEngine):
    engine_name = "integration_hub"
    required_input_fields = ['integration_data', 'hub_config']
    required_output_fields = ['hub_setup', 'connector_library']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} integration hub", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} integration hub: {data}"
