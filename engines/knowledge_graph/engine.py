...
class KnowledgeGraphEngine(BaseEngine):
    engine_name = "knowledge_graph"
    required_input_fields = ['entity_data', 'relationship_config']
    required_output_fields = ['graph_structure', 'query_interface']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} knowledge graph", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} knowledge graph: {data}"
