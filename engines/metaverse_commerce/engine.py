...
class MetaverseCommerceEngine(BaseEngine):
    engine_name = "metaverse_commerce"
    required_input_fields = ['product_data', 'metaverse_config']
    required_output_fields = ['virtual_store', 'avatar_integration']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} metaverse commerce", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} metaverse commerce: {data}"
