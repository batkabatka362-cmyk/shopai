...
class AccountPageEngine(BaseEngine):
    engine_name = "account_page"
    required_input_fields = ['account_data', 'page_config']
    required_output_fields = ['account_layout', 'section_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} account page", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} account page: {data}"
