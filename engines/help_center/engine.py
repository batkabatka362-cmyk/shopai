...
class HelpCenterEngine(BaseEngine):
    engine_name = "help_center"
    required_input_fields = ['support_data', 'center_config']
    required_output_fields = ['help_articles', 'search_index']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} help center", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} help center: {data}"
