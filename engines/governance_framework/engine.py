...
class GovernanceFrameworkEngine(BaseEngine):
    engine_name = "governance_framework"
    required_input_fields = ['org_data', 'governance_config']
    required_output_fields = ['framework_document', 'compliance_checklist']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} governance framework", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} governance framework: {data}"
