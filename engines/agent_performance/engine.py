...
class AgentPerformanceEngine(BaseEngine):
    engine_name = "agent_performance"
    required_input_fields = ['agent_data', 'performance_config']
    required_output_fields = ['performance_scores', 'coaching_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} agent performance", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} agent performance: {data}"
