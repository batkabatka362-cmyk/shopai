...
class DomainManagementEngine(BaseEngine):
    engine_name = "domain_management"
    required_input_fields = ['domain_portfolio', 'management_config']
    required_output_fields = ['domain_status', 'renewal_alerts']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} domain management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} domain management: {data}"
