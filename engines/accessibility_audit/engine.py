...
class AccessibilityAuditEngine(BaseEngine):
    engine_name = "accessibility_audit"
    required_input_fields = ['page_data', 'wcag_config']
    required_output_fields = ['audit_report', 'fix_priorities']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} accessibility audit", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} accessibility audit: {data}"
