...
class SpiffEngineEngine(BaseEngine):
    engine_name = "spiff_engine"
    required_input_fields = ['sales_data', 'spiff_config']
    required_output_fields = ['spiff_calc', 'contest_standings']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} spiff engine", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} spiff engine: {data}"
