...
class SubscriptionTermsEngine(BaseEngine):
    engine_name = "subscription_terms"
    required_input_fields = ['subscription_data', 'terms_config']
    required_output_fields = ['subscription_terms', 'auto_renewal_disclosure']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} subscription terms", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} subscription terms: {data}"
