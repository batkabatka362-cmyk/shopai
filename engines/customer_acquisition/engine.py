...
class CustomerAcquisitionEngine(BaseEngine):
    engine_name = "customer_acquisition"
    required_input_fields = ['acquisition_data', 'channel_data']
    required_output_fields = ['acquisition_report', 'channel_efficiency']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} customer acquisition", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} customer acquisition: {data}"
