...
class ImpactTrackerEngine(BaseEngine):
    engine_name = "impact_tracker"
    required_input_fields = ['impact_data', 'tracker_config']
    required_output_fields = ['impact_display', 'contribution_summary']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} impact tracker", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} impact tracker: {data}"
