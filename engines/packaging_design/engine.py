...
class PackagingDesignEngine(BaseEngine):
    engine_name = "packaging_design"
    required_input_fields = ['product_data', 'brand_guidelines']
    required_output_fields = ['packaging_brief', 'design_specs']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} packaging design", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} packaging design: {data}"
