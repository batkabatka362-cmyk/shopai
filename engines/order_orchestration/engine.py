...
class OrderOrchestrationEngine(BaseEngine):
    engine_name = "order_orchestration"
    required_input_fields = ['order_data', 'orchestration_config']
    required_output_fields = ['orchestration_plan', 'status_tracking']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} order orchestration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} order orchestration: {data}"
