"""
PodcastMarketing Engine — Podcast Marketing
...
class PodcastMarketingEngine(BaseEngine):
    engine_name = "podcast_marketing"
    required_input_fields = ['brand_data', 'podcast_database']
    required_output_fields = ['podcast_matches', 'sponsorship_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} podcast marketing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} podcast marketing: {data}"
