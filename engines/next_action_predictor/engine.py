...
class NextActionPredictorEngine(BaseEngine):
    engine_name = "next_action_predictor"
    required_input_fields = ['history_data', 'context_data']
    required_output_fields = ['predicted_actions', 'confidence_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} next action predictor", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} next action predictor: {data}"
