...
class AgileMetricsEngine(BaseEngine):
    engine_name = "agile_metrics"
    required_input_fields = ['sprint_data', 'metrics_config']
    required_output_fields = ['velocity_report', 'burndown_chart']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} agile metrics", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} agile metrics: {data}"
