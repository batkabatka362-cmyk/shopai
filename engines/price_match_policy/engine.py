...
class PriceMatchPolicyEngine(BaseEngine):
    engine_name = "price_match_policy"
    required_input_fields = ['pricing_data', 'policy_config']
    required_output_fields = ['price_match_policy', 'verification_process']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} price match policy", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} price match policy: {data}"
