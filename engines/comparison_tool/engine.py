...
class ComparisonToolEngine(BaseEngine):
    engine_name = "comparison_tool"
    required_input_fields = ['product_ids', 'comparison_config']
    required_output_fields = ['comparison_matrix', 'recommendation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} comparison tool", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} comparison tool: {data}"
