...
class ContentLockingEngine(BaseEngine):
    engine_name = "content_locking"
    required_input_fields = ['content_data', 'lock_config']
    required_output_fields = ['lock_rules', 'unlock_triggers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} content locking", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} content locking: {data}"
