...
class KeywordBidEngine(BaseEngine):
    engine_name = "keyword_bid"
    required_input_fields = ['keyword_data', 'bid_config']
    required_output_fields = ['bid_adjustments', 'quality_score_impact']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} keyword bid", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} keyword bid: {data}"
