...
class SessionManagementEngine(BaseEngine):
    engine_name = "session_management"
    required_input_fields = ['session_data', 'management_config']
    required_output_fields = ['session_config', 'security_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} session management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} session management: {data}"
