...
class ZeroBasedBudgetingEngine(BaseEngine):
    engine_name = "zero_based_budgeting"
    required_input_fields = ['cost_data', 'budget_config']
    required_output_fields = ['zero_based_budget', 'justification_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} zero based budgeting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} zero based budgeting: {data}"
