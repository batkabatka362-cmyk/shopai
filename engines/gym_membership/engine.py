...
class GymMembershipEngine(BaseEngine):
    engine_name = "gym_membership"
    required_input_fields = ['employee_data', 'gym_config']
    required_output_fields = ['gym_options', 'subsidy_amount']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} gym membership", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} gym membership: {data}"
