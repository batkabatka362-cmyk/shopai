"""
ChargebackReconciliation Engine
...
class ChargebackReconciliationEngine(BaseEngine):
    engine_name = "chargeback_reconciliation"
    required_input_fields = ['chargeback_data', 'order_data']
    required_output_fields = ['reconciliation_report', 'win_rate_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} chargeback reconciliation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} chargeback reconciliation: {data}"
