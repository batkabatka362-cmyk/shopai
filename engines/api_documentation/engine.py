...
class ApiDocumentationEngine(BaseEngine):
    engine_name = "api_documentation"
    required_input_fields = ['api_spec', 'doc_config']
    required_output_fields = ['api_docs', 'code_examples']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} api documentation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} api documentation: {data}"
