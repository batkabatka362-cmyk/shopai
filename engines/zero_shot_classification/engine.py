...
class ZeroShotClassificationEngine(BaseEngine):
    engine_name = "zero_shot_classification"
    required_input_fields = ['text_data', 'label_candidates']
    required_output_fields = ['classifications', 'confidence_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} zero shot classification", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} zero shot classification: {data}"
