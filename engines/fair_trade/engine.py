...
class FairTradeEngine(BaseEngine):
    engine_name = "fair_trade"
    required_input_fields = ['sourcing_data', 'fair_trade_config']
    required_output_fields = ['certification_plan', 'supply_chain_audit']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} fair trade", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} fair trade: {data}"
