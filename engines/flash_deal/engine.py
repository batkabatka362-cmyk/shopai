...
class FlashDealEngine(BaseEngine):
    engine_name = "flash_deal"
    required_input_fields = ['product_data', 'deal_config']
    required_output_fields = ['deal_setup', 'countdown_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} flash deal", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} flash deal: {data}"
