...
class ReorderPointEngine(BaseEngine):
    engine_name = "reorder_point"
    required_input_fields = ['inventory_data', 'demand_data']
    required_output_fields = ['reorder_points', 'review_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} reorder point", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} reorder point: {data}"
