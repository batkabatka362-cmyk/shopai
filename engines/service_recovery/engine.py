...
class ServiceRecoveryEngine(BaseEngine):
    engine_name = "service_recovery"
    required_input_fields = ['failure_data', 'recovery_config']
    required_output_fields = ['recovery_actions', 'follow_up_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} service recovery", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} service recovery: {data}"
