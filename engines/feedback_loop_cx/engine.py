...
class FeedbackLoopCxEngine(BaseEngine):
    engine_name = "feedback_loop_cx"
    required_input_fields = ['feedback_data', 'loop_config']
    required_output_fields = ['action_items', 'improvement_tracking']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} feedback loop cx", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} feedback loop cx: {data}"
