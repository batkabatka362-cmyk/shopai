...
class DemandShapingEngine(BaseEngine):
    engine_name = "demand_shaping"
    required_input_fields = ['demand_data', 'shaping_config']
    required_output_fields = ['shaped_demand', 'incentive_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} demand shaping", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} demand shaping: {data}"
