...
class VisualRegressionEngine(BaseEngine):
    engine_name = "visual_regression"
    required_input_fields = ['screenshot_data', 'baseline_config']
    required_output_fields = ['diff_report', 'pixel_changes']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} visual regression", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} visual regression: {data}"
