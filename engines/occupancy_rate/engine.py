...
class OccupancyRateEngine(BaseEngine):
    engine_name = "occupancy_rate"
    required_input_fields = ['agent_data', 'rate_config']
    required_output_fields = ['occupancy_metrics', 'burnout_risk']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} occupancy rate", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} occupancy rate: {data}"
