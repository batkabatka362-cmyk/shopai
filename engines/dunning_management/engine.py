...
class DunningManagementEngine(BaseEngine):
    engine_name = "dunning_management"
    required_input_fields = ['overdue_data', 'dunning_config']
    required_output_fields = ['dunning_sequence', 'recovery_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dunning management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dunning management: {data}"
