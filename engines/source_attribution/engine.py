...
class SourceAttributionEngine(BaseEngine):
    engine_name = "source_attribution"
    required_input_fields = ['generated_text', 'source_data']
    required_output_fields = ['attributions', 'confidence_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} source attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} source attribution: {data}"
