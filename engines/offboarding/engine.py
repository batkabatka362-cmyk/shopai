...
class OffboardingEngine(BaseEngine):
    engine_name = "offboarding"
    required_input_fields = ['employee_data', 'offboarding_config']
    required_output_fields = ['offboarding_checklist', 'access_revocation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} offboarding", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} offboarding: {data}"
