...
class SeasonalClearanceEngine(BaseEngine):
    engine_name = "seasonal_clearance"
    required_input_fields = ['inventory_data', 'season_config']
    required_output_fields = ['clearance_strategy', 'timing_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} seasonal clearance", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} seasonal clearance: {data}"
