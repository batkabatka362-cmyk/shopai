...
class CoBrandingEngine(BaseEngine):
    engine_name = "co_branding"
    required_input_fields = ['brand_data', 'partner_data']
    required_output_fields = ['co_brand_strategy', 'product_concepts']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} co branding", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} co branding: {data}"
