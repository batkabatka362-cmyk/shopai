...
class DentalVisionEngine(BaseEngine):
    engine_name = "dental_vision"
    required_input_fields = ['employee_data', 'plan_config']
    required_output_fields = ['plan_options', 'coverage_comparison']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dental vision", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dental vision: {data}"
