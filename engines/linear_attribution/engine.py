...
class LinearAttributionEngine(BaseEngine):
    engine_name = "linear_attribution"
    required_input_fields = ['touchpoint_data', 'linear_config']
    required_output_fields = ['equal_weights', 'touchpoint_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} linear attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} linear attribution: {data}"
