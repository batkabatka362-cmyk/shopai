...
class ExclusiveAccessEngine(BaseEngine):
    engine_name = "exclusive_access"
    required_input_fields = ['product_data', 'access_config']
    required_output_fields = ['access_tiers', 'unlock_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} exclusive access", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} exclusive access: {data}"
