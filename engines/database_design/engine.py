...
class DatabaseDesignEngine(BaseEngine):
    engine_name = "database_design"
    required_input_fields = ['requirements_data', 'design_config']
    required_output_fields = ['schema_design', 'index_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} database design", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} database design: {data}"
