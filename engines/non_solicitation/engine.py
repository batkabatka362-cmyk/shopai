...
class NonSolicitationEngine(BaseEngine):
    engine_name = "non_solicitation"
    required_input_fields = ['employee_data', 'ns_config']
    required_output_fields = ['agreement_draft', 'scope_definition']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} non solicitation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} non solicitation: {data}"
