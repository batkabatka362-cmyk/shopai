...
class DeliverrIntegrationEngine(BaseEngine):
    engine_name = "deliverr_integration"
    required_input_fields = ['fulfillment_data', 'integration_config']
    required_output_fields = ['deliverr_sync', 'channel_mapping']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} deliverr integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} deliverr integration: {data}"
