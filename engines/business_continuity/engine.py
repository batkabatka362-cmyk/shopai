...
class BusinessContinuityEngine(BaseEngine):
    engine_name = "business_continuity"
    required_input_fields = ['business_data', 'continuity_config']
    required_output_fields = ['continuity_plan', 'test_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} business continuity", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} business continuity: {data}"
