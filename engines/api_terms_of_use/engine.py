...
class ApiTermsOfUseEngine(BaseEngine):
    engine_name = "api_terms_of_use"
    required_input_fields = ['api_data', 'terms_config']
    required_output_fields = ['api_terms', 'rate_limit_policy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} api terms of use", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} api terms of use: {data}"
