...
class UserResearchEngine(BaseEngine):
    engine_name = "user_research"
    required_input_fields = ['research_goals', 'research_config']
    required_output_fields = ['research_plan', 'methodology']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} user research", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} user research: {data}"
