...
class TermsGeneratorEngine(BaseEngine):
    engine_name = "terms_generator"
    required_input_fields = ['business_data', 'legal_config']
    required_output_fields = ['terms_content', 'privacy_policy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} terms generator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} terms generator: {data}"
