...
class LastMinuteGiftEngine(BaseEngine):
    engine_name = "last_minute_gift"
    required_input_fields = ['shipping_data', 'product_data']
    required_output_fields = ['fast_ship_options', 'digital_alternatives']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} last minute gift", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} last minute gift: {data}"
