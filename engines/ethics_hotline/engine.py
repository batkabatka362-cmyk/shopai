...
class EthicsHotlineEngine(BaseEngine):
    engine_name = "ethics_hotline"
    required_input_fields = ['report_data', 'hotline_config']
    required_output_fields = ['case_tracking', 'response_protocol']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ethics hotline", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ethics hotline: {data}"
