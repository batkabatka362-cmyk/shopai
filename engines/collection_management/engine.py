...
class CollectionManagementEngine(BaseEngine):
    engine_name = "collection_management"
    required_input_fields = ['ar_data', 'collection_config']
    required_output_fields = ['collection_strategy', 'aging_action']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} collection management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} collection management: {data}"
