...
class CustomsFormEngine(BaseEngine):
    engine_name = "customs_form"
    required_input_fields = ['shipment_data', 'customs_config']
    required_output_fields = ['form_content', 'hs_codes']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} customs form", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} customs form: {data}"
