...
class ShopifySeoAiEngine(BaseEngine):
    engine_name = "shopify_seo_ai"
    required_input_fields = ['store_data', 'seo_data']
    required_output_fields = ['seo_audit', 'fix_priorities']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shopify seo ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shopify seo ai: {data}"
