...
class LeadRoutingEngine(BaseEngine):
    engine_name = "lead_routing"
    required_input_fields = ['lead_data', 'routing_config']
    required_output_fields = ['routing_decisions', 'agent_assignments']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} lead routing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} lead routing: {data}"
