...
class PulseSurveyEngine(BaseEngine):
    engine_name = "pulse_survey"
    required_input_fields = ['survey_config', 'audience_data']
    required_output_fields = ['survey_setup', 'analysis_framework']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pulse survey", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pulse survey: {data}"
