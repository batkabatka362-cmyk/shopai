...
class BeneishMEngine(BaseEngine):
    engine_name = "beneish_m"
    required_input_fields = ['financial_data', 'm_config']
    required_output_fields = ['m_score', 'manipulation_flags']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} beneish m", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} beneish m: {data}"
