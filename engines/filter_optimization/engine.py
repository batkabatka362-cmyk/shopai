...
class FilterOptimizationEngine(BaseEngine):
    engine_name = "filter_optimization"
    required_input_fields = ['filter_data', 'usage_data']
    required_output_fields = ['filter_config', 'facet_recommendations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} filter optimization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} filter optimization: {data}"
