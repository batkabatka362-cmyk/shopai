...
class SpeakableSchemaEngine(BaseEngine):
    engine_name = "speakable_schema"
    required_input_fields = ['content_data', 'speakable_config']
    required_output_fields = ['speakable_markup', 'voice_preview']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} speakable schema", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} speakable schema: {data}"
