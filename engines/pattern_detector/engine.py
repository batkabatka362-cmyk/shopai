...
class PatternDetectorEngine(BaseEngine):
    engine_name = "pattern_detector"
    required_input_fields = ['data', 'detection_config']
    required_output_fields = ['patterns', 'significance_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pattern detector", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pattern detector: {data}"
