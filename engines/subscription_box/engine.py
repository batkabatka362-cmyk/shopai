...
class SubscriptionBoxEngine(BaseEngine):
    engine_name = "subscription_box"
    required_input_fields = ['product_pool', 'box_config']
    required_output_fields = ['box_curation', 'billing_setup']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} subscription box", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} subscription box: {data}"
