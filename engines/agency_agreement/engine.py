...
class AgencyAgreementEngine(BaseEngine):
    engine_name = "agency_agreement"
    required_input_fields = ['agency_data', 'agreement_config']
    required_output_fields = ['agreement_draft', 'scope_of_work']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} agency agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} agency agreement: {data}"
