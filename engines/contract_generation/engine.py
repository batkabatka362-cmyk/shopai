...
class ContractGenerationEngine(BaseEngine):
    engine_name = "contract_generation"
    required_input_fields = ['contract_terms', 'template_config']
    required_output_fields = ['contract_document', 'signature_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} contract generation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} contract generation: {data}"
