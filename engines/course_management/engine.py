...
class CourseManagementEngine(BaseEngine):
    engine_name = "course_management"
    required_input_fields = ['course_data', 'management_config']
    required_output_fields = ['course_setup', 'enrollment_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} course management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} course management: {data}"
