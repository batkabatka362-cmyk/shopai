...
class EmbeddingPipelineEngine(BaseEngine):
    engine_name = "embedding_pipeline"
    required_input_fields = ['text_data', 'embedding_config']
    required_output_fields = ['embeddings', 'index_status']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} embedding pipeline", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} embedding pipeline: {data}"
