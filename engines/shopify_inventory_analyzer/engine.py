"""
ShopifyInventoryAnalyzer Engine
...
class ShopifyInventoryAnalyzerEngine(BaseEngine):
    engine_name = "shopify_inventory_analyzer"
    required_input_fields = ['inventory_data', 'sales_data']
    required_output_fields = ['inventory_insights', 'restock_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shopify inventory analyzer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shopify inventory analyzer: {data}"
