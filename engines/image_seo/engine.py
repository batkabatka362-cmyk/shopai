...
class ImageSeoEngine(BaseEngine):
    engine_name = "image_seo"
    required_input_fields = ['image_data', 'seo_config']
    required_output_fields = ['alt_tags', 'file_names']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} image seo", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} image seo: {data}"
