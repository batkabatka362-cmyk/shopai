...
class CommunitySupportEngine(BaseEngine):
    engine_name = "community_support"
    required_input_fields = ['community_data', 'support_config']
    required_output_fields = ['community_response', 'expert_routing']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} community support", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} community support: {data}"
