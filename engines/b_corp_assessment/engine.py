...
class BCorpAssessmentEngine(BaseEngine):
    engine_name = "b_corp_assessment"
    required_input_fields = ['business_data', 'assessment_config']
    required_output_fields = ['bia_score', 'improvement_areas']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} b corp assessment", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} b corp assessment: {data}"
