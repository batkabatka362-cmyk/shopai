...
class CqrsImplementationEngine(BaseEngine):
    engine_name = "cqrs_implementation"
    required_input_fields = ['domain_data', 'cqrs_config']
    required_output_fields = ['command_model', 'query_model']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cqrs implementation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cqrs implementation: {data}"
