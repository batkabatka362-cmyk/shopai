...
class ScheduledDeliveryEngine(BaseEngine):
    engine_name = "scheduled_delivery"
    required_input_fields = ['order_data', 'schedule_config']
    required_output_fields = ['available_slots', 'booking_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} scheduled delivery", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} scheduled delivery: {data}"
