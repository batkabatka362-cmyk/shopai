...
class MondayIntegrationEngine(BaseEngine):
    engine_name = "monday_integration"
    required_input_fields = ['board_data', 'integration_config']
    required_output_fields = ['monday_sync', 'column_mapping']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} monday integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} monday integration: {data}"
