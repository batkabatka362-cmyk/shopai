...
class PersonalizedBoxEngine(BaseEngine):
    engine_name = "personalized_box"
    required_input_fields = ['preference_data', 'product_pool']
    required_output_fields = ['box_contents', 'personalization_score']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} personalized box", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} personalized box: {data}"
