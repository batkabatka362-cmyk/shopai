...
class StreakTrackerEngine(BaseEngine):
    engine_name = "streak_tracker"
    required_input_fields = ['activity_data', 'streak_config']
    required_output_fields = ['streak_status', 'reward_eligibility']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} streak tracker", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} streak tracker: {data}"
