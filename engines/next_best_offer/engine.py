...
class NextBestOfferEngine(BaseEngine):
    engine_name = "next_best_offer"
    required_input_fields = ['customer_data', 'offer_pool']
    required_output_fields = ['best_offers', 'personalization_score']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} next best offer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} next best offer: {data}"
