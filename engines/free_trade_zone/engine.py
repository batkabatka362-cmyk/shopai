...
class FreeTradeZoneEngine(BaseEngine):
    engine_name = "free_trade_zone"
    required_input_fields = ['logistics_data', 'ftz_config']
    required_output_fields = ['ftz_benefits', 'setup_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} free trade zone", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} free trade zone: {data}"
