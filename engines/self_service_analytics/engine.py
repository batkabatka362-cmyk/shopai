...
class SelfServiceAnalyticsEngine(BaseEngine):
    engine_name = "self_service_analytics"
    required_input_fields = ['self_service_data', 'analytics_config']
    required_output_fields = ['deflection_report', 'improvement_areas']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} self service analytics", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} self service analytics: {data}"
