...
class TransactionMonitoringEngine(BaseEngine):
    engine_name = "transaction_monitoring"
    required_input_fields = ['transaction_data', 'monitoring_config']
    required_output_fields = ['monitoring_alerts', 'suspicious_patterns']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} transaction monitoring", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} transaction monitoring: {data}"
