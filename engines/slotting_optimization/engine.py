...
class SlottingOptimizationEngine(BaseEngine):
    engine_name = "slotting_optimization"
    required_input_fields = ['product_data', 'warehouse_layout']
    required_output_fields = ['slot_assignments', 'efficiency_gain']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} slotting optimization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} slotting optimization: {data}"
