...
class NorthStarMetricEngine(BaseEngine):
    engine_name = "north_star_metric"
    required_input_fields = ['metric_data', 'nsm_config']
    required_output_fields = ['north_star_definition', 'tracking_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} north star metric", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} north star metric: {data}"
