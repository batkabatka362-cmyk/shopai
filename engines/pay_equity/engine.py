...
class PayEquityEngine(BaseEngine):
    engine_name = "pay_equity"
    required_input_fields = ['compensation_data', 'equity_config']
    required_output_fields = ['equity_analysis', 'adjustment_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pay equity", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pay equity: {data}"
