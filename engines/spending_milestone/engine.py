...
class SpendingMilestoneEngine(BaseEngine):
    engine_name = "spending_milestone"
    required_input_fields = ['spending_data', 'milestone_config']
    required_output_fields = ['milestone_detection', 'tier_upgrade']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} spending milestone", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} spending milestone: {data}"
