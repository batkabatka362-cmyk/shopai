...
class SocialWidgetAuditEngine(BaseEngine):
    engine_name = "social_widget_audit"
    required_input_fields = ['widget_data', 'audit_config']
    required_output_fields = ['widget_impact', 'optimization_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} social widget audit", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} social widget audit: {data}"
