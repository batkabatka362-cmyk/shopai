...
class AnniversaryCampaignEngine(BaseEngine):
    engine_name = "anniversary_campaign"
    required_input_fields = ['customer_data', 'campaign_config']
    required_output_fields = ['anniversary_offers', 'milestone_rewards']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} anniversary campaign", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} anniversary campaign: {data}"
