...
class AccountingSyncManagerEngine(BaseEngine):
    engine_name = "accounting_sync_manager"
    required_input_fields = ['accounting_data', 'sync_config']
    required_output_fields = ['sync_plan', 'reconciliation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} accounting sync manager", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} accounting sync manager: {data}"
