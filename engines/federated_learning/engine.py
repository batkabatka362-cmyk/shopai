...
class FederatedLearningEngine(BaseEngine):
    engine_name = "federated_learning"
    required_input_fields = ['client_data', 'fl_config']
    required_output_fields = ['global_model', 'aggregation_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} federated learning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} federated learning: {data}"
