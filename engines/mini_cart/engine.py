...
class MiniCartEngine(BaseEngine):
    engine_name = "mini_cart"
    required_input_fields = ['cart_data', 'mini_config']
    required_output_fields = ['mini_layout', 'quick_actions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} mini cart", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} mini cart: {data}"
