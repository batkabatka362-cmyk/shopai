...
class MortgageAgreementEngine(BaseEngine):
    engine_name = "mortgage_agreement"
    required_input_fields = ['property_data', 'mortgage_config']
    required_output_fields = ['mortgage_draft', 'repayment_terms']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} mortgage agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} mortgage agreement: {data}"
