...
class EmployerBrandingEngine(BaseEngine):
    engine_name = "employer_branding"
    required_input_fields = ['brand_data', 'branding_config']
    required_output_fields = ['branding_strategy', 'content_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} employer branding", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} employer branding: {data}"
