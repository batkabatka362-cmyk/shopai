...
class StyleGuideEngine(BaseEngine):
    engine_name = "style_guide"
    required_input_fields = ['brand_data', 'guide_config']
    required_output_fields = ['guide_content', 'usage_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} style guide", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} style guide: {data}"
