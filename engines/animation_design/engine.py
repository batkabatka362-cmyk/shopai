...
class AnimationDesignEngine(BaseEngine):
    engine_name = "animation_design"
    required_input_fields = ['element_data', 'animation_config']
    required_output_fields = ['animation_specs', 'easing_curves']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} animation design", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} animation design: {data}"
