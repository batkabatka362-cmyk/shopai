"""Payment Optimization Engine — memory reader.

Reads past payment optimization records from memory storage.
...
def read_past_payments(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past payment optimization records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_payments(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past payment records."""
    if not records:
        return {
            "avg_total_savings": 0.0,
            "total_runs": 0,
        }

    savings = [r.get("total_savings", 0.0) for r in records]
    n = len(records)
    return {
        "avg_total_savings": round(sum(savings) / n, 2) if n else 0.0,
        "total_runs": n,
    }
