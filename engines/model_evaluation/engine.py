...
class ModelEvaluationEngine(BaseEngine):
    engine_name = "model_evaluation"
    required_input_fields = ['model_data', 'evaluation_config']
    required_output_fields = ['evaluation_report', 'performance_metrics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} model evaluation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} model evaluation: {data}"
