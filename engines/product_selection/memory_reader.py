"""Product Selection Engine — memory reader.

Reads past product selection records from memory storage.
Used to inform current selection with historical context.
"""
...
def read_past_results(
    ...
    """Read past product selection records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_results(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past selection records."""
    if not records:
        return {
            "avg_candidate_count": 0.0,
            "avg_shortlist_count": 0.0,
            "total_runs": 0,
        }
    candidate_counts = [r.get("candidate_count", 0) for r in records]
    shortlist_counts = [r.get("shortlist_count", 0) for r in records]
    n = len(records)
    return {
        "avg_candidate_count": round(sum(candidate_counts) / n, 1) if n else 0.0,
        "avg_shortlist_count": round(sum(shortlist_counts) / n, 1) if n else 0.0,
        "total_runs": n,
    }
