...
class WaterfallAnalysisEngine(BaseEngine):
    engine_name = "waterfall_analysis"
    required_input_fields = ['exit_data', 'waterfall_config']
    required_output_fields = ['distribution_waterfall', 'returns_by_class']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} waterfall analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} waterfall analysis: {data}"
