...
class BenchmarkTestingEngine(BaseEngine):
    engine_name = "benchmark_testing"
    required_input_fields = ['code_data', 'benchmark_config']
    required_output_fields = ['benchmark_results', 'regression_detection']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} benchmark testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} benchmark testing: {data}"
