...
class InventoryVisibilityEngine(BaseEngine):
    engine_name = "inventory_visibility"
    required_input_fields = ['inventory_data', 'visibility_config']
    required_output_fields = ['availability_display', 'channel_allocation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} inventory visibility", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} inventory visibility: {data}"
