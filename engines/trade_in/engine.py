...
class TradeInEngine(BaseEngine):
    engine_name = "trade_in"
    required_input_fields = ['product_data', 'trade_config']
    required_output_fields = ['trade_value', 'processing_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} trade in", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} trade in: {data}"
