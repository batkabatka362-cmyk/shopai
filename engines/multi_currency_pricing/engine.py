...
class MultiCurrencyPricingEngine(BaseEngine):
    engine_name = "multi_currency_pricing"
    required_input_fields = ['product_data', 'currency_config']
    required_output_fields = ['localized_prices', 'rounding_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} multi currency pricing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} multi currency pricing: {data}"
