...
class WishlistPageEngine(BaseEngine):
    engine_name = "wishlist_page"
    required_input_fields = ['wishlist_data', 'page_config']
    required_output_fields = ['wishlist_layout', 'sharing_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} wishlist page", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} wishlist page: {data}"
