...
class EarnedValueEngine(BaseEngine):
    engine_name = "earned_value"
    required_input_fields = ['project_data', 'evm_config']
    required_output_fields = ['evm_metrics', 'forecast_at_completion']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} earned value", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} earned value: {data}"
