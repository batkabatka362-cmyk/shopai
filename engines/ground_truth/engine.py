...
class GroundTruthEngine(BaseEngine):
    engine_name = "ground_truth"
    required_input_fields = ['prediction_data', 'label_data']
    required_output_fields = ['accuracy_report', 'feedback_loop']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ground truth", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ground truth: {data}"
