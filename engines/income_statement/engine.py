...
class IncomeStatementEngine(BaseEngine):
    engine_name = "income_statement"
    required_input_fields = ['gl_data', 'is_config']
    required_output_fields = ['income_statement', 'margin_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} income statement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} income statement: {data}"
