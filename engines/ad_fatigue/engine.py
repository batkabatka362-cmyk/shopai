...
class AdFatigueEngine(BaseEngine):
    engine_name = "ad_fatigue"
    required_input_fields = ['campaign_data', 'frequency_data']
    required_output_fields = ['fatigue_scores', 'refresh_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ad fatigue", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ad fatigue: {data}"
