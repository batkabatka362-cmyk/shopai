"""
ProductRecommendationAi Engine
...
class ProductRecommendationAiEngine(BaseEngine):
    engine_name = "product_recommendation_ai"
    required_input_fields = ['user_data', 'product_catalog']
    required_output_fields = ['recommendations', 'relevance_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} product recommendation ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} product recommendation ai: {data}"
