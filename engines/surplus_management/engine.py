...
class SurplusManagementEngine(BaseEngine):
    engine_name = "surplus_management"
    required_input_fields = ['inventory_data', 'surplus_config']
    required_output_fields = ['surplus_plan', 'disposition_options']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} surplus management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} surplus management: {data}"
