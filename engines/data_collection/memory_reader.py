"""Data Collection Engine — memory reader.

Reads past data collection records from memory storage.
Used to inform current collection with historical context.
"""
...
def read_past_collections(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past data collection records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_collections(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past collection records."""
    if not records:
        return {
            "avg_total_records": 0.0,
            "avg_sources_count": 0.0,
            "total_runs": 0,
        }

    total_records = [r.get("total_records", 0) for r in records]
    source_counts = [r.get("sources_count", 0) for r in records]

    n = len(records)
    return {
        "avg_total_records": round(sum(total_records) / n, 2) if n else 0.0,
        "avg_sources_count": round(sum(source_counts) / n, 1) if n else 0.0,
        "total_runs": n,
    }
