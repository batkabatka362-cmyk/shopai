...
class SafetyStockEngine(BaseEngine):
    engine_name = "safety_stock"
    required_input_fields = ['demand_data', 'service_config']
    required_output_fields = ['safety_stock_levels', 'review_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} safety stock", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} safety stock: {data}"
