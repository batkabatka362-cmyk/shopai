...
class MultiTouchAttributionEngine(BaseEngine):
    engine_name = "multi_touch_attribution"
    required_input_fields = ['touchpoint_data', 'model_config']
    required_output_fields = ['attribution_weights', 'model_comparison']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} multi touch attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} multi touch attribution: {data}"
