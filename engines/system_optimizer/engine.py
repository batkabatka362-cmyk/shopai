...
class SystemOptimizerEngine(BaseEngine):
    engine_name = "system_optimizer"
    required_input_fields = ['system_metrics', 'optimization_config']
    required_output_fields = ['optimization_plan', 'expected_improvement']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} system optimizer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} system optimizer: {data}"
