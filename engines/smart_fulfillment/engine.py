...
class SmartFulfillmentEngine(BaseEngine):
    engine_name = "smart_fulfillment"
    required_input_fields = ['order_data', 'warehouse_data']
    required_output_fields = ['fulfillment_plan', 'routing_optimization']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} smart fulfillment", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} smart fulfillment: {data}"
