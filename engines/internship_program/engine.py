...
class InternshipProgramEngine(BaseEngine):
    engine_name = "internship_program"
    required_input_fields = ['intern_data', 'program_config']
    required_output_fields = ['program_plan', 'evaluation_framework']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} internship program", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} internship program: {data}"
