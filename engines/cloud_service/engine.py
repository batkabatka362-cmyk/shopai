...
class CloudServiceEngine(BaseEngine):
    engine_name = "cloud_service"
    required_input_fields = ['cloud_data', 'service_config']
    required_output_fields = ['service_terms', 'security_addendum']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cloud service", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cloud service: {data}"
