...
class DutyDrawbackEngine(BaseEngine):
    engine_name = "duty_drawback"
    required_input_fields = ['export_data', 'drawback_config']
    required_output_fields = ['drawback_claim', 'recovery_amount']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} duty drawback", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} duty drawback: {data}"
