...
class BulkDiscountEngine(BaseEngine):
    engine_name = "bulk_discount"
    required_input_fields = ['order_data', 'discount_config']
    required_output_fields = ['discount_tiers', 'savings_calculator']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} bulk discount", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} bulk discount: {data}"
