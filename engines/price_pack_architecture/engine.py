...
class PricePackArchitectureEngine(BaseEngine):
    engine_name = "price_pack_architecture"
    required_input_fields = ['product_data', 'pack_config']
    required_output_fields = ['pack_options', 'pricing_ladder']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} price pack architecture", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} price pack architecture: {data}"
