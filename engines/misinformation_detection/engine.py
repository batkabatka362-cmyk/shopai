"""
MisinformationDetection Engine
...
class MisinformationDetectionEngine(BaseEngine):
    engine_name = "misinformation_detection"
    required_input_fields = ['content_data', 'detection_config']
    required_output_fields = ['misinfo_scores', 'fact_check_refs']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} misinformation detection", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} misinformation detection: {data}"
