...
class TextAnalyzerAiEngine(BaseEngine):
    engine_name = "text_analyzer_ai"
    required_input_fields = ['text_data', 'analysis_config']
    required_output_fields = ['text_analysis', 'key_findings']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} text analyzer ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} text analyzer ai: {data}"
