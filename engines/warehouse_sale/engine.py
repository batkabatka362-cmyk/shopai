...
class WarehouseSaleEngine(BaseEngine):
    engine_name = "warehouse_sale"
    required_input_fields = ['inventory_data', 'sale_config']
    required_output_fields = ['sale_setup', 'traffic_management']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} warehouse sale", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} warehouse sale: {data}"
