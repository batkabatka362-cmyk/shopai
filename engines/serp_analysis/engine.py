...
class SerpAnalysisEngine(BaseEngine):
    engine_name = "serp_analysis"
    required_input_fields = ['keyword_data', 'serp_config']
    required_output_fields = ['serp_features', 'ranking_opportunities']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} serp analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} serp analysis: {data}"
