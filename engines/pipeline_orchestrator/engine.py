...
class PipelineOrchestratorEngine(BaseEngine):
    engine_name = "pipeline_orchestrator"
    required_input_fields = ['pipeline_data', 'orchestration_config']
    required_output_fields = ['pipeline_dag', 'execution_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pipeline orchestrator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pipeline orchestrator: {data}"
