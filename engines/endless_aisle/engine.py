...
class EndlessAisleEngine(BaseEngine):
    engine_name = "endless_aisle"
    required_input_fields = ['inventory_data', 'aisle_config']
    required_output_fields = ['extended_catalog', 'fulfillment_routing']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} endless aisle", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} endless aisle: {data}"
