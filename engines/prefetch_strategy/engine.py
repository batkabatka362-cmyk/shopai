...
class PrefetchStrategyEngine(BaseEngine):
    engine_name = "prefetch_strategy"
    required_input_fields = ['navigation_data', 'prefetch_config']
    required_output_fields = ['prefetch_rules', 'cache_warming']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} prefetch strategy", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} prefetch strategy: {data}"
