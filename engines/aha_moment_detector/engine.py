...
class AhaMomentDetectorEngine(BaseEngine):
    engine_name = "aha_moment_detector"
    required_input_fields = ['user_data', 'detection_config']
    required_output_fields = ['aha_moments', 'correlation_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} aha moment detector", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} aha moment detector: {data}"
