...
class TextGenerationEngine(BaseEngine):
    engine_name = "text_generation"
    required_input_fields = ['generation_config', 'context_data']
    required_output_fields = ['generated_text', 'quality_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} text generation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} text generation: {data}"
