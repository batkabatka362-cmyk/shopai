...
class EncryptionManagementEngine(BaseEngine):
    engine_name = "encryption_management"
    required_input_fields = ['data_inventory', 'encryption_config']
    required_output_fields = ['encryption_status', 'key_rotation_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} encryption management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} encryption management: {data}"
