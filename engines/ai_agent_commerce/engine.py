...
class AiAgentCommerceEngine(BaseEngine):
    engine_name = "ai_agent_commerce"
    required_input_fields = ['agent_data', 'commerce_config']
    required_output_fields = ['agent_setup', 'transaction_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ai agent commerce", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ai agent commerce: {data}"
