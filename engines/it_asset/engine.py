...
class ItAssetEngine(BaseEngine):
    engine_name = "it_asset"
    required_input_fields = ['it_data', 'management_config']
    required_output_fields = ['asset_inventory', 'lifecycle_status']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} it asset", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} it asset: {data}"
