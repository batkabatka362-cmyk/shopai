...
class EmailOptimizerAiEngine(BaseEngine):
    engine_name = "email_optimizer_ai"
    required_input_fields = ['email_data', 'engagement_data']
    required_output_fields = ['email_improvements', 'open_rate_forecast']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} email optimizer ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} email optimizer ai: {data}"
