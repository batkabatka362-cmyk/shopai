...
class PackagingComplianceEngine(BaseEngine):
    engine_name = "packaging_compliance"
    required_input_fields = ['packaging_data', 'compliance_config']
    required_output_fields = ['compliance_status', 'labeling_requirements']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} packaging compliance", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} packaging compliance: {data}"
