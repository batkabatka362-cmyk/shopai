...
class RevenueSharingEngine(BaseEngine):
    engine_name = "revenue_sharing"
    required_input_fields = ['revenue_data', 'sharing_config']
    required_output_fields = ['sharing_plan', 'payout_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} revenue sharing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} revenue sharing: {data}"
