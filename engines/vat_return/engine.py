...
class VatReturnEngine(BaseEngine):
    engine_name = "vat_return"
    required_input_fields = ['transaction_data', 'vat_config']
    required_output_fields = ['vat_return', 'filing_status']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} vat return", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} vat return: {data}"
