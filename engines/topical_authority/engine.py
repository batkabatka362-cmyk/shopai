...
class TopicalAuthorityEngine(BaseEngine):
    engine_name = "topical_authority"
    required_input_fields = ['content_data', 'topic_config']
    required_output_fields = ['authority_map', 'content_gaps']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} topical authority", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} topical authority: {data}"
