...
class AdvocacyScoreEngine(BaseEngine):
    engine_name = "advocacy_score"
    required_input_fields = ['customer_data', 'advocacy_config']
    required_output_fields = ['advocacy_scores', 'ambassador_candidates']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} advocacy score", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} advocacy score: {data}"
