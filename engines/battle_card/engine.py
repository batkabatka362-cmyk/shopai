...
class BattleCardEngine(BaseEngine):
    engine_name = "battle_card"
    required_input_fields = ['competitor_data', 'product_data']
    required_output_fields = ['battle_cards', 'objection_handling']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} battle card", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} battle card: {data}"
