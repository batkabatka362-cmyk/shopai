...
class SafetyTrainingEngine(BaseEngine):
    engine_name = "safety_training"
    required_input_fields = ['employee_data', 'safety_config']
    required_output_fields = ['training_plan', 'certification_tracking']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} safety training", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} safety training: {data}"
