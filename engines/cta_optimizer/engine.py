...
class CtaOptimizerEngine(BaseEngine):
    engine_name = "cta_optimizer"
    required_input_fields = ['page_data', 'conversion_data']
    required_output_fields = ['cta_variants', 'placement_recommendations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cta optimizer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cta optimizer: {data}"
