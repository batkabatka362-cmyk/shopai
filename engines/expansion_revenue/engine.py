...
class ExpansionRevenueEngine(BaseEngine):
    engine_name = "expansion_revenue"
    required_input_fields = ['customer_data', 'expansion_config']
    required_output_fields = ['expansion_opportunities', 'upsell_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} expansion revenue", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} expansion revenue: {data}"
