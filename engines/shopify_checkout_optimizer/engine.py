"""
ShopifyCheckoutOptimizer Engine
...
class ShopifyCheckoutOptimizerEngine(BaseEngine):
    engine_name = "shopify_checkout_optimizer"
    required_input_fields = ['checkout_data', 'optimization_config']
    required_output_fields = ['checkout_improvements', 'conversion_estimate']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shopify checkout optimizer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shopify checkout optimizer: {data}"
