...
class FraudRuleEngineEngine(BaseEngine):
    engine_name = "fraud_rule_engine"
    required_input_fields = ['transaction_data', 'rule_config']
    required_output_fields = ['fraud_flags', 'risk_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} fraud rule engine", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} fraud rule engine: {data}"
