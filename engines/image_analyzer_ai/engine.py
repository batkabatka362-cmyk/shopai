...
class ImageAnalyzerAiEngine(BaseEngine):
    engine_name = "image_analyzer_ai"
    required_input_fields = ['image_data', 'analysis_config']
    required_output_fields = ['image_analysis', 'tag_suggestions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} image analyzer ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} image analyzer ai: {data}"
