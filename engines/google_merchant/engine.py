...
class GoogleMerchantEngine(BaseEngine):
    engine_name = "google_merchant"
    required_input_fields = ['product_data', 'merchant_config']
    required_output_fields = ['merchant_sync', 'feed_optimization']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} google merchant", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} google merchant: {data}"
