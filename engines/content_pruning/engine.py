...
class ContentPruningEngine(BaseEngine):
    engine_name = "content_pruning"
    required_input_fields = ['content_data', 'pruning_config']
    required_output_fields = ['pruning_plan', 'redirect_map']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} content pruning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} content pruning: {data}"
