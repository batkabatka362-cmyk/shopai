...
class StockPurchaseEngine(BaseEngine):
    engine_name = "stock_purchase"
    required_input_fields = ['stock_data', 'purchase_config']
    required_output_fields = ['purchase_agreement', 'representations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} stock purchase", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} stock purchase: {data}"
