...
class BadgeSystemEngine(BaseEngine):
    engine_name = "badge_system"
    required_input_fields = ['user_data', 'badge_config']
    required_output_fields = ['badge_assignments', 'unlock_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} badge system", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} badge system: {data}"
