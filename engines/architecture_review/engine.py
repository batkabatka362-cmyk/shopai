...
class ArchitectureReviewEngine(BaseEngine):
    engine_name = "architecture_review"
    required_input_fields = ['system_data', 'review_config']
    required_output_fields = ['review_report', 'recommendations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} architecture review", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} architecture review: {data}"
