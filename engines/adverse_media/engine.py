...
class AdverseMediaEngine(BaseEngine):
    engine_name = "adverse_media"
    required_input_fields = ['entity_data', 'media_config']
    required_output_fields = ['media_hits', 'sentiment_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} adverse media", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} adverse media: {data}"
