...
class KaizenEngine(BaseEngine):
    engine_name = "kaizen"
    required_input_fields = ['process_data', 'improvement_config']
    required_output_fields = ['improvement_plan', 'impact_estimate']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} kaizen", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} kaizen: {data}"
