...
class VolumePricingEngine(BaseEngine):
    engine_name = "volume_pricing"
    required_input_fields = ['order_data', 'volume_config']
    required_output_fields = ['volume_discounts', 'break_points']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} volume pricing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} volume pricing: {data}"
