"""
AffiliateReconciliation Engine
...
class AffiliateReconciliationEngine(BaseEngine):
    engine_name = "affiliate_reconciliation"
    required_input_fields = ['affiliate_data', 'conversion_data']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} affiliate reconciliation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} affiliate reconciliation: {data}"
