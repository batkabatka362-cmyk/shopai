...
class TechnicalDebtEngine(BaseEngine):
    engine_name = "technical_debt"
    required_input_fields = ['codebase_data', 'debt_config']
    required_output_fields = ['debt_inventory', 'payoff_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} technical debt", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} technical debt: {data}"
