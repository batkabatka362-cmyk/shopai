...
class AmpManagementEngine(BaseEngine):
    engine_name = "amp_management"
    required_input_fields = ['page_data', 'amp_config']
    required_output_fields = ['amp_versions', 'validation_status']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} amp management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} amp management: {data}"
