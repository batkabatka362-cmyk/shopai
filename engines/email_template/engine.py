...
class EmailTemplateEngine(BaseEngine):
    engine_name = "email_template"
    required_input_fields = ['template_config', 'brand_assets']
    required_output_fields = ['email_html', 'responsive_preview']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} email template", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} email template: {data}"
