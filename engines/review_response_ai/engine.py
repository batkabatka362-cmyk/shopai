...
class ReviewResponseAiEngine(BaseEngine):
    engine_name = "review_response_ai"
    required_input_fields = ['review_data', 'brand_voice']
    required_output_fields = ['response_drafts', 'escalation_flags']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} review response ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} review response ai: {data}"
