...
class SurveyDesignEngine(BaseEngine):
    engine_name = "survey_design"
    required_input_fields = ['research_goals', 'survey_config']
    required_output_fields = ['survey_questions', 'methodology']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} survey design", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} survey design: {data}"
