...
class ClusteringModelEngine(BaseEngine):
    engine_name = "clustering_model"
    required_input_fields = ['feature_data', 'clustering_config']
    required_output_fields = ['cluster_assignments', 'cluster_profiles']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} clustering model", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} clustering model: {data}"
