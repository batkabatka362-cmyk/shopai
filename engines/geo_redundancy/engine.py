...
class GeoRedundancyEngine(BaseEngine):
    engine_name = "geo_redundancy"
    required_input_fields = ['infrastructure_data', 'redundancy_config']
    required_output_fields = ['redundancy_plan', 'failover_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} geo redundancy", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} geo redundancy: {data}"
