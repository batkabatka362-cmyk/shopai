...
class CapableToPromiseEngine(BaseEngine):
    engine_name = "capable_to_promise"
    required_input_fields = ['capacity_data', 'order_data']
    required_output_fields = ['ctp_dates', 'capacity_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} capable to promise", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} capable to promise: {data}"
