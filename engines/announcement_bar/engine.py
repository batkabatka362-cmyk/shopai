...
class AnnouncementBarEngine(BaseEngine):
    engine_name = "announcement_bar"
    required_input_fields = ['announcement_data', 'display_config']
    required_output_fields = ['bar_content', 'scheduling_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} announcement bar", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} announcement bar: {data}"
