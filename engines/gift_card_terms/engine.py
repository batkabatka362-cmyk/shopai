...
class GiftCardTermsEngine(BaseEngine):
    engine_name = "gift_card_terms"
    required_input_fields = ['card_data', 'terms_config']
    required_output_fields = ['gift_card_terms', 'regulatory_compliance']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} gift card terms", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} gift card terms: {data}"
