...
class DataCleanRoomEngine(BaseEngine):
    engine_name = "data_clean_room"
    required_input_fields = ['dataset_config', 'room_config']
    required_output_fields = ['clean_room_setup', 'query_templates']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} data clean room", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} data clean room: {data}"
