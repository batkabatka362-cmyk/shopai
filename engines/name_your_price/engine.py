...
class NameYourPriceEngine(BaseEngine):
    engine_name = "name_your_price"
    required_input_fields = ['product_data', 'nyp_config']
    required_output_fields = ['nyp_setup', 'minimum_thresholds']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} name your price", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} name your price: {data}"
