...
class TeamingAgreementEngine(BaseEngine):
    engine_name = "teaming_agreement"
    required_input_fields = ['team_data', 'agreement_config']
    required_output_fields = ['teaming_terms', 'bid_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} teaming agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} teaming agreement: {data}"
