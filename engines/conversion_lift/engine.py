...
class ConversionLiftEngine(BaseEngine):
    engine_name = "conversion_lift"
    required_input_fields = ['experiment_data', 'lift_config']
    required_output_fields = ['lift_results', 'incremental_conversions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} conversion lift", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} conversion lift: {data}"
