...
class RepeatContactEngine(BaseEngine):
    engine_name = "repeat_contact"
    required_input_fields = ['contact_data', 'analysis_config']
    required_output_fields = ['repeat_rate', 'root_causes']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} repeat contact", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} repeat contact: {data}"
