...
class FrequencyCappingEngine(BaseEngine):
    engine_name = "frequency_capping"
    required_input_fields = ['contact_data', 'capping_config']
    required_output_fields = ['cap_rules', 'fatigue_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} frequency capping", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} frequency capping: {data}"
