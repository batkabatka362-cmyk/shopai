...
class PenetrationTestingEngine(BaseEngine):
    engine_name = "penetration_testing"
    required_input_fields = ['target_data', 'pentest_config']
    required_output_fields = ['pentest_report', 'exploit_findings']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} penetration testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} penetration testing: {data}"
