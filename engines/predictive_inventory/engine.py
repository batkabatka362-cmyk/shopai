...
class PredictiveInventoryEngine(BaseEngine):
    engine_name = "predictive_inventory"
    required_input_fields = ['sales_data', 'supply_data']
    required_output_fields = ['inventory_forecast', 'order_suggestions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} predictive inventory", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} predictive inventory: {data}"
