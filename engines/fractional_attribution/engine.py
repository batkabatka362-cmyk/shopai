...
class FractionalAttributionEngine(BaseEngine):
    engine_name = "fractional_attribution"
    required_input_fields = ['touchpoint_data', 'fractional_config']
    required_output_fields = ['fractional_weights', 'interaction_credit']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} fractional attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} fractional attribution: {data}"
