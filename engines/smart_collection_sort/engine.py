...
class SmartCollectionSortEngine(BaseEngine):
    engine_name = "smart_collection_sort"
    required_input_fields = ['collection_data', 'performance_data']
    required_output_fields = ['sort_algorithm', 'revenue_impact']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} smart collection sort", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} smart collection sort: {data}"
