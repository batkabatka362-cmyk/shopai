...
class ShopifySyncManagerEngine(BaseEngine):
    engine_name = "shopify_sync_manager"
    required_input_fields = ['sync_config', 'data_sources']
    required_output_fields = ['sync_plan', 'conflict_resolution']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shopify sync manager", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shopify sync manager: {data}"
