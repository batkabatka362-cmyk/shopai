...
class SegmentIntegrationEngine(BaseEngine):
    engine_name = "segment_integration"
    required_input_fields = ['tracking_data', 'integration_config']
    required_output_fields = ['segment_setup', 'source_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} segment integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} segment integration: {data}"
