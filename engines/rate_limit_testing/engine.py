...
class RateLimitTestingEngine(BaseEngine):
    engine_name = "rate_limit_testing"
    required_input_fields = ['endpoint_data', 'test_config']
    required_output_fields = ['rate_limit_report', 'bypass_findings']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} rate limit testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} rate limit testing: {data}"
