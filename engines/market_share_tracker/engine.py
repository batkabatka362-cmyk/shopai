...
class MarketShareTrackerEngine(BaseEngine):
    engine_name = "market_share_tracker"
    required_input_fields = ['sales_data', 'market_data']
    required_output_fields = ['share_estimate', 'trend_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} market share tracker", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} market share tracker: {data}"
