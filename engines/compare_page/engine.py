...
class ComparePageEngine(BaseEngine):
    engine_name = "compare_page"
    required_input_fields = ['product_data', 'compare_config']
    required_output_fields = ['compare_layout', 'attribute_matrix']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} compare page", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} compare page: {data}"
