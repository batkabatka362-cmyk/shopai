...
class MarketplaceSellerTermsEngine(BaseEngine):
    engine_name = "marketplace_seller_terms"
    required_input_fields = ['marketplace_data', 'terms_config']
    required_output_fields = ['seller_terms', 'commission_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} marketplace seller terms", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} marketplace seller terms: {data}"
