...
class ToneAnalyzerEngine(BaseEngine):
    engine_name = "tone_analyzer"
    required_input_fields = ['text_data', 'tone_config']
    required_output_fields = ['tone_scores', 'tone_recommendations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} tone analyzer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} tone analyzer: {data}"
