...
class SensitivityAnalysisEngine(BaseEngine):
    engine_name = "sensitivity_analysis"
    required_input_fields = ['model_data', 'analysis_config']
    required_output_fields = ['tornado_chart', 'key_drivers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sensitivity analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sensitivity analysis: {data}"
