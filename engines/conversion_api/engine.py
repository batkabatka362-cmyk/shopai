...
class ConversionApiEngine(BaseEngine):
    engine_name = "conversion_api"
    required_input_fields = ['event_data', 'api_config']
    required_output_fields = ['capi_setup', 'event_deduplication']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} conversion api", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} conversion api: {data}"
