...
class BrandCampaignEngine(BaseEngine):
    engine_name = "brand_campaign"
    required_input_fields = ['brand_data', 'campaign_config']
    required_output_fields = ['campaign_setup', 'protection_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} brand campaign", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} brand campaign: {data}"
