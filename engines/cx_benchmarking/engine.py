...
class CxBenchmarkingEngine(BaseEngine):
    engine_name = "cx_benchmarking"
    required_input_fields = ['cx_data', 'benchmark_config']
    required_output_fields = ['benchmark_report', 'competitive_position']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cx benchmarking", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cx benchmarking: {data}"
