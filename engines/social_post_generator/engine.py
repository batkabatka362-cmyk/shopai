...
class SocialPostGeneratorEngine(BaseEngine):
    engine_name = "social_post_generator"
    required_input_fields = ['content_brief', 'platform_rules']
    required_output_fields = ['post_variants', 'media_specs']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} social post generator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} social post generator: {data}"
