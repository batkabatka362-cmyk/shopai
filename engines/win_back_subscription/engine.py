...
class WinBackSubscriptionEngine(BaseEngine):
    engine_name = "win_back_subscription"
    required_input_fields = ['churned_data', 'win_back_config']
    required_output_fields = ['win_back_offers', 'reactivation_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} win back subscription", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} win back subscription: {data}"
