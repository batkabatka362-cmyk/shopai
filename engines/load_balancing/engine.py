...
class LoadBalancingEngine(BaseEngine):
    engine_name = "load_balancing"
    required_input_fields = ['server_data', 'balancing_config']
    required_output_fields = ['balancing_strategy', 'health_checks']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} load balancing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} load balancing: {data}"
