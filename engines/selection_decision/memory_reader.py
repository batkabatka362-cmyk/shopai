"""Selection Decision Engine — memory reader.

Reads past selection decision records from memory storage.
...
def read_past_decisions(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past selection decision records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_decisions(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past decision records."""
    if not records:
        return {
    ...
    selected_counts = [r.get("selected_count", 0) for r in records]
    balance_scores = [r.get("balance_score", 0.0) for r in records]

    n = len(records)
    return {
        "avg_selected_count": round(sum(selected_counts) / n, 1) if n else 0.0,
        "avg_balance_score": round(sum(balance_scores) / n, 3) if n else 0.0,
        "total_runs": n,
    }
