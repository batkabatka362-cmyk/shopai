"""
PrintOnDemand Engine — Print On Demand
...
class PrintOnDemandEngine(BaseEngine):
    engine_name = "print_on_demand"
    required_input_fields = ['design_data', 'product_options']
    required_output_fields = ['pod_listings', 'mockup_specs']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} print on demand", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} print on demand: {data}"
