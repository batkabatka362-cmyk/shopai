...
class CreatorProgramEngine(BaseEngine):
    engine_name = "creator_program"
    required_input_fields = ['creator_data', 'program_config']
    required_output_fields = ['program_setup', 'commission_structure']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} creator program", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} creator program: {data}"
