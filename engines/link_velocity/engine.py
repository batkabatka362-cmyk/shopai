...
class LinkVelocityEngine(BaseEngine):
    engine_name = "link_velocity"
    required_input_fields = ['link_data', 'velocity_config']
    required_output_fields = ['velocity_trend', 'anomaly_detection']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} link velocity", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} link velocity: {data}"
