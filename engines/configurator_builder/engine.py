...
class ConfiguratorBuilderEngine(BaseEngine):
    engine_name = "configurator_builder"
    required_input_fields = ['option_data', 'configurator_config']
    required_output_fields = ['configurator_flow', 'pricing_logic']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} configurator builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} configurator builder: {data}"
