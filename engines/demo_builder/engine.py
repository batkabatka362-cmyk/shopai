...
class DemoBuilderEngine(BaseEngine):
    engine_name = "demo_builder"
    required_input_fields = ['product_data', 'demo_config']
    required_output_fields = ['demo_flow', 'talking_points']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} demo builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} demo builder: {data}"
