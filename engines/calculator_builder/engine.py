...
class CalculatorBuilderEngine(BaseEngine):
    engine_name = "calculator_builder"
    required_input_fields = ['formula_data', 'calculator_config']
    required_output_fields = ['calculator_setup', 'ui_layout']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} calculator builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} calculator builder: {data}"
