...
class ChurnModelEngine(BaseEngine):
    engine_name = "churn_model"
    required_input_fields = ['customer_data', 'model_config']
    required_output_fields = ['churn_probabilities', 'risk_factors']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} churn model", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} churn model: {data}"
