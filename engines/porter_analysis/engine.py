...
class PorterAnalysisEngine(BaseEngine):
    engine_name = "porter_analysis"
    required_input_fields = ['industry_data', 'competitive_data']
    required_output_fields = ['five_forces', 'competitive_position']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} porter analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} porter analysis: {data}"
