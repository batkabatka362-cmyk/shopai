...
class OmnichannelRoutingEngine(BaseEngine):
    engine_name = "omnichannel_routing"
    required_input_fields = ['interaction_data', 'routing_config']
    required_output_fields = ['routing_decisions', 'channel_handoff']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} omnichannel routing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} omnichannel routing: {data}"
