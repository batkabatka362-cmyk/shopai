...
class WardrobePlanningEngine(BaseEngine):
    engine_name = "wardrobe_planning"
    required_input_fields = ['wardrobe_data', 'style_config']
    required_output_fields = ['wardrobe_plan', 'gap_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} wardrobe planning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} wardrobe planning: {data}"
