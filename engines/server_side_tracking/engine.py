...
class ServerSideTrackingEngine(BaseEngine):
    engine_name = "server_side_tracking"
    required_input_fields = ['tracking_data', 'server_config']
    required_output_fields = ['server_setup', 'endpoint_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} server side tracking", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} server side tracking: {data}"
