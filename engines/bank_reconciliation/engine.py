...
class BankReconciliationEngine(BaseEngine):
    engine_name = "bank_reconciliation"
    required_input_fields = ['bank_data', 'gl_data']
    required_output_fields = ['reconciliation_report', 'discrepancies']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} bank reconciliation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} bank reconciliation: {data}"
