"""
IncrementalityAttribution Engine
...
class IncrementalityAttributionEngine(BaseEngine):
    engine_name = "incrementality_attribution"
    required_input_fields = ['experiment_data', 'incrementality_config']
    required_output_fields = ['incremental_value', 'counterfactual_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} incrementality attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} incrementality attribution: {data}"
