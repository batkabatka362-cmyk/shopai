...
class FeedbackCategorizationEngine(BaseEngine):
    engine_name = "feedback_categorization"
    required_input_fields = ['feedback_data', 'categorization_config']
    required_output_fields = ['categories', 'priority_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} feedback categorization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} feedback categorization: {data}"
