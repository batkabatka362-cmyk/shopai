...
class PinterestFeedEngine(BaseEngine):
    engine_name = "pinterest_feed"
    required_input_fields = ['pinterest_data', 'feed_config']
    required_output_fields = ['feed_display', 'board_filter']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pinterest feed", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pinterest feed: {data}"
