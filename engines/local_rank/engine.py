...
class LocalRankEngine(BaseEngine):
    engine_name = "local_rank"
    required_input_fields = ['keyword_data', 'location_config']
    required_output_fields = ['local_rankings', 'map_pack_status']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} local rank", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} local rank: {data}"
