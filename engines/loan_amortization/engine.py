...
class LoanAmortizationEngine(BaseEngine):
    engine_name = "loan_amortization"
    required_input_fields = ['loan_data', 'amortization_config']
    required_output_fields = ['payment_schedule', 'interest_breakdown']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} loan amortization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} loan amortization: {data}"
