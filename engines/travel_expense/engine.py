...
class TravelExpenseEngine(BaseEngine):
    engine_name = "travel_expense"
    required_input_fields = ['expense_data', 'policy_config']
    required_output_fields = ['approval_status', 'reimbursement_calc']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} travel expense", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} travel expense: {data}"
