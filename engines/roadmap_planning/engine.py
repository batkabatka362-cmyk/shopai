...
class RoadmapPlanningEngine(BaseEngine):
    engine_name = "roadmap_planning"
    required_input_fields = ['initiative_data', 'timeline_config']
    required_output_fields = ['roadmap', 'milestone_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} roadmap planning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} roadmap planning: {data}"
