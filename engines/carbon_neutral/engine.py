...
class CarbonNeutralEngine(BaseEngine):
    engine_name = "carbon_neutral"
    required_input_fields = ['emission_data', 'neutrality_config']
    required_output_fields = ['neutrality_plan', 'offset_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} carbon neutral", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} carbon neutral: {data}"
