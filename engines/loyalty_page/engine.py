...
class LoyaltyPageEngine(BaseEngine):
    engine_name = "loyalty_page"
    required_input_fields = ['loyalty_data', 'page_config']
    required_output_fields = ['program_display', 'tier_comparison']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} loyalty page", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} loyalty page: {data}"
