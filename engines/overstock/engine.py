...
class OverstockEngine(BaseEngine):
    engine_name = "overstock"
    required_input_fields = ['inventory_data', 'overstock_config']
    required_output_fields = ['liquidation_plan', 'channel_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} overstock", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} overstock: {data}"
