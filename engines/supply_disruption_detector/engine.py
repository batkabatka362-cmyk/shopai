"""
SupplyDisruptionDetector Engine
...
class SupplyDisruptionDetectorEngine(BaseEngine):
    engine_name = "supply_disruption_detector"
    required_input_fields = ['supply_data', 'risk_config']
    required_output_fields = ['disruption_alerts', 'contingency_plans']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} supply disruption detector", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} supply disruption detector: {data}"
