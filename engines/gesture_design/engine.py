...
class GestureDesignEngine(BaseEngine):
    engine_name = "gesture_design"
    required_input_fields = ['interface_data', 'gesture_config']
    required_output_fields = ['gesture_map', 'touch_targets']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} gesture design", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} gesture design: {data}"
