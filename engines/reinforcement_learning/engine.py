...
class ReinforcementLearningEngine(BaseEngine):
    engine_name = "reinforcement_learning"
    required_input_fields = ['environment_data', 'rl_config']
    required_output_fields = ['policy', 'reward_history']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} reinforcement learning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} reinforcement learning: {data}"
