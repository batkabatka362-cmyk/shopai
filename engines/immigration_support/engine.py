...
class ImmigrationSupportEngine(BaseEngine):
    engine_name = "immigration_support"
    required_input_fields = ['employee_data', 'immigration_config']
    required_output_fields = ['visa_tracking', 'compliance_status']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} immigration support", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} immigration support: {data}"
