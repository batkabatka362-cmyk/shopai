...
class ViewthroughAttributionEngine(BaseEngine):
    engine_name = "viewthrough_attribution"
    required_input_fields = ['impression_data', 'attribution_config']
    required_output_fields = ['viewthrough_report', 'assisted_conversions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} viewthrough attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} viewthrough attribution: {data}"
