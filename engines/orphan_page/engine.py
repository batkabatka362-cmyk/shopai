...
class OrphanPageEngine(BaseEngine):
    engine_name = "orphan_page"
    required_input_fields = ['site_data', 'analysis_config']
    required_output_fields = ['orphan_pages', 'linking_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} orphan page", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} orphan page: {data}"
