...
class UrgencyAiEngine(BaseEngine):
    engine_name = "urgency_ai"
    required_input_fields = ['product_data', 'inventory_data']
    required_output_fields = ['urgency_elements', 'scarcity_messages']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} urgency ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} urgency ai: {data}"
