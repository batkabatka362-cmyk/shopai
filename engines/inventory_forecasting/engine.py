"""
InventoryForecasting Engine — Inventory Forecasting
...
class InventoryForecastingEngine(BaseEngine):
    engine_name = "inventory_forecasting"
    required_input_fields = ['inventory_data', 'sales_history']
    required_output_fields = ['forecast', 'reorder_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} inventory forecasting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} inventory forecasting: {data}"
