...
class PortfolioBidEngine(BaseEngine):
    engine_name = "portfolio_bid"
    required_input_fields = ['campaign_data', 'portfolio_config']
    required_output_fields = ['portfolio_strategy', 'cross_campaign_allocation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} portfolio bid", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} portfolio bid: {data}"
