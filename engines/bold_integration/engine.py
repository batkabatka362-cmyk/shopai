...
class BoldIntegrationEngine(BaseEngine):
    engine_name = "bold_integration"
    required_input_fields = ['app_data', 'integration_config']
    required_output_fields = ['bold_setup', 'feature_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} bold integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} bold integration: {data}"
