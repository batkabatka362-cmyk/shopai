...
class EmailAnalyticsDeepEngine(BaseEngine):
    engine_name = "email_analytics_deep"
    required_input_fields = ['campaign_data', 'analytics_config']
    required_output_fields = ['deep_report', 'segment_performance']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} email analytics deep", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} email analytics deep: {data}"
