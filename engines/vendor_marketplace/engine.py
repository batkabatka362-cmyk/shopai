...
class VendorMarketplaceEngine(BaseEngine):
    engine_name = "vendor_marketplace"
    required_input_fields = ['vendor_data', 'marketplace_config']
    required_output_fields = ['marketplace_setup', 'commission_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} vendor marketplace", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} vendor marketplace: {data}"
