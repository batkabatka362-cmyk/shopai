...
class SwotAnalysisEngine(BaseEngine):
    engine_name = "swot_analysis"
    required_input_fields = ['business_data', 'market_data']
    required_output_fields = ['swot_matrix', 'strategic_recommendations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} swot analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} swot analysis: {data}"
