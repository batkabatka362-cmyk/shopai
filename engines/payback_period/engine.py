...
class PaybackPeriodEngine(BaseEngine):
    engine_name = "payback_period"
    required_input_fields = ['cac_data', 'ltv_data']
    required_output_fields = ['payback_analysis', 'investment_recommendation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} payback period", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} payback period: {data}"
