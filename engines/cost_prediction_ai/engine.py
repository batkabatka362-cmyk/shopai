...
class CostPredictionAiEngine(BaseEngine):
    engine_name = "cost_prediction_ai"
    required_input_fields = ['cost_data', 'inflation_data']
    required_output_fields = ['cost_forecast', 'budget_impact']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cost prediction ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cost prediction ai: {data}"
