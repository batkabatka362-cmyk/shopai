...
class TimeDecayAttributionEngine(BaseEngine):
    engine_name = "time_decay_attribution"
    required_input_fields = ['touchpoint_data', 'decay_config']
    required_output_fields = ['attribution_weights', 'half_life_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} time decay attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} time decay attribution: {data}"
