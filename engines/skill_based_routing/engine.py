...
class SkillBasedRoutingEngine(BaseEngine):
    engine_name = "skill_based_routing"
    required_input_fields = ['request_data', 'skill_config']
    required_output_fields = ['routing_decision', 'skill_match_score']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} skill based routing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} skill based routing: {data}"
