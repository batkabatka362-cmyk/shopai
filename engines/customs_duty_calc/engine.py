...
class CustomsDutyCalcEngine(BaseEngine):
    engine_name = "customs_duty_calc"
    required_input_fields = ['product_data', 'destination_config']
    required_output_fields = ['duty_estimate', 'landed_cost']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} customs duty calc", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} customs duty calc: {data}"
