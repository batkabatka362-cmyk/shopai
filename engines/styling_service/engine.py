...
class StylingServiceEngine(BaseEngine):
    engine_name = "styling_service"
    required_input_fields = ['customer_data', 'styling_config']
    required_output_fields = ['style_recommendations', 'outfit_suggestions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} styling service", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} styling service: {data}"
