...
class CanaryDeploymentEngine(BaseEngine):
    engine_name = "canary_deployment"
    required_input_fields = ['deployment_data', 'canary_config']
    required_output_fields = ['canary_plan', 'rollback_criteria']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} canary deployment", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} canary deployment: {data}"
