...
class PortfolioBuilderEngine(BaseEngine):
    engine_name = "portfolio_builder"
    required_input_fields = ['work_data', 'portfolio_config']
    required_output_fields = ['portfolio_layout', 'showcase_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} portfolio builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} portfolio builder: {data}"
