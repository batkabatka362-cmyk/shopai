...
class DcfValuationEngine(BaseEngine):
    engine_name = "dcf_valuation"
    required_input_fields = ['financial_data', 'dcf_config']
    required_output_fields = ['intrinsic_value', 'sensitivity_table']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dcf valuation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dcf valuation: {data}"
