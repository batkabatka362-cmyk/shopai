...
class DataCatalogMgmtEngine(BaseEngine):
    engine_name = "data_catalog_mgmt"
    required_input_fields = ['data_assets', 'catalog_config']
    required_output_fields = ['catalog_entries', 'search_index']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} data catalog mgmt", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} data catalog mgmt: {data}"
