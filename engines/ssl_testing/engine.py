...
class SslTestingEngine(BaseEngine):
    engine_name = "ssl_testing"
    required_input_fields = ['cert_data', 'test_config']
    required_output_fields = ['ssl_report', 'vulnerability_check']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ssl testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ssl testing: {data}"
