...
class BalanceSheetEngine(BaseEngine):
    engine_name = "balance_sheet"
    required_input_fields = ['gl_data', 'bs_config']
    required_output_fields = ['balance_sheet', 'ratio_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} balance sheet", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} balance sheet: {data}"
