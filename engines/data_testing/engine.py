...
class DataTestingEngine(BaseEngine):
    engine_name = "data_testing"
    required_input_fields = ['dataset', 'test_config']
    required_output_fields = ['quality_report', 'anomaly_flags']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} data testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} data testing: {data}"
