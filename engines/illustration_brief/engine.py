...
class IllustrationBriefEngine(BaseEngine):
    engine_name = "illustration_brief"
    required_input_fields = ['concept_data', 'brief_config']
    required_output_fields = ['illustration_brief', 'style_reference']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} illustration brief", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} illustration brief: {data}"
