...
class CrossCulturalEngine(BaseEngine):
    engine_name = "cross_cultural"
    required_input_fields = ['assignment_data', 'cultural_config']
    required_output_fields = ['cultural_training', 'adaptation_support']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cross cultural", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cross cultural: {data}"
