...
class PatentAnalysisEngine(BaseEngine):
    engine_name = "patent_analysis"
    required_input_fields = ['patent_data', 'analysis_config']
    required_output_fields = ['patent_landscape', 'innovation_gaps']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} patent analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} patent analysis: {data}"
