...
class ModelDistillationEngine(BaseEngine):
    engine_name = "model_distillation"
    required_input_fields = ['teacher_model', 'student_config']
    required_output_fields = ['student_model', 'quality_metrics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} model distillation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} model distillation: {data}"
