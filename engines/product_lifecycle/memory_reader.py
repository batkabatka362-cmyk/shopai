"""Product Lifecycle Engine — memory reader.

Reads past lifecycle assessment records from memory storage.
...
def read_past_lifecycles(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past lifecycle records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_lifecycles(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past lifecycle records."""
    if not records:
        return {
            "stage_distribution": {},
            "total_runs": 0,
