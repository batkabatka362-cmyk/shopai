...
class UserFlowDesignEngine(BaseEngine):
    engine_name = "user_flow_design"
    required_input_fields = ['task_data', 'flow_config']
    required_output_fields = ['user_flows', 'decision_points']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} user flow design", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} user flow design: {data}"
