...
class LeanManagementEngine(BaseEngine):
    engine_name = "lean_management"
    required_input_fields = ['process_data', 'lean_config']
    required_output_fields = ['waste_analysis', 'improvement_actions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} lean management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} lean management: {data}"
