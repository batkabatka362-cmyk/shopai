...
class WorkersCompEngine(BaseEngine):
    engine_name = "workers_comp"
    required_input_fields = ['employee_data', 'comp_config']
    required_output_fields = ['coverage_plan', 'claim_process']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} workers comp", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} workers comp: {data}"
