...
class PriceRangeSchemaEngine(BaseEngine):
    engine_name = "price_range_schema"
    required_input_fields = ['pricing_data', 'schema_config']
    required_output_fields = ['price_markup', 'range_display']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} price range schema", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} price range schema: {data}"
