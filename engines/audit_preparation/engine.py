...
class AuditPreparationEngine(BaseEngine):
    engine_name = "audit_preparation"
    required_input_fields = ['financial_data', 'audit_config']
    required_output_fields = ['preparation_checklist', 'document_package']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} audit preparation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} audit preparation: {data}"
