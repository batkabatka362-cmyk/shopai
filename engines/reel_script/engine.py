...
class ReelScriptEngine(BaseEngine):
    engine_name = "reel_script"
    required_input_fields = ['product_data', 'trend_data']
    required_output_fields = ['reel_scripts', 'hook_options']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} reel script", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} reel script: {data}"
