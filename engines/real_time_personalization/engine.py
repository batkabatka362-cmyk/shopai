"""
RealTimePersonalization Engine
...
class RealTimePersonalizationEngine(BaseEngine):
    engine_name = "real_time_personalization"
    required_input_fields = ['visitor_data', 'behavior_data']
    required_output_fields = ['live_recommendations', 'display_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} real time personalization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} real time personalization: {data}"
