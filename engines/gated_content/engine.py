...
class GatedContentEngine(BaseEngine):
    engine_name = "gated_content"
    required_input_fields = ['content_data', 'gate_config']
    required_output_fields = ['gate_setup', 'lead_capture']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} gated content", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} gated content: {data}"
