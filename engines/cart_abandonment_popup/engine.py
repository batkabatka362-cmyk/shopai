...
class CartAbandonmentPopupEngine(BaseEngine):
    engine_name = "cart_abandonment_popup"
    required_input_fields = ['cart_data', 'popup_config']
    required_output_fields = ['popup_content', 'trigger_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cart abandonment popup", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cart abandonment popup: {data}"
