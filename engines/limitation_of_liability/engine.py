...
class LimitationOfLiabilityEngine(BaseEngine):
    engine_name = "limitation_of_liability"
    required_input_fields = ['contract_data', 'liability_config']
    required_output_fields = ['liability_analysis', 'cap_recommendation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} limitation of liability", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} limitation of liability: {data}"
