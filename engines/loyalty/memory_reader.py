"""Loyalty Engine — memory reader.

Reads past loyalty program records from memory storage.
Used to inform current loyalty analysis with historical context.
"""
...
def read_past_loyalty(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past loyalty program records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_loyalty(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past loyalty records."""
    if not records:
        return {
            "avg_total_members": 0,
            "avg_active_members": 0,
            "avg_points_outstanding": 0,
            "total_runs": 0,
        }

    members = [r.get("total_members", 0) for r in records]
    active = [r.get("active_members", 0) for r in records]
    points = [r.get("total_points_outstanding", 0) for r in records]

    n = len(records)
    return {
        "avg_total_members": round(sum(members) / n) if n else 0,
        "avg_active_members": round(sum(active) / n) if n else 0,
        "avg_points_outstanding": round(sum(points) / n) if n else 0,
        "total_runs": n,
    }
