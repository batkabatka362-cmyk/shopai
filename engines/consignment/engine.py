...
class ConsignmentEngine(BaseEngine):
    engine_name = "consignment"
    required_input_fields = ['product_data', 'consignment_config']
    required_output_fields = ['consignment_terms', 'settlement_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} consignment", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} consignment: {data}"
