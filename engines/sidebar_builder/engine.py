...
class SidebarBuilderEngine(BaseEngine):
    engine_name = "sidebar_builder"
    required_input_fields = ['widget_data', 'sidebar_config']
    required_output_fields = ['sidebar_layout', 'widget_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sidebar builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sidebar builder: {data}"
