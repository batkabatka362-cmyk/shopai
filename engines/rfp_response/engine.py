...
class RfpResponseEngine(BaseEngine):
    engine_name = "rfp_response"
    required_input_fields = ['rfp_data', 'company_capabilities']
    required_output_fields = ['rfp_response', 'compliance_matrix']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} rfp response", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} rfp response: {data}"
