...
class AssetPurchaseEngine(BaseEngine):
    engine_name = "asset_purchase"
    required_input_fields = ['asset_data', 'purchase_config']
    required_output_fields = ['purchase_agreement', 'schedule_of_assets']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} asset purchase", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} asset purchase: {data}"
