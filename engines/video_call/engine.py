...
class VideoCallEngine(BaseEngine):
    engine_name = "video_call"
    required_input_fields = ['call_data', 'video_config']
    required_output_fields = ['call_setup', 'recording_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} video call", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} video call: {data}"
