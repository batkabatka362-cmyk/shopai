"""Customer outreach autonomy status surface (Wave 383)."""
...
@dataclass
class CustomerOutreachStatusReport:
    window_hours: float
...
def get_customer_outreach_status(
    *,
    window_hours: float = 168.0,
    store_id: str | None = None,
) -> CustomerOutreachStatusReport:
    report = CustomerOutreachStatusReport(
    ...
    health = analyze_customer_outreach_health(
        window_hours=window_hours,
    )
    ...
    if report.paused:
        ...
        report.next_action = (
            "Resume via `shopai customer-outreach-resume`."
        )
    elif report.health_verdict == "critical":
        report.verdict = "degraded"
        report.verdict_reasons.append(
            f"outreach failure ratio "
            f"{report.health_failure_ratio:.0%} >= critical"
        )
        report.next_action = (
            "`shopai customer-outreach-health --apply-bridge`."
        )
        ...
        report.verdict_reasons.append(
            "no outreach actions in window"
        )
        report.next_action = (
            "Enable via data.apply_customer_outreach=True."
        )
    else:
        report.verdict = "healthy"
        report.verdict_reasons.append(
            f"{report.applied_count} customer(s) tagged"
        )
