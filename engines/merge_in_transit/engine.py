...
class MergeInTransitEngine(BaseEngine):
    engine_name = "merge_in_transit"
    required_input_fields = ['shipment_data', 'merge_config']
    required_output_fields = ['merge_plan', 'synchronization']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} merge in transit", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} merge in transit: {data}"
