...
class EventStormingEngine(BaseEngine):
    engine_name = "event_storming"
    required_input_fields = ['domain_data', 'workshop_config']
    required_output_fields = ['event_map', 'aggregate_boundaries']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} event storming", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} event storming: {data}"
