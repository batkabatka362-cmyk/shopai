"""
PrivacyPreservingAttribution Engine
...
class PrivacyPreservingAttributionEngine(BaseEngine):
    engine_name = "privacy_preserving_attribution"
    required_input_fields = ['aggregated_data', 'privacy_config']
    required_output_fields = ['private_attribution', 'noise_budget']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} privacy preserving attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} privacy preserving attribution: {data}"
