...
class HallucinationDetectionEngine(BaseEngine):
    engine_name = "hallucination_detection"
    required_input_fields = ['generated_text', 'reference_data']
    required_output_fields = ['hallucination_score', 'flagged_claims']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} hallucination detection", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} hallucination detection: {data}"
