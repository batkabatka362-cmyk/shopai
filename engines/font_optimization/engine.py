...
class FontOptimizationEngine(BaseEngine):
    engine_name = "font_optimization"
    required_input_fields = ['font_data', 'optimization_config']
    required_output_fields = ['font_strategy', 'subset_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} font optimization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} font optimization: {data}"
