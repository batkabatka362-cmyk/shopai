...
class WorkflowBuilderEngine(BaseEngine):
    engine_name = "workflow_builder"
    required_input_fields = ['workflow_data', 'step_config']
    required_output_fields = ['workflow_definition', 'trigger_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} workflow builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} workflow builder: {data}"
