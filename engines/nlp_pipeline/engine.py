...
class NlpPipelineEngine(BaseEngine):
    engine_name = "nlp_pipeline"
    required_input_fields = ['text_data', 'pipeline_config']
    required_output_fields = ['nlp_results', 'entity_extractions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} nlp pipeline", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} nlp pipeline: {data}"
