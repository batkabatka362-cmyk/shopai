...
class PermanentEstablishmentEngine(BaseEngine):
    engine_name = "permanent_establishment"
    required_input_fields = ['activity_data', 'pe_config']
    required_output_fields = ['pe_risk_assessment', 'mitigation_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} permanent establishment", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} permanent establishment: {data}"
