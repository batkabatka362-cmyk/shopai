...
class CallRoutingEngine(BaseEngine):
    engine_name = "call_routing"
    required_input_fields = ['call_data', 'routing_config']
    required_output_fields = ['routing_rules', 'queue_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} call routing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} call routing: {data}"
