...
class ResourceLevelingEngine(BaseEngine):
    engine_name = "resource_leveling"
    required_input_fields = ['resource_data', 'project_config']
    required_output_fields = ['leveled_schedule', 'utilization_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} resource leveling", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} resource leveling: {data}"
