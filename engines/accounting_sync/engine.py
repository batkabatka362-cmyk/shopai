...
class AccountingSyncEngine(BaseEngine):
    engine_name = "accounting_sync"
    required_input_fields = ['accounting_config', 'transaction_data']
    required_output_fields = ['sync_status', 'reconciliation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} accounting sync", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} accounting sync: {data}"
