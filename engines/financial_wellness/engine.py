...
class FinancialWellnessEngine(BaseEngine):
    engine_name = "financial_wellness"
    required_input_fields = ['employee_data', 'wellness_config']
    required_output_fields = ['wellness_program', 'resource_library']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} financial wellness", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} financial wellness: {data}"
