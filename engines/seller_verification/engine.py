...
class SellerVerificationEngine(BaseEngine):
    engine_name = "seller_verification"
    required_input_fields = ['seller_data', 'verification_config']
    required_output_fields = ['verification_status', 'trust_badge']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} seller verification", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} seller verification: {data}"
