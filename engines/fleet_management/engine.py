...
class FleetManagementEngine(BaseEngine):
    engine_name = "fleet_management"
    required_input_fields = ['vehicle_data', 'fleet_config']
    required_output_fields = ['fleet_status', 'maintenance_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} fleet management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} fleet management: {data}"
