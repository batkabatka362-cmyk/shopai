...
class TerritoryAnalysisEngine(BaseEngine):
    engine_name = "territory_analysis"
    required_input_fields = ['geo_data', 'territory_config']
    required_output_fields = ['territory_map', 'balance_metrics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} territory analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} territory analysis: {data}"
