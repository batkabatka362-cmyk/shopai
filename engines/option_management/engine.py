...
class OptionManagementEngine(BaseEngine):
    engine_name = "option_management"
    required_input_fields = ['product_data', 'option_config']
    required_output_fields = ['option_sets', 'pricing_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} option management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} option management: {data}"
