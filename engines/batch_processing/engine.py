...
class BatchProcessingEngine(BaseEngine):
    engine_name = "batch_processing"
    required_input_fields = ['batch_data', 'processing_config']
    required_output_fields = ['processed_batches', 'quality_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} batch processing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} batch processing: {data}"
