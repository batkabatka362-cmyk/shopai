...
class OrderMergeEngine(BaseEngine):
    engine_name = "order_merge"
    required_input_fields = ['order_data', 'merge_config']
    required_output_fields = ['merged_order', 'shipping_savings']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} order merge", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} order merge: {data}"
