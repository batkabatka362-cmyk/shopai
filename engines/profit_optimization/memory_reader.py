"""Profit Optimization Engine — memory reader.

Reads past optimization records from memory storage.
Used to inform current optimizations with historical context.
...
def read_past_optimizations(
    ...
    """Read past optimization records.
...
def compute_input_hash(input_data: dict[str, Any]) -> str:
    ...
    Used to detect duplicate / similar optimization requests.
    """
    try:
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    profits = [
        r.get("profit_metrics", {}).get("net_profit", 0.0)
        for r in records
    ]
    strategies = list({
        r.get("selected_strategy", {}).get("name", "unknown")
        for r in records
    })
