...
class RecognitionProgramAiEngine(BaseEngine):
    engine_name = "recognition_program_ai"
    required_input_fields = ['employee_data', 'recognition_config']
    required_output_fields = ['recognition_plan', 'reward_triggers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} recognition program ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} recognition program ai: {data}"
