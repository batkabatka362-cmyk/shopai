...
class PauseSubscriptionEngine(BaseEngine):
    engine_name = "pause_subscription"
    required_input_fields = ['subscription_data', 'pause_config']
    required_output_fields = ['pause_options', 'reactivation_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pause subscription", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pause subscription: {data}"
