"""Store Design Engine — memory reader.

Reads past store design decision records from memory storage.
Used to inform current design analysis with historical context.
"""
...
def read_past_designs(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past store design records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_designs(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past store design records."""
    if not records:
        return {
            "avg_conversion_lift": 0.0,
            "avg_recommendations_count": 0.0,
            "total_runs": 0,
        }

    lifts = [
        r.get("estimated_conversion_lift", 0.0)
        for r in records
    ]
    rec_counts = [r.get("recommendations_count", 0) for r in records]

    n = len(records)
    return {
        "avg_conversion_lift": round(sum(lifts) / n, 2) if n else 0.0,
        "avg_recommendations_count": round(sum(rec_counts) / n, 1) if n else 0.0,
        "total_runs": n,
    }
