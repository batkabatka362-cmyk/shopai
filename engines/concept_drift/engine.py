...
class ConceptDriftEngine(BaseEngine):
    engine_name = "concept_drift"
    required_input_fields = ['model_data', 'monitoring_config']
    required_output_fields = ['drift_detection', 'retrain_trigger']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} concept drift", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} concept drift: {data}"
