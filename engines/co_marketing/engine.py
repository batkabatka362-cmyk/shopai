...
class CoMarketingEngine(BaseEngine):
    engine_name = "co_marketing"
    required_input_fields = ['partner_data', 'campaign_config']
    required_output_fields = ['campaign_plan', 'cost_split']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} co marketing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} co marketing: {data}"
