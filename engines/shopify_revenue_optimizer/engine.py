"""
ShopifyRevenueOptimizer Engine
...
class ShopifyRevenueOptimizerEngine(BaseEngine):
    engine_name = "shopify_revenue_optimizer"
    required_input_fields = ['store_data', 'revenue_data']
    required_output_fields = ['revenue_plan', 'quick_wins']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shopify revenue optimizer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shopify revenue optimizer: {data}"
