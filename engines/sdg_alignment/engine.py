...
class SdgAlignmentEngine(BaseEngine):
    engine_name = "sdg_alignment"
    required_input_fields = ['business_data', 'sdg_config']
    required_output_fields = ['alignment_report', 'contribution_metrics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sdg alignment", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sdg alignment: {data}"
