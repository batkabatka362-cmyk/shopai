...
class ConversationBranchingEngine(BaseEngine):
    engine_name = "conversation_branching"
    required_input_fields = ['conversation_data', 'branching_config']
    required_output_fields = ['branch_tree', 'merge_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} conversation branching", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} conversation branching: {data}"
