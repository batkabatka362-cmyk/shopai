...
class SeatingChartEngine(BaseEngine):
    engine_name = "seating_chart"
    required_input_fields = ['venue_data', 'seating_config']
    required_output_fields = ['seating_map', 'availability_display']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} seating chart", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} seating chart: {data}"
