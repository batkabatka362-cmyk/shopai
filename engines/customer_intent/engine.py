...
class CustomerIntentEngine(BaseEngine):
    engine_name = "customer_intent"
    required_input_fields = ['behavior_data', 'intent_config']
    required_output_fields = ['intent_predictions', 'action_triggers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} customer intent", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} customer intent: {data}"
