...
class NegotiationAiEngine(BaseEngine):
    engine_name = "negotiation_ai"
    required_input_fields = ['deal_data', 'negotiation_config']
    required_output_fields = ['negotiation_strategy', 'counter_offers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} negotiation ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} negotiation ai: {data}"
