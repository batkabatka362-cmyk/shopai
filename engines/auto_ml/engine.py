...
class AutoMlEngine(BaseEngine):
    engine_name = "auto_ml"
    required_input_fields = ['dataset', 'automl_config']
    required_output_fields = ['best_model', 'leaderboard']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} auto ml", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} auto ml: {data}"
