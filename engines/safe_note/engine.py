...
class SafeNoteEngine(BaseEngine):
    engine_name = "safe_note"
    required_input_fields = ['safe_data', 'terms_config']
    required_output_fields = ['conversion_scenarios', 'cap_impact']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} safe note", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} safe note: {data}"
