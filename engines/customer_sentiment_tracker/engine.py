"""
CustomerSentimentTracker Engine
...
class CustomerSentimentTrackerEngine(BaseEngine):
    engine_name = "customer_sentiment_tracker"
    required_input_fields = ['feedback_data', 'social_data']
    required_output_fields = ['sentiment_trend', 'alert_triggers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} customer sentiment tracker", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} customer sentiment tracker: {data}"
