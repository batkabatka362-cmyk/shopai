...
class AiCopywriterEngine(BaseEngine):
    engine_name = "ai_copywriter"
    required_input_fields = ['brief_data', 'brand_config']
    required_output_fields = ['copy_variants', 'tone_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ai copywriter", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ai copywriter: {data}"
