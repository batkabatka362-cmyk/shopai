...
class SpellCheckerEngine(BaseEngine):
    engine_name = "spell_checker"
    required_input_fields = ['text_data', 'dictionary_config']
    required_output_fields = ['corrections', 'error_types']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} spell checker", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} spell checker: {data}"
