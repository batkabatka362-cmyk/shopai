...
class CriteoIntegrationEngine(BaseEngine):
    engine_name = "criteo_integration"
    required_input_fields = ['feed_data', 'integration_config']
    required_output_fields = ['criteo_setup', 'event_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} criteo integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} criteo integration: {data}"
