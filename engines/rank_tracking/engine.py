...
class RankTrackingEngine(BaseEngine):
    engine_name = "rank_tracking"
    required_input_fields = ['keyword_data', 'serp_config']
    required_output_fields = ['rank_history', 'competitor_comparison']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} rank tracking", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} rank tracking: {data}"
