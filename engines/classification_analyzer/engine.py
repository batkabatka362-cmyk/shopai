...
class ClassificationAnalyzerEngine(BaseEngine):
    engine_name = "classification_analyzer"
    required_input_fields = ['dataset', 'classification_config']
    required_output_fields = ['classifications', 'accuracy_metrics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} classification analyzer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} classification analyzer: {data}"
