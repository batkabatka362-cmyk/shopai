...
class GuestCheckoutEngine(BaseEngine):
    engine_name = "guest_checkout"
    required_input_fields = ['checkout_data', 'guest_config']
    required_output_fields = ['guest_flow', 'account_creation_prompt']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} guest checkout", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} guest checkout: {data}"
