...
class UpsellOpportunityAiEngine(BaseEngine):
    engine_name = "upsell_opportunity_ai"
    required_input_fields = ['cart_data', 'product_graph']
    required_output_fields = ['upsell_suggestions', 'timing_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} upsell opportunity ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} upsell opportunity ai: {data}"
