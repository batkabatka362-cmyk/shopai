...
class SystemDiagnosticEngine(BaseEngine):
    engine_name = "system_diagnostic"
    required_input_fields = ['system_data', 'diagnostic_config']
    required_output_fields = ['diagnostic_report', 'fix_recommendations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} system diagnostic", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} system diagnostic: {data}"
