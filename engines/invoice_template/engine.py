...
class InvoiceTemplateEngine(BaseEngine):
    engine_name = "invoice_template"
    required_input_fields = ['order_data', 'template_config']
    required_output_fields = ['invoice_html', 'pdf_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} invoice template", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} invoice template: {data}"
