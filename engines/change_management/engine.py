...
class ChangeManagementEngine(BaseEngine):
    engine_name = "change_management"
    required_input_fields = ['change_data', 'management_config']
    required_output_fields = ['change_plan', 'communication_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} change management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} change management: {data}"
