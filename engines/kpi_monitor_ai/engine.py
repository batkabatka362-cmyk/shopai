...
class KpiMonitorAiEngine(BaseEngine):
    engine_name = "kpi_monitor_ai"
    required_input_fields = ['kpi_data', 'target_config']
    required_output_fields = ['kpi_status', 'alert_triggers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} kpi monitor ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} kpi monitor ai: {data}"
