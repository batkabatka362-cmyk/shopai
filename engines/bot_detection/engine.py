...
class BotDetectionEngine(BaseEngine):
    engine_name = "bot_detection"
    required_input_fields = ['traffic_data', 'detection_config']
    required_output_fields = ['bot_scores', 'blocking_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} bot detection", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} bot detection: {data}"
