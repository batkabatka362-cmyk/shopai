...
class CategoryPageEngine(BaseEngine):
    engine_name = "category_page"
    required_input_fields = ['category_data', 'page_config']
    required_output_fields = ['page_layout', 'product_grid']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} category page", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} category page: {data}"
