...
class CustomerWinBackEngine(BaseEngine):
    engine_name = "customer_win_back"
    required_input_fields = ['inactive_customers', 'win_back_config']
    required_output_fields = ['win_back_offers', 'reactivation_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} customer win back", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} customer win back: {data}"
