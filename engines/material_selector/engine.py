...
class MaterialSelectorEngine(BaseEngine):
    engine_name = "material_selector"
    required_input_fields = ['material_data', 'selector_config']
    required_output_fields = ['selector_setup', 'pricing_impact']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} material selector", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} material selector: {data}"
