...
class CompetitiveUxEngine(BaseEngine):
    engine_name = "competitive_ux"
    required_input_fields = ['competitor_data', 'ux_config']
    required_output_fields = ['ux_comparison', 'best_practices']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} competitive ux", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} competitive ux: {data}"
