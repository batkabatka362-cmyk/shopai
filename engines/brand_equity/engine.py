...
class BrandEquityEngine(BaseEngine):
    engine_name = "brand_equity"
    required_input_fields = ['brand_data', 'survey_data']
    required_output_fields = ['equity_score', 'brand_health']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} brand equity", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} brand equity: {data}"
