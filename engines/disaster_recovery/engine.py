...
class DisasterRecoveryEngine(BaseEngine):
    engine_name = "disaster_recovery"
    required_input_fields = ['system_data', 'recovery_config']
    required_output_fields = ['recovery_plan', 'rto_rpo_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} disaster recovery", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} disaster recovery: {data}"
