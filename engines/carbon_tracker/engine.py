...
class CarbonTrackerEngine(BaseEngine):
    engine_name = "carbon_tracker"
    required_input_fields = ['purchase_data', 'carbon_config']
    required_output_fields = ['carbon_display', 'offset_options']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} carbon tracker", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} carbon tracker: {data}"
