...
class DowngradePathEngine(BaseEngine):
    engine_name = "downgrade_path"
    required_input_fields = ['subscription_data', 'downgrade_config']
    required_output_fields = ['downgrade_options', 'retention_offer']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} downgrade path", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} downgrade path: {data}"
