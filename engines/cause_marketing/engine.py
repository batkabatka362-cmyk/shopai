...
class CauseMarketingEngine(BaseEngine):
    engine_name = "cause_marketing"
    required_input_fields = ['campaign_data', 'cause_config']
    required_output_fields = ['cause_campaign', 'impact_metrics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cause marketing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cause marketing: {data}"
