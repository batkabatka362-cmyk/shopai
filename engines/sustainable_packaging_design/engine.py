"""
SustainablePackagingDesign Engine
...
class SustainablePackagingDesignEngine(BaseEngine):
    engine_name = "sustainable_packaging_design"
    required_input_fields = ['product_data', 'sustainability_config']
    required_output_fields = ['packaging_spec', 'material_alternatives']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sustainable packaging design", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sustainable packaging design: {data}"
