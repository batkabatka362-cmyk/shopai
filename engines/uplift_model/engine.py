...
class UpliftModelEngine(BaseEngine):
    engine_name = "uplift_model"
    required_input_fields = ['experiment_data', 'model_config']
    required_output_fields = ['uplift_scores', 'treatment_recommendations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} uplift model", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} uplift model: {data}"
