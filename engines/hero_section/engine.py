...
class HeroSectionEngine(BaseEngine):
    engine_name = "hero_section"
    required_input_fields = ['content_data', 'hero_config']
    required_output_fields = ['hero_layout', 'cta_placement']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} hero section", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} hero section: {data}"
