...
class CrossSellAiEngine(BaseEngine):
    engine_name = "cross_sell_ai"
    required_input_fields = ['cart_data', 'product_graph']
    required_output_fields = ['cross_sell_offers', 'relevance_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cross sell ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cross sell ai: {data}"
