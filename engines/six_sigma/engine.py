...
class SixSigmaEngine(BaseEngine):
    engine_name = "six_sigma"
    required_input_fields = ['process_data', 'sigma_config']
    required_output_fields = ['sigma_analysis', 'defect_reduction']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} six sigma", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} six sigma: {data}"
