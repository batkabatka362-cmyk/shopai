...
class MaAnalysisEngine(BaseEngine):
    engine_name = "ma_analysis"
    required_input_fields = ['target_data', 'criteria_config']
    required_output_fields = ['target_evaluation', 'synergy_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ma analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ma analysis: {data}"
