...
class EcosystemMappingEngine(BaseEngine):
    engine_name = "ecosystem_mapping"
    required_input_fields = ['partner_data', 'ecosystem_config']
    required_output_fields = ['ecosystem_map', 'opportunity_gaps']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ecosystem mapping", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ecosystem mapping: {data}"
