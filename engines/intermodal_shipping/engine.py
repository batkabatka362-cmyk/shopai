...
class IntermodalShippingEngine(BaseEngine):
    engine_name = "intermodal_shipping"
    required_input_fields = ['shipment_data', 'modal_config']
    required_output_fields = ['routing_options', 'transit_times']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} intermodal shipping", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} intermodal shipping: {data}"
