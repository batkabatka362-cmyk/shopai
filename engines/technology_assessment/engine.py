...
class TechnologyAssessmentEngine(BaseEngine):
    engine_name = "technology_assessment"
    required_input_fields = ['tech_stack', 'assessment_config']
    required_output_fields = ['assessment_report', 'upgrade_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} technology assessment", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} technology assessment: {data}"
