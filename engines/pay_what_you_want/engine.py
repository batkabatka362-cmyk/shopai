...
class PayWhatYouWantEngine(BaseEngine):
    engine_name = "pay_what_you_want"
    required_input_fields = ['product_data', 'pwyw_config']
    required_output_fields = ['pwyw_setup', 'suggested_prices']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pay what you want", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pay what you want: {data}"
