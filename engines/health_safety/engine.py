...
class HealthSafetyEngine(BaseEngine):
    engine_name = "health_safety"
    required_input_fields = ['workplace_data', 'safety_config']
    required_output_fields = ['safety_plan', 'compliance_checklist']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} health safety", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} health safety: {data}"
