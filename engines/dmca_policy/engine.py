...
class DmcaPolicyEngine(BaseEngine):
    engine_name = "dmca_policy"
    required_input_fields = ['platform_data', 'dmca_config']
    required_output_fields = ['dmca_policy', 'takedown_procedure']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dmca policy", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dmca policy: {data}"
