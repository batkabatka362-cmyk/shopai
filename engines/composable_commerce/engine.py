...
class ComposableCommerceEngine(BaseEngine):
    engine_name = "composable_commerce"
    required_input_fields = ['component_data', 'architecture_config']
    required_output_fields = ['component_map', 'integration_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} composable commerce", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} composable commerce: {data}"
