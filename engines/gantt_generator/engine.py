...
class GanttGeneratorEngine(BaseEngine):
    engine_name = "gantt_generator"
    required_input_fields = ['task_data', 'gantt_config']
    required_output_fields = ['gantt_chart', 'milestone_markers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} gantt generator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} gantt generator: {data}"
