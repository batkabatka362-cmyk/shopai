...
class CartSaverEngine(BaseEngine):
    engine_name = "cart_saver"
    required_input_fields = ['cart_data', 'saver_config']
    required_output_fields = ['save_for_later', 'reminder_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cart saver", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cart saver: {data}"
