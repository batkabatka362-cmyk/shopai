...
class FaqSectionEngine(BaseEngine):
    engine_name = "faq_section"
    required_input_fields = ['faq_data', 'section_config']
    required_output_fields = ['faq_layout', 'accordion_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} faq section", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} faq section: {data}"
