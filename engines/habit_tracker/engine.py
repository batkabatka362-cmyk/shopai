...
class HabitTrackerEngine(BaseEngine):
    engine_name = "habit_tracker"
    required_input_fields = ['habit_data', 'tracker_config']
    required_output_fields = ['tracker_setup', 'streak_rewards']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} habit tracker", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} habit tracker: {data}"
