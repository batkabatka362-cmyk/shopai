...
class MfaManagementEngine(BaseEngine):
    engine_name = "mfa_management"
    required_input_fields = ['user_data', 'mfa_config']
    required_output_fields = ['mfa_setup', 'recovery_options']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} mfa management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} mfa management: {data}"
