"""
TermsOfServiceGenerator Engine
...
class TermsOfServiceGeneratorEngine(BaseEngine):
    engine_name = "terms_of_service_generator"
    required_input_fields = ['business_data', 'tos_config']
    required_output_fields = ['terms_of_service', 'liability_limitations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} terms of service generator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} terms of service generator: {data}"
