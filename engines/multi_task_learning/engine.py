...
class MultiTaskLearningEngine(BaseEngine):
    engine_name = "multi_task_learning"
    required_input_fields = ['task_data', 'mtl_config']
    required_output_fields = ['shared_model', 'task_performance']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} multi task learning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} multi task learning: {data}"
