...
class RbacManagementEngine(BaseEngine):
    engine_name = "rbac_management"
    required_input_fields = ['role_data', 'permission_config']
    required_output_fields = ['rbac_matrix', 'access_policies']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} rbac management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} rbac management: {data}"
