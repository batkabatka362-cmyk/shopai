...
class ConflictResolutionSyncEngine(BaseEngine):
    engine_name = "conflict_resolution_sync"
    required_input_fields = ['conflict_data', 'resolution_config']
    required_output_fields = ['resolution_plan', 'merge_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} conflict resolution sync", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} conflict resolution sync: {data}"
