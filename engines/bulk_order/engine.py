...
class BulkOrderEngine(BaseEngine):
    engine_name = "bulk_order"
    required_input_fields = ['order_data', 'bulk_config']
    required_output_fields = ['bulk_pricing', 'fulfillment_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} bulk order", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} bulk order: {data}"
