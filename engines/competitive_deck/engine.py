...
class CompetitiveDeckEngine(BaseEngine):
    engine_name = "competitive_deck"
    required_input_fields = ['competitor_data', 'positioning_data']
    required_output_fields = ['comparison_slides', 'differentiation_points']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} competitive deck", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} competitive deck: {data}"
