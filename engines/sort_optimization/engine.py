...
class SortOptimizationEngine(BaseEngine):
    engine_name = "sort_optimization"
    required_input_fields = ['product_data', 'sort_config']
    required_output_fields = ['sort_algorithms', 'default_sort']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sort optimization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sort optimization: {data}"
