...
class MvpBuilderEngine(BaseEngine):
    engine_name = "mvp_builder"
    required_input_fields = ['feature_data', 'mvp_config']
    required_output_fields = ['mvp_scope', 'build_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} mvp builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} mvp builder: {data}"
