...
class SegmentationBuilderEngine(BaseEngine):
    engine_name = "segmentation_builder"
    required_input_fields = ['data_source', 'segment_config']
    required_output_fields = ['segment_definitions', 'audience_sizes']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} segmentation builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} segmentation builder: {data}"
