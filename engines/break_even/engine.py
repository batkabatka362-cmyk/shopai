...
class BreakEvenEngine(BaseEngine):
    engine_name = "break_even"
    required_input_fields = ['cost_data', 'revenue_data']
    required_output_fields = ['break_even_point', 'sensitivity_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} break even", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} break even: {data}"
