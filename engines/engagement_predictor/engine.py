...
class EngagementPredictorEngine(BaseEngine):
    engine_name = "engagement_predictor"
    required_input_fields = ['content_data', 'prediction_config']
    required_output_fields = ['engagement_forecast', 'optimal_timing']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} engagement predictor", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} engagement predictor: {data}"
