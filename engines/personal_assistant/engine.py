...
class PersonalAssistantEngine(BaseEngine):
    engine_name = "personal_assistant"
    required_input_fields = ['customer_data', 'assistant_config']
    required_output_fields = ['task_list', 'proactive_suggestions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} personal assistant", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} personal assistant: {data}"
