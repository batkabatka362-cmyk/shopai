...
class ChildcareBenefitEngine(BaseEngine):
    engine_name = "childcare_benefit"
    required_input_fields = ['employee_data', 'childcare_config']
    required_output_fields = ['benefit_options', 'provider_network']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} childcare benefit", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} childcare benefit: {data}"
