...
class LoadOptimizationEngine(BaseEngine):
    engine_name = "load_optimization"
    required_input_fields = ['shipment_data', 'container_config']
    required_output_fields = ['loading_plan', 'space_utilization']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} load optimization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} load optimization: {data}"
