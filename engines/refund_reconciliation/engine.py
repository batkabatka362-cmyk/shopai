...
class RefundReconciliationEngine(BaseEngine):
    engine_name = "refund_reconciliation"
    required_input_fields = ['refund_data', 'bank_data']
    required_output_fields = ['reconciliation_report', 'outstanding_refunds']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} refund reconciliation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} refund reconciliation: {data}"
