"""
PaymentGatewayReconciliation Engine
...
class PaymentGatewayReconciliationEngine(BaseEngine):
    engine_name = "payment_gateway_reconciliation"
    required_input_fields = ['gateway_data', 'order_data']
    required_output_fields = ['reconciliation_report', 'fee_breakdown']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} payment gateway reconciliation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} payment gateway reconciliation: {data}"
