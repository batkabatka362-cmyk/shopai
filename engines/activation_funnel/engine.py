...
class ActivationFunnelEngine(BaseEngine):
    engine_name = "activation_funnel"
    required_input_fields = ['funnel_data', 'activation_config']
    required_output_fields = ['funnel_analysis', 'drop_off_fixes']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} activation funnel", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} activation funnel: {data}"
