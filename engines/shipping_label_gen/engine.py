...
class ShippingLabelGenEngine(BaseEngine):
    engine_name = "shipping_label_gen"
    required_input_fields = ['shipment_data', 'label_config']
    required_output_fields = ['label_content', 'barcode_data']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shipping label gen", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shipping label gen: {data}"
