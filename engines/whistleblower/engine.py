...
class WhistleblowerEngine(BaseEngine):
    engine_name = "whistleblower"
    required_input_fields = ['report_data', 'whistleblower_config']
    required_output_fields = ['case_setup', 'investigation_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} whistleblower", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} whistleblower: {data}"
