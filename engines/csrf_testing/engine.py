...
class CsrfTestingEngine(BaseEngine):
    engine_name = "csrf_testing"
    required_input_fields = ['form_data', 'csrf_config']
    required_output_fields = ['csrf_report', 'missing_tokens']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} csrf testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} csrf testing: {data}"
