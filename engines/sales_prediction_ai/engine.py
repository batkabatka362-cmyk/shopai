...
class SalesPredictionAiEngine(BaseEngine):
    engine_name = "sales_prediction_ai"
    required_input_fields = ['sales_history', 'seasonality_data']
    required_output_fields = ['sales_forecast', 'seasonal_factors']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sales prediction ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sales prediction ai: {data}"
