...
class TokenGatingEngine(BaseEngine):
    engine_name = "token_gating"
    required_input_fields = ['token_data', 'gating_config']
    required_output_fields = ['gating_rules', 'access_verification']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} token gating", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} token gating: {data}"
