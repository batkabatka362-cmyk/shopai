...
class PlatformStrategyEngine(BaseEngine):
    engine_name = "platform_strategy"
    required_input_fields = ['platform_data', 'strategy_config']
    required_output_fields = ['platform_plan', 'ecosystem_map']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} platform strategy", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} platform strategy: {data}"
