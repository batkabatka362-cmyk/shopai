...
class ListHygieneEngine(BaseEngine):
    engine_name = "list_hygiene"
    required_input_fields = ['subscriber_data', 'hygiene_config']
    required_output_fields = ['cleaned_list', 'removal_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} list hygiene", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} list hygiene: {data}"
