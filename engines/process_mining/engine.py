...
class ProcessMiningEngine(BaseEngine):
    engine_name = "process_mining"
    required_input_fields = ['event_log', 'mining_config']
    required_output_fields = ['process_model', 'conformance_check']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} process mining", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} process mining: {data}"
