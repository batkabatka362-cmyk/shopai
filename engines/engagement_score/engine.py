...
class EngagementScoreEngine(BaseEngine):
    engine_name = "engagement_score"
    required_input_fields = ['interaction_data', 'scoring_config']
    required_output_fields = ['engagement_scores', 'segment_assignment']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} engagement score", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} engagement score: {data}"
