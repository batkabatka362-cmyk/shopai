...
class EnvironmentalLabelEngine(BaseEngine):
    engine_name = "environmental_label"
    required_input_fields = ['product_data', 'eco_standards']
    required_output_fields = ['eco_labels', 'sustainability_score']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} environmental label", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} environmental label: {data}"
