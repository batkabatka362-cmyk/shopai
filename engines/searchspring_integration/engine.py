"""
SearchspringIntegration Engine
...
class SearchspringIntegrationEngine(BaseEngine):
    engine_name = "searchspring_integration"
    required_input_fields = ['search_data', 'integration_config']
    required_output_fields = ['searchspring_sync', 'merchandising_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} searchspring integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} searchspring integration: {data}"
