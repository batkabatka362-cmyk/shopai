...
class IdentityResolutionEngine(BaseEngine):
    engine_name = "identity_resolution"
    required_input_fields = ['identity_data', 'resolution_config']
    required_output_fields = ['resolved_identities', 'confidence_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} identity resolution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} identity resolution: {data}"
