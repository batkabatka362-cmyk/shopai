...
class LtvBidEngine(BaseEngine):
    engine_name = "ltv_bid"
    required_input_fields = ['ltv_data', 'bid_config']
    required_output_fields = ['ltv_based_bids', 'value_optimization']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ltv bid", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ltv bid: {data}"
