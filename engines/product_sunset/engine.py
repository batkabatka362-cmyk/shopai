...
class ProductSunsetEngine(BaseEngine):
    engine_name = "product_sunset"
    required_input_fields = ['product_data', 'sunset_config']
    required_output_fields = ['sunset_plan', 'communication_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} product sunset", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} product sunset: {data}"
