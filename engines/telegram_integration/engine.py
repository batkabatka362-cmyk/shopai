...
class TelegramIntegrationEngine(BaseEngine):
    engine_name = "telegram_integration"
    required_input_fields = ['bot_data', 'integration_config']
    required_output_fields = ['telegram_setup', 'command_handlers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} telegram integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} telegram integration: {data}"
