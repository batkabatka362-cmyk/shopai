...
class WelcomeBoxEngine(BaseEngine):
    engine_name = "welcome_box"
    required_input_fields = ['customer_data', 'box_config']
    required_output_fields = ['box_contents', 'onboarding_insert']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} welcome box", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} welcome box: {data}"
