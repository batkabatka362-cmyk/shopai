...
class RiskScoringEngineEngine(BaseEngine):
    engine_name = "risk_scoring_engine"
    required_input_fields = ['entity_data', 'scoring_config']
    required_output_fields = ['risk_score', 'factor_breakdown']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} risk scoring engine", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} risk scoring engine: {data}"
