...
class EnhancedConversionEngine(BaseEngine):
    engine_name = "enhanced_conversion"
    required_input_fields = ['conversion_data', 'enhancement_config']
    required_output_fields = ['enhancement_setup', 'match_improvement']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} enhanced conversion", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} enhanced conversion: {data}"
