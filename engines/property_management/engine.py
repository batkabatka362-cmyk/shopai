...
class PropertyManagementEngine(BaseEngine):
    engine_name = "property_management"
    required_input_fields = ['property_data', 'management_config']
    required_output_fields = ['management_agreement', 'fee_structure']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} property management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} property management: {data}"
