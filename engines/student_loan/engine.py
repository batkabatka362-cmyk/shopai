...
class StudentLoanEngine(BaseEngine):
    engine_name = "student_loan"
    required_input_fields = ['employee_data', 'loan_config']
    required_output_fields = ['assistance_plan', 'payment_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} student loan", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} student loan: {data}"
