...
class DividendCalcEngine(BaseEngine):
    engine_name = "dividend_calc"
    required_input_fields = ['financial_data', 'dividend_config']
    required_output_fields = ['dividend_amount', 'payment_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dividend calc", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dividend calc: {data}"
