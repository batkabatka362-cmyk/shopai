...
class RichResultEngine(BaseEngine):
    engine_name = "rich_result"
    required_input_fields = ['page_data', 'rich_config']
    required_output_fields = ['rich_markup', 'eligibility_check']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} rich result", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} rich result: {data}"
