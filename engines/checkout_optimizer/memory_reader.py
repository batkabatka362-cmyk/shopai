"""Checkout Optimizer Engine — memory reader.

Reads past checkout optimization records from memory storage.
...
def read_past_optimizations(
    ...
    """Read past checkout optimization records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_optimizations(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past checkout optimization records."""
    if not records:
        return {
            "avg_friction_points": 0.0,
            "avg_optimizations_count": 0.0,
            "total_runs": 0,
        }

    friction_counts = [r.get("friction_points_count", 0) for r in records]
    opt_counts = [r.get("optimizations_count", 0) for r in records]

    n = len(records)
    return {
        "avg_friction_points": round(sum(friction_counts) / n, 1) if n else 0.0,
        "avg_optimizations_count": round(sum(opt_counts) / n, 1) if n else 0.0,
        "total_runs": n,
    }
