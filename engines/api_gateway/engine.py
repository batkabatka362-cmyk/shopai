...
class ApiGatewayEngine(BaseEngine):
    engine_name = "api_gateway"
    required_input_fields = ['api_data', 'gateway_config']
    required_output_fields = ['gateway_setup', 'rate_limit_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} api gateway", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} api gateway: {data}"
