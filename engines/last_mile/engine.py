...
class LastMileEngine(BaseEngine):
    engine_name = "last_mile"
    required_input_fields = ['delivery_data', 'route_config']
    required_output_fields = ['delivery_routes', 'driver_assignments']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} last mile", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} last mile: {data}"
