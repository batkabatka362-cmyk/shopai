...
class MergeTagEngine(BaseEngine):
    engine_name = "merge_tag"
    required_input_fields = ['template_data', 'tag_config']
    required_output_fields = ['tag_mapping', 'fallback_values']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} merge tag", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} merge tag: {data}"
