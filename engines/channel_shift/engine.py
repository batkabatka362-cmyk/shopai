...
class ChannelShiftEngine(BaseEngine):
    engine_name = "channel_shift"
    required_input_fields = ['channel_data', 'shift_config']
    required_output_fields = ['shift_plan', 'cost_savings']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} channel shift", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} channel shift: {data}"
