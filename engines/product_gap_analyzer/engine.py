...
class ProductGapAnalyzerEngine(BaseEngine):
    engine_name = "product_gap_analyzer"
    required_input_fields = ['catalog_data', 'market_data']
    required_output_fields = ['gaps', 'product_suggestions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} product gap analyzer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} product gap analyzer: {data}"
