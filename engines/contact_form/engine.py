...
class ContactFormEngine(BaseEngine):
    engine_name = "contact_form"
    required_input_fields = ['form_data', 'contact_config']
    required_output_fields = ['form_layout', 'submission_handling']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} contact form", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} contact form: {data}"
