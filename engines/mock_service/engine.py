...
class MockServiceEngine(BaseEngine):
    engine_name = "mock_service"
    required_input_fields = ['api_spec', 'mock_config']
    required_output_fields = ['mock_endpoints', 'response_templates']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} mock service", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} mock service: {data}"
