...
class CompensationPlanningEngine(BaseEngine):
    engine_name = "compensation_planning"
    required_input_fields = ['sales_data', 'comp_config']
    required_output_fields = ['comp_plan', 'payout_forecast']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} compensation planning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} compensation planning: {data}"
