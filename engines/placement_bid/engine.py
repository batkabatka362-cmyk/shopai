...
class PlacementBidEngine(BaseEngine):
    engine_name = "placement_bid"
    required_input_fields = ['placement_data', 'bid_config']
    required_output_fields = ['placement_bids', 'performance_by_site']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} placement bid", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} placement bid: {data}"
