...
class SuppressionManagementEngine(BaseEngine):
    engine_name = "suppression_management"
    required_input_fields = ['suppression_data', 'management_config']
    required_output_fields = ['suppression_list', 'compliance_status']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} suppression management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} suppression management: {data}"
