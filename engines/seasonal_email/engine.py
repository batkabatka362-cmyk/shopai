...
class SeasonalEmailEngine(BaseEngine):
    engine_name = "seasonal_email"
    required_input_fields = ['season_data', 'email_config']
    required_output_fields = ['email_series', 'content_calendar']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} seasonal email", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} seasonal email: {data}"
