...
class ApiTestingEngine(BaseEngine):
    engine_name = "api_testing"
    required_input_fields = ['api_spec', 'test_config']
    required_output_fields = ['test_results', 'coverage_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} api testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} api testing: {data}"
