...
class AttentionMapEngine(BaseEngine):
    engine_name = "attention_map"
    required_input_fields = ['interaction_data', 'map_config']
    required_output_fields = ['attention_heatmap', 'engagement_score']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} attention map", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} attention map: {data}"
