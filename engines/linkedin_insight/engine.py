...
class LinkedinInsightEngine(BaseEngine):
    engine_name = "linkedin_insight"
    required_input_fields = ['tag_data', 'integration_config']
    required_output_fields = ['tag_setup', 'conversion_tracking']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} linkedin insight", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} linkedin insight: {data}"
