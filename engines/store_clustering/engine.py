...
class StoreClusteringEngine(BaseEngine):
    engine_name = "store_clustering"
    required_input_fields = ['store_data', 'clustering_config']
    required_output_fields = ['store_clusters', 'cluster_profiles']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} store clustering", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} store clustering: {data}"
