...
class InventoryReportAiEngine(BaseEngine):
    engine_name = "inventory_report_ai"
    required_input_fields = ['inventory_data', 'report_config']
    required_output_fields = ['inventory_report', 'health_summary']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} inventory report ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} inventory report ai: {data}"
