...
class CostOfGoodsEngine(BaseEngine):
    engine_name = "cost_of_goods"
    required_input_fields = ['product_data', 'cogs_config']
    required_output_fields = ['cogs_report', 'margin_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cost of goods", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cost of goods: {data}"
