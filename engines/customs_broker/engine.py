...
class CustomsBrokerEngine(BaseEngine):
    engine_name = "customs_broker"
    required_input_fields = ['shipment_data', 'customs_config']
    required_output_fields = ['broker_selection', 'documentation_prep']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} customs broker", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} customs broker: {data}"
