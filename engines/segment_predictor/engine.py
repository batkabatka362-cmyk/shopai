...
class SegmentPredictorEngine(BaseEngine):
    engine_name = "segment_predictor"
    required_input_fields = ['customer_data', 'segment_config']
    required_output_fields = ['segment_predictions', 'migration_forecast']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} segment predictor", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} segment predictor: {data}"
