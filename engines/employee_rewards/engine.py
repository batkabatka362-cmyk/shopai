...
class EmployeeRewardsEngine(BaseEngine):
    engine_name = "employee_rewards"
    required_input_fields = ['employee_data', 'reward_config']
    required_output_fields = ['reward_options', 'recognition_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} employee rewards", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} employee rewards: {data}"
