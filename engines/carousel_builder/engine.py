...
class CarouselBuilderEngine(BaseEngine):
    engine_name = "carousel_builder"
    required_input_fields = ['content_data', 'carousel_config']
    required_output_fields = ['carousel_slides', 'swipe_logic']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} carousel builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} carousel builder: {data}"
