...
class RecommendationModelEngine(BaseEngine):
    engine_name = "recommendation_model"
    required_input_fields = ['interaction_data', 'model_config']
    required_output_fields = ['recommendations', 'similarity_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} recommendation model", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} recommendation model: {data}"
