...
class NativeAdsEngine(BaseEngine):
    engine_name = "native_ads"
    required_input_fields = ['content_data', 'placement_config']
    required_output_fields = ['native_content', 'distribution_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} native ads", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} native ads: {data}"
