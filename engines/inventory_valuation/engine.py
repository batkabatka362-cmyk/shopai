...
class InventoryValuationEngine(BaseEngine):
    engine_name = "inventory_valuation"
    required_input_fields = ['inventory_data', 'valuation_config']
    required_output_fields = ['valuation_report', 'method_comparison']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} inventory valuation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} inventory valuation: {data}"
