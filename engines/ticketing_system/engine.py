...
class TicketingSystemEngine(BaseEngine):
    engine_name = "ticketing_system"
    required_input_fields = ['event_data', 'ticketing_config']
    required_output_fields = ['ticket_types', 'sales_tracking']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ticketing system", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ticketing system: {data}"
