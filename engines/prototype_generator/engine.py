...
class PrototypeGeneratorEngine(BaseEngine):
    engine_name = "prototype_generator"
    required_input_fields = ['design_data', 'prototype_config']
    required_output_fields = ['prototype_spec', 'click_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} prototype generator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} prototype generator: {data}"
