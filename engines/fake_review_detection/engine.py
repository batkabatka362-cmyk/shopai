...
class FakeReviewDetectionEngine(BaseEngine):
    engine_name = "fake_review_detection"
    required_input_fields = ['review_data', 'detection_config']
    required_output_fields = ['authenticity_scores', 'suspicious_patterns']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} fake review detection", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} fake review detection: {data}"
