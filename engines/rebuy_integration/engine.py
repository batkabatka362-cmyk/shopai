...
class RebuyIntegrationEngine(BaseEngine):
    engine_name = "rebuy_integration"
    required_input_fields = ['recommendation_data', 'integration_config']
    required_output_fields = ['rebuy_sync', 'widget_placement']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} rebuy integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} rebuy integration: {data}"
