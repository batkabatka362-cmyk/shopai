...
class Customer360Engine(BaseEngine):
    engine_name = "customer_360"
    required_input_fields = ['customer_data', 'integration_config']
    required_output_fields = ['unified_profile', 'data_completeness']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} customer 360", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} customer 360: {data}"
