...
class AdventCalendarEngine(BaseEngine):
    engine_name = "advent_calendar"
    required_input_fields = ['daily_offers', 'calendar_config']
    required_output_fields = ['calendar_setup', 'reveal_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} advent calendar", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} advent calendar: {data}"
