...
class AutoResponseEngine(BaseEngine):
    engine_name = "auto_response"
    required_input_fields = ['query_data', 'auto_config']
    required_output_fields = ['auto_reply', 'confidence_score']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} auto response", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} auto response: {data}"
