...
class DataSamplingEngine(BaseEngine):
    engine_name = "data_sampling"
    required_input_fields = ['dataset', 'sampling_config']
    required_output_fields = ['sample_data', 'representativeness_score']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} data sampling", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} data sampling: {data}"
