...
class VoiceUiEngine(BaseEngine):
    engine_name = "voice_ui"
    required_input_fields = ['dialog_data', 'vui_config']
    required_output_fields = ['vui_flow', 'utterance_mapping']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} voice ui", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} voice ui: {data}"
