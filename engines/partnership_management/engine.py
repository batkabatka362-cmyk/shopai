...
class PartnershipManagementEngine(BaseEngine):
    engine_name = "partnership_management"
    required_input_fields = ['partner_data', 'management_config']
    required_output_fields = ['partnership_status', 'action_items']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} partnership management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} partnership management: {data}"
