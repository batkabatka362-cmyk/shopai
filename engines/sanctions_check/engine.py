...
class SanctionsCheckEngine(BaseEngine):
    engine_name = "sanctions_check"
    required_input_fields = ['entity_data', 'sanctions_config']
    required_output_fields = ['check_result', 'match_details']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sanctions check", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sanctions check: {data}"
