...
class FeatureSectionEngine(BaseEngine):
    engine_name = "feature_section"
    required_input_fields = ['feature_data', 'section_config']
    required_output_fields = ['section_layout', 'icon_mapping']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} feature section", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} feature section: {data}"
