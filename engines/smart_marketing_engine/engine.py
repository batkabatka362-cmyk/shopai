...
class SmartMarketingEngineEngine(BaseEngine):
    engine_name = "smart_marketing_engine"
    required_input_fields = ['campaign_data', 'audience_data']
    required_output_fields = ['campaign_actions', 'budget_optimization']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} smart marketing engine", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} smart marketing engine: {data}"
