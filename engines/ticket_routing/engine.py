...
class TicketRoutingEngine(BaseEngine):
    engine_name = "ticket_routing"
    required_input_fields = ['ticket_data', 'routing_rules']
    required_output_fields = ['routing_decision', 'priority_score']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ticket routing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ticket routing: {data}"
