...
class PeerSupportEngine(BaseEngine):
    engine_name = "peer_support"
    required_input_fields = ['question_data', 'peer_config']
    required_output_fields = ['peer_matches', 'incentive_triggers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} peer support", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} peer support: {data}"
