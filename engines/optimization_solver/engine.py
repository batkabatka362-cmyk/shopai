...
class OptimizationSolverEngine(BaseEngine):
    engine_name = "optimization_solver"
    required_input_fields = ['constraint_data', 'objective_config']
    required_output_fields = ['optimal_solution', 'sensitivity_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} optimization solver", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} optimization solver: {data}"
