...
class SessionReplayEngine(BaseEngine):
    engine_name = "session_replay"
    required_input_fields = ['session_data', 'replay_config']
    required_output_fields = ['replay_url', 'interaction_timeline']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} session replay", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} session replay: {data}"
