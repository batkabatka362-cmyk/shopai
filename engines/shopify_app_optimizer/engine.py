...
class ShopifyAppOptimizerEngine(BaseEngine):
    engine_name = "shopify_app_optimizer"
    required_input_fields = ['app_data', 'optimization_config']
    required_output_fields = ['app_audit', 'removal_recommendations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shopify app optimizer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shopify app optimizer: {data}"
