...
class MediaTransparencyEngine(BaseEngine):
    engine_name = "media_transparency"
    required_input_fields = ['media_data', 'transparency_config']
    required_output_fields = ['transparency_report', 'fee_breakdown']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} media transparency", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} media transparency: {data}"
