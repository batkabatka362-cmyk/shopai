...
class TemperatureSensitiveEngine(BaseEngine):
    engine_name = "temperature_sensitive"
    required_input_fields = ['product_data', 'temp_config']
    required_output_fields = ['shipping_requirements', 'packaging_spec']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} temperature sensitive", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} temperature sensitive: {data}"
