...
class DataDrivenAttributionEngine(BaseEngine):
    engine_name = "data_driven_attribution"
    required_input_fields = ['touchpoint_data', 'model_config']
    required_output_fields = ['attribution_model', 'channel_weights']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} data driven attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} data driven attribution: {data}"
