...
class LabelStudioEngine(BaseEngine):
    engine_name = "label_studio"
    required_input_fields = ['data_source', 'labeling_config']
    required_output_fields = ['labeled_dataset', 'agreement_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} label studio", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} label studio: {data}"
