...
class GoogleShoppingEngine(BaseEngine):
    engine_name = "google_shopping"
    required_input_fields = ['product_feed', 'shopping_config']
    required_output_fields = ['shopping_listings', 'bid_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} google shopping", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} google shopping: {data}"
