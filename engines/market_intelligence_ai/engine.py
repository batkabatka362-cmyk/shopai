...
class MarketIntelligenceAiEngine(BaseEngine):
    engine_name = "market_intelligence_ai"
    required_input_fields = ['market_data', 'competitor_data']
    required_output_fields = ['market_score', 'opportunity_matrix']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} market intelligence ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} market intelligence ai: {data}"
