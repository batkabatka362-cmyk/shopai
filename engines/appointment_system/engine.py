...
class AppointmentSystemEngine(BaseEngine):
    engine_name = "appointment_system"
    required_input_fields = ['provider_data', 'appointment_config']
    required_output_fields = ['appointment_flow', 'reminder_setup']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} appointment system", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} appointment system: {data}"
