...
class VisitorManagementEngine(BaseEngine):
    engine_name = "visitor_management"
    required_input_fields = ['visitor_data', 'management_config']
    required_output_fields = ['check_in_flow', 'security_protocol']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} visitor management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} visitor management: {data}"
