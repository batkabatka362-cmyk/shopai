...
class StandupSummaryEngine(BaseEngine):
    engine_name = "standup_summary"
    required_input_fields = ['standup_data', 'summary_config']
    required_output_fields = ['summary_content', 'blocker_alerts']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} standup summary", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} standup summary: {data}"
