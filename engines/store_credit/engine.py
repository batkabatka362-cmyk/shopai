...
class StoreCreditEngine(BaseEngine):
    engine_name = "store_credit"
    required_input_fields = ['credit_data', 'credit_config']
    required_output_fields = ['credit_setup', 'expiry_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} store credit", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} store credit: {data}"
