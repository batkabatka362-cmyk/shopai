...
class DiscordIntegrationEngine(BaseEngine):
    engine_name = "discord_integration"
    required_input_fields = ['server_data', 'integration_config']
    required_output_fields = ['discord_setup', 'bot_commands']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} discord integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} discord integration: {data}"
