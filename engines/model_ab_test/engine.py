...
class ModelAbTestEngine(BaseEngine):
    engine_name = "model_ab_test"
    required_input_fields = ['model_variants', 'test_config']
    required_output_fields = ['test_results', 'winner_model']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} model ab test", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} model ab test: {data}"
