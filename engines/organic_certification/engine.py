...
class OrganicCertificationEngine(BaseEngine):
    engine_name = "organic_certification"
    required_input_fields = ['product_data', 'organic_config']
    required_output_fields = ['certification_plan', 'compliance_checklist']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} organic certification", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} organic certification: {data}"
