...
class RegressionModelEngine(BaseEngine):
    engine_name = "regression_model"
    required_input_fields = ['feature_data', 'model_config']
    required_output_fields = ['predictions', 'error_metrics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} regression model", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} regression model: {data}"
