...
class SkuRationalizationEngine(BaseEngine):
    engine_name = "sku_rationalization"
    required_input_fields = ['sku_data', 'rationalization_config']
    required_output_fields = ['keep_discontinue_list', 'impact_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sku rationalization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sku rationalization: {data}"
