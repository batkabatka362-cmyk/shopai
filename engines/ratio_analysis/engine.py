...
class RatioAnalysisEngine(BaseEngine):
    engine_name = "ratio_analysis"
    required_input_fields = ['financial_data', 'ratio_config']
    required_output_fields = ['ratio_report', 'trend_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ratio analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ratio analysis: {data}"
