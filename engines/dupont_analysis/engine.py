...
class DupontAnalysisEngine(BaseEngine):
    engine_name = "dupont_analysis"
    required_input_fields = ['financial_data', 'analysis_config']
    required_output_fields = ['roe_decomposition', 'driver_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dupont analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dupont analysis: {data}"
