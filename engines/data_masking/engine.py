...
class DataMaskingEngine(BaseEngine):
    engine_name = "data_masking"
    required_input_fields = ['sensitive_data', 'masking_config']
    required_output_fields = ['masked_data', 'masking_log']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} data masking", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} data masking: {data}"
