...
class BusinessPlanEngine(BaseEngine):
    engine_name = "business_plan"
    required_input_fields = ['business_data', 'plan_config']
    required_output_fields = ['plan_content', 'financial_model']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} business plan", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} business plan: {data}"
