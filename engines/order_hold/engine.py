...
class OrderHoldEngine(BaseEngine):
    engine_name = "order_hold"
    required_input_fields = ['order_data', 'hold_config']
    required_output_fields = ['hold_status', 'release_conditions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} order hold", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} order hold: {data}"
