...
class FreemiumStrategyEngine(BaseEngine):
    engine_name = "freemium_strategy"
    required_input_fields = ['product_data', 'freemium_config']
    required_output_fields = ['tier_design', 'conversion_triggers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} freemium strategy", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} freemium strategy: {data}"
