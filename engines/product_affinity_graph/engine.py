...
class ProductAffinityGraphEngine(BaseEngine):
    engine_name = "product_affinity_graph"
    required_input_fields = ['purchase_data', 'catalog_data']
    required_output_fields = ['affinity_graph', 'co_purchase_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} product affinity graph", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} product affinity graph: {data}"
