...
class AbacManagementEngine(BaseEngine):
    engine_name = "abac_management"
    required_input_fields = ['attribute_data', 'policy_config']
    required_output_fields = ['abac_policies', 'evaluation_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} abac management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} abac management: {data}"
