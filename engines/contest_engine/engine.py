...
class ContestEngineEngine(BaseEngine):
    engine_name = "contest_engine"
    required_input_fields = ['participant_data', 'contest_config']
    required_output_fields = ['contest_setup', 'leaderboard']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} contest engine", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} contest engine: {data}"
