...
class ReadabilityScorerEngine(BaseEngine):
    engine_name = "readability_scorer"
    required_input_fields = ['text_data', 'scoring_config']
    required_output_fields = ['readability_scores', 'improvement_suggestions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} readability scorer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} readability scorer: {data}"
