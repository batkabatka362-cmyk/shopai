...
class InteractionDesignEngine(BaseEngine):
    engine_name = "interaction_design"
    required_input_fields = ['component_data', 'interaction_config']
    required_output_fields = ['interaction_specs', 'micro_interactions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} interaction design", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} interaction design: {data}"
