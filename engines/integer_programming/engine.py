...
class IntegerProgrammingEngine(BaseEngine):
    engine_name = "integer_programming"
    required_input_fields = ['constraint_data', 'objective_config']
    required_output_fields = ['optimal_solution', 'gap_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} integer programming", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} integer programming: {data}"
