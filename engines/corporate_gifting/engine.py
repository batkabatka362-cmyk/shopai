...
class CorporateGiftingEngine(BaseEngine):
    engine_name = "corporate_gifting"
    required_input_fields = ['corporate_data', 'gifting_config']
    required_output_fields = ['gifting_program', 'bulk_pricing']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} corporate gifting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} corporate gifting: {data}"
