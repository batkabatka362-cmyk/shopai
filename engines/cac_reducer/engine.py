...
class CacReducerEngine(BaseEngine):
    engine_name = "cac_reducer"
    required_input_fields = ['acquisition_data', 'channel_data']
    required_output_fields = ['reduction_plan', 'efficiency_gains']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cac reducer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cac reducer: {data}"
