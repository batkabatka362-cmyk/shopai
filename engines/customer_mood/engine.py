...
class CustomerMoodEngine(BaseEngine):
    engine_name = "customer_mood"
    required_input_fields = ['interaction_data', 'mood_config']
    required_output_fields = ['mood_scores', 'engagement_adjustments']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} customer mood", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} customer mood: {data}"
