...
class DemographicBidEngine(BaseEngine):
    engine_name = "demographic_bid"
    required_input_fields = ['demo_data', 'bid_config']
    required_output_fields = ['demo_adjustments', 'segment_roi']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} demographic bid", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} demographic bid: {data}"
