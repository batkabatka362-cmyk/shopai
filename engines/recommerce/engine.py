...
class RecommerceEngine(BaseEngine):
    engine_name = "recommerce"
    required_input_fields = ['used_product_data', 'recommerce_config']
    required_output_fields = ['listing_plan', 'pricing_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} recommerce", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} recommerce: {data}"
