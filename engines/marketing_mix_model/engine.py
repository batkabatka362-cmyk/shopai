...
class MarketingMixModelEngine(BaseEngine):
    engine_name = "marketing_mix_model"
    required_input_fields = ['marketing_data', 'model_config']
    required_output_fields = ['channel_effectiveness', 'budget_recommendation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} marketing mix model", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} marketing mix model: {data}"
