"""
SocialScheduler Engine — Social Scheduler
...
class SocialSchedulerEngine(BaseEngine):
    engine_name = "social_scheduler"
    required_input_fields = ['content_queue', 'platform_config']
    required_output_fields = ['posting_schedule', 'optimal_times']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} social scheduler", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} social scheduler: {data}"
