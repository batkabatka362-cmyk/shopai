"""
HashtagResearch Engine — Hashtag Research
...
class HashtagResearchEngine(BaseEngine):
    engine_name = "hashtag_research"
    required_input_fields = ['content_data', 'platform']
    required_output_fields = ['hashtag_sets', 'reach_estimates']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} hashtag research", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} hashtag research: {data}"
