...
class BatchSyncEngine(BaseEngine):
    engine_name = "batch_sync"
    required_input_fields = ['sync_config', 'schedule_data']
    required_output_fields = ['batch_schedule', 'sync_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} batch sync", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} batch sync: {data}"
