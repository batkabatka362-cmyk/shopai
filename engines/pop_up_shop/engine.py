...
class PopUpShopEngine(BaseEngine):
    engine_name = "pop_up_shop"
    required_input_fields = ['location_data', 'pop_up_config']
    required_output_fields = ['pop_up_plan', 'logistics_checklist']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pop up shop", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pop up shop: {data}"
