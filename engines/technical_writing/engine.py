...
class TechnicalWritingEngine(BaseEngine):
    engine_name = "technical_writing"
    required_input_fields = ['topic_data', 'writing_config']
    required_output_fields = ['technical_content', 'review_checklist']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} technical writing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} technical writing: {data}"
