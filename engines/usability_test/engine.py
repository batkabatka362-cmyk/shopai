...
class UsabilityTestEngine(BaseEngine):
    engine_name = "usability_test"
    required_input_fields = ['test_data', 'usability_config']
    required_output_fields = ['test_plan', 'task_scenarios']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} usability test", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} usability test: {data}"
