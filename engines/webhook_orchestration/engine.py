...
class WebhookOrchestrationEngine(BaseEngine):
    engine_name = "webhook_orchestration"
    required_input_fields = ['webhook_data', 'orchestration_config']
    required_output_fields = ['webhook_flows', 'retry_policies']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} webhook orchestration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} webhook orchestration: {data}"
