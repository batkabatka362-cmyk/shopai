...
class OutfitBuilderEngine(BaseEngine):
    engine_name = "outfit_builder"
    required_input_fields = ['product_catalog', 'style_config']
    required_output_fields = ['outfit_combinations', 'style_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} outfit builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} outfit builder: {data}"
