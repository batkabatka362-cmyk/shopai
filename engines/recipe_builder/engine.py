...
class RecipeBuilderEngine(BaseEngine):
    engine_name = "recipe_builder"
    required_input_fields = ['ingredient_catalog', 'dietary_config']
    required_output_fields = ['recipe_suggestions', 'shopping_list']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} recipe builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} recipe builder: {data}"
