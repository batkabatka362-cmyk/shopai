...
class SmartContractCommerceEngine(BaseEngine):
    engine_name = "smart_contract_commerce"
    required_input_fields = ['commerce_data', 'contract_config']
    required_output_fields = ['contract_spec', 'deployment_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} smart contract commerce", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} smart contract commerce: {data}"
