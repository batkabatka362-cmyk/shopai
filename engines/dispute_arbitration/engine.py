...
class DisputeArbitrationEngine(BaseEngine):
    engine_name = "dispute_arbitration"
    required_input_fields = ['dispute_data', 'arbitration_config']
    required_output_fields = ['arbitration_decision', 'enforcement_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dispute arbitration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dispute arbitration: {data}"
