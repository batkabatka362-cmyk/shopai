...
class ShoppingCampaignEngine(BaseEngine):
    engine_name = "shopping_campaign"
    required_input_fields = ['feed_data', 'campaign_config']
    required_output_fields = ['campaign_setup', 'bid_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shopping campaign", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shopping campaign: {data}"
