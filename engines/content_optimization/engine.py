...
class ContentOptimizationEngine(BaseEngine):
    engine_name = "content_optimization"
    required_input_fields = ['content_data', 'seo_config']
    required_output_fields = ['optimization_plan', 'keyword_density']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} content optimization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} content optimization: {data}"
