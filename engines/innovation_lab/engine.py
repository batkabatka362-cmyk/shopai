...
class InnovationLabEngine(BaseEngine):
    engine_name = "innovation_lab"
    required_input_fields = ['idea_data', 'lab_config']
    required_output_fields = ['experiment_plan', 'resource_allocation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} innovation lab", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} innovation lab: {data}"
