...
class ChannelAttributionAiEngine(BaseEngine):
    engine_name = "channel_attribution_ai"
    required_input_fields = ['touchpoint_data', 'attribution_config']
    required_output_fields = ['channel_weights', 'roi_by_channel']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} channel attribution ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} channel attribution ai: {data}"
