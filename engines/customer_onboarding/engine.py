...
class CustomerOnboardingEngine(BaseEngine):
    engine_name = "customer_onboarding"
    required_input_fields = ['customer_data', 'onboarding_config']
    required_output_fields = ['onboarding_flow', 'milestone_tracking']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} customer onboarding", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} customer onboarding: {data}"
