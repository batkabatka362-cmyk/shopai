...
class MemoryManagementAiEngine(BaseEngine):
    engine_name = "memory_management_ai"
    required_input_fields = ['conversation_data', 'memory_config']
    required_output_fields = ['memory_state', 'retrieval_index']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} memory management ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} memory management ai: {data}"
