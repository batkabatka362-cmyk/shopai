...
class PhotographyContractEngine(BaseEngine):
    engine_name = "photography_contract"
    required_input_fields = ['shoot_data', 'contract_config']
    required_output_fields = ['contract_draft', 'usage_rights']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} photography contract", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} photography contract: {data}"
