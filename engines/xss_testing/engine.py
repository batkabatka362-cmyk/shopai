...
class XssTestingEngine(BaseEngine):
    engine_name = "xss_testing"
    required_input_fields = ['input_data', 'xss_config']
    required_output_fields = ['xss_report', 'vulnerable_endpoints']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} xss testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} xss testing: {data}"
