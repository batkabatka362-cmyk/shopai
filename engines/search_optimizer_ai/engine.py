...
class SearchOptimizerAiEngine(BaseEngine):
    engine_name = "search_optimizer_ai"
    required_input_fields = ['search_data', 'catalog_data']
    required_output_fields = ['search_improvements', 'relevance_boost']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} search optimizer ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} search optimizer ai: {data}"
