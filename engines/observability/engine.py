...
class ObservabilityEngine(BaseEngine):
    engine_name = "observability"
    required_input_fields = ['system_data', 'observability_config']
    required_output_fields = ['observability_plan', 'dashboard_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} observability", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} observability: {data}"
