...
class DemandSensingEngine(BaseEngine):
    engine_name = "demand_sensing"
    required_input_fields = ['signal_data', 'sensing_config']
    required_output_fields = ['demand_signals', 'adjustment_recommendations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} demand sensing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} demand sensing: {data}"
