...
class RevenueRecognitionAscEngine(BaseEngine):
    engine_name = "revenue_recognition_asc"
    required_input_fields = ['contract_data', 'asc606_config']
    required_output_fields = ['recognition_schedule', 'performance_obligations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} revenue recognition asc", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} revenue recognition asc: {data}"
