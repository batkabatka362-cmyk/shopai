...
class ViewabilityEngine(BaseEngine):
    engine_name = "viewability"
    required_input_fields = ['impression_data', 'viewability_config']
    required_output_fields = ['viewability_scores', 'optimization_actions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} viewability", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} viewability: {data}"
