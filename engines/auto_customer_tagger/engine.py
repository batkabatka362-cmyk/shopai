...
class AutoCustomerTaggerEngine(BaseEngine):
    engine_name = "auto_customer_tagger"
    required_input_fields = ['customer_data', 'tag_rules']
    required_output_fields = ['tag_assignments', 'segment_updates']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} auto customer tagger", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} auto customer tagger: {data}"
