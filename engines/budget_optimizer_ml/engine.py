...
class BudgetOptimizerMlEngine(BaseEngine):
    engine_name = "budget_optimizer_ml"
    required_input_fields = ['budget_data', 'performance_data']
    required_output_fields = ['optimized_budget', 'allocation_rationale']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} budget optimizer ml", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} budget optimizer ml: {data}"
