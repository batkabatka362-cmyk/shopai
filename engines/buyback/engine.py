...
class BuybackEngine(BaseEngine):
    engine_name = "buyback"
    required_input_fields = ['product_data', 'buyback_config']
    required_output_fields = ['buyback_offer', 'condition_assessment']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} buyback", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} buyback: {data}"
