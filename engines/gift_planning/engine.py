...
class GiftPlanningEngine(BaseEngine):
    engine_name = "gift_planning"
    required_input_fields = ['recipient_data', 'occasion_config']
    required_output_fields = ['gift_plan', 'budget_allocation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} gift planning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} gift planning: {data}"
