...
class DigitalOohEngine(BaseEngine):
    engine_name = "digital_ooh"
    required_input_fields = ['campaign_data', 'dooh_config']
    required_output_fields = ['dooh_plan', 'location_targeting']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} digital ooh", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} digital ooh: {data}"
