...
class TreeShakingEngine(BaseEngine):
    engine_name = "tree_shaking"
    required_input_fields = ['module_data', 'analysis_config']
    required_output_fields = ['unused_exports', 'reduction_estimate']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} tree shaking", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} tree shaking: {data}"
