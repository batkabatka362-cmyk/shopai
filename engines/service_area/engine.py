...
class ServiceAreaEngine(BaseEngine):
    engine_name = "service_area"
    required_input_fields = ['location_data', 'service_config']
    required_output_fields = ['service_boundaries', 'coverage_map']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} service area", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} service area: {data}"
