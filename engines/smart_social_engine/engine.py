...
class SmartSocialEngineEngine(BaseEngine):
    engine_name = "smart_social_engine"
    required_input_fields = ['social_data', 'trend_data']
    required_output_fields = ['social_actions', 'content_calendar']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} smart social engine", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} smart social engine: {data}"
