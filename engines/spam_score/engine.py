...
class SpamScoreEngine(BaseEngine):
    engine_name = "spam_score"
    required_input_fields = ['domain_data', 'spam_config']
    required_output_fields = ['spam_score', 'risk_factors']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} spam score", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} spam score: {data}"
