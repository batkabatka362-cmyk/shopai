...
class OnboardingGuideEngine(BaseEngine):
    engine_name = "onboarding_guide"
    required_input_fields = ['product_data', 'user_persona']
    required_output_fields = ['guide_content', 'milestone_checklist']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} onboarding guide", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} onboarding guide: {data}"
