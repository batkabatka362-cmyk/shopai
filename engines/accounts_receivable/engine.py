...
class AccountsReceivableEngine(BaseEngine):
    engine_name = "accounts_receivable"
    required_input_fields = ['ar_data', 'management_config']
    required_output_fields = ['ar_aging', 'collection_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} accounts receivable", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} accounts receivable: {data}"
