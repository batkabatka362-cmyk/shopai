...
class StyleTileEngine(BaseEngine):
    engine_name = "style_tile"
    required_input_fields = ['brand_data', 'tile_config']
    required_output_fields = ['style_tile', 'mood_reference']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} style tile", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} style tile: {data}"
