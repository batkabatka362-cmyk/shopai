...
class IncidentResponseEngine(BaseEngine):
    engine_name = "incident_response"
    required_input_fields = ['incident_data', 'response_config']
    required_output_fields = ['response_plan', 'communication_draft']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} incident response", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} incident response: {data}"
