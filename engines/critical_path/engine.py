...
class CriticalPathEngine(BaseEngine):
    engine_name = "critical_path"
    required_input_fields = ['task_data', 'project_config']
    required_output_fields = ['critical_path', 'slack_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} critical path", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} critical path: {data}"
