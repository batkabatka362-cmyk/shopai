...
class LogAnalysisEngine(BaseEngine):
    engine_name = "log_analysis"
    required_input_fields = ['log_data', 'analysis_config']
    required_output_fields = ['log_insights', 'anomaly_alerts']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} log analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} log analysis: {data}"
