...
class SkillAssessmentEngine(BaseEngine):
    engine_name = "skill_assessment"
    required_input_fields = ['employee_data', 'assessment_config']
    required_output_fields = ['skill_matrix', 'training_needs']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} skill assessment", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} skill assessment: {data}"
