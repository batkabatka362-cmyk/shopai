...
class PaginationStrategyEngine(BaseEngine):
    engine_name = "pagination_strategy"
    required_input_fields = ['catalog_data', 'ux_config']
    required_output_fields = ['pagination_config', 'load_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pagination strategy", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pagination strategy: {data}"
