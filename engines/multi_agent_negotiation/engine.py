...
class MultiAgentNegotiationEngine(BaseEngine):
    engine_name = "multi_agent_negotiation"
    required_input_fields = ['agent_data', 'negotiation_config']
    required_output_fields = ['agreement', 'trade_offs']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} multi agent negotiation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} multi agent negotiation: {data}"
