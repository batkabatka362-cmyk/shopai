...
class RelocationAssistanceEngine(BaseEngine):
    engine_name = "relocation_assistance"
    required_input_fields = ['employee_data', 'relocation_config']
    required_output_fields = ['assistance_package', 'vendor_coordination']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} relocation assistance", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} relocation assistance: {data}"
