...
class StatusPageEngine(BaseEngine):
    engine_name = "status_page"
    required_input_fields = ['service_data', 'page_config']
    required_output_fields = ['status_display', 'incident_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} status page", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} status page: {data}"
