"""
EmailDeliverability Engine — Email Deliverability
...
class EmailDeliverabilityEngine(BaseEngine):
    engine_name = "email_deliverability"
    required_input_fields = ['email_config', 'sender_data']
    required_output_fields = ['deliverability_score', 'improvement_actions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} email deliverability", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} email deliverability: {data}"
