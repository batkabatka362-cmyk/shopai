...
class DataAugmentationEngine(BaseEngine):
    engine_name = "data_augmentation"
    required_input_fields = ['training_data', 'augmentation_config']
    required_output_fields = ['augmented_data', 'diversity_metrics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} data augmentation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} data augmentation: {data}"
