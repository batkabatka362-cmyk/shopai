"""Returns Management Engine — memory reader.

Reads past returns processing records from memory storage.
Used to inform current processing with historical context.
"""
...
def read_past_returns(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past returns records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_returns(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past returns records."""
    if not records:
        return {
            "avg_return_rate": 0.0,
            "avg_total_cost": 0.0,
    ...
    return_rates = [r.get("return_rate", 0.0) for r in records]
    costs = [r.get("total_cost", 0.0) for r in records]
    fraud_counts = [r.get("fraud_flags_count", 0) for r in records]

    n = len(records)
    return {
        "avg_return_rate": round(sum(return_rates) / n, 3) if n else 0.0,
        "avg_total_cost": round(sum(costs) / n, 2) if n else 0.0,
        "avg_fraud_flags": round(sum(fraud_counts) / n, 1) if n else 0.0,
        "total_runs": n,
    }
