...
class CohortRetentionEngine(BaseEngine):
    engine_name = "cohort_retention"
    required_input_fields = ['cohort_data', 'retention_config']
    required_output_fields = ['retention_curves', 'churn_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cohort retention", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cohort retention: {data}"
