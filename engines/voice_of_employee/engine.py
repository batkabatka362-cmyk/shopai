...
class VoiceOfEmployeeEngine(BaseEngine):
    engine_name = "voice_of_employee"
    required_input_fields = ['survey_data', 'analysis_config']
    required_output_fields = ['employee_insights', 'action_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} voice of employee", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} voice of employee: {data}"
