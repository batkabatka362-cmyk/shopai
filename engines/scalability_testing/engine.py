...
class ScalabilityTestingEngine(BaseEngine):
    engine_name = "scalability_testing"
    required_input_fields = ['system_data', 'scalability_config']
    required_output_fields = ['scaling_curve', 'capacity_limits']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} scalability testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} scalability testing: {data}"
