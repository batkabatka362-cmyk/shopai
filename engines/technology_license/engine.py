...
class TechnologyLicenseEngine(BaseEngine):
    engine_name = "technology_license"
    required_input_fields = ['tech_data', 'license_config']
    required_output_fields = ['license_terms', 'usage_restrictions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} technology license", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} technology license: {data}"
