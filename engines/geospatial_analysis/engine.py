...
class GeospatialAnalysisEngine(BaseEngine):
    engine_name = "geospatial_analysis"
    required_input_fields = ['location_data', 'analysis_config']
    required_output_fields = ['spatial_report', 'cluster_map']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} geospatial analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} geospatial analysis: {data}"
