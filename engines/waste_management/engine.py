...
class WasteManagementEngine(BaseEngine):
    engine_name = "waste_management"
    required_input_fields = ['waste_data', 'reduction_config']
    required_output_fields = ['waste_report', 'reduction_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} waste management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} waste management: {data}"
