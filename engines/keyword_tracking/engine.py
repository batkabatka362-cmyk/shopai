...
class KeywordTrackingEngine(BaseEngine):
    engine_name = "keyword_tracking"
    required_input_fields = ['keyword_data', 'tracking_config']
    required_output_fields = ['ranking_report', 'movement_alerts']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} keyword tracking", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} keyword tracking: {data}"
