...
class TypographySelectorEngine(BaseEngine):
    engine_name = "typography_selector"
    required_input_fields = ['brand_data', 'typography_config']
    required_output_fields = ['font_selection', 'hierarchy_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} typography selector", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} typography selector: {data}"
