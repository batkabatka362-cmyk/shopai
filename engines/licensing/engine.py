...
class LicensingEngine(BaseEngine):
    engine_name = "licensing"
    required_input_fields = ['ip_data', 'licensing_config']
    required_output_fields = ['license_terms', 'royalty_structure']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} licensing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} licensing: {data}"
