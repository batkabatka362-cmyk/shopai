...
class SellerRatingEngine(BaseEngine):
    engine_name = "seller_rating"
    required_input_fields = ['seller_data', 'rating_config']
    required_output_fields = ['seller_scores', 'ranking_position']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} seller rating", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} seller rating: {data}"
