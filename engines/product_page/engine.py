...
class ProductPageEngine(BaseEngine):
    engine_name = "product_page"
    required_input_fields = ['product_data', 'page_config']
    required_output_fields = ['page_layout', 'component_order']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} product page", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} product page: {data}"
