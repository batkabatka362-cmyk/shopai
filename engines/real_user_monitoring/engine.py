...
class RealUserMonitoringEngine(BaseEngine):
    engine_name = "real_user_monitoring"
    required_input_fields = ['rum_data', 'monitoring_config']
    required_output_fields = ['user_experience_report', 'performance_breakdown']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} real user monitoring", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} real user monitoring: {data}"
