...
class SurveyBuilderEngine(BaseEngine):
    engine_name = "survey_builder"
    required_input_fields = ['question_data', 'survey_config']
    required_output_fields = ['survey_flow', 'logic_branching']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} survey builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} survey builder: {data}"
