...
class AnchorTextEngine(BaseEngine):
    engine_name = "anchor_text"
    required_input_fields = ['link_data', 'optimization_config']
    required_output_fields = ['anchor_recommendations', 'diversity_score']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} anchor text", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} anchor text: {data}"
