...
class FlightRiskEngine(BaseEngine):
    engine_name = "flight_risk"
    required_input_fields = ['employee_data', 'risk_config']
    required_output_fields = ['flight_risk_scores', 'retention_actions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} flight risk", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} flight risk: {data}"
