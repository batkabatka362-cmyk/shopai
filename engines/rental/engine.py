...
class RentalEngine(BaseEngine):
    engine_name = "rental"
    required_input_fields = ['product_data', 'rental_config']
    required_output_fields = ['rental_terms', 'availability_calendar']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} rental", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} rental: {data}"
