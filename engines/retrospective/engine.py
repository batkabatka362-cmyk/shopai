...
class RetrospectiveEngine(BaseEngine):
    engine_name = "retrospective"
    required_input_fields = ['sprint_data', 'retro_config']
    required_output_fields = ['retro_summary', 'action_items']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} retrospective", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} retrospective: {data}"
