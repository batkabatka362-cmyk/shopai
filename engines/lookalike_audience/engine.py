...
class LookalikeAudienceEngine(BaseEngine):
    engine_name = "lookalike_audience"
    required_input_fields = ['seed_audience', 'platform_config']
    required_output_fields = ['lookalike_segments', 'expansion_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} lookalike audience", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} lookalike audience: {data}"
