...
class MixAndMatchEngine(BaseEngine):
    engine_name = "mix_and_match"
    required_input_fields = ['product_data', 'mix_config']
    required_output_fields = ['mix_deals', 'pricing_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} mix and match", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} mix and match: {data}"
