...
class RealTimeSyncEngine(BaseEngine):
    engine_name = "real_time_sync"
    required_input_fields = ['sync_config', 'source_data']
    required_output_fields = ['sync_status', 'conflict_resolution']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} real time sync", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} real time sync: {data}"
