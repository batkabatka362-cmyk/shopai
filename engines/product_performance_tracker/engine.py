"""
ProductPerformanceTracker Engine
...
class ProductPerformanceTrackerEngine(BaseEngine):
    engine_name = "product_performance_tracker"
    required_input_fields = ['product_data', 'performance_config']
    required_output_fields = ['performance_report', 'trend_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} product performance tracker", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} product performance tracker: {data}"
