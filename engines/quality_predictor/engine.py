...
class QualityPredictorEngine(BaseEngine):
    engine_name = "quality_predictor"
    required_input_fields = ['production_data', 'quality_data']
    required_output_fields = ['quality_forecast', 'defect_prevention']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} quality predictor", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} quality predictor: {data}"
