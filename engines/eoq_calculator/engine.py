...
class EoqCalculatorEngine(BaseEngine):
    engine_name = "eoq_calculator"
    required_input_fields = ['demand_data', 'cost_data']
    required_output_fields = ['optimal_quantities', 'reorder_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} eoq calculator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} eoq calculator: {data}"
