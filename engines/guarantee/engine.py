...
class GuaranteeEngine(BaseEngine):
    engine_name = "guarantee"
    required_input_fields = ['guarantor_data', 'guarantee_config']
    required_output_fields = ['guarantee_draft', 'liability_scope']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} guarantee", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} guarantee: {data}"
