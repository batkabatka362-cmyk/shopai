...
class PalletOptimizationEngine(BaseEngine):
    engine_name = "pallet_optimization"
    required_input_fields = ['product_data', 'pallet_config']
    required_output_fields = ['pallet_plan', 'stacking_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pallet optimization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pallet optimization: {data}"
