...
class ScreenSharingEngine(BaseEngine):
    engine_name = "screen_sharing"
    required_input_fields = ['session_data', 'sharing_config']
    required_output_fields = ['sharing_setup', 'recording_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} screen sharing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} screen sharing: {data}"
