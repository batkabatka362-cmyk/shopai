...
class VeganCertificationEngine(BaseEngine):
    engine_name = "vegan_certification"
    required_input_fields = ['product_data', 'vegan_config']
    required_output_fields = ['certification_plan', 'ingredient_audit']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} vegan certification", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} vegan certification: {data}"
