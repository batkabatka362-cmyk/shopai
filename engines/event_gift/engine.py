...
class EventGiftEngine(BaseEngine):
    engine_name = "event_gift"
    required_input_fields = ['event_data', 'gift_config']
    required_output_fields = ['gift_selection', 'delivery_coordination']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} event gift", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} event gift: {data}"
