"""
CustomerScoring Engine — Customer Scoring
...
class CustomerScoringEngine(BaseEngine):
    engine_name = "customer_scoring"
    required_input_fields = ['customer_data', 'scoring_model']
    required_output_fields = ['customer_scores', 'score_breakdown']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} customer scoring", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} customer scoring: {data}"
