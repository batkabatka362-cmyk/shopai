...
class MinificationAnalysisEngine(BaseEngine):
    engine_name = "minification_analysis"
    required_input_fields = ['asset_data', 'analysis_config']
    required_output_fields = ['minification_report', 'size_reduction']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} minification analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} minification analysis: {data}"
