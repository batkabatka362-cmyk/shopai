...
class ModernSlaveryEngine(BaseEngine):
    engine_name = "modern_slavery"
    required_input_fields = ['supply_data', 'slavery_config']
    required_output_fields = ['slavery_statement', 'due_diligence_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} modern slavery", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} modern slavery: {data}"
