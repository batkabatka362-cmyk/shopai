...
class OpenSourceStrategyEngine(BaseEngine):
    engine_name = "open_source_strategy"
    required_input_fields = ['project_data', 'oss_config']
    required_output_fields = ['oss_plan', 'contribution_guidelines']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} open source strategy", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} open source strategy: {data}"
