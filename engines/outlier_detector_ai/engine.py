...
class OutlierDetectorAiEngine(BaseEngine):
    engine_name = "outlier_detector_ai"
    required_input_fields = ['dataset', 'detection_config']
    required_output_fields = ['outliers', 'explanation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} outlier detector ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} outlier detector ai: {data}"
