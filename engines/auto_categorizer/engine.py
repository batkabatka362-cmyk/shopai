...
class AutoCategorizerEngine(BaseEngine):
    engine_name = "auto_categorizer"
    required_input_fields = ['item_data', 'category_config']
    required_output_fields = ['category_assignments', 'confidence_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} auto categorizer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} auto categorizer: {data}"
