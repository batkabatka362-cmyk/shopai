...
class PetInsuranceEngine(BaseEngine):
    engine_name = "pet_insurance"
    required_input_fields = ['employee_data', 'pet_config']
    required_output_fields = ['plan_options', 'coverage_details']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pet insurance", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pet insurance: {data}"
