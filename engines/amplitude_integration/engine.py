...
class AmplitudeIntegrationEngine(BaseEngine):
    engine_name = "amplitude_integration"
    required_input_fields = ['event_data', 'integration_config']
    required_output_fields = ['amplitude_setup', 'taxonomy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} amplitude integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} amplitude integration: {data}"
