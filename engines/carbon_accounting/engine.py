...
class CarbonAccountingEngine(BaseEngine):
    engine_name = "carbon_accounting"
    required_input_fields = ['emission_data', 'accounting_config']
    required_output_fields = ['carbon_report', 'scope_breakdown']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} carbon accounting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} carbon accounting: {data}"
