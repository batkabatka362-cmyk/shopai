...
class GrossMarginAnalysisEngine(BaseEngine):
    engine_name = "gross_margin_analysis"
    required_input_fields = ['revenue_data', 'cost_data']
    required_output_fields = ['margin_report', 'optimization_actions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} gross margin analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} gross margin analysis: {data}"
