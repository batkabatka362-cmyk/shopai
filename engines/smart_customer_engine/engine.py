...
class SmartCustomerEngineEngine(BaseEngine):
    engine_name = "smart_customer_engine"
    required_input_fields = ['customer_data', 'interaction_data']
    required_output_fields = ['customer_actions', 'engagement_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} smart customer engine", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} smart customer engine: {data}"
