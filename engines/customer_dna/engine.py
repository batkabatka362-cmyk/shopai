...
class CustomerDnaEngine(BaseEngine):
    engine_name = "customer_dna"
    required_input_fields = ['customer_data', 'behavior_data']
    required_output_fields = ['dna_profile', 'preference_map']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} customer dna", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} customer dna: {data}"
