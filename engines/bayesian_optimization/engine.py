...
class BayesianOptimizationEngine(BaseEngine):
    engine_name = "bayesian_optimization"
    required_input_fields = ['parameter_space', 'bo_config']
    required_output_fields = ['best_parameters', 'acquisition_function']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} bayesian optimization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} bayesian optimization: {data}"
