...
class SreDashboardEngine(BaseEngine):
    engine_name = "sre_dashboard"
    required_input_fields = ['sli_data', 'slo_config']
    required_output_fields = ['dashboard_config', 'error_budget']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sre dashboard", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sre dashboard: {data}"
