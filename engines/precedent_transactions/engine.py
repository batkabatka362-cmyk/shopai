...
class PrecedentTransactionsEngine(BaseEngine):
    engine_name = "precedent_transactions"
    required_input_fields = ['deal_data', 'analysis_config']
    required_output_fields = ['transaction_multiples', 'implied_value']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} precedent transactions", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} precedent transactions: {data}"
