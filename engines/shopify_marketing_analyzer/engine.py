"""
ShopifyMarketingAnalyzer Engine
...
class ShopifyMarketingAnalyzerEngine(BaseEngine):
    engine_name = "shopify_marketing_analyzer"
    required_input_fields = ['marketing_data', 'channel_data']
    required_output_fields = ['marketing_insights', 'channel_performance']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shopify marketing analyzer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shopify marketing analyzer: {data}"
