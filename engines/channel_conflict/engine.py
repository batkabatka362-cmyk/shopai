...
class ChannelConflictEngine(BaseEngine):
    engine_name = "channel_conflict"
    required_input_fields = ['channel_data', 'pricing_data']
    required_output_fields = ['conflict_report', 'resolution_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} channel conflict", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} channel conflict: {data}"
