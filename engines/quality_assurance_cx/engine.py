...
class QualityAssuranceCxEngine(BaseEngine):
    engine_name = "quality_assurance_cx"
    required_input_fields = ['interaction_data', 'qa_config']
    required_output_fields = ['qa_scores', 'coaching_feedback']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} quality assurance cx", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} quality assurance cx: {data}"
