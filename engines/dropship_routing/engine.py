...
class DropshipRoutingEngine(BaseEngine):
    engine_name = "dropship_routing"
    required_input_fields = ['order_data', 'supplier_config']
    required_output_fields = ['routing_plan', 'supplier_notifications']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dropship routing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dropship routing: {data}"
