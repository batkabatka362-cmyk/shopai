...
class PurchaseOrderEngine(BaseEngine):
    engine_name = "purchase_order"
    required_input_fields = ['requisition_data', 'po_config']
    required_output_fields = ['purchase_order', 'approval_status']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} purchase order", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} purchase order: {data}"
