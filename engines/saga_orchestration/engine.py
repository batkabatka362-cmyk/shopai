...
class SagaOrchestrationEngine(BaseEngine):
    engine_name = "saga_orchestration"
    required_input_fields = ['transaction_data', 'saga_config']
    required_output_fields = ['saga_definition', 'compensation_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} saga orchestration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} saga orchestration: {data}"
