"""
DataEncryptionInTransit Engine
...
class DataEncryptionInTransitEngine(BaseEngine):
    engine_name = "data_encryption_in_transit"
    required_input_fields = ['endpoint_data', 'tls_config']
    required_output_fields = ['tls_status', 'certificate_health']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} data encryption in transit", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} data encryption in transit: {data}"
