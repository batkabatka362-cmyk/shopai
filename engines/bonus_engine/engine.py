...
class BonusEngineEngine(BaseEngine):
    engine_name = "bonus_engine"
    required_input_fields = ['performance_data', 'bonus_config']
    required_output_fields = ['bonus_calc', 'eligibility_check']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} bonus engine", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} bonus engine: {data}"
