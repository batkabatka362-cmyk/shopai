...
class SmartSearchAiEngine(BaseEngine):
    engine_name = "smart_search_ai"
    required_input_fields = ['search_query', 'catalog_data']
    required_output_fields = ['search_results', 'nlp_understanding']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} smart search ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} smart search ai: {data}"
