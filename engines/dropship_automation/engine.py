...
class DropshipAutomationEngine(BaseEngine):
    engine_name = "dropship_automation"
    required_input_fields = ['order_data', 'supplier_config']
    required_output_fields = ['automation_rules', 'routing_logic']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dropship automation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dropship automation: {data}"
