...
class MarketEntryEngine(BaseEngine):
    engine_name = "market_entry"
    required_input_fields = ['market_data', 'entry_config']
    required_output_fields = ['entry_strategy', 'risk_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} market entry", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} market entry: {data}"
