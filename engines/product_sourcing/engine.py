...
class ProductSourcingEngine(BaseEngine):
    engine_name = "product_sourcing"
    required_input_fields = ['product_requirements', 'supplier_database']
    required_output_fields = ['sourcing_options', 'cost_comparison']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} product sourcing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} product sourcing: {data}"
