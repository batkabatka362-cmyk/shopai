...
class PricePredictionAiEngine(BaseEngine):
    engine_name = "price_prediction_ai"
    required_input_fields = ['price_history', 'market_data']
    required_output_fields = ['price_forecast', 'confidence_bands']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} price prediction ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} price prediction ai: {data}"
