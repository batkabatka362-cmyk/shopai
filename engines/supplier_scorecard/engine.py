...
class SupplierScorecardEngine(BaseEngine):
    engine_name = "supplier_scorecard"
    required_input_fields = ['performance_data', 'scorecard_config']
    required_output_fields = ['supplier_scores', 'improvement_targets']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} supplier scorecard", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} supplier scorecard: {data}"
