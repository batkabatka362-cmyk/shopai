"""Marketplace Engine — memory reader.

Reads past marketplace sync records from memory storage.
Used to inform current sync with historical context.
"""
...
def read_past_marketplace(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past marketplace records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_marketplace(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past marketplace records."""
    if not records:
        return {
            "avg_sync_errors": 0.0,
            "avg_platforms": 0.0,
    ...
    platform_counts = []
    for r in records:
        statuses = r.get("sync_status", [])
        total_errors = sum(len(s.get("errors", [])) for s in statuses)
        sync_errors.append(total_errors)
        platform_counts.append(len(statuses))

    n = len(records)
    return {
        "avg_sync_errors": round(sum(sync_errors) / n, 1) if n else 0.0,
        "avg_platforms": round(sum(platform_counts) / n, 1) if n else 0.0,
        "total_runs": n,
    }
