...
class CitationVerificationEngine(BaseEngine):
    engine_name = "citation_verification"
    required_input_fields = ['citation_data', 'source_data']
    required_output_fields = ['verification_status', 'accuracy_score']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} citation verification", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} citation verification: {data}"
