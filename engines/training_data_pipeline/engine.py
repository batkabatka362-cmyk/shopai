...
class TrainingDataPipelineEngine(BaseEngine):
    engine_name = "training_data_pipeline"
    required_input_fields = ['raw_data', 'pipeline_config']
    required_output_fields = ['processed_dataset', 'quality_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} training data pipeline", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} training data pipeline: {data}"
