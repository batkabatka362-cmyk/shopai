...
class WarrantyClauseEngine(BaseEngine):
    engine_name = "warranty_clause"
    required_input_fields = ['product_data', 'warranty_config']
    required_output_fields = ['warranty_terms', 'limitation_language']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} warranty clause", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} warranty clause: {data}"
