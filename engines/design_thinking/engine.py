...
class DesignThinkingEngine(BaseEngine):
    engine_name = "design_thinking"
    required_input_fields = ['problem_data', 'dt_config']
    required_output_fields = ['design_sprint', 'prototype_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} design thinking", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} design thinking: {data}"
