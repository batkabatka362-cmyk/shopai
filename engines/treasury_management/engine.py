...
class TreasuryManagementEngine(BaseEngine):
    engine_name = "treasury_management"
    required_input_fields = ['cash_data', 'treasury_config']
    required_output_fields = ['treasury_plan', 'investment_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} treasury management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} treasury management: {data}"
