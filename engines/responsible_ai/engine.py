...
class ResponsibleAiEngine(BaseEngine):
    engine_name = "responsible_ai"
    required_input_fields = ['system_data', 'rai_config']
    required_output_fields = ['rai_scorecard', 'improvement_actions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} responsible ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} responsible ai: {data}"
