...
class NonprofitPricingEngine(BaseEngine):
    engine_name = "nonprofit_pricing"
    required_input_fields = ['org_data', 'pricing_config']
    required_output_fields = ['nonprofit_prices', 'verification_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} nonprofit pricing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} nonprofit pricing: {data}"
