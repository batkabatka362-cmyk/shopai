...
class CoverageAnalysisEngine(BaseEngine):
    engine_name = "coverage_analysis"
    required_input_fields = ['code_data', 'coverage_config']
    required_output_fields = ['coverage_report', 'uncovered_paths']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} coverage analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} coverage analysis: {data}"
