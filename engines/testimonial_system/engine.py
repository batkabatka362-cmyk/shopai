...
class TestimonialSystemEngine(BaseEngine):
    engine_name = "testimonial_system"
    required_input_fields = ['testimonial_data', 'system_config']
    required_output_fields = ['testimonial_flow', 'display_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} testimonial system", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} testimonial system: {data}"
