...
class EmailValidationEngine(BaseEngine):
    engine_name = "email_validation"
    required_input_fields = ['email_data', 'validation_config']
    required_output_fields = ['validated_emails', 'deliverability_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} email validation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} email validation: {data}"
