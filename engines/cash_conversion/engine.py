...
class CashConversionEngine(BaseEngine):
    engine_name = "cash_conversion"
    required_input_fields = ['financial_data', 'analysis_config']
    required_output_fields = ['ccc_metrics', 'improvement_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cash conversion", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cash conversion: {data}"
