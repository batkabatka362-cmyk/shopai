...
class EconomyDeliveryEngine(BaseEngine):
    engine_name = "economy_delivery"
    required_input_fields = ['order_data', 'economy_config']
    required_output_fields = ['economy_options', 'transit_estimate']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} economy delivery", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} economy delivery: {data}"
