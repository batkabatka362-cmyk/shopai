...
class RevenueRecognitionEngine(BaseEngine):
    engine_name = "revenue_recognition"
    required_input_fields = ['revenue_data', 'recognition_config']
    required_output_fields = ['recognition_report', 'deferred_revenue']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} revenue recognition", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} revenue recognition: {data}"
