...
class PitchDeckEngine(BaseEngine):
    engine_name = "pitch_deck"
    required_input_fields = ['company_data', 'investor_profile']
    required_output_fields = ['deck_content', 'financial_projections']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pitch deck", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pitch deck: {data}"
