...
class PandemicPlanningEngine(BaseEngine):
    engine_name = "pandemic_planning"
    required_input_fields = ['scenario_data', 'planning_config']
    required_output_fields = ['response_plan', 'continuity_measures']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pandemic planning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pandemic planning: {data}"
