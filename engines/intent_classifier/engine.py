...
class IntentClassifierEngine(BaseEngine):
    engine_name = "intent_classifier"
    required_input_fields = ['text_data', 'intent_config']
    required_output_fields = ['intents', 'confidence_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} intent classifier", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} intent classifier: {data}"
