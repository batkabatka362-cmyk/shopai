...
class DocumentGenerationEngine(BaseEngine):
    engine_name = "document_generation"
    required_input_fields = ['template_data', 'data_source']
    required_output_fields = ['generated_document', 'format_options']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} document generation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} document generation: {data}"
