...
class CompetitorCampaignEngine(BaseEngine):
    engine_name = "competitor_campaign"
    required_input_fields = ['competitor_data', 'campaign_config']
    required_output_fields = ['campaign_setup', 'bidding_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} competitor campaign", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} competitor campaign: {data}"
