...
class PostEventEngine(BaseEngine):
    engine_name = "post_event"
    required_input_fields = ['event_data', 'follow_up_config']
    required_output_fields = ['follow_up_plan', 'content_repurpose']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} post event", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} post event: {data}"
