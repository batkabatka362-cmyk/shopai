...
class FlashSaleAlertEngine(BaseEngine):
    engine_name = "flash_sale_alert"
    required_input_fields = ['sale_data', 'subscriber_data']
    required_output_fields = ['sale_notifications', 'countdown_elements']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} flash sale alert", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} flash sale alert: {data}"
