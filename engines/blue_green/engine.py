...
class BlueGreenEngine(BaseEngine):
    engine_name = "blue_green"
    required_input_fields = ['deployment_data', 'bg_config']
    required_output_fields = ['deployment_plan', 'switch_criteria']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} blue green", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} blue green: {data}"
