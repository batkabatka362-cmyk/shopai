...
class AmbassadorProgramEngine(BaseEngine):
    engine_name = "ambassador_program"
    required_input_fields = ['ambassador_data', 'program_config']
    required_output_fields = ['program_setup', 'reward_structure']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ambassador program", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ambassador program: {data}"
