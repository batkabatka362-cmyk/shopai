...
class PhysicalTicketEngine(BaseEngine):
    engine_name = "physical_ticket"
    required_input_fields = ['event_data', 'ticket_config']
    required_output_fields = ['ticket_design', 'barcode_generation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} physical ticket", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} physical ticket: {data}"
