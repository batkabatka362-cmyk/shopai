...
class IdentityGraphEngine(BaseEngine):
    engine_name = "identity_graph"
    required_input_fields = ['identity_data', 'graph_config']
    required_output_fields = ['graph_setup', 'resolution_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} identity graph", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} identity graph: {data}"
