...
class CodeRefactoringEngine(BaseEngine):
    ...
    required_input_fields = ['code_data', 'refactor_config']
    required_output_fields = ['refactored_code', 'improvement_summary']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} code refactoring", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} code refactoring: {data}"
