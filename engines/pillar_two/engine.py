...
class PillarTwoEngine(BaseEngine):
    engine_name = "pillar_two"
    required_input_fields = ['financial_data', 'pillar_config']
    required_output_fields = ['minimum_tax_calc', 'top_up_liability']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pillar two", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pillar two: {data}"
