...
class AccessCodeEngine(BaseEngine):
    engine_name = "access_code"
    required_input_fields = ['product_data', 'code_config']
    required_output_fields = ['access_codes', 'redemption_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} access code", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} access code: {data}"
