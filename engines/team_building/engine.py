...
class TeamBuildingEngine(BaseEngine):
    engine_name = "team_building"
    required_input_fields = ['team_data', 'activity_config']
    required_output_fields = ['activity_suggestions', 'scheduling']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} team building", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} team building: {data}"
