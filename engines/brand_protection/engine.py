...
class BrandProtectionEngine(BaseEngine):
    engine_name = "brand_protection"
    required_input_fields = ['marketplace_data', 'brand_config']
    required_output_fields = ['infringement_alerts', 'takedown_actions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} brand protection", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} brand protection: {data}"
