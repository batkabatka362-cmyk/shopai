...
class SdkBuilderEngine(BaseEngine):
    engine_name = "sdk_builder"
    required_input_fields = ['api_spec', 'sdk_config']
    required_output_fields = ['sdk_structure', 'code_samples']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sdk builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sdk builder: {data}"
