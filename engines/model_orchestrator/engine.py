...
class ModelOrchestratorEngine(BaseEngine):
    engine_name = "model_orchestrator"
    required_input_fields = ['task_data', 'model_config']
    required_output_fields = ['orchestration_plan', 'execution_order']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} model orchestrator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} model orchestrator: {data}"
