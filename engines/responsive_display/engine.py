...
class ResponsiveDisplayEngine(BaseEngine):
    engine_name = "responsive_display"
    required_input_fields = ['asset_data', 'display_config']
    required_output_fields = ['responsive_setup', 'asset_combinations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} responsive display", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} responsive display: {data}"
