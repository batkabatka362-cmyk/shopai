...
class SalesCommissionEngine(BaseEngine):
    engine_name = "sales_commission"
    required_input_fields = ['sales_data', 'commission_config']
    required_output_fields = ['commission_report', 'payout_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sales commission", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sales commission: {data}"
