...
class QuestionAnsweringEngine(BaseEngine):
    engine_name = "question_answering"
    required_input_fields = ['question', 'context_data']
    required_output_fields = ['answer', 'confidence_score']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} question answering", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} question answering: {data}"
