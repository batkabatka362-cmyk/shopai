...
class CashbackEngineEngine(BaseEngine):
    engine_name = "cashback_engine"
    required_input_fields = ['transaction_data', 'cashback_config']
    required_output_fields = ['cashback_calc', 'payout_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cashback engine", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cashback engine: {data}"
