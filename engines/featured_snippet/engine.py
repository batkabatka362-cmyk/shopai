...
class FeaturedSnippetEngine(BaseEngine):
    engine_name = "featured_snippet"
    required_input_fields = ['content_data', 'snippet_config']
    required_output_fields = ['snippet_targets', 'content_format']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} featured snippet", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} featured snippet: {data}"
