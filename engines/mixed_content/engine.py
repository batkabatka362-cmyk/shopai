...
class MixedContentEngine(BaseEngine):
    engine_name = "mixed_content"
    required_input_fields = ['page_data', 'analysis_config']
    required_output_fields = ['mixed_content_report', 'fix_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} mixed content", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} mixed content: {data}"
