...
class SponsorshipAgreementEngine(BaseEngine):
    engine_name = "sponsorship_agreement"
    required_input_fields = ['sponsor_data', 'agreement_config']
    required_output_fields = ['agreement_draft', 'activation_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sponsorship agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sponsorship agreement: {data}"
