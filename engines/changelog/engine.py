...
class ChangelogEngine(BaseEngine):
    engine_name = "changelog"
    required_input_fields = ['release_data', 'changelog_config']
    required_output_fields = ['changelog_content', 'notification_draft']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} changelog", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} changelog: {data}"
