...
class SplitShipmentEngine(BaseEngine):
    engine_name = "split_shipment"
    required_input_fields = ['order_data', 'split_config']
    required_output_fields = ['split_plan', 'tracking_setup']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} split shipment", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} split shipment: {data}"
