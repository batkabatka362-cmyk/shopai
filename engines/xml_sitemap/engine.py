...
class XmlSitemapEngine(BaseEngine):
    engine_name = "xml_sitemap"
    required_input_fields = ['page_data', 'sitemap_config']
    required_output_fields = ['sitemap_content', 'submission_status']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} xml sitemap", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} xml sitemap: {data}"
