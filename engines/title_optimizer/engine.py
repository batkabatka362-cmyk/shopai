...
class TitleOptimizerEngine(BaseEngine):
    engine_name = "title_optimizer"
    required_input_fields = ['page_data', 'keyword_data']
    required_output_fields = ['optimized_titles', 'ctr_predictions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} title optimizer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} title optimizer: {data}"
