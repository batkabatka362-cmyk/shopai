...
class AnomalyDetectorAiEngine(BaseEngine):
    engine_name = "anomaly_detector_ai"
    required_input_fields = ['metric_data', 'baseline_config']
    required_output_fields = ['anomalies', 'root_cause_suggestions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} anomaly detector ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} anomaly detector ai: {data}"
