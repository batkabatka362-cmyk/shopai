...
class RunbookAutomationEngine(BaseEngine):
    engine_name = "runbook_automation"
    required_input_fields = ['runbook_data', 'automation_config']
    required_output_fields = ['automated_runbook', 'execution_log']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} runbook automation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} runbook automation: {data}"
