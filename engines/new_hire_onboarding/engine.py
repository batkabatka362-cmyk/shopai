...
class NewHireOnboardingEngine(BaseEngine):
    engine_name = "new_hire_onboarding"
    required_input_fields = ['employee_data', 'onboarding_config']
    required_output_fields = ['onboarding_plan', 'checklist']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} new hire onboarding", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} new hire onboarding: {data}"
