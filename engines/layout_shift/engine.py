...
class LayoutShiftEngine(BaseEngine):
    engine_name = "layout_shift"
    required_input_fields = ['layout_data', 'analysis_config']
    required_output_fields = ['cls_score', 'shift_elements']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} layout shift", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} layout shift: {data}"
