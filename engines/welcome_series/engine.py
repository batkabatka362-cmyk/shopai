...
class WelcomeSeriesEngine(BaseEngine):
    engine_name = "welcome_series"
    required_input_fields = ['signup_data', 'series_config']
    required_output_fields = ['email_sequence', 'trigger_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} welcome series", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} welcome series: {data}"
