...
class BingUetEngine(BaseEngine):
    engine_name = "bing_uet"
    required_input_fields = ['tag_data', 'integration_config']
    required_output_fields = ['uet_setup', 'goal_mapping']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} bing uet", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} bing uet: {data}"
