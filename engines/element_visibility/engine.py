...
class ElementVisibilityEngine(BaseEngine):
    engine_name = "element_visibility"
    required_input_fields = ['page_data', 'visibility_config']
    required_output_fields = ['visibility_report', 'above_fold_score']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} element visibility", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} element visibility: {data}"
