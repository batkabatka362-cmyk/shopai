...
class E2eTestingEngine(BaseEngine):
    engine_name = "e2e_testing"
    required_input_fields = ['flow_data', 'test_config']
    required_output_fields = ['test_results', 'screenshot_diffs']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} e2e testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} e2e testing: {data}"
