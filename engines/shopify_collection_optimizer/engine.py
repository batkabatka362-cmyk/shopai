"""
ShopifyCollectionOptimizer Engine
...
class ShopifyCollectionOptimizerEngine(BaseEngine):
    engine_name = "shopify_collection_optimizer"
    required_input_fields = ['collection_data', 'optimization_config']
    required_output_fields = ['collection_plan', 'merchandising_order']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shopify collection optimizer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shopify collection optimizer: {data}"
