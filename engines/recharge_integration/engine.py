...
class RechargeIntegrationEngine(BaseEngine):
    engine_name = "recharge_integration"
    required_input_fields = ['subscription_data', 'integration_config']
    required_output_fields = ['recharge_sync', 'plan_mapping']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} recharge integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} recharge integration: {data}"
