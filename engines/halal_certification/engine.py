...
class HalalCertificationEngine(BaseEngine):
    engine_name = "halal_certification"
    required_input_fields = ['product_data', 'halal_config']
    required_output_fields = ['certification_plan', 'compliance_audit']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} halal certification", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} halal certification: {data}"
