...
class GrowthHackingEngine(BaseEngine):
    engine_name = "growth_hacking"
    required_input_fields = ['growth_data', 'experiment_config']
    required_output_fields = ['growth_experiments', 'prioritized_tactics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} growth hacking", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} growth hacking: {data}"
