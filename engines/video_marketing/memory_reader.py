"""Video Marketing Engine — memory reader.

Reads past video marketing records from memory storage.
Used to inform current content strategy with historical context.
"""
...
def read_past_videos(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past video marketing records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_videos(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past video records."""
    if not records:
        return {
            "avg_duration": 0.0,
    ...
    n = len(records)
    return {
        "avg_duration": round(sum(durations) / n, 1) if n else 0.0,
