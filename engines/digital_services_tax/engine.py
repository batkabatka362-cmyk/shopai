...
class DigitalServicesTaxEngine(BaseEngine):
    engine_name = "digital_services_tax"
    required_input_fields = ['revenue_data', 'dst_config']
    required_output_fields = ['dst_liability', 'filing_requirements']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} digital services tax", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} digital services tax: {data}"
