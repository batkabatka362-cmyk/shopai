...
class ScoringModelBuilderEngine(BaseEngine):
    engine_name = "scoring_model_builder"
    required_input_fields = ['criteria_data', 'model_config']
    required_output_fields = ['scoring_model', 'calibration_data']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} scoring model builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} scoring model builder: {data}"
