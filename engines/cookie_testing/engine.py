...
class CookieTestingEngine(BaseEngine):
    engine_name = "cookie_testing"
    required_input_fields = ['cookie_data', 'test_config']
    required_output_fields = ['cookie_report', 'compliance_check']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cookie testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cookie testing: {data}"
