...
class ReferralReminderEngine(BaseEngine):
    engine_name = "referral_reminder"
    required_input_fields = ['referral_data', 'reminder_config']
    required_output_fields = ['reminder_campaigns', 'reward_status']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} referral reminder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} referral reminder: {data}"
