...
class SearchConsoleEngine(BaseEngine):
    engine_name = "search_console"
    required_input_fields = ['gsc_data', 'analysis_config']
    required_output_fields = ['performance_report', 'opportunity_list']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} search console", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} search console: {data}"
