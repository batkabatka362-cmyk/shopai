...
class VisualSearchAiEngine(BaseEngine):
    engine_name = "visual_search_ai"
    required_input_fields = ['image_data', 'catalog_data']
    required_output_fields = ['visual_matches', 'similarity_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} visual search ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} visual search ai: {data}"
