...
class DnsTestingEngine(BaseEngine):
    engine_name = "dns_testing"
    required_input_fields = ['dns_data', 'test_config']
    required_output_fields = ['resolution_report', 'propagation_status']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dns testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dns testing: {data}"
