...
class ScienceBasedTargetsEngine(BaseEngine):
    engine_name = "science_based_targets"
    required_input_fields = ['emission_data', 'sbt_config']
    required_output_fields = ['target_setting', 'progress_tracking']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} science based targets", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} science based targets: {data}"
