...
class CookiePolicyGeneratorEngine(BaseEngine):
    engine_name = "cookie_policy_generator"
    required_input_fields = ['cookie_data', 'policy_config']
    required_output_fields = ['cookie_policy', 'consent_mechanism']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cookie policy generator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cookie policy generator: {data}"
