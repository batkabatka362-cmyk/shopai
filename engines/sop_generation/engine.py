...
class SopGenerationEngine(BaseEngine):
    engine_name = "sop_generation"
    required_input_fields = ['process_data', 'sop_config']
    required_output_fields = ['sop_document', 'checklist']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sop generation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sop generation: {data}"
