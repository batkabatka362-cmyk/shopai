...
class TopicModelingEngine(BaseEngine):
    engine_name = "topic_modeling"
    required_input_fields = ['document_data', 'topic_config']
    required_output_fields = ['topics', 'document_assignments']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} topic modeling", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} topic modeling: {data}"
