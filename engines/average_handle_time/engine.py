...
class AverageHandleTimeEngine(BaseEngine):
    engine_name = "average_handle_time"
    required_input_fields = ['interaction_data', 'aht_config']
    required_output_fields = ['aht_metrics', 'optimization_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} average handle time", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} average handle time: {data}"
