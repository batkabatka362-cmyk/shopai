...
class InfluenceScoreEngine(BaseEngine):
    engine_name = "influence_score"
    required_input_fields = ['social_data', 'influence_config']
    required_output_fields = ['influence_scores', 'influencer_tier']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} influence score", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} influence score: {data}"
