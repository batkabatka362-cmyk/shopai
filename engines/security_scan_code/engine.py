...
class SecurityScanCodeEngine(BaseEngine):
    engine_name = "security_scan_code"
    required_input_fields = ['code_data', 'scan_config']
    required_output_fields = ['vulnerabilities', 'fix_suggestions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} security scan code", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} security scan code: {data}"
