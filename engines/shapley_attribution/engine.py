...
class ShapleyAttributionEngine(BaseEngine):
    engine_name = "shapley_attribution"
    required_input_fields = ['touchpoint_data', 'shapley_config']
    required_output_fields = ['shapley_values', 'channel_contribution']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shapley attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shapley attribution: {data}"
