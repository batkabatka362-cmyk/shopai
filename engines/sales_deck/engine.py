...
class SalesDeckEngine(BaseEngine):
    engine_name = "sales_deck"
    required_input_fields = ['product_data', 'prospect_data']
    required_output_fields = ['deck_content', 'objection_handling']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sales deck", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sales deck: {data}"
