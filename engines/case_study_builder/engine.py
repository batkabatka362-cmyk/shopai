...
class CaseStudyBuilderEngine(BaseEngine):
    engine_name = "case_study_builder"
    required_input_fields = ['client_data', 'builder_config']
    required_output_fields = ['case_study_template', 'data_collection']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} case study builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} case study builder: {data}"
