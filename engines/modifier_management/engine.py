...
class ModifierManagementEngine(BaseEngine):
    engine_name = "modifier_management"
    required_input_fields = ['product_data', 'modifier_config']
    required_output_fields = ['modifier_groups', 'price_adjustments']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} modifier management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} modifier management: {data}"
