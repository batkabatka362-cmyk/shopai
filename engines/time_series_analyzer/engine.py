...
class TimeSeriesAnalyzerEngine(BaseEngine):
    engine_name = "time_series_analyzer"
    required_input_fields = ['time_series', 'analysis_config']
    required_output_fields = ['decomposition', 'forecast']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} time series analyzer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} time series analyzer: {data}"
