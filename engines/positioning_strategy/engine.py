...
class PositioningStrategyEngine(BaseEngine):
    engine_name = "positioning_strategy"
    required_input_fields = ['brand_data', 'competitor_data']
    required_output_fields = ['positioning_map', 'differentiation_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} positioning strategy", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} positioning strategy: {data}"
