...
class RfmAnalysisEngine(BaseEngine):
    engine_name = "rfm_analysis"
    required_input_fields = ['transaction_data', 'rfm_config']
    required_output_fields = ['rfm_scores', 'segment_assignments']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} rfm analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} rfm analysis: {data}"
