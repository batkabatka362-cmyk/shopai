...
class SpinWheelEngine(BaseEngine):
    engine_name = "spin_wheel"
    required_input_fields = ['prize_data', 'wheel_config']
    required_output_fields = ['wheel_setup', 'probability_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} spin wheel", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} spin wheel: {data}"
