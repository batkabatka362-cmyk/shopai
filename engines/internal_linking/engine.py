...
class InternalLinkingEngine(BaseEngine):
    engine_name = "internal_linking"
    required_input_fields = ['site_structure', 'linking_config']
    required_output_fields = ['link_suggestions', 'authority_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} internal linking", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} internal linking: {data}"
