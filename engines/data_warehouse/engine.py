...
class DataWarehouseEngine(BaseEngine):
    engine_name = "data_warehouse"
    required_input_fields = ['data_sources', 'warehouse_config']
    required_output_fields = ['warehouse_schema', 'loading_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} data warehouse", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} data warehouse: {data}"
