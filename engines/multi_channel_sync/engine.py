...
class MultiChannelSyncEngine(BaseEngine):
    engine_name = "multi_channel_sync"
    required_input_fields = ['channel_data', 'sync_config']
    required_output_fields = ['sync_status', 'channel_health']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} multi channel sync", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} multi channel sync: {data}"
