...
class TotalRewardsEngine(BaseEngine):
    engine_name = "total_rewards"
    required_input_fields = ['employee_data', 'rewards_config']
    required_output_fields = ['total_rewards_statement', 'value_breakdown']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} total rewards", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} total rewards: {data}"
