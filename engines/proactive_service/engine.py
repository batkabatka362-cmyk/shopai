...
class ProactiveServiceEngine(BaseEngine):
    engine_name = "proactive_service"
    required_input_fields = ['customer_data', 'service_config']
    required_output_fields = ['proactive_actions', 'trigger_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} proactive service", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} proactive service: {data}"
