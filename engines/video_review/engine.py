...
class VideoReviewEngine(BaseEngine):
    engine_name = "video_review"
    required_input_fields = ['review_data', 'video_config']
    required_output_fields = ['video_review_campaign', 'content_library']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} video review", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} video review: {data}"
