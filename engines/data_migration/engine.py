...
class DataMigrationEngine(BaseEngine):
    engine_name = "data_migration"
    required_input_fields = ['source_data', 'migration_config']
    required_output_fields = ['migration_plan', 'validation_checks']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} data migration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} data migration: {data}"
