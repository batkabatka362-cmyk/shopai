...
class InsuranceClauseEngine(BaseEngine):
    engine_name = "insurance_clause"
    required_input_fields = ['risk_data', 'insurance_config']
    required_output_fields = ['insurance_requirements', 'coverage_minimums']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} insurance clause", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} insurance clause: {data}"
