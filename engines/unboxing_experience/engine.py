...
class UnboxingExperienceEngine(BaseEngine):
    engine_name = "unboxing_experience"
    required_input_fields = ['product_data', 'brand_config']
    required_output_fields = ['unboxing_plan', 'insert_cards']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} unboxing experience", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} unboxing experience: {data}"
