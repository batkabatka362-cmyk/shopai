...
class ConsentPerformanceEngine(BaseEngine):
    engine_name = "consent_performance"
    required_input_fields = ['consent_data', 'performance_config']
    required_output_fields = ['consent_impact', 'optimization_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} consent performance", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} consent performance: {data}"
