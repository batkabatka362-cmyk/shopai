...
class EscalationProcedureEngine(BaseEngine):
    engine_name = "escalation_procedure"
    required_input_fields = ['issue_data', 'escalation_config']
    required_output_fields = ['escalation_levels', 'contact_matrix']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} escalation procedure", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} escalation procedure: {data}"
