...
class LocalSeoEngine(BaseEngine):
    engine_name = "local_seo"
    required_input_fields = ['business_data', 'local_config']
    required_output_fields = ['local_listings', 'citation_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} local seo", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} local seo: {data}"
