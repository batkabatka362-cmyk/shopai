...
class UserRecordingEngine(BaseEngine):
    engine_name = "user_recording"
    required_input_fields = ['recording_data', 'recording_config']
    required_output_fields = ['recording_url', 'highlight_reel']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} user recording", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} user recording: {data}"
