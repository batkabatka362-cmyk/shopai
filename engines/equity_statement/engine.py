...
class EquityStatementEngine(BaseEngine):
    engine_name = "equity_statement"
    required_input_fields = ['equity_data', 'statement_config']
    required_output_fields = ['equity_statement', 'change_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} equity statement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} equity statement: {data}"
