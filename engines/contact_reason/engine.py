...
class ContactReasonEngine(BaseEngine):
    engine_name = "contact_reason"
    required_input_fields = ['contact_data', 'analysis_config']
    required_output_fields = ['reason_taxonomy', 'volume_by_reason']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} contact reason", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} contact reason: {data}"
