...
class ExperienceGiftEngine(BaseEngine):
    engine_name = "experience_gift"
    required_input_fields = ['experience_data', 'pricing_config']
    required_output_fields = ['experience_packages', 'booking_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} experience gift", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} experience gift: {data}"
