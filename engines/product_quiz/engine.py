...
class ProductQuizEngine(BaseEngine):
    engine_name = "product_quiz"
    required_input_fields = ['quiz_config', 'product_catalog']
    required_output_fields = ['quiz_flow', 'recommendation_logic']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} product quiz", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} product quiz: {data}"
