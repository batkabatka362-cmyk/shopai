...
class GrpCalculatorEngine(BaseEngine):
    engine_name = "grp_calculator"
    required_input_fields = ['media_data', 'audience_data']
    required_output_fields = ['grp_scores', 'efficiency_metrics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} grp calculator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} grp calculator: {data}"
