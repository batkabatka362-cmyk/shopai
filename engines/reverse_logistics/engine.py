...
class ReverseLogisticsEngine(BaseEngine):
    engine_name = "reverse_logistics"
    required_input_fields = ['return_data', 'logistics_config']
    required_output_fields = ['reverse_plan', 'cost_recovery']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} reverse logistics", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} reverse logistics: {data}"
