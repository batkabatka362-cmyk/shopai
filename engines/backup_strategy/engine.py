...
class BackupStrategyEngine(BaseEngine):
    engine_name = "backup_strategy"
    required_input_fields = ['data_inventory', 'backup_config']
    required_output_fields = ['backup_plan', 'recovery_testing']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} backup strategy", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} backup strategy: {data}"
