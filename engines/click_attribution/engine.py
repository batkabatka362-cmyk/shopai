...
class ClickAttributionEngine(BaseEngine):
    engine_name = "click_attribution"
    required_input_fields = ['click_data', 'attribution_config']
    required_output_fields = ['click_report', 'last_click_model']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} click attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} click attribution: {data}"
