...
class OvertimeAnalysisEngine(BaseEngine):
    engine_name = "overtime_analysis"
    required_input_fields = ['time_data', 'analysis_config']
    required_output_fields = ['overtime_report', 'cost_impact']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} overtime analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} overtime analysis: {data}"
