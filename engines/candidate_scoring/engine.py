...
class CandidateScoringEngine(BaseEngine):
    engine_name = "candidate_scoring"
    required_input_fields = ['candidate_data', 'scoring_config']
    required_output_fields = ['candidate_scores', 'ranking']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} candidate scoring", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} candidate scoring: {data}"
