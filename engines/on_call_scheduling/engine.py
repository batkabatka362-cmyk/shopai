...
class OnCallSchedulingEngine(BaseEngine):
    engine_name = "on_call_scheduling"
    required_input_fields = ['team_data', 'schedule_config']
    required_output_fields = ['on_call_schedule', 'escalation_policy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} on call scheduling", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} on call scheduling: {data}"
