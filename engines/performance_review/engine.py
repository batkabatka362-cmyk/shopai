...
class PerformanceReviewEngine(BaseEngine):
    engine_name = "performance_review"
    required_input_fields = ['employee_data', 'review_config']
    required_output_fields = ['review_content', 'development_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} performance review", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} performance review: {data}"
