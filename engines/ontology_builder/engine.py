...
class OntologyBuilderEngine(BaseEngine):
    engine_name = "ontology_builder"
    required_input_fields = ['domain_data', 'ontology_config']
    required_output_fields = ['ontology_schema', 'relationship_map']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ontology builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ontology builder: {data}"
