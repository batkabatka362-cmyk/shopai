...
class CrmSyncEngine(BaseEngine):
    engine_name = "crm_sync"
    required_input_fields = ['crm_config', 'customer_data']
    required_output_fields = ['sync_mapping', 'sync_status']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} crm sync", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} crm sync: {data}"
