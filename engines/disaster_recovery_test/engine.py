...
class DisasterRecoveryTestEngine(BaseEngine):
    engine_name = "disaster_recovery_test"
    required_input_fields = ['dr_plan', 'test_config']
    required_output_fields = ['test_results', 'rto_rpo_actual']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} disaster recovery test", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} disaster recovery test: {data}"
