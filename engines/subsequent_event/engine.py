...
class SubsequentEventEngine(BaseEngine):
    engine_name = "subsequent_event"
    required_input_fields = ['event_data', 'reporting_config']
    required_output_fields = ['event_assessment', 'disclosure_requirement']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} subsequent event", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} subsequent event: {data}"
