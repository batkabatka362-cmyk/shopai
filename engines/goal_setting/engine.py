...
class GoalSettingEngine(BaseEngine):
    engine_name = "goal_setting"
    required_input_fields = ['business_data', 'goal_config']
    required_output_fields = ['goal_framework', 'measurement_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} goal setting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} goal setting: {data}"
