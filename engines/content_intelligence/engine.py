...
class ContentIntelligenceEngine(BaseEngine):
    ...
    required_input_fields = ['content_data', 'performance_data']
    required_output_fields = ['content_scores', 'improvement_map']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} content intelligence", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} content intelligence: {data}"
