...
class SweepstakesRulesEngine(BaseEngine):
    engine_name = "sweepstakes_rules"
    required_input_fields = ['sweepstakes_data', 'rules_config']
    required_output_fields = ['official_rules', 'void_jurisdictions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sweepstakes rules", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sweepstakes rules: {data}"
