...
class UShapedAttributionEngine(BaseEngine):
    engine_name = "u_shaped_attribution"
    required_input_fields = ['touchpoint_data', 'u_config']
    required_output_fields = ['u_weights', 'opener_closer_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} u shaped attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} u shaped attribution: {data}"
