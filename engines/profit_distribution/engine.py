...
class ProfitDistributionEngine(BaseEngine):
    engine_name = "profit_distribution"
    required_input_fields = ['profit_data', 'distribution_config']
    required_output_fields = ['distribution_plan', 'tax_implications']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} profit distribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} profit distribution: {data}"
