...
class SupplierAuditEngine(BaseEngine):
    engine_name = "supplier_audit"
    required_input_fields = ['supplier_data', 'audit_config']
    required_output_fields = ['audit_report', 'corrective_actions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} supplier audit", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} supplier audit: {data}"
