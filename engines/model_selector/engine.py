...
class ModelSelectorEngine(BaseEngine):
    engine_name = "model_selector"
    required_input_fields = ['task_data', 'model_inventory']
    required_output_fields = ['selected_model', 'selection_rationale']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} model selector", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} model selector: {data}"
