...
class ProposalBuilderEngine(BaseEngine):
    engine_name = "proposal_builder"
    required_input_fields = ['client_data', 'proposal_config']
    required_output_fields = ['proposal_content', 'pricing_options']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} proposal builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} proposal builder: {data}"
