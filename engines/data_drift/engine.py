...
class DataDriftEngine(BaseEngine):
    engine_name = "data_drift"
    required_input_fields = ['input_data', 'reference_data']
    required_output_fields = ['drift_metrics', 'feature_drift_map']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} data drift", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} data drift: {data}"
