...
class LoyaltyAiEngine(BaseEngine):
    engine_name = "loyalty_ai"
    required_input_fields = ['loyalty_data', 'behavior_data']
    required_output_fields = ['loyalty_score', 'reward_recommendations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} loyalty ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} loyalty ai: {data}"
