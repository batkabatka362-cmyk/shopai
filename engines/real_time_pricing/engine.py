...
class RealTimePricingEngine(BaseEngine):
    engine_name = "real_time_pricing"
    required_input_fields = ['market_data', 'competitor_data']
    required_output_fields = ['live_prices', 'adjustment_log']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} real time pricing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} real time pricing: {data}"
