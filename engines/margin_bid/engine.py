...
class MarginBidEngine(BaseEngine):
    engine_name = "margin_bid"
    required_input_fields = ['margin_data', 'bid_config']
    required_output_fields = ['margin_based_bids', 'profitability_targeting']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} margin bid", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} margin bid: {data}"
