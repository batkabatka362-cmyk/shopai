...
class TradeAgreementEngine(BaseEngine):
    engine_name = "trade_agreement"
    required_input_fields = ['trade_data', 'agreement_config']
    required_output_fields = ['agreement_benefits', 'duty_savings']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} trade agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} trade agreement: {data}"
