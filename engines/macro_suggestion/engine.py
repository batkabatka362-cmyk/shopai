...
class MacroSuggestionEngine(BaseEngine):
    engine_name = "macro_suggestion"
    required_input_fields = ['ticket_data', 'macro_library']
    required_output_fields = ['suggested_macros', 'relevance_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} macro suggestion", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} macro suggestion: {data}"
