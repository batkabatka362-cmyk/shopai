...
class RollbackManagementEngine(BaseEngine):
    engine_name = "rollback_management"
    required_input_fields = ['deployment_data', 'rollback_config']
    required_output_fields = ['rollback_plan', 'verification_steps']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} rollback management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} rollback management: {data}"
