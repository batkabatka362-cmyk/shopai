...
class AiProductTaggerEngine(BaseEngine):
    engine_name = "ai_product_tagger"
    required_input_fields = ['product_data', 'taxonomy']
    required_output_fields = ['auto_tags', 'confidence_map']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ai product tagger", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ai product tagger: {data}"
