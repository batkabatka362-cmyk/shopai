...
class ChannelPreferenceEngine(BaseEngine):
    engine_name = "channel_preference"
    required_input_fields = ['customer_data', 'preference_config']
    required_output_fields = ['preferred_channels', 'prediction_model']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} channel preference", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} channel preference: {data}"
