...
class PageTimingEngine(BaseEngine):
    engine_name = "page_timing"
    required_input_fields = ['timing_data', 'analysis_config']
    required_output_fields = ['timing_breakdown', 'bottleneck_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} page timing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} page timing: {data}"
