"""
ShopifyProductOptimizer Engine
...
class ShopifyProductOptimizerEngine(BaseEngine):
    engine_name = "shopify_product_optimizer"
    required_input_fields = ['product_data', 'performance_data']
    required_output_fields = ['optimization_plan', 'expected_impact']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shopify product optimizer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shopify product optimizer: {data}"
