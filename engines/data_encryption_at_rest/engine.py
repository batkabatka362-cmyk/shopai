...
class DataEncryptionAtRestEngine(BaseEngine):
    engine_name = "data_encryption_at_rest"
    required_input_fields = ['data_inventory', 'encryption_config']
    required_output_fields = ['encryption_status', 'key_management']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} data encryption at rest", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} data encryption at rest: {data}"
