...
class ReceiptTemplateEngine(BaseEngine):
    engine_name = "receipt_template"
    required_input_fields = ['order_data', 'template_config']
    required_output_fields = ['receipt_html', 'print_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} receipt template", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} receipt template: {data}"
