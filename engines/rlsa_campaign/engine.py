...
class RlsaCampaignEngine(BaseEngine):
    engine_name = "rlsa_campaign"
    required_input_fields = ['audience_data', 'campaign_config']
    required_output_fields = ['campaign_setup', 'bid_adjustments']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} rlsa campaign", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} rlsa campaign: {data}"
