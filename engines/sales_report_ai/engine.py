...
class SalesReportAiEngine(BaseEngine):
    engine_name = "sales_report_ai"
    required_input_fields = ['sales_data', 'report_config']
    required_output_fields = ['sales_report', 'performance_summary']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sales report ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sales report ai: {data}"
