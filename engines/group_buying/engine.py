...
class GroupBuyingEngine(BaseEngine):
    engine_name = "group_buying"
    required_input_fields = ['product_data', 'group_config']
    required_output_fields = ['group_deal', 'threshold_pricing']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} group buying", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} group buying: {data}"
