...
class InternationalSeoEngine(BaseEngine):
    engine_name = "international_seo"
    required_input_fields = ['site_data', 'international_config']
    required_output_fields = ['seo_strategy', 'localization_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} international seo", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} international seo: {data}"
