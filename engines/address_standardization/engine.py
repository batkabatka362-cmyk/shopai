...
class AddressStandardizationEngine(BaseEngine):
    engine_name = "address_standardization"
    required_input_fields = ['address_data', 'standard_config']
    required_output_fields = ['standardized_addresses', 'geocodes']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} address standardization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} address standardization: {data}"
