...
class RetargetingEngine(BaseEngine):
    engine_name = "retargeting"
    required_input_fields = ['audience_data', 'retarget_config']
    required_output_fields = ['retarget_segments', 'ad_creative']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} retargeting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} retargeting: {data}"
