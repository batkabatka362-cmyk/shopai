...
class MakeIntegrationEngine(BaseEngine):
    engine_name = "make_integration"
    required_input_fields = ['module_data', 'scenario_config']
    required_output_fields = ['scenario_template', 'connection_setup']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} make integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} make integration: {data}"
