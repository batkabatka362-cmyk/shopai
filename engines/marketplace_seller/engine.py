...
class MarketplaceSellerEngine(BaseEngine):
    engine_name = "marketplace_seller"
    required_input_fields = ['seller_data', 'marketplace_config']
    required_output_fields = ['seller_setup', 'onboarding_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} marketplace seller", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} marketplace seller: {data}"
