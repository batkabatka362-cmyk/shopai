...
class VoiceSearchEngine(BaseEngine):
    engine_name = "voice_search"
    required_input_fields = ['query_data', 'voice_config']
    required_output_fields = ['voice_optimizations', 'faq_schema']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} voice search", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} voice search: {data}"
