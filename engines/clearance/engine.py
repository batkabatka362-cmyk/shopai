...
class ClearanceEngine(BaseEngine):
    engine_name = "clearance"
    required_input_fields = ['inventory_data', 'clearance_config']
    required_output_fields = ['clearance_plan', 'markdown_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} clearance", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} clearance: {data}"
