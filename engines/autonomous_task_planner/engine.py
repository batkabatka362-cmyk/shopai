...
class AutonomousTaskPlannerEngine(BaseEngine):
    engine_name = "autonomous_task_planner"
    required_input_fields = ['goal_data', 'constraint_config']
    required_output_fields = ['task_plan', 'execution_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} autonomous task planner", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} autonomous task planner: {data}"
