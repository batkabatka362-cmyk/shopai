...
class MonteCarloEngine(BaseEngine):
    ...
    required_input_fields = ['scenario_data', 'simulation_config']
    required_output_fields = ['probability_distribution', 'confidence_intervals']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} monte carlo", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} monte carlo: {data}"
