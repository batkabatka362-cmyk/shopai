...
class SendgridIntegrationEngine(BaseEngine):
    engine_name = "sendgrid_integration"
    required_input_fields = ['email_data', 'integration_config']
    required_output_fields = ['sendgrid_setup', 'template_sync']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sendgrid integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sendgrid integration: {data}"
