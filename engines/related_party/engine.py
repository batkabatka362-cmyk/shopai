...
class RelatedPartyEngine(BaseEngine):
    engine_name = "related_party"
    required_input_fields = ['transaction_data', 'rp_config']
    required_output_fields = ['rp_disclosures', 'arm_length_test']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} related party", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} related party: {data}"
