"""
SubscriptionChurnPrevention Engine
...
class SubscriptionChurnPreventionEngine(BaseEngine):
    engine_name = "subscription_churn_prevention"
    required_input_fields = ['subscription_data', 'prevention_config']
    required_output_fields = ['at_risk_subscriptions', 'intervention_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} subscription churn prevention", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} subscription churn prevention: {data}"
