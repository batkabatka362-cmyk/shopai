...
class FtlFreightEngine(BaseEngine):
    engine_name = "ftl_freight"
    required_input_fields = ['shipment_data', 'freight_config']
    required_output_fields = ['freight_quotes', 'scheduling_options']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ftl freight", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ftl freight: {data}"
