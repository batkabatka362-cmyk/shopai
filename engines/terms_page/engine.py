...
class TermsPageEngine(BaseEngine):
    engine_name = "terms_page"
    required_input_fields = ['terms_data', 'page_config']
    required_output_fields = ['terms_content', 'version_tracking']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} terms page", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} terms page: {data}"
