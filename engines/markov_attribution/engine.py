...
class MarkovAttributionEngine(BaseEngine):
    engine_name = "markov_attribution"
    required_input_fields = ['path_data', 'markov_config']
    required_output_fields = ['transition_matrix', 'removal_effects']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} markov attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} markov attribution: {data}"
