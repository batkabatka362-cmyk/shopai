...
class FailoverTestingEngine(BaseEngine):
    engine_name = "failover_testing"
    required_input_fields = ['infrastructure_data', 'failover_config']
    required_output_fields = ['failover_results', 'recovery_metrics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} failover testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} failover testing: {data}"
