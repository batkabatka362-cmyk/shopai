...
class BudgetOptimizerAiEngine(BaseEngine):
    engine_name = "budget_optimizer_ai"
    required_input_fields = ['performance_data', 'budget_config']
    required_output_fields = ['optimized_budget', 'expected_roi']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} budget optimizer ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} budget optimizer ai: {data}"
