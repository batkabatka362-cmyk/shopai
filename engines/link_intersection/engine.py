...
class LinkIntersectionEngine(BaseEngine):
    engine_name = "link_intersection"
    required_input_fields = ['competitor_data', 'intersection_config']
    required_output_fields = ['common_links', 'unique_opportunities']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} link intersection", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} link intersection: {data}"
