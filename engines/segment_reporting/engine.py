...
class SegmentReportingEngine(BaseEngine):
    engine_name = "segment_reporting"
    required_input_fields = ['financial_data', 'segment_config']
    required_output_fields = ['segment_report', 'elimination_entries']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} segment reporting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} segment reporting: {data}"
