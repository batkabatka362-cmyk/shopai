...
class ConsultingAgreementEngine(BaseEngine):
    engine_name = "consulting_agreement"
    required_input_fields = ['consultant_data', 'agreement_config']
    required_output_fields = ['agreement_draft', 'deliverable_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} consulting agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} consulting agreement: {data}"
