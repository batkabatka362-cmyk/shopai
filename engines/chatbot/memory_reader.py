"""Chatbot Engine — memory reader.

Reads past chatbot interaction records from memory storage.
Used to inform current conversation with historical context.
"""
...
def read_past_conversations(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past chatbot conversation records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_conversations(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past chatbot records."""
    if not records:
        return {
            "avg_confidence": 0.0,
            "escalation_rate": 0.0,
    ...
    n = len(records)
    return {
        "avg_confidence": round(sum(confidences) / n, 2) if n else 0.0,
        "escalation_rate": round(escalations / n * 100, 1) if n else 0.0,
