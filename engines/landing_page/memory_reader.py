"""Landing Page Engine — memory reader.

Reads past landing page generation records from memory storage.
Used to inform current page generation with historical context.
"""
...
def read_past_pages(
    ...
    """Read past landing page records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_pages(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past landing page records."""
    if not records:
        return {
            "avg_page_count": 0.0,
            "avg_conversion_estimate": 0.0,
            "total_runs": 0,
        }

    page_counts = [len(r.get("pages", [])) for r in records]
    conversion_ests = [float(r.get("estimated_conversion", 0)) for r in records]

    n = len(records)
    return {
        "avg_page_count": round(sum(page_counts) / n, 1) if n else 0.0,
        "avg_conversion_estimate": round(sum(conversion_ests) / n, 4) if n else 0.0,
        "total_runs": n,
    }
