...
class AccessibilityTestingEngine(BaseEngine):
    engine_name = "accessibility_testing"
    required_input_fields = ['page_data', 'a11y_config']
    required_output_fields = ['a11y_report', 'wcag_violations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} accessibility testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} accessibility testing: {data}"
