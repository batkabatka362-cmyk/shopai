...
class AdPerformanceImpactEngine(BaseEngine):
    engine_name = "ad_performance_impact"
    required_input_fields = ['ad_data', 'impact_config']
    required_output_fields = ['performance_impact', 'lazy_load_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ad performance impact", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ad performance impact: {data}"
