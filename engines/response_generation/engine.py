...
class ResponseGenerationEngine(BaseEngine):
    engine_name = "response_generation"
    required_input_fields = ['query_data', 'response_config']
    required_output_fields = ['response_draft', 'tone_check']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} response generation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} response generation: {data}"
