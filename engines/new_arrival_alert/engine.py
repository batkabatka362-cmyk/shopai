...
class NewArrivalAlertEngine(BaseEngine):
    engine_name = "new_arrival_alert"
    required_input_fields = ['new_products', 'preference_data']
    required_output_fields = ['alert_campaigns', 'personalized_picks']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} new arrival alert", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} new arrival alert: {data}"
