...
class ResourcePredictorEngine(BaseEngine):
    engine_name = "resource_predictor"
    required_input_fields = ['usage_data', 'prediction_config']
    required_output_fields = ['resource_forecast', 'scaling_recommendations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} resource predictor", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} resource predictor: {data}"
