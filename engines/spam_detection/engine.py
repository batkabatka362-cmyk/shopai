...
class SpamDetectionEngine(BaseEngine):
    engine_name = "spam_detection"
    required_input_fields = ['message_data', 'detection_config']
    required_output_fields = ['spam_scores', 'filter_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} spam detection", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} spam detection: {data}"
