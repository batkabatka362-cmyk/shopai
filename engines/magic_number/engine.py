...
class MagicNumberEngine(BaseEngine):
    engine_name = "magic_number"
    required_input_fields = ['sales_data', 'marketing_data']
    required_output_fields = ['magic_number', 'efficiency_trend']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} magic number", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} magic number: {data}"
