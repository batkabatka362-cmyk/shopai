...
class CertificationProgramEngine(BaseEngine):
    engine_name = "certification_program"
    required_input_fields = ['program_data', 'cert_config']
    required_output_fields = ['program_setup', 'assessment_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} certification program", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} certification program: {data}"
