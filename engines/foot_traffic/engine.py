...
class FootTrafficEngine(BaseEngine):
    engine_name = "foot_traffic"
    required_input_fields = ['traffic_data', 'analysis_config']
    required_output_fields = ['traffic_report', 'peak_hours']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} foot traffic", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} foot traffic: {data}"
