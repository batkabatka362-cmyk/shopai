...
class HomePlanningEngine(BaseEngine):
    engine_name = "home_planning"
    required_input_fields = ['room_data', 'product_catalog']
    required_output_fields = ['design_plan', 'shopping_list']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} home planning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} home planning: {data}"
