...
class CorsTestingEngine(BaseEngine):
    engine_name = "cors_testing"
    required_input_fields = ['api_data', 'cors_config']
    required_output_fields = ['cors_report', 'misconfiguration_flags']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cors testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cors testing: {data}"
