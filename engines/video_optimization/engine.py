...
class VideoOptimizationEngine(BaseEngine):
    engine_name = "video_optimization"
    required_input_fields = ['video_data', 'optimization_config']
    required_output_fields = ['encoding_plan', 'adaptive_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} video optimization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} video optimization: {data}"
