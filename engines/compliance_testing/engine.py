...
class ComplianceTestingEngine(BaseEngine):
    engine_name = "compliance_testing"
    required_input_fields = ['system_data', 'compliance_config']
    required_output_fields = ['compliance_results', 'gap_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} compliance testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} compliance testing: {data}"
