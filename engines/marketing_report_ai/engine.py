...
class MarketingReportAiEngine(BaseEngine):
    engine_name = "marketing_report_ai"
    required_input_fields = ['marketing_data', 'report_config']
    required_output_fields = ['marketing_report', 'channel_breakdown']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} marketing report ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} marketing report ai: {data}"
