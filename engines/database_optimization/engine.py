...
class DatabaseOptimizationEngine(BaseEngine):
    engine_name = "database_optimization"
    required_input_fields = ['query_data', 'db_config']
    required_output_fields = ['optimization_plan', 'index_recommendations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} database optimization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} database optimization: {data}"
