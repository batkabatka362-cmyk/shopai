...
class TextSummarizationEngine(BaseEngine):
    engine_name = "text_summarization"
    required_input_fields = ['source_text', 'summary_config']
    required_output_fields = ['summary', 'key_points']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} text summarization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} text summarization: {data}"
