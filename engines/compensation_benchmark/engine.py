...
class CompensationBenchmarkEngine(BaseEngine):
    engine_name = "compensation_benchmark"
    required_input_fields = ['comp_data', 'benchmark_config']
    required_output_fields = ['benchmark_report', 'market_position']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} compensation benchmark", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} compensation benchmark: {data}"
