...
class PickPackEngine(BaseEngine):
    engine_name = "pick_pack"
    required_input_fields = ['order_data', 'warehouse_config']
    required_output_fields = ['pick_list', 'pack_instructions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pick pack", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pick pack: {data}"
