...
class IntegrationPlanningEngine(BaseEngine):
    engine_name = "integration_planning"
    required_input_fields = ['acquisition_data', 'integration_config']
    required_output_fields = ['integration_plan', 'synergy_timeline']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} integration planning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} integration planning: {data}"
