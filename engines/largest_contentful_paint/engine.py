...
class LargestContentfulPaintEngine(BaseEngine):
    engine_name = "largest_contentful_paint"
    required_input_fields = ['paint_data', 'analysis_config']
    required_output_fields = ['lcp_score', 'optimization_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} largest contentful paint", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} largest contentful paint: {data}"
