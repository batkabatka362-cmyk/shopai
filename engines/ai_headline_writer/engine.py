...
class AiHeadlineWriterEngine(BaseEngine):
    engine_name = "ai_headline_writer"
    required_input_fields = ['topic_data', 'audience_data']
    required_output_fields = ['headlines', 'ctr_predictions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ai headline writer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ai headline writer: {data}"
