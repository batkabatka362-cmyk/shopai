...
class DataQualityEngine(BaseEngine):
    engine_name = "data_quality"
    required_input_fields = ['dataset', 'quality_rules']
    required_output_fields = ['quality_scores', 'remediation_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} data quality", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} data quality: {data}"
