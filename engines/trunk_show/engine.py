...
class TrunkShowEngine(BaseEngine):
    engine_name = "trunk_show"
    required_input_fields = ['product_data', 'show_config']
    required_output_fields = ['show_plan', 'invitation_list']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} trunk show", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} trunk show: {data}"
