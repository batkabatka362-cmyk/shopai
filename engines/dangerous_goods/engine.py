...
class DangerousGoodsEngine(BaseEngine):
    engine_name = "dangerous_goods"
    required_input_fields = ['product_data', 'dg_config']
    required_output_fields = ['dg_classification', 'shipping_restrictions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dangerous goods", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dangerous goods: {data}"
