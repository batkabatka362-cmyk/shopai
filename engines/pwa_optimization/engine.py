...
class PwaOptimizationEngine(BaseEngine):
    engine_name = "pwa_optimization"
    required_input_fields = ['app_data', 'pwa_config']
    required_output_fields = ['pwa_manifest', 'optimization_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pwa optimization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pwa optimization: {data}"
