...
class ApprovalWorkflowEngine(BaseEngine):
    engine_name = "approval_workflow"
    required_input_fields = ['request_data', 'approval_config']
    required_output_fields = ['approval_chain', 'notification_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} approval workflow", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} approval workflow: {data}"
