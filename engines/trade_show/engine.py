...
class TradeShowEngine(BaseEngine):
    engine_name = "trade_show"
    required_input_fields = ['event_data', 'product_catalog']
    required_output_fields = ['booth_plan', 'lead_capture_setup']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} trade show", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} trade show: {data}"
