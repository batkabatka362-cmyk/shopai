...
class MigrationPlanningEngine(BaseEngine):
    engine_name = "migration_planning"
    required_input_fields = ['current_system', 'target_system']
    required_output_fields = ['migration_plan', 'risk_mitigation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} migration planning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} migration planning: {data}"
