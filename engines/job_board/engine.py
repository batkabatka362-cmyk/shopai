...
class JobBoardEngine(BaseEngine):
    engine_name = "job_board"
    required_input_fields = ['job_data', 'board_config']
    required_output_fields = ['job_postings', 'distribution_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} job board", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} job board: {data}"
