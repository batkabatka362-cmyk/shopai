...
class LastClickAttributionEngine(BaseEngine):
    engine_name = "last_click_attribution"
    required_input_fields = ['touchpoint_data', 'lc_config']
    required_output_fields = ['last_touch_report', 'closing_channels']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} last click attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} last click attribution: {data}"
