...
class CircuitBreakerEngine(BaseEngine):
    engine_name = "circuit_breaker"
    required_input_fields = ['service_data', 'breaker_config']
    required_output_fields = ['breaker_config', 'fallback_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} circuit breaker", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} circuit breaker: {data}"
