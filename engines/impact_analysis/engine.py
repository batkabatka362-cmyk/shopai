...
class ImpactAnalysisEngine(BaseEngine):
    engine_name = "impact_analysis"
    required_input_fields = ['change_data', 'system_config']
    required_output_fields = ['impact_report', 'affected_areas']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} impact analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} impact analysis: {data}"
