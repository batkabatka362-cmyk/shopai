...
class AiReviewWriterEngine(BaseEngine):
    engine_name = "ai_review_writer"
    required_input_fields = ['product_data', 'review_config']
    required_output_fields = ['review_responses', 'tone_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ai review writer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ai review writer: {data}"
