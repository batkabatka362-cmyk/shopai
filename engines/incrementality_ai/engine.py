...
class IncrementalityAiEngine(BaseEngine):
    engine_name = "incrementality_ai"
    required_input_fields = ['experiment_data', 'baseline_data']
    required_output_fields = ['incremental_lift', 'confidence_interval']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} incrementality ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} incrementality ai: {data}"
