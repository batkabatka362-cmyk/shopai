...
class SalesLiftEngine(BaseEngine):
    engine_name = "sales_lift"
    required_input_fields = ['sales_data', 'lift_config']
    required_output_fields = ['lift_results', 'revenue_impact']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sales lift", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sales lift: {data}"
