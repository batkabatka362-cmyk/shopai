...
class SeasonalGiftEngine(BaseEngine):
    engine_name = "seasonal_gift"
    required_input_fields = ['season_data', 'product_data']
    required_output_fields = ['gift_recommendations', 'promotion_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} seasonal gift", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} seasonal gift: {data}"
