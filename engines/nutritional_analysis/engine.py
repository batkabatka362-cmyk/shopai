...
class NutritionalAnalysisEngine(BaseEngine):
    engine_name = "nutritional_analysis"
    required_input_fields = ['product_data', 'nutrition_config']
    required_output_fields = ['nutrition_facts', 'label_compliance']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} nutritional analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} nutritional analysis: {data}"
