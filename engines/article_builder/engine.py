...
class ArticleBuilderEngine(BaseEngine):
    engine_name = "article_builder"
    required_input_fields = ['article_data', 'builder_config']
    required_output_fields = ['article_layout', 'seo_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} article builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} article builder: {data}"
