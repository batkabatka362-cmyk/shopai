...
class LockerDeliveryEngine(BaseEngine):
    engine_name = "locker_delivery"
    required_input_fields = ['delivery_data', 'locker_config']
    required_output_fields = ['locker_options', 'pickup_instructions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} locker delivery", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} locker delivery: {data}"
