...
class NewProductIntroEngine(BaseEngine):
    engine_name = "new_product_intro"
    required_input_fields = ['product_data', 'launch_config']
    required_output_fields = ['intro_plan', 'pricing_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} new product intro", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} new product intro: {data}"
