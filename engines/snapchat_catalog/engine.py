...
class SnapchatCatalogEngine(BaseEngine):
    engine_name = "snapchat_catalog"
    required_input_fields = ['product_data', 'catalog_config']
    required_output_fields = ['catalog_sync', 'ad_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} snapchat catalog", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} snapchat catalog: {data}"
