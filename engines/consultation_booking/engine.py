...
class ConsultationBookingEngine(BaseEngine):
    engine_name = "consultation_booking"
    required_input_fields = ['service_data', 'booking_config']
    required_output_fields = ['booking_flow', 'calendar_integration']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} consultation booking", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} consultation booking: {data}"
