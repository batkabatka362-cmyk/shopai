...
class ServiceWorkerEngine(BaseEngine):
    engine_name = "service_worker"
    required_input_fields = ['app_data', 'sw_config']
    required_output_fields = ['sw_strategy', 'offline_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} service worker", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} service worker: {data}"
