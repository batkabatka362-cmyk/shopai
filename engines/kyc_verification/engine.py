...
class KycVerificationEngine(BaseEngine):
    engine_name = "kyc_verification"
    required_input_fields = ['customer_data', 'kyc_config']
    required_output_fields = ['verification_status', 'document_check']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} kyc verification", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} kyc verification: {data}"
