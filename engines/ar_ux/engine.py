...
class ArUxEngine(BaseEngine):
    engine_name = "ar_ux"
    required_input_fields = ['ar_data', 'ux_config']
    required_output_fields = ['ar_experience', 'interaction_design']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ar ux", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ar ux: {data}"
