...
class ActivityBasedCostingEngine(BaseEngine):
    engine_name = "activity_based_costing"
    required_input_fields = ['activity_data', 'costing_config']
    required_output_fields = ['cost_allocation', 'profitability_view']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} activity based costing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} activity based costing: {data}"
