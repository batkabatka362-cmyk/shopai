...
class DeliveryFeedbackEngine(BaseEngine):
    engine_name = "delivery_feedback"
    required_input_fields = ['delivery_data', 'feedback_config']
    required_output_fields = ['feedback_request', 'survey_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} delivery feedback", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} delivery feedback: {data}"
