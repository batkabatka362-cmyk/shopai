"""Trend Detection Engine — memory reader.

Reads past trend detection records from memory storage.
...
def read_past_trends(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past trend detection records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_trends(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past trend records."""
    if not records:
        return {
            "avg_trend_count": 0.0,
    ...
    trend_counts = [r.get("trend_count", 0) for r in records]
    ...
    n = len(records)
    return {
        "avg_trend_count": round(sum(trend_counts) / n, 1) if n else 0.0,
