...
class TermSheetEngine(BaseEngine):
    engine_name = "term_sheet"
    required_input_fields = ['deal_data', 'term_config']
    required_output_fields = ['term_sheet_draft', 'key_terms']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} term sheet", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} term sheet: {data}"
