...
class AutoReportGeneratorEngine(BaseEngine):
    engine_name = "auto_report_generator"
    required_input_fields = ['data_sources', 'report_config']
    required_output_fields = ['report_content', 'distribution_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} auto report generator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} auto report generator: {data}"
