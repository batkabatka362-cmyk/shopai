...
class PromptOptimizerAiEngine(BaseEngine):
    engine_name = "prompt_optimizer_ai"
    required_input_fields = ['prompt_data', 'performance_data']
    required_output_fields = ['optimized_prompts', 'improvement_metrics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} prompt optimizer ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} prompt optimizer ai: {data}"
