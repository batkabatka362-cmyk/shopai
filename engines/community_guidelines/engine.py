...
class CommunityGuidelinesEngine(BaseEngine):
    engine_name = "community_guidelines"
    required_input_fields = ['community_data', 'guideline_config']
    required_output_fields = ['guidelines_content', 'enforcement_tiers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} community guidelines", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} community guidelines: {data}"
