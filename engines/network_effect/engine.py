...
class NetworkEffectEngine(BaseEngine):
    engine_name = "network_effect"
    required_input_fields = ['user_data', 'network_config']
    required_output_fields = ['network_metrics', 'growth_levers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} network effect", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} network effect: {data}"
