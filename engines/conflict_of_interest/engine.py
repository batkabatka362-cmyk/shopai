...
class ConflictOfInterestEngine(BaseEngine):
    engine_name = "conflict_of_interest"
    required_input_fields = ['disclosure_data', 'coi_config']
    required_output_fields = ['coi_assessment', 'mitigation_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} conflict of interest", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} conflict of interest: {data}"
