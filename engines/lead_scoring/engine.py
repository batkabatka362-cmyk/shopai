"""
LeadScoring Engine — Lead Scoring
...
class LeadScoringEngine(BaseEngine):
    engine_name = "lead_scoring"
    required_input_fields = ['lead_data', 'scoring_criteria']
    required_output_fields = ['lead_scores', 'qualification_status']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} lead scoring", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} lead scoring: {data}"
