...
class Inventory360Engine(BaseEngine):
    engine_name = "inventory_360"
    required_input_fields = ['inventory_data', 'integration_config']
    required_output_fields = ['unified_inventory', 'availability_status']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} inventory 360", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} inventory 360: {data}"
