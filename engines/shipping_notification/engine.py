...
class ShippingNotificationEngine(BaseEngine):
    engine_name = "shipping_notification"
    required_input_fields = ['shipment_data', 'template_config']
    required_output_fields = ['notification_content', 'tracking_link']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shipping notification", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shipping notification: {data}"
