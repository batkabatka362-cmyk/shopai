...
class WellnessProgramEngine(BaseEngine):
    engine_name = "wellness_program"
    required_input_fields = ['employee_data', 'wellness_config']
    required_output_fields = ['program_plan', 'engagement_metrics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} wellness program", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} wellness program: {data}"
