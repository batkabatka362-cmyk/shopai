...
class LaborPlanningEngine(BaseEngine):
    engine_name = "labor_planning"
    required_input_fields = ['demand_forecast', 'labor_config']
    required_output_fields = ['labor_schedule', 'cost_projection']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} labor planning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} labor planning: {data}"
