...
class SimulatedAnnealingEngine(BaseEngine):
    engine_name = "simulated_annealing"
    required_input_fields = ['solution_space', 'sa_config']
    required_output_fields = ['best_solution', 'cooling_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} simulated annealing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} simulated annealing: {data}"
