...
class SchemaMigrationEngine(BaseEngine):
    engine_name = "schema_migration"
    required_input_fields = ['schema_data', 'migration_config']
    required_output_fields = ['migration_plan', 'rollback_script']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} schema migration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} schema migration: {data}"
