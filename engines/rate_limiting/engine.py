...
class RateLimitingEngine(BaseEngine):
    engine_name = "rate_limiting"
    required_input_fields = ['traffic_data', 'limit_config']
    required_output_fields = ['rate_limits', 'throttle_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} rate limiting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} rate limiting: {data}"
