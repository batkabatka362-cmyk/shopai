...
class DifferentialPrivacyEngine(BaseEngine):
    engine_name = "differential_privacy"
    required_input_fields = ['dataset', 'privacy_config']
    required_output_fields = ['private_output', 'privacy_budget']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} differential privacy", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} differential privacy: {data}"
