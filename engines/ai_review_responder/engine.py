...
class AiReviewResponderEngine(BaseEngine):
    engine_name = "ai_review_responder"
    required_input_fields = ['review_data', 'brand_voice']
    required_output_fields = ['response_drafts', 'sentiment_tags']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ai review responder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ai review responder: {data}"
