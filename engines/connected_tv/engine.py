...
class ConnectedTvEngine(BaseEngine):
    engine_name = "connected_tv"
    required_input_fields = ['campaign_data', 'ctv_config']
    required_output_fields = ['ctv_plan', 'targeting_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} connected tv", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} connected tv: {data}"
