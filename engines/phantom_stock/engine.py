...
class PhantomStockEngine(BaseEngine):
    engine_name = "phantom_stock"
    required_input_fields = ['stock_data', 'phantom_config']
    required_output_fields = ['phantom_plan', 'valuation_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} phantom stock", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} phantom stock: {data}"
