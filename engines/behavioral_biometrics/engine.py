...
class BehavioralBiometricsEngine(BaseEngine):
    engine_name = "behavioral_biometrics"
    required_input_fields = ['behavior_data', 'biometric_config']
    required_output_fields = ['biometric_score', 'anomaly_flags']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} behavioral biometrics", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} behavioral biometrics: {data}"
