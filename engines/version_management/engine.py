...
class VersionManagementEngine(BaseEngine):
    engine_name = "version_management"
    required_input_fields = ['product_data', 'version_config']
    required_output_fields = ['version_tree', 'compatibility_matrix']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} version management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} version management: {data}"
