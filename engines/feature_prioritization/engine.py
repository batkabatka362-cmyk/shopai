...
class FeaturePrioritizationEngine(BaseEngine):
    engine_name = "feature_prioritization"
    required_input_fields = ['feature_list', 'scoring_criteria']
    required_output_fields = ['prioritized_features', 'impact_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} feature prioritization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} feature prioritization: {data}"
