"""
ProductCustomization Engine — Product Customization
...
class ProductCustomizationEngine(BaseEngine):
    engine_name = "product_customization"
    required_input_fields = ['product_data', 'customization_options']
    required_output_fields = ['customizer_config', 'pricing_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} product customization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} product customization: {data}"
