"""
PositionBasedAttribution Engine
...
class PositionBasedAttributionEngine(BaseEngine):
    engine_name = "position_based_attribution"
    required_input_fields = ['touchpoint_data', 'position_config']
    required_output_fields = ['attribution_weights', 'interaction_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} position based attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} position based attribution: {data}"
