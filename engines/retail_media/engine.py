...
class RetailMediaEngine(BaseEngine):
    engine_name = "retail_media"
    required_input_fields = ['campaign_data', 'retail_config']
    required_output_fields = ['media_plan', 'retailer_targeting']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} retail media", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} retail media: {data}"
