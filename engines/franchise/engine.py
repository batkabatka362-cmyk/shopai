...
class FranchiseEngine(BaseEngine):
    engine_name = "franchise"
    required_input_fields = ['business_data', 'franchise_config']
    required_output_fields = ['franchise_model', 'fee_structure']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} franchise", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} franchise: {data}"
