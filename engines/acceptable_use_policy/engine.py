...
class AcceptableUsePolicyEngine(BaseEngine):
    engine_name = "acceptable_use_policy"
    required_input_fields = ['platform_data', 'policy_config']
    required_output_fields = ['aup_content', 'enforcement_procedure']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} acceptable use policy", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} acceptable use policy: {data}"
