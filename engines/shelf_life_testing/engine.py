...
class ShelfLifeTestingEngine(BaseEngine):
    engine_name = "shelf_life_testing"
    required_input_fields = ['product_data', 'testing_config']
    required_output_fields = ['shelf_life_estimate', 'storage_recommendations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shelf life testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shelf life testing: {data}"
