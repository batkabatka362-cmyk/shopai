...
class AssignmentAgreementEngine(BaseEngine):
    engine_name = "assignment_agreement"
    required_input_fields = ['ip_data', 'assignment_config']
    required_output_fields = ['agreement_draft', 'consideration_terms']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} assignment agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} assignment agreement: {data}"
