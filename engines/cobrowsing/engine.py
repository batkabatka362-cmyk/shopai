...
class CobrowsingEngine(BaseEngine):
    engine_name = "cobrowsing"
    required_input_fields = ['session_data', 'cobrowse_config']
    required_output_fields = ['cobrowse_setup', 'permission_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cobrowsing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cobrowsing: {data}"
