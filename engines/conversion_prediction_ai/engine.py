...
class ConversionPredictionAiEngine(BaseEngine):
    engine_name = "conversion_prediction_ai"
    required_input_fields = ['visitor_data', 'funnel_data']
    required_output_fields = ['conversion_forecast', 'optimization_targets']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} conversion prediction ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} conversion prediction ai: {data}"
