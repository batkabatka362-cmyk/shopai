...
class AutoInventoryManagerEngine(BaseEngine):
    engine_name = "auto_inventory_manager"
    required_input_fields = ['inventory_data', 'policy_config']
    required_output_fields = ['inventory_actions', 'order_queue']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} auto inventory manager", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} auto inventory manager: {data}"
