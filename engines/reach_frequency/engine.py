...
class ReachFrequencyEngine(BaseEngine):
    engine_name = "reach_frequency"
    required_input_fields = ['media_data', 'audience_data']
    required_output_fields = ['reach_estimate', 'frequency_distribution']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} reach frequency", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} reach frequency: {data}"
