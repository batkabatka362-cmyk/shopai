...
class FinancingOptionEngine(BaseEngine):
    engine_name = "financing_option"
    required_input_fields = ['product_data', 'finance_config']
    required_output_fields = ['payment_plans', 'interest_calc']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} financing option", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} financing option: {data}"
