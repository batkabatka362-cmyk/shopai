...
class CodeGenerationEngine(BaseEngine):
    engine_name = "code_generation"
    required_input_fields = ['spec_data', 'generation_config']
    required_output_fields = ['generated_code', 'test_cases']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} code generation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} code generation: {data}"
