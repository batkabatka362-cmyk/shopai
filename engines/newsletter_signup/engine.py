...
class NewsletterSignupEngine(BaseEngine):
    engine_name = "newsletter_signup"
    required_input_fields = ['signup_config', 'incentive_data']
    required_output_fields = ['signup_form', 'automation_trigger']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} newsletter signup", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} newsletter signup: {data}"
