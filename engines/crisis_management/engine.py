...
class CrisisManagementEngine(BaseEngine):
    engine_name = "crisis_management"
    required_input_fields = ['crisis_data', 'response_config']
    required_output_fields = ['crisis_plan', 'communication_draft']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} crisis management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} crisis management: {data}"
