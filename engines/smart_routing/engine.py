...
class SmartRoutingEngine(BaseEngine):
    engine_name = "smart_routing"
    required_input_fields = ['ticket_data', 'routing_config']
    required_output_fields = ['routing_decision', 'skill_matching']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} smart routing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} smart routing: {data}"
