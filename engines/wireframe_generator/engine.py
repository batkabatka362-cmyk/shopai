...
class WireframeGeneratorEngine(BaseEngine):
    engine_name = "wireframe_generator"
    required_input_fields = ['requirements_data', 'wireframe_config']
    required_output_fields = ['wireframe_specs', 'interaction_notes']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} wireframe generator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} wireframe generator: {data}"
