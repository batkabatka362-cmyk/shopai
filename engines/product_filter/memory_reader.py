"""Product Filter Engine — memory reader.

Reads past filter decision records from memory storage.
Used to inform current filtering with historical context.
"""
...
def read_past_filters(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past filter records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_filters(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past filter records."""
    if not records:
        return {
            "avg_pass_rate": 0.0,
            "avg_input_count": 0.0,
    ...
    pass_rates = [
        r.get("filter_summary", {}).get("total_passed", 0)
        / max(r.get("filter_summary", {}).get("total_input", 1), 1)
        for r in records
    ]
    input_counts = [
        r.get("filter_summary", {}).get("total_input", 0)
        for r in records
    ]

    n = len(records)
    return {
        "avg_pass_rate": round(sum(pass_rates) / n, 3) if n else 0.0,
        "avg_input_count": round(sum(input_counts) / n, 1) if n else 0.0,
        "total_runs": n,
    }
