...
class DeliverabilityMonitorEngine(BaseEngine):
    engine_name = "deliverability_monitor"
    required_input_fields = ['email_data', 'monitoring_config']
    required_output_fields = ['deliverability_report', 'reputation_score']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} deliverability monitor", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} deliverability monitor: {data}"
