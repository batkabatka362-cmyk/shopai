...
class PrStrategyEngine(BaseEngine):
    engine_name = "pr_strategy"
    required_input_fields = ['brand_data', 'pr_config']
    required_output_fields = ['pr_plan', 'media_targets']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pr strategy", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pr strategy: {data}"
