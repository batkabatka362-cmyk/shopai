...
class PricingDecisionEngine(BaseEngine):
    engine_name = "pricing_decision"
    required_input_fields = ['product_data', 'market_context']
    required_output_fields = ['pricing_decision', 'impact_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pricing decision", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pricing decision: {data}"
