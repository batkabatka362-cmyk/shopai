...
class StoreIntelligenceEngine(BaseEngine):
    engine_name = "store_intelligence"
    required_input_fields = ['store_data', 'benchmark_data']
    required_output_fields = ['store_score', 'improvement_roadmap']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} store intelligence", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} store intelligence: {data}"
