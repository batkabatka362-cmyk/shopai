...
class TuitionReimbursementEngine(BaseEngine):
    engine_name = "tuition_reimbursement"
    required_input_fields = ['employee_data', 'tuition_config']
    required_output_fields = ['reimbursement_plan', 'eligibility_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} tuition reimbursement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} tuition reimbursement: {data}"
