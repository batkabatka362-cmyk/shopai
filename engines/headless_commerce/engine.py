...
class HeadlessCommerceEngine(BaseEngine):
    engine_name = "headless_commerce"
    required_input_fields = ['storefront_config', 'api_config']
    required_output_fields = ['api_endpoints', 'frontend_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} headless commerce", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} headless commerce: {data}"
