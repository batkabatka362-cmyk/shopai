...
class HeaderTestingEngine(BaseEngine):
    engine_name = "header_testing"
    required_input_fields = ['response_data', 'test_config']
    required_output_fields = ['header_report', 'security_score']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} header testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} header testing: {data}"
