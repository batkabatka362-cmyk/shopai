...
class ShoppableEmailEngine(BaseEngine):
    engine_name = "shoppable_email"
    required_input_fields = ['email_data', 'shopping_config']
    required_output_fields = ['email_setup', 'product_blocks']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shoppable email", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shoppable email: {data}"
