...
class WeddingRegistryEngine(BaseEngine):
    engine_name = "wedding_registry"
    required_input_fields = ['couple_data', 'registry_config']
    required_output_fields = ['registry_page', 'thank_you_tracker']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} wedding registry", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} wedding registry: {data}"
