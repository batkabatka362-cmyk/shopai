...
class PressPageEngine(BaseEngine):
    engine_name = "press_page"
    required_input_fields = ['press_data', 'page_config']
    required_output_fields = ['page_layout', 'media_kit']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} press page", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} press page: {data}"
