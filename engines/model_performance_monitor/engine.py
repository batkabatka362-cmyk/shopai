"""
ModelPerformanceMonitor Engine
...
class ModelPerformanceMonitorEngine(BaseEngine):
    engine_name = "model_performance_monitor"
    required_input_fields = ['model_data', 'monitor_config']
    required_output_fields = ['model_health', 'drift_alerts']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} model performance monitor", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} model performance monitor: {data}"
