...
class EsgReportingEngine(BaseEngine):
    engine_name = "esg_reporting"
    required_input_fields = ['esg_data', 'reporting_config']
    required_output_fields = ['esg_report', 'metric_tracking']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} esg reporting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} esg reporting: {data}"
