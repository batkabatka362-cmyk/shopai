...
class SponsorManagementEngine(BaseEngine):
    engine_name = "sponsor_management"
    required_input_fields = ['sponsor_data', 'management_config']
    required_output_fields = ['sponsor_tiers', 'benefit_delivery']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sponsor management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sponsor management: {data}"
