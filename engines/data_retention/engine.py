...
class DataRetentionEngine(BaseEngine):
    engine_name = "data_retention"
    required_input_fields = ['data_inventory', 'retention_policy']
    required_output_fields = ['retention_schedule', 'purge_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} data retention", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} data retention: {data}"
