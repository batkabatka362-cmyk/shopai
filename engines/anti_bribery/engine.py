...
class AntiBriberyEngine(BaseEngine):
    engine_name = "anti_bribery"
    required_input_fields = ['party_data', 'anti_bribery_config']
    required_output_fields = ['anti_bribery_provisions', 'training_requirements']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} anti bribery", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} anti bribery: {data}"
