...
class ThirdPartyRiskEngine(BaseEngine):
    engine_name = "third_party_risk"
    required_input_fields = ['third_party_data', 'risk_config']
    required_output_fields = ['risk_score', 'due_diligence_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} third party risk", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} third party risk: {data}"
