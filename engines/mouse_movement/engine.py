...
class MouseMovementEngine(BaseEngine):
    engine_name = "mouse_movement"
    required_input_fields = ['movement_data', 'tracking_config']
    required_output_fields = ['movement_patterns', 'hesitation_zones']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} mouse movement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} mouse movement: {data}"
