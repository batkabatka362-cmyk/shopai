...
class LeaderboardEngine(BaseEngine):
    engine_name = "leaderboard"
    required_input_fields = ['score_data', 'leaderboard_config']
    required_output_fields = ['rankings', 'display_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} leaderboard", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} leaderboard: {data}"
