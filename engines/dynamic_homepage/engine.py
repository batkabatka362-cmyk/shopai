...
class DynamicHomepageEngine(BaseEngine):
    engine_name = "dynamic_homepage"
    required_input_fields = ['visitor_data', 'product_data']
    required_output_fields = ['homepage_layout', 'personalization_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dynamic homepage", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dynamic homepage: {data}"
