"""Simulation Lab — memory reader.

Reads past simulation records from memory storage.
Used to inform current simulations with historical context.
...
def read_past_simulations(
    strategy_type: str = "",
    limit: int = 10,
) -> dict[str, Any]:
    """Read past simulation records filtered by strategy type.

    Args:
        strategy_type: Filter by strategy type (empty = all).
...
def compute_input_hash(input_data: dict[str, Any]) -> str:
    ...
    Used to detect duplicate / similar simulation requests.
    """
    try:
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_simulations(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    strategy_types = list({r.get("strategy_type", "unknown") for r in records})
