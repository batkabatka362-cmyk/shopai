...
class SiteSelectionEngine(BaseEngine):
    engine_name = "site_selection"
    required_input_fields = ['location_data', 'criteria_config']
    required_output_fields = ['site_scores', 'recommendation_list']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} site selection", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} site selection: {data}"
