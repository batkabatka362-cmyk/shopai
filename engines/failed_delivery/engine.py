...
class FailedDeliveryEngine(BaseEngine):
    engine_name = "failed_delivery"
    required_input_fields = ['delivery_data', 'retry_config']
    required_output_fields = ['retry_plan', 'communication_draft']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} failed delivery", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} failed delivery: {data}"
