...
class SubleaseAgreementEngine(BaseEngine):
    engine_name = "sublease_agreement"
    required_input_fields = ['lease_data', 'sublease_config']
    required_output_fields = ['sublease_draft', 'landlord_consent']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sublease agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sublease agreement: {data}"
