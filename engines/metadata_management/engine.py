...
class MetadataManagementEngine(BaseEngine):
    engine_name = "metadata_management"
    required_input_fields = ['metadata', 'management_config']
    required_output_fields = ['metadata_registry', 'lineage_map']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} metadata management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} metadata management: {data}"
