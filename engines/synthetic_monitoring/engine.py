...
class SyntheticMonitoringEngine(BaseEngine):
    engine_name = "synthetic_monitoring"
    required_input_fields = ['test_config', 'endpoint_data']
    required_output_fields = ['test_results', 'availability_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} synthetic monitoring", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} synthetic monitoring: {data}"
