...
class StreamProcessingEngine(BaseEngine):
    engine_name = "stream_processing"
    required_input_fields = ['stream_data', 'processing_config']
    required_output_fields = ['stream_pipeline', 'scaling_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} stream processing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} stream processing: {data}"
