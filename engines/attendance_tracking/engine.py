...
class AttendanceTrackingEngine(BaseEngine):
    engine_name = "attendance_tracking"
    required_input_fields = ['attendance_data', 'tracking_config']
    required_output_fields = ['attendance_report', 'anomaly_flags']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} attendance tracking", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} attendance tracking: {data}"
