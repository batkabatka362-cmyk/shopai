...
class CxJourneyOrchestrationEngine(BaseEngine):
    engine_name = "cx_journey_orchestration"
    required_input_fields = ['journey_data', 'orchestration_config']
    required_output_fields = ['journey_flow', 'trigger_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cx journey orchestration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cx journey orchestration: {data}"
