...
class NavigationOptimizationEngine(BaseEngine):
    engine_name = "navigation_optimization"
    required_input_fields = ['navigation_data', 'optimization_config']
    required_output_fields = ['nav_structure', 'ux_improvements']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} navigation optimization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} navigation optimization: {data}"
