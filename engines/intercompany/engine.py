...
class IntercompanyEngine(BaseEngine):
    engine_name = "intercompany"
    required_input_fields = ['entity_data', 'transaction_config']
    required_output_fields = ['ic_transactions', 'elimination_entries']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} intercompany", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} intercompany: {data}"
