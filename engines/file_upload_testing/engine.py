...
class FileUploadTestingEngine(BaseEngine):
    engine_name = "file_upload_testing"
    required_input_fields = ['upload_data', 'test_config']
    required_output_fields = ['upload_report', 'security_findings']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} file upload testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} file upload testing: {data}"
