...
class PageAuthorityEngine(BaseEngine):
    engine_name = "page_authority"
    required_input_fields = ['page_data', 'authority_config']
    required_output_fields = ['authority_scores', 'link_equity_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} page authority", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} page authority: {data}"
