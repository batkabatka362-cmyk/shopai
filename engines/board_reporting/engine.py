...
class BoardReportingEngine(BaseEngine):
    engine_name = "board_reporting"
    required_input_fields = ['business_data', 'report_config']
    required_output_fields = ['board_report', 'executive_summary']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} board reporting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} board reporting: {data}"
