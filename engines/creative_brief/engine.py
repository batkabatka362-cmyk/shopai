...
class CreativeBriefEngine(BaseEngine):
    engine_name = "creative_brief"
    required_input_fields = ['campaign_goals', 'brand_guidelines']
    required_output_fields = ['creative_brief', 'asset_requirements']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} creative brief", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} creative brief: {data}"
