...
class CycleCountingEngine(BaseEngine):
    engine_name = "cycle_counting"
    required_input_fields = ['inventory_data', 'cycle_config']
    required_output_fields = ['cycle_schedule', 'accuracy_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cycle counting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cycle counting: {data}"
