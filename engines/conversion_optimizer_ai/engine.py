...
class ConversionOptimizerAiEngine(BaseEngine):
    engine_name = "conversion_optimizer_ai"
    required_input_fields = ['funnel_data', 'test_data']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} conversion optimizer ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} conversion optimizer ai: {data}"
