"""
CompetitiveIntelligenceAi Engine
...
class CompetitiveIntelligenceAiEngine(BaseEngine):
    engine_name = "competitive_intelligence_ai"
    required_input_fields = ['competitor_data', 'market_data']
    required_output_fields = ['competitive_position', 'threat_assessment']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} competitive intelligence ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} competitive intelligence ai: {data}"
