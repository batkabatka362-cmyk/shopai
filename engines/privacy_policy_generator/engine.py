...
class PrivacyPolicyGeneratorEngine(BaseEngine):
    engine_name = "privacy_policy_generator"
    required_input_fields = ['business_data', 'policy_config']
    required_output_fields = ['privacy_policy', 'compliance_mapping']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} privacy policy generator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} privacy policy generator: {data}"
