...
class OnboardingOptimizerEngine(BaseEngine):
    engine_name = "onboarding_optimizer"
    required_input_fields = ['onboarding_data', 'optimization_config']
    required_output_fields = ['optimized_flow', 'completion_forecast']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} onboarding optimizer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} onboarding optimizer: {data}"
