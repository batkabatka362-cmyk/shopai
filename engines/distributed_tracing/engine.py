...
class DistributedTracingEngine(BaseEngine):
    engine_name = "distributed_tracing"
    required_input_fields = ['trace_data', 'tracing_config']
    required_output_fields = ['trace_map', 'latency_breakdown']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} distributed tracing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} distributed tracing: {data}"
