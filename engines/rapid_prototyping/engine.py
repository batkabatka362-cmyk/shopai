...
class RapidPrototypingEngine(BaseEngine):
    engine_name = "rapid_prototyping"
    required_input_fields = ['concept_data', 'prototype_config']
    required_output_fields = ['prototype_spec', 'testing_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} rapid prototyping", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} rapid prototyping: {data}"
