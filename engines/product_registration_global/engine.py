"""
ProductRegistrationGlobal Engine
...
class ProductRegistrationGlobalEngine(BaseEngine):
    engine_name = "product_registration_global"
    required_input_fields = ['product_data', 'registration_config']
    required_output_fields = ['registration_plan', 'market_requirements']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} product registration global", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} product registration global: {data}"
