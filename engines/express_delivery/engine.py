...
class ExpressDeliveryEngine(BaseEngine):
    engine_name = "express_delivery"
    required_input_fields = ['order_data', 'express_config']
    required_output_fields = ['express_options', 'surcharge_calc']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} express delivery", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} express delivery: {data}"
