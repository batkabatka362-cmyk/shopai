...
class InsuranceEngine(BaseEngine):
    engine_name = "insurance"
    required_input_fields = ['product_data', 'insurance_config']
    required_output_fields = ['insurance_options', 'premium_calculations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} insurance", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} insurance: {data}"
