...
class MaintenanceAgreementEngine(BaseEngine):
    engine_name = "maintenance_agreement"
    required_input_fields = ['maintenance_data', 'agreement_config']
    required_output_fields = ['maintenance_terms', 'response_times']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} maintenance agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} maintenance agreement: {data}"
