...
class SmartSeoEngineEngine(BaseEngine):
    engine_name = "smart_seo_engine"
    required_input_fields = ['seo_data', 'competitor_data']
    required_output_fields = ['seo_actions', 'ranking_forecast']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} smart seo engine", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} smart seo engine: {data}"
