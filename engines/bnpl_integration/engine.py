...
class BnplIntegrationEngine(BaseEngine):
    engine_name = "bnpl_integration"
    required_input_fields = ['order_data', 'bnpl_config']
    required_output_fields = ['bnpl_options', 'eligibility']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} bnpl integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} bnpl integration: {data}"
