...
class InvestorRelationsEngine(BaseEngine):
    engine_name = "investor_relations"
    required_input_fields = ['financial_data', 'ir_config']
    required_output_fields = ['ir_content', 'shareholder_update']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} investor relations", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} investor relations: {data}"
