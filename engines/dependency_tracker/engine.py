...
class DependencyTrackerEngine(BaseEngine):
    engine_name = "dependency_tracker"
    required_input_fields = ['project_data', 'dependency_config']
    required_output_fields = ['dependency_map', 'risk_flags']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dependency tracker", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dependency tracker: {data}"
