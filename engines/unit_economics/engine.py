...
class UnitEconomicsEngine(BaseEngine):
    engine_name = "unit_economics"
    required_input_fields = ['cost_data', 'revenue_data']
    required_output_fields = ['unit_metrics', 'profitability_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} unit economics", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} unit economics: {data}"
