...
class AttributionAiEngine(BaseEngine):
    engine_name = "attribution_ai"
    required_input_fields = ['touchpoint_data', 'conversion_data']
    required_output_fields = ['attribution_weights', 'channel_roi']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} attribution ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} attribution ai: {data}"
