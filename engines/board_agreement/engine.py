...
class BoardAgreementEngine(BaseEngine):
    engine_name = "board_agreement"
    required_input_fields = ['director_data', 'agreement_config']
    required_output_fields = ['agreement_terms', 'fiduciary_duties']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} board agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} board agreement: {data}"
