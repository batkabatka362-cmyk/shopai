...
class RetentionOptimizerEngine(BaseEngine):
    engine_name = "retention_optimizer"
    required_input_fields = ['churn_data', 'engagement_data']
    required_output_fields = ['retention_plan', 'projected_savings']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} retention optimizer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} retention optimizer: {data}"
