...
class BrowseAbandonmentEngine(BaseEngine):
    engine_name = "browse_abandonment"
    required_input_fields = ['browse_data', 'retarget_config']
    required_output_fields = ['retarget_campaigns', 'product_recommendations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} browse abandonment", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} browse abandonment: {data}"
