...
class AffinityAnalysisEngine(BaseEngine):
    engine_name = "affinity_analysis"
    required_input_fields = ['purchase_data', 'affinity_config']
    required_output_fields = ['affinity_matrix', 'recommendations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} affinity analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} affinity analysis: {data}"
