"""
CartAbandonmentRecoveryAi Engine
...
class CartAbandonmentRecoveryAiEngine(BaseEngine):
    engine_name = "cart_abandonment_recovery_ai"
    required_input_fields = ['abandoned_carts', 'recovery_config']
    required_output_fields = ['recovery_campaigns', 'expected_recovery']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cart abandonment recovery ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cart abandonment recovery ai: {data}"
