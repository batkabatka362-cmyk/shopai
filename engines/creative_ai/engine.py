...
class CreativeAiEngine(BaseEngine):
    engine_name = "creative_ai"
    required_input_fields = ['brief_data', 'brand_assets']
    required_output_fields = ['creative_concepts', 'asset_specs']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} creative ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} creative ai: {data}"
