...
class ComingSoonPageEngine(BaseEngine):
    engine_name = "coming_soon_page"
    required_input_fields = ['launch_data', 'page_config']
    required_output_fields = ['page_layout', 'countdown_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} coming soon page", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} coming soon page: {data}"
