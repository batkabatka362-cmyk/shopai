...
class BrandSafetyEngine(BaseEngine):
    engine_name = "brand_safety"
    required_input_fields = ['placement_data', 'safety_config']
    required_output_fields = ['safety_report', 'blocklist_updates']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} brand safety", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} brand safety: {data}"
