...
class NextBestChannelEngine(BaseEngine):
    engine_name = "next_best_channel"
    required_input_fields = ['customer_data', 'channel_config']
    required_output_fields = ['channel_recommendation', 'timing_suggestion']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} next best channel", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} next best channel: {data}"
