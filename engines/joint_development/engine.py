...
class JointDevelopmentEngine(BaseEngine):
    engine_name = "joint_development"
    required_input_fields = ['project_data', 'jda_config']
    required_output_fields = ['agreement_draft', 'ip_ownership']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} joint development", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} joint development: {data}"
