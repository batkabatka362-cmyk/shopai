...
class LeadershipDevelopmentEngine(BaseEngine):
    engine_name = "leadership_development"
    required_input_fields = ['leader_data', 'development_config']
    required_output_fields = ['development_plan', 'competency_assessment']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} leadership development", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} leadership development: {data}"
