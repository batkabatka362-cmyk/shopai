...
class CxMaturityEngine(BaseEngine):
    engine_name = "cx_maturity"
    required_input_fields = ['organization_data', 'maturity_config']
    required_output_fields = ['maturity_assessment', 'roadmap']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cx maturity", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cx maturity: {data}"
