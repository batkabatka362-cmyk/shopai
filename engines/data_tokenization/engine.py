...
class DataTokenizationEngine(BaseEngine):
    engine_name = "data_tokenization"
    required_input_fields = ['sensitive_data', 'tokenization_config']
    required_output_fields = ['tokenized_data', 'token_vault']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} data tokenization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} data tokenization: {data}"
