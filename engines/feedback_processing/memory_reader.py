"""Feedback Processing Engine — memory reader.

Reads past feedback processing records from memory storage.
...
def read_past_feedback(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past feedback processing records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_feedback(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past feedback processing records."""
    if not records:
        return {
    ...
    action_counts = [r.get("action_count", 0) for r in records]
    n = len(records)
    return {
        "avg_feedback_count": round(sum(feedback_counts) / n, 1) if n else 0.0,
        "avg_action_count": round(sum(action_counts) / n, 1) if n else 0.0,
        "total_runs": n,
    }
