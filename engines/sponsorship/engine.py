...
class SponsorshipEngine(BaseEngine):
    engine_name = "sponsorship"
    required_input_fields = ['brand_data', 'opportunity_data']
    required_output_fields = ['sponsorship_plan', 'roi_estimate']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sponsorship", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sponsorship: {data}"
