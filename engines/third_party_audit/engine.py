...
class ThirdPartyAuditEngine(BaseEngine):
    engine_name = "third_party_audit"
    required_input_fields = ['script_data', 'audit_config']
    required_output_fields = ['third_party_report', 'impact_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} third party audit", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} third party audit: {data}"
