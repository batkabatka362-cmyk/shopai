...
class NotesToFinancialEngine(BaseEngine):
    engine_name = "notes_to_financial"
    required_input_fields = ['financial_data', 'notes_config']
    required_output_fields = ['financial_notes', 'disclosure_checklist']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} notes to financial", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} notes to financial: {data}"
