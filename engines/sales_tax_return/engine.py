...
class SalesTaxReturnEngine(BaseEngine):
    engine_name = "sales_tax_return"
    required_input_fields = ['transaction_data', 'tax_config']
    required_output_fields = ['tax_return', 'nexus_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sales tax return", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sales tax return: {data}"
