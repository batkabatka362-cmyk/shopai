...
class IngredientSourcingEngine(BaseEngine):
    engine_name = "ingredient_sourcing"
    required_input_fields = ['ingredient_data', 'sourcing_config']
    required_output_fields = ['sourcing_plan', 'supplier_options']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ingredient sourcing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ingredient sourcing: {data}"
