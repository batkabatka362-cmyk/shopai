...
class IdentityProtectionEngine(BaseEngine):
    engine_name = "identity_protection"
    required_input_fields = ['employee_data', 'protection_config']
    required_output_fields = ['protection_plan', 'monitoring_setup']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} identity protection", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} identity protection: {data}"
