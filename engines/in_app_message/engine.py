...
class InAppMessageEngine(BaseEngine):
    engine_name = "in_app_message"
    required_input_fields = ['message_data', 'targeting_config']
    required_output_fields = ['message_display', 'trigger_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} in app message", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} in app message: {data}"
