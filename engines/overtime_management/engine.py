...
class OvertimeManagementEngine(BaseEngine):
    engine_name = "overtime_management"
    required_input_fields = ['time_data', 'overtime_config']
    required_output_fields = ['overtime_report', 'compliance_check']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} overtime management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} overtime management: {data}"
