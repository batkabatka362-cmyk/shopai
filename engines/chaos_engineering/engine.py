...
class ChaosEngineeringEngine(BaseEngine):
    engine_name = "chaos_engineering"
    required_input_fields = ['system_data', 'chaos_config']
    required_output_fields = ['experiment_plan', 'resilience_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} chaos engineering", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} chaos engineering: {data}"
