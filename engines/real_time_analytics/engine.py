...
class RealTimeAnalyticsEngine(BaseEngine):
    engine_name = "real_time_analytics"
    required_input_fields = ['event_stream', 'metric_config']
    required_output_fields = ['live_metrics', 'anomaly_alerts']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} real time analytics", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} real time analytics: {data}"
