...
class ExecutiveCoachingEngine(BaseEngine):
    engine_name = "executive_coaching"
    required_input_fields = ['executive_data', 'coaching_config']
    required_output_fields = ['coaching_plan', 'coach_matching']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} executive coaching", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} executive coaching: {data}"
