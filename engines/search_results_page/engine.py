...
class SearchResultsPageEngine(BaseEngine):
    engine_name = "search_results_page"
    required_input_fields = ['search_data', 'page_config']
    required_output_fields = ['results_layout', 'filter_sidebar']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} search results page", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} search results page: {data}"
