...
class ForecastEnsembleEngine(BaseEngine):
    engine_name = "forecast_ensemble"
    required_input_fields = ['historical_data', 'model_config']
    required_output_fields = ['ensemble_forecast', 'model_weights']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} forecast ensemble", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} forecast ensemble: {data}"
