...
class AchievementSystemEngine(BaseEngine):
    engine_name = "achievement_system"
    required_input_fields = ['user_data', 'achievement_config']
    required_output_fields = ['achievement_setup', 'unlock_conditions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} achievement system", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} achievement system: {data}"
