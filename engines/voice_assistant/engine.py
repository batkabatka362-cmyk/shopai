...
class VoiceAssistantEngine(BaseEngine):
    engine_name = "voice_assistant"
    required_input_fields = ['voice_data', 'assistant_config']
    required_output_fields = ['voice_response', 'action_triggers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} voice assistant", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} voice assistant: {data}"
