...
class ConcurrencyTestingEngine(BaseEngine):
    engine_name = "concurrency_testing"
    required_input_fields = ['system_data', 'concurrency_config']
    required_output_fields = ['race_conditions', 'deadlock_detection']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} concurrency testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} concurrency testing: {data}"
