...
class ApiSecurityEngine(BaseEngine):
    ...
    required_input_fields = ['api_data', 'security_config']
    required_output_fields = ['security_assessment', 'hardening_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} api security", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} api security: {data}"
