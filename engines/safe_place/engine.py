...
class SafePlaceEngine(BaseEngine):
    engine_name = "safe_place"
    required_input_fields = ['delivery_data', 'safe_place_config']
    required_output_fields = ['safe_place_options', 'instruction_display']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} safe place", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} safe place: {data}"
