...
class GainSharingEngine(BaseEngine):
    engine_name = "gain_sharing"
    required_input_fields = ['performance_data', 'sharing_config']
    required_output_fields = ['gain_distribution', 'metrics_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} gain sharing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} gain sharing: {data}"
