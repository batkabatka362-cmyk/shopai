...
class ApiEconomyEngine(BaseEngine):
    engine_name = "api_economy"
    required_input_fields = ['api_data', 'economy_config']
    required_output_fields = ['monetization_plan', 'developer_portal']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} api economy", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} api economy: {data}"
