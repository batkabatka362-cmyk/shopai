...
class ValueChainEngine(BaseEngine):
    engine_name = "value_chain"
    required_input_fields = ['operations_data', 'cost_data']
    required_output_fields = ['value_chain_map', 'optimization_opportunities']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} value chain", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} value chain: {data}"
