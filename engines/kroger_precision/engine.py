...
class KrogerPrecisionEngine(BaseEngine):
    engine_name = "kroger_precision"
    required_input_fields = ['campaign_data', 'kroger_config']
    required_output_fields = ['campaign_setup', 'shopper_targeting']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} kroger precision", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} kroger precision: {data}"
