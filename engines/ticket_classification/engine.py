...
class TicketClassificationEngine(BaseEngine):
    engine_name = "ticket_classification"
    required_input_fields = ['ticket_data', 'classification_config']
    required_output_fields = ['ticket_categories', 'confidence_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ticket classification", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ticket classification: {data}"
