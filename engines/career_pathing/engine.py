...
class CareerPathingEngine(BaseEngine):
    engine_name = "career_pathing"
    required_input_fields = ['employee_data', 'career_config']
    required_output_fields = ['career_paths', 'skill_gaps']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} career pathing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} career pathing: {data}"
