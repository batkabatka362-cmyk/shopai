...
class RedirectChainEngine(BaseEngine):
    engine_name = "redirect_chain"
    required_input_fields = ['url_data', 'analysis_config']
    required_output_fields = ['chain_report', 'fix_recommendations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} redirect chain", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} redirect chain: {data}"
