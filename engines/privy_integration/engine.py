...
class PrivyIntegrationEngine(BaseEngine):
    engine_name = "privy_integration"
    required_input_fields = ['popup_data', 'integration_config']
    required_output_fields = ['privy_sync', 'campaign_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} privy integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} privy integration: {data}"
