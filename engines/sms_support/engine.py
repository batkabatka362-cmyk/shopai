...
class SmsSupportEngine(BaseEngine):
    engine_name = "sms_support"
    required_input_fields = ['sms_data', 'support_config']
    required_output_fields = ['sms_response', 'automation_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sms support", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sms support: {data}"
