"""
SubscriptionReconciliation Engine
...
class SubscriptionReconciliationEngine(BaseEngine):
    engine_name = "subscription_reconciliation"
    required_input_fields = ['subscription_data', 'billing_data']
    required_output_fields = ['reconciliation_report', 'mrr_verification']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} subscription reconciliation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} subscription reconciliation: {data}"
