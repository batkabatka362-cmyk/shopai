...
class StatutoryReportingEngine(BaseEngine):
    engine_name = "statutory_reporting"
    required_input_fields = ['financial_data', 'reporting_config']
    required_output_fields = ['statutory_reports', 'filing_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} statutory reporting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} statutory reporting: {data}"
