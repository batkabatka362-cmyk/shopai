...
class MultiVendorEngine(BaseEngine):
    engine_name = "multi_vendor"
    required_input_fields = ['vendor_data', 'platform_config']
    required_output_fields = ['vendor_setup', 'commission_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} multi vendor", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} multi vendor: {data}"
