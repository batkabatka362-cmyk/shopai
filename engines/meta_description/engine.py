...
class MetaDescriptionEngine(BaseEngine):
    engine_name = "meta_description"
    required_input_fields = ['page_data', 'keyword_data']
    required_output_fields = ['meta_descriptions', 'preview_snippets']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} meta description", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} meta description: {data}"
