...
class SquareIntegrationEngine(BaseEngine):
    engine_name = "square_integration"
    required_input_fields = ['pos_data', 'integration_config']
    required_output_fields = ['square_sync', 'inventory_mapping']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} square integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} square integration: {data}"
