"""
LandingPageBuilder Engine — Landing Page Builder
...
class LandingPageBuilderEngine(BaseEngine):
    engine_name = "landing_page_builder"
    required_input_fields = ['campaign_data', 'template_config']
    required_output_fields = ['page_structure', 'copy_blocks']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} landing page builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} landing page builder: {data}"
