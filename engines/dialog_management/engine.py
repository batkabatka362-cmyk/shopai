...
class DialogManagementEngine(BaseEngine):
    engine_name = "dialog_management"
    required_input_fields = ['conversation_data', 'dialog_config']
    required_output_fields = ['next_action', 'response_template']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dialog management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dialog management: {data}"
