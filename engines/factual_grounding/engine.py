...
class FactualGroundingEngine(BaseEngine):
    engine_name = "factual_grounding"
    required_input_fields = ['generated_text', 'knowledge_base']
    required_output_fields = ['grounding_score', 'source_citations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} factual grounding", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} factual grounding: {data}"
