...
class AmlScreeningEngine(BaseEngine):
    engine_name = "aml_screening"
    required_input_fields = ['customer_data', 'screening_config']
    required_output_fields = ['screening_result', 'risk_level']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} aml screening", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} aml screening: {data}"
