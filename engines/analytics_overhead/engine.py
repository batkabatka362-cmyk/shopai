...
class AnalyticsOverheadEngine(BaseEngine):
    engine_name = "analytics_overhead"
    required_input_fields = ['analytics_data', 'audit_config']
    required_output_fields = ['overhead_report', 'reduction_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} analytics overhead", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} analytics overhead: {data}"
