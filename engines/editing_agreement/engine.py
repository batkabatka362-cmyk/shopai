...
class EditingAgreementEngine(BaseEngine):
    engine_name = "editing_agreement"
    required_input_fields = ['editing_data', 'agreement_config']
    required_output_fields = ['agreement_terms', 'revision_rounds']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} editing agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} editing agreement: {data}"
