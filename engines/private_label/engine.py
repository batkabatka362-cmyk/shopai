...
class PrivateLabelEngine(BaseEngine):
    engine_name = "private_label"
    required_input_fields = ['category_data', 'brand_config']
    required_output_fields = ['private_label_plan', 'supplier_requirements']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} private label", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} private label: {data}"
