...
class LicenseManagementEngine(BaseEngine):
    engine_name = "license_management"
    required_input_fields = ['software_data', 'license_config']
    required_output_fields = ['license_inventory', 'renewal_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} license management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} license management: {data}"
