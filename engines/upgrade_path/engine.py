...
class UpgradePathEngine(BaseEngine):
    engine_name = "upgrade_path"
    required_input_fields = ['subscription_data', 'upgrade_config']
    required_output_fields = ['upgrade_options', 'migration_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} upgrade path", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} upgrade path: {data}"
