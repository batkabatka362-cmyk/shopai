...
class CxPersonalizationEngine(BaseEngine):
    engine_name = "cx_personalization"
    required_input_fields = ['customer_data', 'personalization_config']
    required_output_fields = ['personalized_experience', 'variant_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cx personalization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cx personalization: {data}"
