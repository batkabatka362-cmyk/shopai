...
class MeetingRoomEngine(BaseEngine):
    engine_name = "meeting_room"
    required_input_fields = ['room_data', 'booking_config']
    required_output_fields = ['booking_system', 'utilization_analytics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} meeting room", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} meeting room: {data}"
