...
class ConfidentialityClauseEngine(BaseEngine):
    engine_name = "confidentiality_clause"
    required_input_fields = ['scope_data', 'conf_config']
    required_output_fields = ['confidentiality_terms', 'exclusion_list']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} confidentiality clause", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} confidentiality clause: {data}"
