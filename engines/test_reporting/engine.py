...
class TestReportingEngine(BaseEngine):
    engine_name = "test_reporting"
    required_input_fields = ['test_results', 'reporting_config']
    required_output_fields = ['test_report', 'trend_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} test reporting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} test reporting: {data}"
