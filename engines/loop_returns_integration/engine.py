...
class LoopReturnsIntegrationEngine(BaseEngine):
    engine_name = "loop_returns_integration"
    required_input_fields = ['return_data', 'integration_config']
    required_output_fields = ['loop_sync', 'workflow_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} loop returns integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} loop returns integration: {data}"
