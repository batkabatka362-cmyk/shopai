...
class SalesEnablementEngine(BaseEngine):
    engine_name = "sales_enablement"
    required_input_fields = ['content_data', 'enablement_config']
    required_output_fields = ['content_library', 'training_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sales enablement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sales enablement: {data}"
