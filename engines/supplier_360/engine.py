...
class Supplier360Engine(BaseEngine):
    engine_name = "supplier_360"
    required_input_fields = ['supplier_data', 'integration_config']
    required_output_fields = ['unified_supplier', 'risk_overview']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} supplier 360", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} supplier 360: {data}"
