...
class ModelComparisonEngine(BaseEngine):
    engine_name = "model_comparison"
    required_input_fields = ['model_list', 'comparison_config']
    required_output_fields = ['comparison_report', 'recommendation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} model comparison", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} model comparison: {data}"
