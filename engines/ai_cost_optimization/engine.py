...
class AiCostOptimizationEngine(BaseEngine):
    engine_name = "ai_cost_optimization"
    required_input_fields = ['usage_data', 'cost_config']
    required_output_fields = ['optimization_plan', 'savings_estimate']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ai cost optimization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ai cost optimization: {data}"
