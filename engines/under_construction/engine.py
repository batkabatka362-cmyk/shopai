...
class UnderConstructionEngine(BaseEngine):
    engine_name = "under_construction"
    required_input_fields = ['project_data', 'page_config']
    required_output_fields = ['page_content', 'progress_display']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} under construction", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} under construction: {data}"
