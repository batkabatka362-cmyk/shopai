...
class DaypartingEngine(BaseEngine):
    engine_name = "dayparting"
    required_input_fields = ['performance_data', 'schedule_config']
    required_output_fields = ['ad_schedule', 'hourly_modifiers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dayparting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dayparting: {data}"
