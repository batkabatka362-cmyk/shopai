...
class AlumniNetworkEngine(BaseEngine):
    engine_name = "alumni_network"
    required_input_fields = ['alumni_data', 'network_config']
    required_output_fields = ['network_setup', 'engagement_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} alumni network", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} alumni network: {data}"
