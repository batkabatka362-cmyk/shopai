...
class AudienceBidEngine(BaseEngine):
    engine_name = "audience_bid"
    required_input_fields = ['audience_data', 'bid_config']
    required_output_fields = ['bid_modifiers', 'segment_performance']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} audience bid", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} audience bid: {data}"
