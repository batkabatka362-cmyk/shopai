...
class PostPurchaseSurveyEngine(BaseEngine):
    engine_name = "post_purchase_survey"
    required_input_fields = ['order_data', 'survey_config']
    required_output_fields = ['survey_content', 'trigger_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} post purchase survey", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} post purchase survey: {data}"
