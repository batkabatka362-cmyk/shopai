...
class SupplyAgreementEngine(BaseEngine):
    engine_name = "supply_agreement"
    required_input_fields = ['supplier_data', 'agreement_config']
    required_output_fields = ['agreement_draft', 'volume_commitments']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} supply agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} supply agreement: {data}"
