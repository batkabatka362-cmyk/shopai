...
class MealPlanningEngine(BaseEngine):
    engine_name = "meal_planning"
    required_input_fields = ['preference_data', 'recipe_database']
    required_output_fields = ['meal_plan', 'grocery_list']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} meal planning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} meal planning: {data}"
