...
class RouteOptimizationEngine(BaseEngine):
    engine_name = "route_optimization"
    required_input_fields = ['location_data', 'route_config']
    required_output_fields = ['optimized_routes', 'distance_matrix']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} route optimization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} route optimization: {data}"
