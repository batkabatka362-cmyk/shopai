...
class DigitalAssetEngine(BaseEngine):
    engine_name = "digital_asset"
    required_input_fields = ['asset_data', 'dam_config']
    required_output_fields = ['asset_library', 'usage_tracking']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} digital asset", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} digital asset: {data}"
