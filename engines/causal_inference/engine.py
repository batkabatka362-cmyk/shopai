...
class CausalInferenceEngine(BaseEngine):
    engine_name = "causal_inference"
    required_input_fields = ['observational_data', 'inference_config']
    required_output_fields = ['causal_estimates', 'confounding_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} causal inference", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} causal inference: {data}"
