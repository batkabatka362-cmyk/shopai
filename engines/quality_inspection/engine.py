...
class QualityInspectionEngine(BaseEngine):
    engine_name = "quality_inspection"
    required_input_fields = ['product_samples', 'inspection_criteria']
    required_output_fields = ['inspection_report', 'pass_fail_status']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} quality inspection", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} quality inspection: {data}"
