...
class PagePerformanceEngine(BaseEngine):
    engine_name = "page_performance"
    required_input_fields = ['page_data', 'performance_config']
    required_output_fields = ['performance_scores', 'bottlenecks']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} page performance", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} page performance: {data}"
