...
class PartsCatalogEngine(BaseEngine):
    engine_name = "parts_catalog"
    required_input_fields = ['product_data', 'parts_config']
    required_output_fields = ['parts_listing', 'compatibility_map']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} parts catalog", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} parts catalog: {data}"
