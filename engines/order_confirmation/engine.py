...
class OrderConfirmationEngine(BaseEngine):
    engine_name = "order_confirmation"
    required_input_fields = ['order_data', 'template_config']
    required_output_fields = ['confirmation_email', 'tracking_setup']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} order confirmation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} order confirmation: {data}"
