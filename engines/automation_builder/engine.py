...
class AutomationBuilderEngine(BaseEngine):
    engine_name = "automation_builder"
    required_input_fields = ['automation_data', 'builder_config']
    required_output_fields = ['automation_flows', 'trigger_library']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} automation builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} automation builder: {data}"
