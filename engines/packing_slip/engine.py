...
class PackingSlipEngine(BaseEngine):
    engine_name = "packing_slip"
    required_input_fields = ['order_data', 'slip_config']
    required_output_fields = ['slip_content', 'print_layout']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} packing slip", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} packing slip: {data}"
