...
class CuratedCollectionEngine(BaseEngine):
    engine_name = "curated_collection"
    required_input_fields = ['product_data', 'curation_config']
    required_output_fields = ['collection_setup', 'rotation_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} curated collection", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} curated collection: {data}"
