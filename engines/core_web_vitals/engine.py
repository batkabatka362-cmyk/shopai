...
class CoreWebVitalsEngine(BaseEngine):
    engine_name = "core_web_vitals"
    required_input_fields = ['performance_data', 'vitals_config']
    required_output_fields = ['cwv_scores', 'fix_priorities']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} core web vitals", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} core web vitals: {data}"
