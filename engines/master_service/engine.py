...
class MasterServiceEngine(BaseEngine):
    engine_name = "master_service"
    required_input_fields = ['service_data', 'msa_config']
    required_output_fields = ['msa_draft', 'sow_template']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} master service", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} master service: {data}"
