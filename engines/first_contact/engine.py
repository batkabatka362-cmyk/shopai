...
class FirstContactEngine(BaseEngine):
    engine_name = "first_contact"
    required_input_fields = ['resolution_data', 'fcr_config']
    required_output_fields = ['fcr_rate', 'improvement_actions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} first contact", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} first contact: {data}"
