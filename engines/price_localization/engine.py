...
class PriceLocalizationEngine(BaseEngine):
    engine_name = "price_localization"
    required_input_fields = ['product_data', 'market_config']
    required_output_fields = ['localized_prices', 'purchasing_power_adj']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} price localization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} price localization: {data}"
