...
class SmartDiscountEngineEngine(BaseEngine):
    engine_name = "smart_discount_engine"
    required_input_fields = ['cart_data', 'customer_data']
    required_output_fields = ['discount_offer', 'margin_impact']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} smart discount engine", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} smart discount engine: {data}"
