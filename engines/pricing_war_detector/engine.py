...
class PricingWarDetectorEngine(BaseEngine):
    engine_name = "pricing_war_detector"
    required_input_fields = ['competitor_data', 'pricing_data']
    required_output_fields = ['war_alerts', 'response_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pricing war detector", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pricing war detector: {data}"
