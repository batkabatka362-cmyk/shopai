...
class ApiMarketplaceEngine(BaseEngine):
    engine_name = "api_marketplace"
    required_input_fields = ['api_data', 'marketplace_config']
    required_output_fields = ['api_catalog', 'usage_plans']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} api marketplace", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} api marketplace: {data}"
