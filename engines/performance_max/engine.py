...
class PerformanceMaxEngine(BaseEngine):
    engine_name = "performance_max"
    required_input_fields = ['campaign_data', 'pmax_config']
    required_output_fields = ['pmax_setup', 'asset_group_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} performance max", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} performance max: {data}"
