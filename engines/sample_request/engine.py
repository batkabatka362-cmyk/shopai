"""
SampleRequest Engine — Sample Request
...
class SampleRequestEngine(BaseEngine):
    engine_name = "sample_request"
    required_input_fields = ['product_data', 'sample_config']
    required_output_fields = ['sample_program', 'qualification_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sample request", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sample request: {data}"
