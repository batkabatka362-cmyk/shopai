...
class RuleOf40Engine(BaseEngine):
    engine_name = "rule_of_40"
    required_input_fields = ['growth_data', 'profitability_data']
    required_output_fields = ['rule_of_40_score', 'benchmark_comparison']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} rule of 40", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} rule of 40: {data}"
