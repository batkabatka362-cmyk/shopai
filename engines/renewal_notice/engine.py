...
class RenewalNoticeEngine(BaseEngine):
    engine_name = "renewal_notice"
    required_input_fields = ['contract_data', 'renewal_config']
    required_output_fields = ['notice_draft', 'updated_terms']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} renewal notice", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} renewal notice: {data}"
