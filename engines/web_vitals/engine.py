...
class WebVitalsEngine(BaseEngine):
    engine_name = "web_vitals"
    required_input_fields = ['performance_data', 'vitals_config']
    required_output_fields = ['vitals_scores', 'improvement_actions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} web vitals", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} web vitals: {data}"
