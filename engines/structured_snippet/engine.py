...
class StructuredSnippetEngine(BaseEngine):
    engine_name = "structured_snippet"
    required_input_fields = ['content_data', 'snippet_config']
    required_output_fields = ['snippet_markup', 'preview']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} structured snippet", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} structured snippet: {data}"
