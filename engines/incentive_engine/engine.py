...
class IncentiveEngineEngine(BaseEngine):
    engine_name = "incentive_engine"
    required_input_fields = ['performance_data', 'incentive_config']
    required_output_fields = ['incentive_plan', 'payout_forecast']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} incentive engine", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} incentive engine: {data}"
