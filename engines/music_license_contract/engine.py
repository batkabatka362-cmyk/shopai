...
class MusicLicenseContractEngine(BaseEngine):
    engine_name = "music_license_contract"
    required_input_fields = ['music_data', 'license_config']
    required_output_fields = ['license_terms', 'usage_scope']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} music license contract", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} music license contract: {data}"
