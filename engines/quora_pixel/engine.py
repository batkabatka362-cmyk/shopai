...
class QuoraPixelEngine(BaseEngine):
    engine_name = "quora_pixel"
    required_input_fields = ['pixel_data', 'integration_config']
    required_output_fields = ['pixel_setup', 'conversion_events']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} quora pixel", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} quora pixel: {data}"
