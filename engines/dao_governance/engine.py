...
class DaoGovernanceEngine(BaseEngine):
    engine_name = "dao_governance"
    required_input_fields = ['dao_data', 'governance_config']
    required_output_fields = ['governance_setup', 'voting_mechanism']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dao governance", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dao governance: {data}"
