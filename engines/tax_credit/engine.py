...
class TaxCreditEngine(BaseEngine):
    engine_name = "tax_credit"
    required_input_fields = ['activity_data', 'credit_config']
    required_output_fields = ['credit_identification', 'claim_documentation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} tax credit", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} tax credit: {data}"
