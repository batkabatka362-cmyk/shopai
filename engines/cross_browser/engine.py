...
class CrossBrowserEngine(BaseEngine):
    engine_name = "cross_browser"
    required_input_fields = ['page_data', 'browser_config']
    required_output_fields = ['compatibility_report', 'rendering_diffs']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cross browser", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cross browser: {data}"
