"""Image Optimization Engine — memory reader.

Reads past image optimization records from memory storage.
...
def read_past_optimizations(
    ...
    """Read past image optimization records.
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past optimization records."""
    if not records:
        return {
    ...
    scores = [r.get("avg_quality_score", 0.0) for r in records]
    savings = [r.get("avg_savings_pct", 0.0) for r in records]

    n = len(records)
    return {
        "avg_quality_score": round(sum(scores) / n, 1) if n else 0.0,
