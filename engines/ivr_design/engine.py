...
class IvrDesignEngine(BaseEngine):
    engine_name = "ivr_design"
    required_input_fields = ['call_flow', 'ivr_config']
    required_output_fields = ['ivr_tree', 'prompt_scripts']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ivr design", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ivr design: {data}"
