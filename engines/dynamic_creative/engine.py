...
class DynamicCreativeEngine(BaseEngine):
    engine_name = "dynamic_creative"
    required_input_fields = ['creative_data', 'dynamic_config']
    required_output_fields = ['dco_setup', 'variant_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dynamic creative", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dynamic creative: {data}"
