...
class CacheStrategyEngine(BaseEngine):
    engine_name = "cache_strategy"
    required_input_fields = ['traffic_data', 'cache_config']
    required_output_fields = ['cache_plan', 'invalidation_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cache strategy", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cache strategy: {data}"
