...
class FundraisingEngine(BaseEngine):
    engine_name = "fundraising"
    required_input_fields = ['company_data', 'fundraising_config']
    required_output_fields = ['fundraising_strategy', 'investor_targets']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} fundraising", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} fundraising: {data}"
