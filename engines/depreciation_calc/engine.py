...
class DepreciationCalcEngine(BaseEngine):
    engine_name = "depreciation_calc"
    required_input_fields = ['asset_data', 'depreciation_config']
    required_output_fields = ['depreciation_schedule', 'tax_impact']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} depreciation calc", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} depreciation calc: {data}"
