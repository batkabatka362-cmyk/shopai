...
class StoreHealthEngine(BaseEngine):
    engine_name = "store_health"
    required_input_fields = ['store_metrics', 'health_config']
    required_output_fields = ['health_score', 'issue_alerts']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} store health", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} store health: {data}"
