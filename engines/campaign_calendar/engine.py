...
class CampaignCalendarEngine(BaseEngine):
    engine_name = "campaign_calendar"
    required_input_fields = ['campaign_data', 'calendar_config']
    required_output_fields = ['calendar_view', 'conflict_alerts']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} campaign calendar", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} campaign calendar: {data}"
