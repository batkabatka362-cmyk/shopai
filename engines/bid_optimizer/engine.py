...
class BidOptimizerEngine(BaseEngine):
    engine_name = "bid_optimizer"
    required_input_fields = ['auction_data', 'bid_config']
    required_output_fields = ['bid_recommendations', 'win_rate_forecast']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} bid optimizer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} bid optimizer: {data}"
