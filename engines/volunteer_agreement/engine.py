...
class VolunteerAgreementEngine(BaseEngine):
    engine_name = "volunteer_agreement"
    required_input_fields = ['volunteer_data', 'agreement_config']
    required_output_fields = ['agreement_terms', 'liability_waiver']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} volunteer agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} volunteer agreement: {data}"
