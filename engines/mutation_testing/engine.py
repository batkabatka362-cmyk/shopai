...
class MutationTestingEngine(BaseEngine):
    engine_name = "mutation_testing"
    required_input_fields = ['code_data', 'mutation_config']
    required_output_fields = ['mutation_score', 'surviving_mutants']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} mutation testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} mutation testing: {data}"
