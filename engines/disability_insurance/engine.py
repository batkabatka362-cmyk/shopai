...
class DisabilityInsuranceEngine(BaseEngine):
    engine_name = "disability_insurance"
    required_input_fields = ['employee_data', 'disability_config']
    required_output_fields = ['coverage_options', 'claim_process']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} disability insurance", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} disability insurance: {data}"
