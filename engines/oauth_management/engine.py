...
class OauthManagementEngine(BaseEngine):
    engine_name = "oauth_management"
    required_input_fields = ['oauth_config', 'client_data']
    required_output_fields = ['oauth_setup', 'token_management']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} oauth management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} oauth management: {data}"
