...
class BondedWarehouseEngine(BaseEngine):
    engine_name = "bonded_warehouse"
    required_input_fields = ['warehouse_data', 'bonded_config']
    required_output_fields = ['bonded_setup', 'duty_deferral']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} bonded warehouse", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} bonded warehouse: {data}"
