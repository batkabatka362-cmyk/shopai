...
class TestGenerationEngine(BaseEngine):
    engine_name = "test_generation"
    required_input_fields = ['code_data', 'test_config']
    required_output_fields = ['test_cases', 'coverage_estimate']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} test generation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} test generation: {data}"
