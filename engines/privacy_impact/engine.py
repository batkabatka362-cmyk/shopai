...
class PrivacyImpactEngine(BaseEngine):
    engine_name = "privacy_impact"
    required_input_fields = ['system_data', 'privacy_config']
    required_output_fields = ['impact_assessment', 'risk_rating']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} privacy impact", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} privacy impact: {data}"
