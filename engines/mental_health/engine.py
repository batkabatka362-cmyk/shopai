...
class MentalHealthEngine(BaseEngine):
    engine_name = "mental_health"
    required_input_fields = ['employee_data', 'mental_health_config']
    required_output_fields = ['resource_guide', 'provider_access']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} mental health", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} mental health: {data}"
