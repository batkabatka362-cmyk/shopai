"""
PromptInjectionDetection Engine
...
class PromptInjectionDetectionEngine(BaseEngine):
    engine_name = "prompt_injection_detection"
    required_input_fields = ['input_data', 'detection_config']
    required_output_fields = ['injection_score', 'blocked_inputs']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} prompt injection detection", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} prompt injection detection: {data}"
