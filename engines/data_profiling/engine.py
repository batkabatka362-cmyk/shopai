...
class DataProfilingEngine(BaseEngine):
    engine_name = "data_profiling"
    required_input_fields = ['dataset', 'profiling_config']
    required_output_fields = ['profile_report', 'anomaly_flags']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} data profiling", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} data profiling: {data}"
