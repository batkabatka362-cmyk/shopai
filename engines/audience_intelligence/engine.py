...
class AudienceIntelligenceEngine(BaseEngine):
    engine_name = "audience_intelligence"
    required_input_fields = ['audience_data', 'market_data']
    required_output_fields = ['audience_insights', 'persona_refinement']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} audience intelligence", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} audience intelligence: {data}"
