...
class SubcontractAgreementEngine(BaseEngine):
    engine_name = "subcontract_agreement"
    required_input_fields = ['subcontract_data', 'agreement_config']
    required_output_fields = ['subcontract_terms', 'flow_down_provisions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} subcontract agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} subcontract agreement: {data}"
