...
class DeadStockEngine(BaseEngine):
    engine_name = "dead_stock"
    required_input_fields = ['inventory_data', 'disposal_config']
    required_output_fields = ['dead_stock_list', 'disposal_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dead stock", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dead stock: {data}"
