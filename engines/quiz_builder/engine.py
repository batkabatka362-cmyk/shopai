...
class QuizBuilderEngine(BaseEngine):
    engine_name = "quiz_builder"
    required_input_fields = ['question_data', 'quiz_config']
    required_output_fields = ['quiz_flow', 'scoring_logic']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} quiz builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} quiz builder: {data}"
