...
class NoCodeBuilderEngine(BaseEngine):
    engine_name = "no_code_builder"
    required_input_fields = ['workflow_data', 'builder_config']
    required_output_fields = ['builder_setup', 'template_library']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} no code builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} no code builder: {data}"
