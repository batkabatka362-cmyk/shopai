...
class CrossDeviceEngine(BaseEngine):
    engine_name = "cross_device"
    required_input_fields = ['device_data', 'tracking_config']
    required_output_fields = ['device_graph', 'attribution_model']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cross device", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cross device: {data}"
