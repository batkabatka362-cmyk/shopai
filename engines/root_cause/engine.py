...
class RootCauseEngine(BaseEngine):
    engine_name = "root_cause"
    required_input_fields = ['incident_data', 'analysis_config']
    required_output_fields = ['root_causes', 'prevention_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} root cause", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} root cause: {data}"
