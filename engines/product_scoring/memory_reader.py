"""Product Scoring Engine — memory reader.

Reads past scoring records from memory storage.
"""
...
def read_past_scores(limit: int = 10) -> dict[str, Any]:
    """Read past scoring records."""
    try:
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_scores(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    if not records:
        return {"avg_composite_score": 0.0, "total_runs": 0}
    scores = [r.get("avg_composite_score", 0.0) for r in records]
    n = len(records)
    return {"avg_composite_score": round(sum(scores) / n, 2) if n else 0.0, "total_runs": n}
