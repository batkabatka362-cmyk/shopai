...
class DomainAuthorityEngine(BaseEngine):
    engine_name = "domain_authority"
    required_input_fields = ['domain_data', 'authority_config']
    required_output_fields = ['authority_score', 'improvement_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} domain authority", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} domain authority: {data}"
