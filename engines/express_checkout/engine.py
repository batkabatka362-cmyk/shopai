...
class ExpressCheckoutEngine(BaseEngine):
    engine_name = "express_checkout"
    required_input_fields = ['payment_data', 'express_config']
    required_output_fields = ['express_flow', 'wallet_integration']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} express checkout", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} express checkout: {data}"
