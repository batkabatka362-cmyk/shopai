...
class WarrantyClaimEngine(BaseEngine):
    engine_name = "warranty_claim"
    required_input_fields = ['claim_data', 'warranty_terms']
    required_output_fields = ['claim_decision', 'resolution_steps']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} warranty claim", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} warranty claim: {data}"
