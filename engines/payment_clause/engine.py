...
class PaymentClauseEngine(BaseEngine):
    engine_name = "payment_clause"
    required_input_fields = ['payment_data', 'clause_config']
    required_output_fields = ['payment_terms', 'late_payment_provisions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} payment clause", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} payment clause: {data}"
