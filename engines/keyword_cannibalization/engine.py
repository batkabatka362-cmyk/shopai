...
class KeywordCannibalizationEngine(BaseEngine):
    engine_name = "keyword_cannibalization"
    required_input_fields = ['keyword_data', 'analysis_config']
    required_output_fields = ['cannibalization_report', 'consolidation_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} keyword cannibalization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} keyword cannibalization: {data}"
