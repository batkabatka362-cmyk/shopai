...
class AccessControlEngine(BaseEngine):
    engine_name = "access_control"
    required_input_fields = ['user_data', 'permission_config']
    required_output_fields = ['access_matrix', 'audit_log']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} access control", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} access control: {data}"
