...
class ModelRobustnessEngine(BaseEngine):
    engine_name = "model_robustness"
    required_input_fields = ['model_data', 'robustness_config']
    required_output_fields = ['robustness_score', 'adversarial_examples']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} model robustness", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} model robustness: {data}"
