...
class RdTaxCreditEngine(BaseEngine):
    engine_name = "rd_tax_credit"
    required_input_fields = ['rd_data', 'credit_config']
    required_output_fields = ['qualifying_activities', 'credit_calculation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} rd tax credit", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} rd tax credit: {data}"
