...
class PredictiveRevenueEngine(BaseEngine):
    engine_name = "predictive_revenue"
    required_input_fields = ['revenue_history', 'prediction_config']
    required_output_fields = ['revenue_forecast', 'confidence_bands']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} predictive revenue", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} predictive revenue: {data}"
