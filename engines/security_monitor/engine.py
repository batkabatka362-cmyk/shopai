...
class SecurityMonitorEngine(BaseEngine):
    engine_name = "security_monitor"
    required_input_fields = ['security_data', 'monitor_config']
    required_output_fields = ['security_report', 'threat_alerts']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} security monitor", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} security monitor: {data}"
