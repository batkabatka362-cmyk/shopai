...
class UptimeMonitoringEngine(BaseEngine):
    engine_name = "uptime_monitoring"
    required_input_fields = ['endpoint_data', 'monitoring_config']
    required_output_fields = ['uptime_report', 'incident_alerts']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} uptime monitoring", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} uptime monitoring: {data}"
