...
class CashFlowStatementEngine(BaseEngine):
    engine_name = "cash_flow_statement"
    required_input_fields = ['financial_data', 'cf_config']
    required_output_fields = ['cash_flow_statement', 'free_cash_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cash flow statement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cash flow statement: {data}"
