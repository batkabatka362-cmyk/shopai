...
class PeriodEndEngine(BaseEngine):
    engine_name = "period_end"
    required_input_fields = ['accounting_data', 'period_config']
    required_output_fields = ['close_tasks', 'reconciliation_list']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} period end", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} period end: {data}"
