...
class EventPlanningEngine(BaseEngine):
    engine_name = "event_planning"
    required_input_fields = ['event_data', 'planning_config']
    required_output_fields = ['event_plan', 'logistics_checklist']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} event planning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} event planning: {data}"
