...
class LicenseKeyEngine(BaseEngine):
    engine_name = "license_key"
    required_input_fields = ['product_data', 'license_config']
    required_output_fields = ['license_keys', 'activation_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} license key", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} license key: {data}"
