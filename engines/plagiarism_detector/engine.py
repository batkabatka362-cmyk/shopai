...
class PlagiarismDetectorEngine(BaseEngine):
    engine_name = "plagiarism_detector"
    required_input_fields = ['text_data', 'reference_corpus']
    required_output_fields = ['similarity_scores', 'matched_sources']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} plagiarism detector", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} plagiarism detector: {data}"
