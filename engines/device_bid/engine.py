...
class DeviceBidEngine(BaseEngine):
    engine_name = "device_bid"
    required_input_fields = ['device_data', 'bid_config']
    required_output_fields = ['device_adjustments', 'cross_device_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} device bid", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} device bid: {data}"
