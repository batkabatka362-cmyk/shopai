...
class MultivariateTestEngine(BaseEngine):
    engine_name = "multivariate_test"
    required_input_fields = ['test_config', 'variant_data']
    required_output_fields = ['test_plan', 'interaction_effects']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} multivariate test", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} multivariate test: {data}"
