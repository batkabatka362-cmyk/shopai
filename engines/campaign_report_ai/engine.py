...
class CampaignReportAiEngine(BaseEngine):
    engine_name = "campaign_report_ai"
    required_input_fields = ['campaign_data', 'report_config']
    required_output_fields = ['campaign_report', 'roi_summary']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} campaign report ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} campaign report ai: {data}"
