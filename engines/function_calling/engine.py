...
class FunctionCallingEngine(BaseEngine):
    engine_name = "function_calling"
    required_input_fields = ['task_data', 'function_registry']
    required_output_fields = ['function_calls', 'parameter_mapping']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} function calling", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} function calling: {data}"
