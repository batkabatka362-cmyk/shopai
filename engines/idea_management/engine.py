...
class IdeaManagementEngine(BaseEngine):
    engine_name = "idea_management"
    required_input_fields = ['idea_data', 'management_config']
    required_output_fields = ['idea_pipeline', 'evaluation_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} idea management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} idea management: {data}"
