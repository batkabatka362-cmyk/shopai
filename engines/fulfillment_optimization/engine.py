"""
FulfillmentOptimization Engine
...
class FulfillmentOptimizationEngine(BaseEngine):
    engine_name = "fulfillment_optimization"
    required_input_fields = ['order_data', 'fulfillment_config']
    required_output_fields = ['fulfillment_plan', 'cost_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} fulfillment optimization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} fulfillment optimization: {data}"
