...
class PillarPageEngine(BaseEngine):
    engine_name = "pillar_page"
    required_input_fields = ['topic_data', 'pillar_config']
    required_output_fields = ['pillar_content', 'internal_link_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pillar page", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pillar page: {data}"
