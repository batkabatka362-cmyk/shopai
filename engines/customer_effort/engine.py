...
class CustomerEffortEngine(BaseEngine):
    engine_name = "customer_effort"
    required_input_fields = ['interaction_data', 'effort_config']
    required_output_fields = ['effort_scores', 'friction_points']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} customer effort", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} customer effort: {data}"
