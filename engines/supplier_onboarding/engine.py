...
class SupplierOnboardingEngine(BaseEngine):
    engine_name = "supplier_onboarding"
    required_input_fields = ['supplier_data', 'onboarding_config']
    required_output_fields = ['onboarding_checklist', 'compliance_verification']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} supplier onboarding", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} supplier onboarding: {data}"
