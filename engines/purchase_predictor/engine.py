...
class PurchasePredictorEngine(BaseEngine):
    engine_name = "purchase_predictor"
    required_input_fields = ['customer_data', 'product_data']
    required_output_fields = ['purchase_probability', 'timing_prediction']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} purchase predictor", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} purchase predictor: {data}"
