...
class ReportingObligationEngine(BaseEngine):
    engine_name = "reporting_obligation"
    required_input_fields = ['contract_data', 'reporting_config']
    required_output_fields = ['reporting_requirements', 'template_set']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} reporting obligation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} reporting obligation: {data}"
