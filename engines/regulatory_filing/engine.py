...
class RegulatoryFilingEngine(BaseEngine):
    engine_name = "regulatory_filing"
    required_input_fields = ['filing_data', 'regulation_config']
    required_output_fields = ['filing_document', 'submission_status']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} regulatory filing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} regulatory filing: {data}"
