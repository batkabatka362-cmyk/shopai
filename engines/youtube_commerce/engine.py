...
class YoutubeCommerceEngine(BaseEngine):
    engine_name = "youtube_commerce"
    required_input_fields = ['product_data', 'video_config']
    required_output_fields = ['product_shelves', 'merch_links']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} youtube commerce", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} youtube commerce: {data}"
