...
class CyberInsuranceEngine(BaseEngine):
    engine_name = "cyber_insurance"
    required_input_fields = ['risk_data', 'insurance_config']
    required_output_fields = ['coverage_plan', 'premium_estimate']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cyber insurance", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cyber insurance: {data}"
