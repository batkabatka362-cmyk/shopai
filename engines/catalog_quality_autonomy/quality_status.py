"""Catalog quality autonomy status surface (Wave 440)."""
...
@dataclass
class CatalogQualityStatusReport:
    window_hours: float
...
def get_catalog_quality_status(
    *,
    window_hours: float = 168.0,
    store_id: str | None = None,
) -> CatalogQualityStatusReport:
    report = CatalogQualityStatusReport(
    ...
    health = analyze_catalog_quality_health(
        window_hours=window_hours,
    )
    ...
    if report.paused:
        ...
        report.next_action = (
            "Resume via `shopai catalog-quality-resume`."
        )
    elif report.health_verdict == "critical":
        report.verdict = "degraded"
        report.verdict_reasons.append(
            f"quality failure ratio "
            f"{report.health_failure_ratio:.0%} >= critical"
        )
        report.next_action = (
            "`shopai catalog-quality-health --apply-bridge`."
        )
        ...
        report.verdict_reasons.append(
            "no catalog quality actions in window"
        )
        report.next_action = (
            "Enable via data.apply_catalog_quality=True."
        )
    else:
        report.verdict = "healthy"
        report.verdict_reasons.append(
            f"{report.applied_count} product(s) tagged"
        )
