...
class CrossChannelEngine(BaseEngine):
    engine_name = "cross_channel"
    required_input_fields = ['channel_data', 'measurement_config']
    required_output_fields = ['unified_report', 'channel_interaction']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cross channel", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cross channel: {data}"
