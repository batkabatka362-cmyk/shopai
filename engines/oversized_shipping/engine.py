...
class OversizedShippingEngine(BaseEngine):
    engine_name = "oversized_shipping"
    required_input_fields = ['product_data', 'shipping_config']
    required_output_fields = ['freight_options', 'cost_estimate']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} oversized shipping", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} oversized shipping: {data}"
