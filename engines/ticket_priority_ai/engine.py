...
class TicketPriorityAiEngine(BaseEngine):
    engine_name = "ticket_priority_ai"
    required_input_fields = ['ticket_data', 'priority_config']
    required_output_fields = ['priority_scores', 'escalation_flags']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ticket priority ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ticket priority ai: {data}"
