...
class AttentionAttributionEngine(BaseEngine):
    engine_name = "attention_attribution"
    required_input_fields = ['engagement_data', 'attention_config']
    required_output_fields = ['attention_weights', 'dwell_time_credit']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} attention attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} attention attribution: {data}"
