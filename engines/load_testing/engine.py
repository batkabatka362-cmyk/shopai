...
class LoadTestingEngine(BaseEngine):
    engine_name = "load_testing"
    required_input_fields = ['endpoint_data', 'load_config']
    required_output_fields = ['load_results', 'bottleneck_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} load testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} load testing: {data}"
