...
class SatisfactionPredictorEngine(BaseEngine):
    engine_name = "satisfaction_predictor"
    required_input_fields = ['interaction_data', 'prediction_config']
    required_output_fields = ['satisfaction_scores', 'intervention_triggers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} satisfaction predictor", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} satisfaction predictor: {data}"
