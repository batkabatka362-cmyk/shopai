...
class BlockchainTrackingEngine(BaseEngine):
    engine_name = "blockchain_tracking"
    required_input_fields = ['product_data', 'blockchain_config']
    required_output_fields = ['tracking_setup', 'verification_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} blockchain tracking", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} blockchain tracking: {data}"
