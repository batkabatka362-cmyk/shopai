...
class TechnologyTrendEngine(BaseEngine):
    engine_name = "technology_trend"
    required_input_fields = ['tech_data', 'trend_config']
    required_output_fields = ['tech_trends', 'adoption_timeline']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} technology trend", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} technology trend: {data}"
