...
class PromotionalPlanningEngine(BaseEngine):
    engine_name = "promotional_planning"
    required_input_fields = ['promo_data', 'planning_config']
    required_output_fields = ['promo_calendar', 'budget_allocation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} promotional planning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} promotional planning: {data}"
