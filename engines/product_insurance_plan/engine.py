...
class ProductInsurancePlanEngine(BaseEngine):
    engine_name = "product_insurance_plan"
    required_input_fields = ['product_data', 'insurance_config']
    required_output_fields = ['insurance_options', 'premium_calc']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} product insurance plan", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} product insurance plan: {data}"
