...
class WhiteGloveEngine(BaseEngine):
    engine_name = "white_glove"
    required_input_fields = ['order_data', 'service_config']
    required_output_fields = ['service_plan', 'premium_experience']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} white glove", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} white glove: {data}"
