"""Product Optimization Engine — memory reader.
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if not records:
        return {
            "avg_optimization_count": 0.0,
            "total_runs": 0,
        }

    counts = [r.get("optimization_count", 0) for r in records]
    n = len(records)
    return {
        "avg_optimization_count": round(sum(counts) / n, 1) if n else 0.0,
        "total_runs": n,
    }
