"""KPI Tracking Engine — memory reader.

Reads past KPI tracking records from memory storage.
Used to inform trend analysis with historical context.
"""
...
def read_past_kpis(
    ...
    """Read past KPI tracking records.
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past KPI records."""
    if not records:
        return {
            "avg_revenue": 0.0,
            "avg_aov": 0.0,
    ...
    revenues = [float(r.get("kpis", {}).get("revenue", 0)) for r in records]
    aovs = [float(r.get("kpis", {}).get("aov", 0)) for r in records]

    n = len(records)
    return {
        "avg_revenue": round(sum(revenues) / n, 2) if n else 0.0,
        "avg_aov": round(sum(aovs) / n, 2) if n else 0.0,
        "total_runs": n,
    }
