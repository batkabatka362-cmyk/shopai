...
class ExecutiveReportEngine(BaseEngine):
    engine_name = "executive_report"
    required_input_fields = ['business_data', 'report_config']
    required_output_fields = ['executive_summary', 'kpi_dashboard']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} executive report", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} executive report: {data}"
