...
class GraphAnalyticsEngine(BaseEngine):
    engine_name = "graph_analytics"
    required_input_fields = ['graph_data', 'analytics_config']
    required_output_fields = ['centrality_scores', 'path_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} graph analytics", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} graph analytics: {data}"
