...
class RunwayCalculatorEngine(BaseEngine):
    engine_name = "runway_calculator"
    required_input_fields = ['cash_data', 'expense_data']
    required_output_fields = ['runway_months', 'scenario_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} runway calculator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} runway calculator: {data}"
