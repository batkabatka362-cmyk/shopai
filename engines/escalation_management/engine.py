...
class EscalationManagementEngine(BaseEngine):
    engine_name = "escalation_management"
    required_input_fields = ['ticket_data', 'escalation_rules']
    required_output_fields = ['escalation_path', 'notification_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} escalation management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} escalation management: {data}"
