...
class InstagramShopEngine(BaseEngine):
    engine_name = "instagram_shop"
    required_input_fields = ['product_data', 'instagram_config']
    required_output_fields = ['shop_listings', 'tag_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} instagram shop", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} instagram shop: {data}"
