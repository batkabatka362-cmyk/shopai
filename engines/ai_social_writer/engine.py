...
class AiSocialWriterEngine(BaseEngine):
    engine_name = "ai_social_writer"
    required_input_fields = ['content_data', 'platform_config']
    required_output_fields = ['social_posts', 'hashtag_sets']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ai social writer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ai social writer: {data}"
