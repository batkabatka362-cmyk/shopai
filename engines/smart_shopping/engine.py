...
class SmartShoppingEngine(BaseEngine):
    engine_name = "smart_shopping"
    required_input_fields = ['feed_data', 'smart_config']
    required_output_fields = ['campaign_setup', 'automation_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} smart shopping", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} smart shopping: {data}"
