...
class DataFreshnessEngine(BaseEngine):
    engine_name = "data_freshness"
    required_input_fields = ['data_catalog', 'freshness_config']
    required_output_fields = ['freshness_report', 'stale_alerts']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} data freshness", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} data freshness: {data}"
