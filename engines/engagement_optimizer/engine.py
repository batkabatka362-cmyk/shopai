...
class EngagementOptimizerEngine(BaseEngine):
    engine_name = "engagement_optimizer"
    required_input_fields = ['engagement_data', 'content_data']
    required_output_fields = ['engagement_plan', 'metric_targets']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} engagement optimizer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} engagement optimizer: {data}"
