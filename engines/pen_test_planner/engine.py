...
class PenTestPlannerEngine(BaseEngine):
    engine_name = "pen_test_planner"
    required_input_fields = ['system_data', 'test_config']
    required_output_fields = ['test_plan', 'vulnerability_checklist']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pen test planner", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pen test planner: {data}"
