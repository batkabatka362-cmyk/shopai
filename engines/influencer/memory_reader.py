"""Influencer Engine — memory reader.

Reads past influencer campaign records from memory storage.
Used to inform current campaigns with historical context.
"""
...
def read_past_campaigns(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past campaign records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_campaigns(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past campaign records."""
    if not records:
        return {
            "avg_roi": 0.0,
            "avg_influencer_count": 0.0,
            "total_runs": 0,
        }

    rois = [r.get("estimated_roi", 0.0) for r in records]
    inf_counts = [r.get("influencer_count", 0) for r in records]

    n = len(records)
    return {
        "avg_roi": round(sum(rois) / n, 2) if n else 0.0,
        "avg_influencer_count": round(sum(inf_counts) / n, 1) if n else 0.0,
        "total_runs": n,
    }
