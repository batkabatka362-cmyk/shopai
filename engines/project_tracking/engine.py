...
class ProjectTrackingEngine(BaseEngine):
    engine_name = "project_tracking"
    required_input_fields = ['project_data', 'tracking_config']
    required_output_fields = ['status_report', 'milestone_tracking']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} project tracking", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} project tracking: {data}"
