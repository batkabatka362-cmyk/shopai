...
class RevenueAttributionEngine(BaseEngine):
    engine_name = "revenue_attribution"
    required_input_fields = ['revenue_data', 'attribution_config']
    required_output_fields = ['revenue_by_channel', 'touchpoint_value']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} revenue attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} revenue attribution: {data}"
