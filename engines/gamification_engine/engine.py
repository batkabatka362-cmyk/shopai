...
class GamificationEngineEngine(BaseEngine):
    engine_name = "gamification_engine"
    required_input_fields = ['performance_data', 'gamification_config']
    required_output_fields = ['game_mechanics', 'leaderboard_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} gamification engine", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} gamification engine: {data}"
