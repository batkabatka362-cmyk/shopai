...
class DigitalDeliveryEngine(BaseEngine):
    engine_name = "digital_delivery"
    required_input_fields = ['product_data', 'delivery_config']
    required_output_fields = ['download_links', 'access_tokens']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} digital delivery", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} digital delivery: {data}"
