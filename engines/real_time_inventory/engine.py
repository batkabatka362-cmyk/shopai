...
class RealTimeInventoryEngine(BaseEngine):
    engine_name = "real_time_inventory"
    required_input_fields = ['sales_data', 'stock_data']
    required_output_fields = ['live_stock', 'alert_triggers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} real time inventory", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} real time inventory: {data}"
