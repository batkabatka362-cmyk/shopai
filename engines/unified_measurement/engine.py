...
class UnifiedMeasurementEngine(BaseEngine):
    engine_name = "unified_measurement"
    required_input_fields = ['measurement_data', 'config']
    required_output_fields = ['measurement_framework', 'reporting_dashboard']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} unified measurement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} unified measurement: {data}"
