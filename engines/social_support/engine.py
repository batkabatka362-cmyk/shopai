...
class SocialSupportEngine(BaseEngine):
    engine_name = "social_support"
    required_input_fields = ['social_data', 'support_config']
    required_output_fields = ['response_draft', 'public_private_decision']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} social support", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} social support: {data}"
