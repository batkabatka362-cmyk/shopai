...
class NotificationCenterEngine(BaseEngine):
    engine_name = "notification_center"
    required_input_fields = ['notification_data', 'center_config']
    required_output_fields = ['notification_feed', 'preference_settings']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} notification center", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} notification center: {data}"
