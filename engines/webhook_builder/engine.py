...
class WebhookBuilderEngine(BaseEngine):
    engine_name = "webhook_builder"
    required_input_fields = ['event_data', 'webhook_config']
    required_output_fields = ['webhook_setup', 'payload_schema']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} webhook builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} webhook builder: {data}"
