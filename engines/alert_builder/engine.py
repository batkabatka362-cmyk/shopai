...
class AlertBuilderEngine(BaseEngine):
    engine_name = "alert_builder"
    required_input_fields = ['metric_data', 'alert_config']
    required_output_fields = ['alert_rules', 'notification_channels']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} alert builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} alert builder: {data}"
