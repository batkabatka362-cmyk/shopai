...
class DsaCampaignEngine(BaseEngine):
    engine_name = "dsa_campaign"
    required_input_fields = ['site_data', 'campaign_config']
    required_output_fields = ['campaign_setup', 'page_feed']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dsa campaign", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dsa campaign: {data}"
