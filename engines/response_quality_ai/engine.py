...
class ResponseQualityAiEngine(BaseEngine):
    engine_name = "response_quality_ai"
    required_input_fields = ['response_data', 'quality_config']
    required_output_fields = ['quality_scores', 'improvement_actions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} response quality ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} response quality ai: {data}"
