"""
UserGeneratedContentRights Engine
...
class UserGeneratedContentRightsEngine(BaseEngine):
    engine_name = "user_generated_content_rights"
    required_input_fields = ['ugc_data', 'rights_config']
    required_output_fields = ['rights_terms', 'attribution_requirements']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} user generated content rights", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} user generated content rights: {data}"
