...
class LookbookEngine(BaseEngine):
    engine_name = "lookbook"
    required_input_fields = ['product_data', 'lookbook_config']
    required_output_fields = ['lookbook_layout', 'product_tags']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} lookbook", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} lookbook: {data}"
