...
class SecurityTestingEngine(BaseEngine):
    engine_name = "security_testing"
    required_input_fields = ['app_data', 'security_config']
    required_output_fields = ['vulnerability_report', 'risk_assessment']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} security testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} security testing: {data}"
