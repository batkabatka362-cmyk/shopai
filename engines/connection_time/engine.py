...
class ConnectionTimeEngine(BaseEngine):
    engine_name = "connection_time"
    required_input_fields = ['network_data', 'analysis_config']
    required_output_fields = ['connection_metrics', 'dns_optimization']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} connection time", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} connection time: {data}"
