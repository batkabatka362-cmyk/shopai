...
class ErgonomicsEngine(BaseEngine):
    engine_name = "ergonomics"
    required_input_fields = ['workspace_data', 'ergonomic_config']
    required_output_fields = ['assessment_report', 'improvement_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ergonomics", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ergonomics: {data}"
