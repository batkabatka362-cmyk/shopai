"""
TransferPricingCompliance Engine
...
class TransferPricingComplianceEngine(BaseEngine):
    engine_name = "transfer_pricing_compliance"
    required_input_fields = ['ic_data', 'compliance_config']
    required_output_fields = ['tp_documentation', 'benchmarking_study']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} transfer pricing compliance", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} transfer pricing compliance: {data}"
