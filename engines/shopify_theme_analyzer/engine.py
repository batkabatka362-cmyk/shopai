...
class ShopifyThemeAnalyzerEngine(BaseEngine):
    engine_name = "shopify_theme_analyzer"
    required_input_fields = ['theme_data', 'analysis_config']
    required_output_fields = ['theme_report', 'optimization_actions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shopify theme analyzer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shopify theme analyzer: {data}"
