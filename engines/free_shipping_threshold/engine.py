...
class FreeShippingThresholdEngine(BaseEngine):
    engine_name = "free_shipping_threshold"
    required_input_fields = ['cart_data', 'threshold_config']
    required_output_fields = ['threshold_setup', 'progress_display']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} free shipping threshold", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} free shipping threshold: {data}"
