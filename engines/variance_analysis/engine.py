...
class VarianceAnalysisEngine(BaseEngine):
    engine_name = "variance_analysis"
    required_input_fields = ['actual_data', 'budget_data']
    required_output_fields = ['variance_report', 'root_causes']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} variance analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} variance analysis: {data}"
