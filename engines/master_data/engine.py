...
class MasterDataEngine(BaseEngine):
    engine_name = "master_data"
    required_input_fields = ['data_sources', 'mdm_config']
    required_output_fields = ['golden_record', 'data_quality_score']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} master data", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} master data: {data}"
