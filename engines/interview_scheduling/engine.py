...
class InterviewSchedulingEngine(BaseEngine):
    engine_name = "interview_scheduling"
    required_input_fields = ['candidate_data', 'scheduling_config']
    required_output_fields = ['interview_slots', 'calendar_sync']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} interview scheduling", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} interview scheduling: {data}"
