...
class SmartAnalyticsEngineEngine(BaseEngine):
    engine_name = "smart_analytics_engine"
    required_input_fields = ['analytics_data', 'goal_data']
    required_output_fields = ['analytics_actions', 'insight_feed']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} smart analytics engine", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} smart analytics engine: {data}"
