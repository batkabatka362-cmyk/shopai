...
class MultiAgentCoordinationEngine(BaseEngine):
    engine_name = "multi_agent_coordination"
    required_input_fields = ['agent_data', 'coordination_config']
    required_output_fields = ['coordination_plan', 'message_routing']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} multi agent coordination", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} multi agent coordination: {data}"
