...
class AvailableToPromiseEngine(BaseEngine):
    engine_name = "available_to_promise"
    required_input_fields = ['inventory_data', 'demand_data']
    required_output_fields = ['atp_quantities', 'promise_dates']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} available to promise", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} available to promise: {data}"
