...
class SemanticSearchEngine(BaseEngine):
    engine_name = "semantic_search"
    required_input_fields = ['query_data', 'semantic_config']
    required_output_fields = ['semantic_results', 'intent_matching']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} semantic search", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} semantic search: {data}"
