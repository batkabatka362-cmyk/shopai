...
class GeoBidEngine(BaseEngine):
    engine_name = "geo_bid"
    required_input_fields = ['geo_data', 'bid_config']
    required_output_fields = ['geo_adjustments', 'location_performance']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} geo bid", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} geo bid: {data}"
