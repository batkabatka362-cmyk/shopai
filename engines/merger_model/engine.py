...
class MergerModelEngine(BaseEngine):
    engine_name = "merger_model"
    required_input_fields = ['acquirer_data', 'target_data']
    required_output_fields = ['accretion_dilution', 'synergy_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} merger model", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} merger model: {data}"
