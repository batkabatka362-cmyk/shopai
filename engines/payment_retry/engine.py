...
class PaymentRetryEngine(BaseEngine):
    engine_name = "payment_retry"
    required_input_fields = ['failed_payment', 'retry_config']
    required_output_fields = ['retry_strategy', 'recovery_rate']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} payment retry", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} payment retry: {data}"
