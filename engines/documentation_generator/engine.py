...
class DocumentationGeneratorEngine(BaseEngine):
    engine_name = "documentation_generator"
    required_input_fields = ['code_data', 'doc_config']
    required_output_fields = ['documentation', 'api_reference']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} documentation generator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} documentation generator: {data}"
