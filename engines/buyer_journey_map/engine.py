...
class BuyerJourneyMapEngine(BaseEngine):
    engine_name = "buyer_journey_map"
    required_input_fields = ['touchpoint_data', 'conversion_data']
    required_output_fields = ['journey_stages', 'content_needs']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} buyer journey map", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} buyer journey map: {data}"
