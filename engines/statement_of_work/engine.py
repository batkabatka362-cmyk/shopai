...
class StatementOfWorkEngine(BaseEngine):
    engine_name = "statement_of_work"
    required_input_fields = ['project_data', 'sow_config']
    required_output_fields = ['sow_draft', 'milestone_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} statement of work", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} statement of work: {data}"
