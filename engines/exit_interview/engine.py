...
class ExitInterviewEngine(BaseEngine):
    engine_name = "exit_interview"
    required_input_fields = ['interview_data', 'analysis_config']
    required_output_fields = ['interview_insights', 'trend_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} exit interview", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} exit interview: {data}"
