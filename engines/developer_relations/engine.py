...
class DeveloperRelationsEngine(BaseEngine):
    engine_name = "developer_relations"
    required_input_fields = ['developer_data', 'devrel_config']
    required_output_fields = ['devrel_plan', 'community_metrics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} developer relations", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} developer relations: {data}"
