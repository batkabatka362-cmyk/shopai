...
class NextBestActionEngine(BaseEngine):
    engine_name = "next_best_action"
    required_input_fields = ['customer_data', 'action_config']
    required_output_fields = ['recommended_actions', 'expected_value']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} next best action", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} next best action: {data}"
