...
class SearchRankingAiEngine(BaseEngine):
    engine_name = "search_ranking_ai"
    required_input_fields = ['search_data', 'ranking_config']
    required_output_fields = ['ranked_results', 'ranking_factors']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} search ranking ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} search ranking ai: {data}"
