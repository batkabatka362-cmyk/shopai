...
class ApiTermsEngine(BaseEngine):
    engine_name = "api_terms"
    required_input_fields = ['api_data', 'terms_config']
    required_output_fields = ['terms_draft', 'rate_limits']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} api terms", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} api terms: {data}"
