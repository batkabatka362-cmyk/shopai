...
class ShoppableVideoEngine(BaseEngine):
    engine_name = "shoppable_video"
    required_input_fields = ['video_data', 'shopping_config']
    required_output_fields = ['video_setup', 'product_hotspots']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shoppable video", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shoppable video: {data}"
