...
class RefundPolicyGeneratorEngine(BaseEngine):
    engine_name = "refund_policy_generator"
    required_input_fields = ['business_data', 'policy_config']
    required_output_fields = ['refund_policy', 'exception_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} refund policy generator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} refund policy generator: {data}"
