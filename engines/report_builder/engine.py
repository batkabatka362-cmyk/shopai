...
class ReportBuilderEngine(BaseEngine):
    engine_name = "report_builder"
    required_input_fields = ['data_source', 'report_config']
    required_output_fields = ['report_template', 'visualization_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} report builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} report builder: {data}"
