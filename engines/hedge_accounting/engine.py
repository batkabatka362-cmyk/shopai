...
class HedgeAccountingEngine(BaseEngine):
    engine_name = "hedge_accounting"
    required_input_fields = ['hedge_data', 'accounting_config']
    required_output_fields = ['hedge_effectiveness', 'journal_entries']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} hedge accounting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} hedge accounting: {data}"
