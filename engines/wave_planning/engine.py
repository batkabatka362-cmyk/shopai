...
class WavePlanningEngine(BaseEngine):
    engine_name = "wave_planning"
    required_input_fields = ['order_data', 'wave_config']
    required_output_fields = ['wave_plan', 'resource_allocation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} wave planning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} wave planning: {data}"
