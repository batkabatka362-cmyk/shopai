...
class OrderEditEngine(BaseEngine):
    engine_name = "order_edit"
    required_input_fields = ['order_data', 'edit_config']
    required_output_fields = ['edit_options', 'recalculation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} order edit", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} order edit: {data}"
