...
class ContinuousFeedbackEngine(BaseEngine):
    engine_name = "continuous_feedback"
    required_input_fields = ['feedback_data', 'feedback_config']
    required_output_fields = ['feedback_summary', 'action_items']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} continuous feedback", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} continuous feedback: {data}"
