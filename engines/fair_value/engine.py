...
class FairValueEngine(BaseEngine):
    engine_name = "fair_value"
    required_input_fields = ['asset_data', 'valuation_config']
    required_output_fields = ['fair_value_estimate', 'hierarchy_level']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} fair value", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} fair value: {data}"
