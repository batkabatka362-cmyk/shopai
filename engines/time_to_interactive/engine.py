...
class TimeToInteractiveEngine(BaseEngine):
    engine_name = "time_to_interactive"
    required_input_fields = ['page_data', 'analysis_config']
    required_output_fields = ['tti_score', 'blocking_resources']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} time to interactive", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} time to interactive: {data}"
