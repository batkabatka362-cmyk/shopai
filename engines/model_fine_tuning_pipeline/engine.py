"""
ModelFineTuningPipeline Engine
...
class ModelFineTuningPipelineEngine(BaseEngine):
    engine_name = "model_fine_tuning_pipeline"
    required_input_fields = ['training_data', 'tuning_config']
    required_output_fields = ['tuned_model', 'training_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} model fine tuning pipeline", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} model fine tuning pipeline: {data}"
