...
class EconomicIndicatorEngine(BaseEngine):
    engine_name = "economic_indicator"
    required_input_fields = ['economic_data', 'indicator_config']
    required_output_fields = ['indicator_report', 'impact_assessment']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} economic indicator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} economic indicator: {data}"
