...
class CounterfeitDetectionEngine(BaseEngine):
    engine_name = "counterfeit_detection"
    required_input_fields = ['product_data', 'verification_config']
    required_output_fields = ['authenticity_score', 'risk_flags']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} counterfeit detection", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} counterfeit detection: {data}"
