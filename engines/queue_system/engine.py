...
class QueueSystemEngine(BaseEngine):
    engine_name = "queue_system"
    required_input_fields = ['service_data', 'queue_config']
    required_output_fields = ['queue_flow', 'wait_time_display']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} queue system", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} queue system: {data}"
