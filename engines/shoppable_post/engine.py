...
class ShoppablePostEngine(BaseEngine):
    engine_name = "shoppable_post"
    required_input_fields = ['post_data', 'shopping_config']
    required_output_fields = ['post_setup', 'product_tags']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shoppable post", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shoppable post: {data}"
