...
class EngravingEngine(BaseEngine):
    engine_name = "engraving"
    required_input_fields = ['product_data', 'engraving_config']
    required_output_fields = ['engraving_setup', 'preview_generator']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} engraving", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} engraving: {data}"
