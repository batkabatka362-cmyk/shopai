...
class FacebookPixelEngine(BaseEngine):
    engine_name = "facebook_pixel"
    required_input_fields = ['pixel_data', 'integration_config']
    required_output_fields = ['pixel_setup', 'event_mapping']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} facebook pixel", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} facebook pixel: {data}"
