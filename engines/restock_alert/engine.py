...
class RestockAlertEngine(BaseEngine):
    engine_name = "restock_alert"
    required_input_fields = ['inventory_changes', 'subscriber_data']
    required_output_fields = ['restock_notifications', 'demand_capture']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} restock alert", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} restock alert: {data}"
