...
class LegalAssistanceEngine(BaseEngine):
    engine_name = "legal_assistance"
    required_input_fields = ['employee_data', 'legal_config']
    required_output_fields = ['legal_plan', 'provider_access']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} legal assistance", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} legal assistance: {data}"
