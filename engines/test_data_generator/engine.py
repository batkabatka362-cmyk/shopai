...
class TestDataGeneratorEngine(BaseEngine):
    engine_name = "test_data_generator"
    required_input_fields = ['schema_data', 'generation_config']
    required_output_fields = ['test_data', 'coverage_matrix']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} test data generator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} test data generator: {data}"
