...
class DeliveryPromiseEngine(BaseEngine):
    engine_name = "delivery_promise"
    required_input_fields = ['order_data', 'logistics_config']
    required_output_fields = ['promise_date', 'confidence_level']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} delivery promise", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} delivery promise: {data}"
