...
class BiodiversityImpactEngine(BaseEngine):
    engine_name = "biodiversity_impact"
    required_input_fields = ['operations_data', 'biodiversity_config']
    required_output_fields = ['impact_assessment', 'mitigation_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} biodiversity impact", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} biodiversity impact: {data}"
