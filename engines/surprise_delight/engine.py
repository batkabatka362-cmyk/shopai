...
class SurpriseDelightEngine(BaseEngine):
    engine_name = "surprise_delight"
    required_input_fields = ['customer_data', 'surprise_config']
    required_output_fields = ['surprise_selection', 'delivery_timing']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} surprise delight", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} surprise delight: {data}"
