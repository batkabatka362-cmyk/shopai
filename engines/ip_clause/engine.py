...
class IpClauseEngine(BaseEngine):
    engine_name = "ip_clause"
    required_input_fields = ['ip_data', 'clause_config']
    required_output_fields = ['ip_provisions', 'ownership_terms']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ip clause", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ip clause: {data}"
