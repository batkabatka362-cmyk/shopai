...
class ResourceTimingEngine(BaseEngine):
    engine_name = "resource_timing"
    required_input_fields = ['resource_data', 'analysis_config']
    required_output_fields = ['resource_waterfall', 'optimization_targets']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} resource timing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} resource timing: {data}"
