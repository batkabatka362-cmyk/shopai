"""Supplier Communication Engine — memory reader.

Reads past supplier communication records from memory storage.
Used to inform current supplier interactions with historical context.
"""
...
def read_past_communications(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past supplier communication records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_communications(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past supplier communication records."""
    if not records:
        return {
            "avg_po_count": 0,
            "avg_po_total": 0.0,
    ...
    po_counts = [r.get("po_count", 0) for r in records]
    po_totals = [r.get("total_po_value", 0.0) for r in records]
    msg_counts = [r.get("messages_count", 0) for r in records]

    n = len(records)
    return {
        "avg_po_count": round(sum(po_counts) / n) if n else 0,
        "avg_po_total": round(sum(po_totals) / n, 2) if n else 0.0,
        "avg_messages_count": round(sum(msg_counts) / n) if n else 0,
        "total_runs": n,
    }
