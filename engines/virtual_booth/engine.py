...
class VirtualBoothEngine(BaseEngine):
    engine_name = "virtual_booth"
    required_input_fields = ['exhibitor_data', 'booth_config']
    required_output_fields = ['booth_setup', 'interaction_tools']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} virtual booth", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} virtual booth: {data}"
