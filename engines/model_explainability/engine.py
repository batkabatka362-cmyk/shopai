...
class ModelExplainabilityEngine(BaseEngine):
    engine_name = "model_explainability"
    required_input_fields = ['model_data', 'explain_config']
    required_output_fields = ['explanations', 'feature_importance']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} model explainability", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} model explainability: {data}"
