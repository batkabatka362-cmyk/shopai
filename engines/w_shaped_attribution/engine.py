...
class WShapedAttributionEngine(BaseEngine):
    engine_name = "w_shaped_attribution"
    required_input_fields = ['touchpoint_data', 'w_config']
    required_output_fields = ['w_weights', 'key_touchpoints']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} w shaped attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} w shaped attribution: {data}"
