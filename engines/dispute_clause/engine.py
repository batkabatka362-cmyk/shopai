...
class DisputeClauseEngine(BaseEngine):
    engine_name = "dispute_clause"
    required_input_fields = ['contract_data', 'dispute_config']
    required_output_fields = ['dispute_mechanism', 'escalation_ladder']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dispute clause", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dispute clause: {data}"
