...
class GuardrailsEngine(BaseEngine):
    engine_name = "guardrails"
    required_input_fields = ['input_data', 'guardrail_config']
    required_output_fields = ['validation_result', 'blocked_content']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} guardrails", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} guardrails: {data}"
