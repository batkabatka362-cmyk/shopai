...
class InfiniteScrollEngine(BaseEngine):
    engine_name = "infinite_scroll"
    required_input_fields = ['content_data', 'scroll_config']
    required_output_fields = ['scroll_strategy', 'preload_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} infinite scroll", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} infinite scroll: {data}"
