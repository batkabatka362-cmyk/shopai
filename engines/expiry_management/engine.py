...
class ExpiryManagementEngine(BaseEngine):
    engine_name = "expiry_management"
    required_input_fields = ['product_data', 'expiry_config']
    required_output_fields = ['expiry_alerts', 'rotation_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} expiry management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} expiry management: {data}"
