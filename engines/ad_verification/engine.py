...
class AdVerificationEngine(BaseEngine):
    engine_name = "ad_verification"
    required_input_fields = ['ad_data', 'verification_config']
    required_output_fields = ['verification_report', 'fraud_detection']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ad verification", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ad verification: {data}"
