...
class JointVentureAgreementEngine(BaseEngine):
    engine_name = "joint_venture_agreement"
    required_input_fields = ['jv_data', 'agreement_config']
    required_output_fields = ['jv_agreement', 'governance_structure']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} joint venture agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} joint venture agreement: {data}"
