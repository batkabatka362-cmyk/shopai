...
class PriceSensitivityEngine(BaseEngine):
    engine_name = "price_sensitivity"
    required_input_fields = ['survey_data', 'pricing_data']
    required_output_fields = ['sensitivity_curve', 'optimal_range']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} price sensitivity", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} price sensitivity: {data}"
