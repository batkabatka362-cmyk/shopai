...
class DarkModeEngine(BaseEngine):
    engine_name = "dark_mode"
    required_input_fields = ['theme_data', 'dark_config']
    required_output_fields = ['dark_theme', 'color_mapping']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dark mode", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dark mode: {data}"
