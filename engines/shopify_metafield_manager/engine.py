"""
ShopifyMetafieldManager Engine
...
class ShopifyMetafieldManagerEngine(BaseEngine):
    engine_name = "shopify_metafield_manager"
    required_input_fields = ['metafield_data', 'management_config']
    required_output_fields = ['metafield_plan', 'seo_impact']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shopify metafield manager", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shopify metafield manager: {data}"
