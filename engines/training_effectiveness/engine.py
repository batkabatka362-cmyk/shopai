...
class TrainingEffectivenessEngine(BaseEngine):
    engine_name = "training_effectiveness"
    required_input_fields = ['training_data', 'effectiveness_config']
    required_output_fields = ['effectiveness_report', 'roi_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} training effectiveness", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} training effectiveness: {data}"
