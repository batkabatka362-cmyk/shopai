...
class RecognitionProgramEngine(BaseEngine):
    engine_name = "recognition_program"
    required_input_fields = ['employee_data', 'program_config']
    required_output_fields = ['recognition_setup', 'reward_options']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} recognition program", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} recognition program: {data}"
