...
class GeneticAlgorithmEngine(BaseEngine):
    engine_name = "genetic_algorithm"
    required_input_fields = ['population_data', 'ga_config']
    required_output_fields = ['best_solution', 'evolution_history']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} genetic algorithm", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} genetic algorithm: {data}"
