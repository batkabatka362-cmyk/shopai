...
class WorkloadBalancingEngine(BaseEngine):
    engine_name = "workload_balancing"
    required_input_fields = ['agent_data', 'balancing_config']
    required_output_fields = ['assignment_plan', 'utilization_forecast']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} workload balancing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} workload balancing: {data}"
