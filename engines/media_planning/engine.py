...
class MediaPlanningEngine(BaseEngine):
    engine_name = "media_planning"
    required_input_fields = ['campaign_goals', 'media_options']
    required_output_fields = ['media_plan', 'budget_allocation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} media planning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} media planning: {data}"
