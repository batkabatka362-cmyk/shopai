...
class FineTuningEngine(BaseEngine):
    engine_name = "fine_tuning"
    required_input_fields = ['training_data', 'tuning_config']
    required_output_fields = ['tuned_model', 'performance_comparison']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} fine tuning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} fine tuning: {data}"
