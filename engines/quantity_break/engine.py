...
class QuantityBreakEngine(BaseEngine):
    engine_name = "quantity_break"
    required_input_fields = ['product_data', 'break_config']
    required_output_fields = ['break_points', 'pricing_table']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} quantity break", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} quantity break: {data}"
