...
class SocialProofAiEngine(BaseEngine):
    engine_name = "social_proof_ai"
    required_input_fields = ['review_data', 'product_data']
    required_output_fields = ['social_proof_elements', 'trust_score']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} social proof ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} social proof ai: {data}"
