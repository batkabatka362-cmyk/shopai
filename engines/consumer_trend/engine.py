...
class ConsumerTrendEngine(BaseEngine):
    engine_name = "consumer_trend"
    required_input_fields = ['consumer_data', 'trend_config']
    required_output_fields = ['trend_report', 'opportunity_map']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} consumer trend", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} consumer trend: {data}"
