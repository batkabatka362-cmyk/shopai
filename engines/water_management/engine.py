...
class WaterManagementEngine(BaseEngine):
    engine_name = "water_management"
    required_input_fields = ['water_data', 'conservation_config']
    required_output_fields = ['water_report', 'conservation_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} water management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} water management: {data}"
