...
class ToolSelectionEngine(BaseEngine):
    engine_name = "tool_selection"
    required_input_fields = ['task_data', 'tool_inventory']
    required_output_fields = ['selected_tools', 'execution_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} tool selection", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} tool selection: {data}"
