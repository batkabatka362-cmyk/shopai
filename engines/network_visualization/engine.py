...
class NetworkVisualizationEngine(BaseEngine):
    engine_name = "network_visualization"
    required_input_fields = ['network_data', 'viz_config']
    required_output_fields = ['visualization_spec', 'layout_algorithm']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} network visualization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} network visualization: {data}"
