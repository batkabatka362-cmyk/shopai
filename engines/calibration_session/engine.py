...
class CalibrationSessionEngine(BaseEngine):
    engine_name = "calibration_session"
    required_input_fields = ['rating_data', 'calibration_config']
    required_output_fields = ['calibrated_ratings', 'distribution_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} calibration session", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} calibration session: {data}"
