...
class OptionPricingEngine(BaseEngine):
    engine_name = "option_pricing"
    required_input_fields = ['option_data', 'pricing_config']
    required_output_fields = ['option_value', 'greeks']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} option pricing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} option pricing: {data}"
