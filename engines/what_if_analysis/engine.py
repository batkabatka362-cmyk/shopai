...
class WhatIfAnalysisEngine(BaseEngine):
    engine_name = "what_if_analysis"
    required_input_fields = ['base_data', 'scenario_config']
    required_output_fields = ['scenario_results', 'comparison_matrix']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} what if analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} what if analysis: {data}"
