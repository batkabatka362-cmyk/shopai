...
class LowCodePlatformEngine(BaseEngine):
    engine_name = "low_code_platform"
    required_input_fields = ['app_data', 'platform_config']
    required_output_fields = ['platform_setup', 'component_library']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} low code platform", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} low code platform: {data}"
