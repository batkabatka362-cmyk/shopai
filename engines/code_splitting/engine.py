...
class CodeSplittingEngine(BaseEngine):
    engine_name = "code_splitting"
    required_input_fields = ['route_data', 'splitting_config']
    required_output_fields = ['split_plan', 'chunk_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} code splitting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} code splitting: {data}"
