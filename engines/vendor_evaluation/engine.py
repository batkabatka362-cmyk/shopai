...
class VendorEvaluationEngine(BaseEngine):
    engine_name = "vendor_evaluation"
    required_input_fields = ['vendor_data', 'evaluation_criteria']
    required_output_fields = ['vendor_scores', 'recommendation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} vendor evaluation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} vendor evaluation: {data}"
