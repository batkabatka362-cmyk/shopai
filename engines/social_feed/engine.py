...
class SocialFeedEngine(BaseEngine):
    engine_name = "social_feed"
    required_input_fields = ['social_data', 'feed_config']
    required_output_fields = ['feed_layout', 'refresh_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} social feed", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} social feed: {data}"
