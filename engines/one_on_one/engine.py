...
class OneOnOneEngine(BaseEngine):
    engine_name = "one_on_one"
    required_input_fields = ['meeting_data', 'one_on_one_config']
    required_output_fields = ['meeting_agenda', 'talking_points']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} one on one", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} one on one: {data}"
