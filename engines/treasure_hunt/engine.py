...
class TreasureHuntEngine(BaseEngine):
    engine_name = "treasure_hunt"
    required_input_fields = ['hunt_data', 'hunt_config']
    required_output_fields = ['hunt_setup', 'clue_sequence']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} treasure hunt", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} treasure hunt: {data}"
