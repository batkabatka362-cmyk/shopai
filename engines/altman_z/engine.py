...
class AltmanZEngine(BaseEngine):
    engine_name = "altman_z"
    required_input_fields = ['financial_data', 'z_config']
    required_output_fields = ['z_score', 'distress_probability']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} altman z", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} altman z: {data}"
