...
class MultiStepCheckoutEngine(BaseEngine):
    engine_name = "multi_step_checkout"
    required_input_fields = ['checkout_data', 'step_config']
    required_output_fields = ['step_flow', 'progress_indicator']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} multi step checkout", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} multi step checkout: {data}"
