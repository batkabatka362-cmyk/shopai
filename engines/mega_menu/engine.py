...
class MegaMenuEngine(BaseEngine):
    engine_name = "mega_menu"
    required_input_fields = ['navigation_data', 'menu_config']
    required_output_fields = ['menu_structure', 'display_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} mega menu", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} mega menu: {data}"
