...
class SmartReturnPredictorEngine(BaseEngine):
    engine_name = "smart_return_predictor"
    required_input_fields = ['order_data', 'product_data']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} smart return predictor", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} smart return predictor: {data}"
