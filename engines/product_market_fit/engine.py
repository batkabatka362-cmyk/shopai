...
class ProductMarketFitEngine(BaseEngine):
    engine_name = "product_market_fit"
    required_input_fields = ['product_data', 'market_data']
    required_output_fields = ['pmf_score', 'pivot_recommendations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} product market fit", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} product market fit: {data}"
