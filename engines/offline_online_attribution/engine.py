"""
OfflineOnlineAttribution Engine
...
class OfflineOnlineAttributionEngine(BaseEngine):
    engine_name = "offline_online_attribution"
    required_input_fields = ['touchpoint_data', 'oo_config']
    required_output_fields = ['blended_attribution', 'channel_bridge']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} offline online attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} offline online attribution: {data}"
