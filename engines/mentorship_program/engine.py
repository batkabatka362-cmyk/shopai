...
class MentorshipProgramEngine(BaseEngine):
    engine_name = "mentorship_program"
    required_input_fields = ['participant_data', 'program_config']
    required_output_fields = ['mentor_matching', 'progress_tracking']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} mentorship program", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} mentorship program: {data}"
