...
class BogoEngine(BaseEngine):
    engine_name = "bogo"
    required_input_fields = ['product_data', 'bogo_config']
    required_output_fields = ['bogo_setup', 'stacking_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} bogo", required=req, stop_on_reject=req))
