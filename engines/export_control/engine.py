...
class ExportControlEngine(BaseEngine):
    engine_name = "export_control"
    required_input_fields = ['product_data', 'export_config']
    required_output_fields = ['export_classification', 'license_requirements']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} export control", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} export control: {data}"
