...
class InvestorReportAiEngine(BaseEngine):
    engine_name = "investor_report_ai"
    required_input_fields = ['financial_data', 'report_config']
    required_output_fields = ['investor_report', 'key_metrics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} investor report ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} investor report ai: {data}"
