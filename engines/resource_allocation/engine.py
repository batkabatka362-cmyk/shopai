...
class ResourceAllocationEngine(BaseEngine):
    engine_name = "resource_allocation"
    required_input_fields = ['resource_data', 'project_needs']
    required_output_fields = ['allocation_plan', 'utilization_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} resource allocation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} resource allocation: {data}"
