...
class PersonalShopperEngine(BaseEngine):
    engine_name = "personal_shopper"
    required_input_fields = ['customer_data', 'shopper_config']
    required_output_fields = ['curated_selection', 'shopping_experience']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} personal shopper", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} personal shopper: {data}"
