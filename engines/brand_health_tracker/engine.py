...
class BrandHealthTrackerEngine(BaseEngine):
    engine_name = "brand_health_tracker"
    required_input_fields = ['brand_data', 'mention_data']
    required_output_fields = ['brand_score', 'health_trend']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} brand health tracker", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} brand health tracker: {data}"
