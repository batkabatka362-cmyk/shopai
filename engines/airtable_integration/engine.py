...
class AirtableIntegrationEngine(BaseEngine):
    engine_name = "airtable_integration"
    required_input_fields = ['base_data', 'integration_config']
    required_output_fields = ['airtable_sync', 'field_mapping']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} airtable integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} airtable integration: {data}"
