...
class ChallengeSystemEngine(BaseEngine):
    engine_name = "challenge_system"
    required_input_fields = ['challenge_data', 'participation_data']
    required_output_fields = ['active_challenges', 'progress_tracking']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} challenge system", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} challenge system: {data}"
