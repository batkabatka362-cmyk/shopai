...
class SelfHealingSystemEngine(BaseEngine):
    engine_name = "self_healing_system"
    required_input_fields = ['error_data', 'system_config']
    required_output_fields = ['healing_plan', 'recovery_actions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} self healing system", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} self healing system: {data}"
