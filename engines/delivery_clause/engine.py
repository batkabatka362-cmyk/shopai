...
class DeliveryClauseEngine(BaseEngine):
    engine_name = "delivery_clause"
    required_input_fields = ['logistics_data', 'clause_config']
    required_output_fields = ['delivery_terms', 'risk_transfer']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} delivery clause", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} delivery clause: {data}"
