...
class SavingsTrackerEngine(BaseEngine):
    engine_name = "savings_tracker"
    required_input_fields = ['savings_data', 'tracker_config']
    required_output_fields = ['savings_display', 'goal_progress']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} savings tracker", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} savings tracker: {data}"
