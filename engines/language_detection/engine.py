...
class LanguageDetectionEngine(BaseEngine):
    engine_name = "language_detection"
    required_input_fields = ['text_data', 'detection_config']
    required_output_fields = ['detected_languages', 'confidence_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} language detection", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} language detection: {data}"
