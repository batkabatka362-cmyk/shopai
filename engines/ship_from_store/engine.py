...
class ShipFromStoreEngine(BaseEngine):
    engine_name = "ship_from_store"
    required_input_fields = ['order_data', 'store_inventory']
    required_output_fields = ['store_selection', 'fulfillment_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ship from store", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ship from store: {data}"
