...
class CapacityPlanningEngine(BaseEngine):
    engine_name = "capacity_planning"
    required_input_fields = ['usage_data', 'growth_forecast']
    required_output_fields = ['capacity_plan', 'scaling_timeline']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} capacity planning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} capacity planning: {data}"
