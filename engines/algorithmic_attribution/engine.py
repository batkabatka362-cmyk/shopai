...
class AlgorithmicAttributionEngine(BaseEngine):
    engine_name = "algorithmic_attribution"
    required_input_fields = ['touchpoint_data', 'algo_config']
    required_output_fields = ['ml_attribution', 'feature_importance']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} algorithmic attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} algorithmic attribution: {data}"
