...
class MlTestingEngine(BaseEngine):
    engine_name = "ml_testing"
    required_input_fields = ['model_data', 'test_config']
    required_output_fields = ['test_results', 'performance_bounds']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ml testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ml testing: {data}"
