...
class ConversionByStoreEngine(BaseEngine):
    engine_name = "conversion_by_store"
    required_input_fields = ['store_data', 'conversion_config']
    required_output_fields = ['conversion_rates', 'improvement_actions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} conversion by store", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} conversion by store: {data}"
