...
class CountdownTimerEngine(BaseEngine):
    engine_name = "countdown_timer"
    required_input_fields = ['timer_data', 'display_config']
    required_output_fields = ['timer_config', 'urgency_copy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} countdown timer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} countdown timer: {data}"
