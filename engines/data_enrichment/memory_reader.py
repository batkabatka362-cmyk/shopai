"""Data Enrichment Engine — memory reader.

Reads past data enrichment records from memory storage.
Used to inform current enrichment with historical context.
"""
...
def read_past_enrichments(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past data enrichment records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_enrichments(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past enrichment records."""
    if not records:
        return {
            "avg_quality_score": 0.0,
            "avg_features_count": 0.0,
            "avg_gaps_count": 0.0,
            "total_runs": 0,
        }

    scores = [r.get("quality_score", 0.0) for r in records]
    features = [r.get("features_count", 0) for r in records]
    gaps = [r.get("gaps_count", 0) for r in records]

    n = len(records)
    return {
        "avg_quality_score": round(sum(scores) / n, 3) if n else 0.0,
        "avg_features_count": round(sum(features) / n, 1) if n else 0.0,
        "avg_gaps_count": round(sum(gaps) / n, 1) if n else 0.0,
        "total_runs": n,
    }
