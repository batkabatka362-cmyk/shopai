...
class EnsembleBuilderEngine(BaseEngine):
    engine_name = "ensemble_builder"
    required_input_fields = ['model_list', 'ensemble_config']
    required_output_fields = ['ensemble_model', 'weight_allocation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ensemble builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ensemble builder: {data}"
