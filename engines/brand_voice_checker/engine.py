...
class BrandVoiceCheckerEngine(BaseEngine):
    engine_name = "brand_voice_checker"
    required_input_fields = ['content_data', 'brand_guidelines']
    required_output_fields = ['compliance_score', 'violation_details']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} brand voice checker", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} brand voice checker: {data}"
