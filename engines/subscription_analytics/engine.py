...
class SubscriptionAnalyticsEngine(BaseEngine):
    engine_name = "subscription_analytics"
    required_input_fields = ['subscription_data', 'analytics_config']
    required_output_fields = ['subscription_report', 'mrr_metrics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} subscription analytics", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} subscription analytics: {data}"
