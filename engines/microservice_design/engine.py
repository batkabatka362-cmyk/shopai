...
class MicroserviceDesignEngine(BaseEngine):
    engine_name = "microservice_design"
    required_input_fields = ['domain_data', 'design_config']
    required_output_fields = ['service_boundaries', 'api_contracts']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} microservice design", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} microservice design: {data}"
