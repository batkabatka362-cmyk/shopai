...
class PerformanceProfilerEngine(BaseEngine):
    engine_name = "performance_profiler"
    required_input_fields = ['profile_data', 'profiler_config']
    required_output_fields = ['profile_report', 'bottlenecks']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} performance profiler", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} performance profiler: {data}"
