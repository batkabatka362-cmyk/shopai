...
class FlywheelEngine(BaseEngine):
    engine_name = "flywheel"
    required_input_fields = ['business_data', 'flywheel_config']
    required_output_fields = ['flywheel_map', 'momentum_metrics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} flywheel", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} flywheel: {data}"
