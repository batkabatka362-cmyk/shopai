...
class MessengerCommerceEngine(BaseEngine):
    engine_name = "messenger_commerce"
    required_input_fields = ['catalog_data', 'messenger_config']
    required_output_fields = ['shop_setup', 'auto_replies']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} messenger commerce", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} messenger commerce: {data}"
