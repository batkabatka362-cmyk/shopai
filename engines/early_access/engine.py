...
class EarlyAccessEngine(BaseEngine):
    engine_name = "early_access"
    required_input_fields = ['product_data', 'access_config']
    required_output_fields = ['early_access_plan', 'notification_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} early access", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} early access: {data}"
