...
class PushTemplateEngine(BaseEngine):
    engine_name = "push_template"
    required_input_fields = ['notification_config', 'audience_data']
    required_output_fields = ['push_content', 'deep_link_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} push template", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} push template: {data}"
