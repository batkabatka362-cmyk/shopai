...
class PhoneValidationEngine(BaseEngine):
    engine_name = "phone_validation"
    required_input_fields = ['phone_data', 'validation_config']
    required_output_fields = ['validated_phones', 'carrier_info']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} phone validation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} phone validation: {data}"
