...
class AnnotationPipelineEngine(BaseEngine):
    engine_name = "annotation_pipeline"
    required_input_fields = ['raw_data', 'annotation_config']
    required_output_fields = ['annotated_data', 'quality_metrics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} annotation pipeline", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} annotation pipeline: {data}"
