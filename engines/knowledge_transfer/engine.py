...
class KnowledgeTransferEngine(BaseEngine):
    engine_name = "knowledge_transfer"
    required_input_fields = ['knowledge_data', 'transfer_config']
    required_output_fields = ['transfer_plan', 'documentation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} knowledge transfer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} knowledge transfer: {data}"
