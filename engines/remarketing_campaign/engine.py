...
class RemarketingCampaignEngine(BaseEngine):
    engine_name = "remarketing_campaign"
    required_input_fields = ['audience_data', 'campaign_config']
    required_output_fields = ['campaign_setup', 'list_management']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} remarketing campaign", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} remarketing campaign: {data}"
