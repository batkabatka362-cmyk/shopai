...
class ViralCoefficientEngine(BaseEngine):
    engine_name = "viral_coefficient"
    required_input_fields = ['referral_data', 'viral_config']
    required_output_fields = ['k_factor', 'viral_loop_optimization']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} viral coefficient", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} viral coefficient: {data}"
