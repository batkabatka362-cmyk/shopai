...
class TrademarkMonitoringEngine(BaseEngine):
    engine_name = "trademark_monitoring"
    required_input_fields = ['trademark_data', 'monitoring_config']
    required_output_fields = ['infringement_alerts', 'action_recommendations']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} trademark monitoring", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} trademark monitoring: {data}"
