...
class TrustFlowEngine(BaseEngine):
    engine_name = "trust_flow"
    required_input_fields = ['link_data', 'trust_config']
    required_output_fields = ['trust_score', 'citation_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} trust flow", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} trust flow: {data}"
