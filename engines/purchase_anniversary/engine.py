...
class PurchaseAnniversaryEngine(BaseEngine):
    engine_name = "purchase_anniversary"
    required_input_fields = ['purchase_data', 'anniversary_config']
    required_output_fields = ['anniversary_detection', 'offer_generation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} purchase anniversary", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} purchase anniversary: {data}"
