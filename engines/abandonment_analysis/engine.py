...
class AbandonmentAnalysisEngine(BaseEngine):
    engine_name = "abandonment_analysis"
    required_input_fields = ['abandonment_data', 'analysis_config']
    required_output_fields = ['abandonment_report', 'recovery_actions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} abandonment analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} abandonment analysis: {data}"
