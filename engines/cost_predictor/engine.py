...
class CostPredictorEngine(BaseEngine):
    engine_name = "cost_predictor"
    required_input_fields = ['cost_data', 'growth_config']
    required_output_fields = ['cost_forecast', 'saving_opportunities']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cost predictor", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cost predictor: {data}"
