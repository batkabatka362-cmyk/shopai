...
class ReviewSentimentAiEngine(BaseEngine):
    engine_name = "review_sentiment_ai"
    required_input_fields = ['review_data', 'sentiment_config']
    required_output_fields = ['sentiment_analysis', 'actionable_insights']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} review sentiment ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} review sentiment ai: {data}"
