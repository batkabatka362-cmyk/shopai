...
class RemoteWorkEngine(BaseEngine):
    engine_name = "remote_work"
    required_input_fields = ['employee_data', 'remote_config']
    required_output_fields = ['remote_policy', 'productivity_metrics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} remote work", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} remote work: {data}"
