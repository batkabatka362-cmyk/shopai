...
class BrandPartnershipEngine(BaseEngine):
    engine_name = "brand_partnership"
    required_input_fields = ['brand_data', 'partnership_criteria']
    required_output_fields = ['partner_matches', 'collaboration_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} brand partnership", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} brand partnership: {data}"
