"""
LeaseAgreementCommercial Engine
...
class LeaseAgreementCommercialEngine(BaseEngine):
    engine_name = "lease_agreement_commercial"
    required_input_fields = ['property_data', 'lease_config']
    required_output_fields = ['lease_draft', 'rent_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} lease agreement commercial", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} lease agreement commercial: {data}"
