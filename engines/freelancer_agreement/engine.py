...
class FreelancerAgreementEngine(BaseEngine):
    engine_name = "freelancer_agreement"
    required_input_fields = ['freelancer_data', 'agreement_config']
    required_output_fields = ['agreement_terms', 'payment_milestones']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} freelancer agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} freelancer agreement: {data}"
