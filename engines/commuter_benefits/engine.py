...
class CommuterBenefitsEngine(BaseEngine):
    engine_name = "commuter_benefits"
    required_input_fields = ['employee_data', 'commuter_config']
    required_output_fields = ['benefit_options', 'tax_savings']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} commuter benefits", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} commuter benefits: {data}"
