...
class SslManagementEngine(BaseEngine):
    engine_name = "ssl_management"
    required_input_fields = ['certificate_data', 'ssl_config']
    required_output_fields = ['ssl_status', 'renewal_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ssl management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ssl management: {data}"
