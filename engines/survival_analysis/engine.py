...
class SurvivalAnalysisEngine(BaseEngine):
    engine_name = "survival_analysis"
    required_input_fields = ['customer_data', 'analysis_config']
    required_output_fields = ['survival_curves', 'hazard_rates']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} survival analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} survival analysis: {data}"
