...
class PasswordPolicyEngine(BaseEngine):
    engine_name = "password_policy"
    required_input_fields = ['policy_config', 'compliance_config']
    required_output_fields = ['policy_rules', 'strength_checker']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} password policy", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} password policy: {data}"
