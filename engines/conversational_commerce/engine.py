...
class ConversationalCommerceEngine(BaseEngine):
    engine_name = "conversational_commerce"
    required_input_fields = ['chat_data', 'commerce_config']
    required_output_fields = ['conversation_flow', 'purchase_triggers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} conversational commerce", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} conversational commerce: {data}"
