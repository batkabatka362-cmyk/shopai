...
class TransferLearningEngine(BaseEngine):
    engine_name = "transfer_learning"
    required_input_fields = ['source_model', 'target_data']
    required_output_fields = ['adapted_model', 'performance_comparison']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} transfer learning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} transfer learning: {data}"
