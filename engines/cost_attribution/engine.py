...
class CostAttributionEngine(BaseEngine):
    engine_name = "cost_attribution"
    required_input_fields = ['cost_data', 'activity_config']
    required_output_fields = ['cost_allocation', 'efficiency_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cost attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cost attribution: {data}"
