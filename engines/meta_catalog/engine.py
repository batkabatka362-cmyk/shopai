...
class MetaCatalogEngine(BaseEngine):
    engine_name = "meta_catalog"
    required_input_fields = ['product_data', 'catalog_config']
    required_output_fields = ['catalog_sync', 'pixel_integration']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} meta catalog", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} meta catalog: {data}"
