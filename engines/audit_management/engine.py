...
class AuditManagementEngine(BaseEngine):
    engine_name = "audit_management"
    required_input_fields = ['audit_data', 'management_config']
    required_output_fields = ['audit_plan', 'finding_tracker']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} audit management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} audit management: {data}"
