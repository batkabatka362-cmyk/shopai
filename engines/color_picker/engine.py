...
class ColorPickerEngine(BaseEngine):
    engine_name = "color_picker"
    required_input_fields = ['product_data', 'color_config']
    required_output_fields = ['picker_setup', 'swatch_display']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} color picker", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} color picker: {data}"
