...
class OutputValidationEngine(BaseEngine):
    engine_name = "output_validation"
    required_input_fields = ['model_output', 'validation_config']
    required_output_fields = ['validation_result', 'correction_suggestions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} output validation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} output validation: {data}"
