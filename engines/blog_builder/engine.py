...
class BlogBuilderEngine(BaseEngine):
    engine_name = "blog_builder"
    required_input_fields = ['blog_data', 'builder_config']
    required_output_fields = ['blog_layout', 'category_structure']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} blog builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} blog builder: {data}"
