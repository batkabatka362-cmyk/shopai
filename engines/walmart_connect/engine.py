...
class WalmartConnectEngine(BaseEngine):
    engine_name = "walmart_connect"
    required_input_fields = ['campaign_data', 'walmart_config']
    required_output_fields = ['campaign_setup', 'targeting_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} walmart connect", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} walmart connect: {data}"
