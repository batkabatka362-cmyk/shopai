...
class SocialNetworkAnalysisEngine(BaseEngine):
    engine_name = "social_network_analysis"
    required_input_fields = ['network_data', 'analysis_config']
    required_output_fields = ['network_metrics', 'community_detection']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} social network analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} social network analysis: {data}"
