...
class ShopifyAnalyticsAiEngine(BaseEngine):
    engine_name = "shopify_analytics_ai"
    required_input_fields = ['store_data', 'analytics_config']
    required_output_fields = ['analytics_dashboard', 'insight_feed']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shopify analytics ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shopify analytics ai: {data}"
