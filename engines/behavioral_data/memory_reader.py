"""Behavioral Data Engine — memory reader.

Reads past behavioral data records from memory storage.
...
def read_past_behavioral(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past behavioral data records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_behavioral(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past behavioral records."""
    if not records:
        return {
    ...
    scores = [r.get("avg_engagement_score", 0.0) for r in records]
    sessions = [r.get("total_sessions", 0) for r in records]

    n = len(records)
    return {
        "avg_engagement_score": round(sum(scores) / n, 3) if n else 0.0,
        "avg_sessions": round(sum(sessions) / n, 1) if n else 0.0,
        "total_runs": n,
    }
