...
class SocialCommerceAiEngine(BaseEngine):
    engine_name = "social_commerce_ai"
    required_input_fields = ['social_data', 'product_catalog']
    required_output_fields = ['social_strategy', 'shoppable_content']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} social commerce ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} social commerce ai: {data}"
