...
class OrderFraudCheckEngine(BaseEngine):
    engine_name = "order_fraud_check"
    required_input_fields = ['order_data', 'fraud_config']
    required_output_fields = ['fraud_decision', 'risk_score']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} order fraud check", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} order fraud check: {data}"
