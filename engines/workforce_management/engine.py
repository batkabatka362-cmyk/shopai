...
class WorkforceManagementEngine(BaseEngine):
    engine_name = "workforce_management"
    required_input_fields = ['demand_data', 'workforce_config']
    required_output_fields = ['staffing_plan', 'schedule_forecast']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} workforce management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} workforce management: {data}"
