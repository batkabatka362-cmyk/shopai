...
class TalentAcquisitionEngine(BaseEngine):
    engine_name = "talent_acquisition"
    required_input_fields = ['role_data', 'hiring_config']
    required_output_fields = ['job_posting', 'screening_criteria']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} talent acquisition", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} talent acquisition: {data}"
