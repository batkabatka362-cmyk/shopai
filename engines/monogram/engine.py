...
class MonogramEngine(BaseEngine):
    engine_name = "monogram"
    required_input_fields = ['product_data', 'monogram_config']
    required_output_fields = ['monogram_setup', 'font_options']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} monogram", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} monogram: {data}"
