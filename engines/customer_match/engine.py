...
class CustomerMatchEngine(BaseEngine):
    engine_name = "customer_match"
    required_input_fields = ['customer_data', 'match_config']
    required_output_fields = ['audience_upload', 'match_rate']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} customer match", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} customer match: {data}"
