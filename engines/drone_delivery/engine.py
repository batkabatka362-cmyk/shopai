...
class DroneDeliveryEngine(BaseEngine):
    engine_name = "drone_delivery"
    required_input_fields = ['delivery_data', 'drone_config']
    required_output_fields = ['eligibility_check', 'delivery_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} drone delivery", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} drone delivery: {data}"
