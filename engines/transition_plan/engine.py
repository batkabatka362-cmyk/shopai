...
class TransitionPlanEngine(BaseEngine):
    engine_name = "transition_plan"
    required_input_fields = ['service_data', 'transition_config']
    required_output_fields = ['transition_schedule', 'knowledge_transfer']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} transition plan", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} transition plan: {data}"
