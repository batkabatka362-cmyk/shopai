...
class DigitalCatalogEngine(BaseEngine):
    engine_name = "digital_catalog"
    required_input_fields = ['catalog_data', 'design_config']
    required_output_fields = ['catalog_layout', 'interactive_elements']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} digital catalog", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} digital catalog: {data}"
