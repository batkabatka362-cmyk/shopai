...
class ConsentModeEngine(BaseEngine):
    engine_name = "consent_mode"
    required_input_fields = ['consent_data', 'mode_config']
    required_output_fields = ['consent_setup', 'modeling_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} consent mode", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} consent mode: {data}"
