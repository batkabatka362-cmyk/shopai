...
class TotalBlockingTimeEngine(BaseEngine):
    engine_name = "total_blocking_time"
    required_input_fields = ['task_data', 'analysis_config']
    required_output_fields = ['tbt_score', 'long_tasks']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} total blocking time", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} total blocking time: {data}"
