...
class EmployeeStoreEngine(BaseEngine):
    engine_name = "employee_store"
    required_input_fields = ['product_data', 'store_config']
    required_output_fields = ['store_setup', 'discount_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} employee store", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} employee store: {data}"
