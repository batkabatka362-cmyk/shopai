...
class AbandonmentPredictorEngine(BaseEngine):
    engine_name = "abandonment_predictor"
    required_input_fields = ['session_data', 'behavior_data']
    required_output_fields = ['abandonment_risk', 'intervention_triggers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} abandonment predictor", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} abandonment predictor: {data}"
