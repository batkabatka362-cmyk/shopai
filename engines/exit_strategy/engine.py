...
class ExitStrategyEngine(BaseEngine):
    engine_name = "exit_strategy"
    required_input_fields = ['business_data', 'exit_config']
    required_output_fields = ['exit_options', 'valuation_estimate']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} exit strategy", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} exit strategy: {data}"
