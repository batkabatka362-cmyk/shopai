...
class ApmMonitoringEngine(BaseEngine):
    engine_name = "apm_monitoring"
    required_input_fields = ['app_data', 'apm_config']
    required_output_fields = ['performance_report', 'bottleneck_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} apm monitoring", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} apm monitoring: {data}"
