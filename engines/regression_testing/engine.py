...
class RegressionTestingEngine(BaseEngine):
    engine_name = "regression_testing"
    required_input_fields = ['test_suite', 'regression_config']
    required_output_fields = ['test_results', 'regression_flags']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} regression testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} regression testing: {data}"
