...
class ParticleSwarmEngine(BaseEngine):
    engine_name = "particle_swarm"
    required_input_fields = ['search_space', 'pso_config']
    required_output_fields = ['best_position', 'convergence_history']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} particle swarm", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} particle swarm: {data}"
