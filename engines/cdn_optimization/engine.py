...
class CdnOptimizationEngine(BaseEngine):
    engine_name = "cdn_optimization"
    required_input_fields = ['traffic_data', 'cdn_config']
    required_output_fields = ['cdn_plan', 'cache_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cdn optimization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cdn optimization: {data}"
