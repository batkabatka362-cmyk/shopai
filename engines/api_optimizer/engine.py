...
class ApiOptimizerEngine(BaseEngine):
    engine_name = "api_optimizer"
    required_input_fields = ['api_data', 'performance_config']
    required_output_fields = ['optimization_plan', 'latency_reduction']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} api optimizer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} api optimizer: {data}"
