...
class OpenRatePredictorEngine(BaseEngine):
    engine_name = "open_rate_predictor"
    required_input_fields = ['email_data', 'prediction_config']
    required_output_fields = ['open_probability', 'subject_optimization']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} open rate predictor", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} open rate predictor: {data}"
