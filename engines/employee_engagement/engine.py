...
class EmployeeEngagementEngine(BaseEngine):
    engine_name = "employee_engagement"
    required_input_fields = ['survey_data', 'engagement_config']
    required_output_fields = ['engagement_score', 'action_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} employee engagement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} employee engagement: {data}"
