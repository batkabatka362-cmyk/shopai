...
class SecretManagementEngine(BaseEngine):
    engine_name = "secret_management"
    required_input_fields = ['secret_inventory', 'rotation_config']
    required_output_fields = ['rotation_plan', 'access_audit']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} secret management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} secret management: {data}"
