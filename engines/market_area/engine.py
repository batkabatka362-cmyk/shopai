...
class MarketAreaEngine(BaseEngine):
    engine_name = "market_area"
    required_input_fields = ['store_data', 'area_config']
    required_output_fields = ['trade_area', 'demographic_profile']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} market area", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} market area: {data}"
