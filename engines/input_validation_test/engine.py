...
class InputValidationTestEngine(BaseEngine):
    engine_name = "input_validation_test"
    required_input_fields = ['form_data', 'test_config']
    required_output_fields = ['validation_report', 'bypass_findings']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} input validation test", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} input validation test: {data}"
