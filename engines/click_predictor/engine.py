...
class ClickPredictorEngine(BaseEngine):
    engine_name = "click_predictor"
    required_input_fields = ['content_data', 'prediction_config']
    required_output_fields = ['click_probability', 'ctr_optimization']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} click predictor", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} click predictor: {data}"
