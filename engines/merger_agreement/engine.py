...
class MergerAgreementEngine(BaseEngine):
    engine_name = "merger_agreement"
    required_input_fields = ['merger_data', 'agreement_config']
    required_output_fields = ['agreement_draft', 'closing_conditions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} merger agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} merger agreement: {data}"
