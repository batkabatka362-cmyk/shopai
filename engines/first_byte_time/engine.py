...
class FirstByteTimeEngine(BaseEngine):
    engine_name = "first_byte_time"
    required_input_fields = ['server_data', 'analysis_config']
    required_output_fields = ['ttfb', 'server_optimization']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} first byte time", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} first byte time: {data}"
