...
class PhotoReviewEngine(BaseEngine):
    engine_name = "photo_review"
    required_input_fields = ['review_data', 'incentive_config']
    required_output_fields = ['photo_review_campaign', 'ugc_gallery']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} photo review", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} photo review: {data}"
