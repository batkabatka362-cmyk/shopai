...
class RpaBuilderEngine(BaseEngine):
    engine_name = "rpa_builder"
    required_input_fields = ['process_data', 'rpa_config']
    required_output_fields = ['rpa_workflow', 'bot_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} rpa builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} rpa builder: {data}"
