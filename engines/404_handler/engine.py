...
class NotFoundHandlerEngine(BaseEngine):
    engine_name = "404_handler"
    required_input_fields = ['page_data', 'handler_config']
    required_output_fields = ['error_page', 'redirect_suggestions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} 404 handler", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} 404 handler: {data}"
