...
class ViewportTrackingEngine(BaseEngine):
    engine_name = "viewport_tracking"
    required_input_fields = ['viewport_data', 'tracking_config']
    required_output_fields = ['visible_elements', 'exposure_time']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} viewport tracking", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} viewport tracking: {data}"
