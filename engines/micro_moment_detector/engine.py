...
class MicroMomentDetectorEngine(BaseEngine):
    engine_name = "micro_moment_detector"
    required_input_fields = ['behavior_data', 'event_data']
    required_output_fields = ['micro_moments', 'trigger_actions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} micro moment detector", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} micro moment detector: {data}"
