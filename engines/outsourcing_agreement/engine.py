...
class OutsourcingAgreementEngine(BaseEngine):
    engine_name = "outsourcing_agreement"
    required_input_fields = ['outsource_data', 'agreement_config']
    required_output_fields = ['outsource_terms', 'governance_model']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} outsourcing agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} outsourcing agreement: {data}"
