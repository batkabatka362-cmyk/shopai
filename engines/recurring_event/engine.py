...
class RecurringEventEngine(BaseEngine):
    engine_name = "recurring_event"
    required_input_fields = ['event_data', 'recurrence_config']
    required_output_fields = ['recurrence_setup', 'variation_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} recurring event", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} recurring event: {data}"
