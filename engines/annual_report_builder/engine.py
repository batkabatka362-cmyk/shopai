...
class AnnualReportBuilderEngine(BaseEngine):
    engine_name = "annual_report_builder"
    required_input_fields = ['financial_data', 'report_config']
    required_output_fields = ['report_content', 'design_brief']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} annual report builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} annual report builder: {data}"
