...
class RollingForecastEngine(BaseEngine):
    engine_name = "rolling_forecast"
    required_input_fields = ['actual_data', 'forecast_config']
    required_output_fields = ['rolling_forecast', 'accuracy_metrics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} rolling forecast", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} rolling forecast: {data}"
