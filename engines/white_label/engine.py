...
class WhiteLabelEngine(BaseEngine):
    engine_name = "white_label"
    required_input_fields = ['product_data', 'client_config']
    required_output_fields = ['white_label_plan', 'customization_options']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} white label", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} white label: {data}"
