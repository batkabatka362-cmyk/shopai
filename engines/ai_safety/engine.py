...
class AiSafetyEngine(BaseEngine):
    engine_name = "ai_safety"
    required_input_fields = ['model_data', 'safety_config']
    required_output_fields = ['safety_assessment', 'guardrail_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ai safety", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ai safety: {data}"
