...
class ContentModerationEngine(BaseEngine):
    engine_name = "content_moderation"
    required_input_fields = ['content_data', 'moderation_config']
    required_output_fields = ['moderation_results', 'flagged_content']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} content moderation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} content moderation: {data}"
