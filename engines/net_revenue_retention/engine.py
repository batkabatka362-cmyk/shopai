...
class NetRevenueRetentionEngine(BaseEngine):
    engine_name = "net_revenue_retention"
    required_input_fields = ['revenue_data', 'retention_config']
    required_output_fields = ['nrr_metrics', 'improvement_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} net revenue retention", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} net revenue retention: {data}"
