...
class EmailBuilderEngine(BaseEngine):
    engine_name = "email_builder"
    required_input_fields = ['content_data', 'email_config']
    required_output_fields = ['email_template', 'responsive_preview']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} email builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} email builder: {data}"
