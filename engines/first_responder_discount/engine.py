...
class FirstResponderDiscountEngine(BaseEngine):
    engine_name = "first_responder_discount"
    required_input_fields = ['verification_data', 'discount_config']
    required_output_fields = ['discount_setup', 'verification_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} first responder discount", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} first responder discount: {data}"
