...
class ResolutionTimeEngine(BaseEngine):
    engine_name = "resolution_time"
    required_input_fields = ['ticket_data', 'analysis_config']
    required_output_fields = ['resolution_metrics', 'benchmark_comparison']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} resolution time", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} resolution time: {data}"
