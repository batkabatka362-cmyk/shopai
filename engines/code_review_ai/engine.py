...
class CodeReviewAiEngine(BaseEngine):
    engine_name = "code_review_ai"
    required_input_fields = ['code_data', 'review_config']
    required_output_fields = ['review_comments', 'quality_score']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} code review ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} code review ai: {data}"
