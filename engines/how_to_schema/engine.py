...
class HowToSchemaEngine(BaseEngine):
    engine_name = "how_to_schema"
    required_input_fields = ['tutorial_data', 'schema_config']
    required_output_fields = ['howto_markup', 'step_structure']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} how to schema", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} how to schema: {data}"
