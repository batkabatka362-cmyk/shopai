...
class InvestmentDecisionEngine(BaseEngine):
    engine_name = "investment_decision"
    required_input_fields = ['opportunity_data', 'budget_config']
    required_output_fields = ['investment_recommendation', 'roi_forecast']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} investment decision", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} investment decision: {data}"
