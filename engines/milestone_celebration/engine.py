...
class MilestoneCelebrationEngine(BaseEngine):
    engine_name = "milestone_celebration"
    required_input_fields = ['milestone_data', 'celebration_config']
    required_output_fields = ['celebration_plan', 'communication']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} milestone celebration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} milestone celebration: {data}"
