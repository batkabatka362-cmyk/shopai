...
class InteractiveLookbookEngine(BaseEngine):
    engine_name = "interactive_lookbook"
    required_input_fields = ['lookbook_data', 'interaction_config']
    required_output_fields = ['lookbook_setup', 'hotspot_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} interactive lookbook", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} interactive lookbook: {data}"
