...
class AmendmentEngine(BaseEngine):
    engine_name = "amendment"
    required_input_fields = ['contract_data', 'amendment_config']
    required_output_fields = ['amendment_draft', 'effective_terms']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} amendment", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} amendment: {data}"
