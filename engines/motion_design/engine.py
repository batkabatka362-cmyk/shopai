...
class MotionDesignEngine(BaseEngine):
    engine_name = "motion_design"
    required_input_fields = ['content_data', 'motion_config']
    required_output_fields = ['animation_spec', 'timing_curves']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} motion design", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} motion design: {data}"
