...
class SoakTestingEngine(BaseEngine):
    engine_name = "soak_testing"
    required_input_fields = ['system_data', 'soak_config']
    required_output_fields = ['stability_report', 'memory_leak_detection']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} soak testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} soak testing: {data}"
