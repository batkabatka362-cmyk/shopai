...
class SlotExtractionEngine(BaseEngine):
    engine_name = "slot_extraction"
    required_input_fields = ['text_data', 'slot_config']
    required_output_fields = ['extracted_slots', 'slot_values']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} slot extraction", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} slot extraction: {data}"
