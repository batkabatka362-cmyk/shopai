...
class ReferringDomainsEngine(BaseEngine):
    engine_name = "referring_domains"
    required_input_fields = ['backlink_data', 'analysis_config']
    required_output_fields = ['domain_report', 'quality_distribution']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} referring domains", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} referring domains: {data}"
