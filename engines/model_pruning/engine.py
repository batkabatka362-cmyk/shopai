...
class ModelPruningEngine(BaseEngine):
    engine_name = "model_pruning"
    required_input_fields = ['model_data', 'pruning_config']
    required_output_fields = ['pruned_model', 'sparsity_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} model pruning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} model pruning: {data}"
