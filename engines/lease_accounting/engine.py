...
class LeaseAccountingEngine(BaseEngine):
    engine_name = "lease_accounting"
    required_input_fields = ['lease_data', 'accounting_config']
    required_output_fields = ['lease_schedule', 'journal_entries']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} lease accounting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} lease accounting: {data}"
