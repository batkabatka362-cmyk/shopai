...
class SupportAgreementEngine(BaseEngine):
    engine_name = "support_agreement"
    required_input_fields = ['support_data', 'agreement_config']
    required_output_fields = ['support_terms', 'coverage_hours']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} support agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} support agreement: {data}"
