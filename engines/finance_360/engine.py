...
class Finance360Engine(BaseEngine):
    engine_name = "finance_360"
    required_input_fields = ['financial_data', 'integration_config']
    required_output_fields = ['unified_finance', 'health_indicators']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} finance 360", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} finance 360: {data}"
