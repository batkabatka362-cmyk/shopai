...
class CostMonitoringEngine(BaseEngine):
    engine_name = "cost_monitoring"
    required_input_fields = ['billing_data', 'budget_config']
    required_output_fields = ['cost_report', 'alert_thresholds']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cost monitoring", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cost monitoring: {data}"
