...
class LtvMaximizerEngine(BaseEngine):
    engine_name = "ltv_maximizer"
    required_input_fields = ['customer_data', 'program_data']
    required_output_fields = ['ltv_strategies', 'projected_impact']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ltv maximizer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ltv maximizer: {data}"
