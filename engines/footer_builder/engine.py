...
class FooterBuilderEngine(BaseEngine):
    engine_name = "footer_builder"
    required_input_fields = ['content_data', 'footer_config']
    required_output_fields = ['footer_layout', 'link_structure']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} footer builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} footer builder: {data}"
