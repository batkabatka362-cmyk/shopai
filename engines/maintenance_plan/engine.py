...
class MaintenancePlanEngine(BaseEngine):
    engine_name = "maintenance_plan"
    required_input_fields = ['product_data', 'plan_config']
    required_output_fields = ['maintenance_schedule', 'cost_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} maintenance plan", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} maintenance plan: {data}"
