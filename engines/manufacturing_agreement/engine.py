...
class ManufacturingAgreementEngine(BaseEngine):
    engine_name = "manufacturing_agreement"
    required_input_fields = ['product_data', 'mfg_config']
    required_output_fields = ['agreement_draft', 'quality_standards']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} manufacturing agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} manufacturing agreement: {data}"
