...
class ForeignCurrencyEngine(BaseEngine):
    engine_name = "foreign_currency"
    required_input_fields = ['transaction_data', 'fx_config']
    required_output_fields = ['translated_amounts', 'gain_loss_calc']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} foreign currency", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} foreign currency: {data}"
