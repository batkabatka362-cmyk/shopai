...
class HyperparameterTuningEngine(BaseEngine):
    engine_name = "hyperparameter_tuning"
    required_input_fields = ['model_config', 'search_space']
    required_output_fields = ['best_hyperparameters', 'tuning_history']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} hyperparameter tuning", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} hyperparameter tuning: {data}"
