...
class CheckoutOptimizerAiEngine(BaseEngine):
    engine_name = "checkout_optimizer_ai"
    required_input_fields = ['checkout_data', 'abandonment_data']
    required_output_fields = ['checkout_fixes', 'conversion_forecast']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} checkout optimizer ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} checkout optimizer ai: {data}"
