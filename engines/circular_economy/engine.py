...
class CircularEconomyEngine(BaseEngine):
    engine_name = "circular_economy"
    required_input_fields = ['product_data', 'circular_config']
    required_output_fields = ['circular_plan', 'impact_metrics']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} circular economy", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} circular economy: {data}"
