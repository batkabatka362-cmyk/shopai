...
class ImageCdnEngine(BaseEngine):
    engine_name = "image_cdn"
    required_input_fields = ['image_data', 'optimization_config']
    required_output_fields = ['optimized_urls', 'format_strategy']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} image cdn", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} image cdn: {data}"
