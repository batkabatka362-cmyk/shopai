...
class Channel360Engine(BaseEngine):
    engine_name = "channel_360"
    required_input_fields = ['channel_data', 'integration_config']
    required_output_fields = ['unified_channel', 'performance_overview']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} channel 360", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} channel 360: {data}"
