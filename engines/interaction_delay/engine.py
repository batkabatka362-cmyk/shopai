...
class InteractionDelayEngine(BaseEngine):
    engine_name = "interaction_delay"
    required_input_fields = ['interaction_data', 'analysis_config']
    required_output_fields = ['inp_score', 'delay_breakdown']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} interaction delay", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} interaction delay: {data}"
