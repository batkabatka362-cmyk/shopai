...
class ChatCommerceAiEngine(BaseEngine):
    engine_name = "chat_commerce_ai"
    required_input_fields = ['chat_data', 'catalog_data']
    required_output_fields = ['chat_response', 'product_suggestions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} chat commerce ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} chat commerce ai: {data}"
