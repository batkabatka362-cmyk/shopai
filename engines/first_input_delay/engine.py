...
class FirstInputDelayEngine(BaseEngine):
    engine_name = "first_input_delay"
    required_input_fields = ['input_data', 'analysis_config']
    required_output_fields = ['fid_score', 'main_thread_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} first input delay", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} first input delay: {data}"
