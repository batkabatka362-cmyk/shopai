...
class SelfServiceEngine(BaseEngine):
    engine_name = "self_service"
    required_input_fields = ['customer_query', 'knowledge_base']
    required_output_fields = ['self_service_response', 'deflection_score']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} self service", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} self service: {data}"
