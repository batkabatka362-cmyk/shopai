...
class NextBestProductEngine(BaseEngine):
    engine_name = "next_best_product"
    required_input_fields = ['customer_data', 'catalog_data']
    required_output_fields = ['product_recommendations', 'affinity_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} next best product", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} next best product: {data}"
