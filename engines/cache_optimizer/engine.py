...
class CacheOptimizerEngine(BaseEngine):
    engine_name = "cache_optimizer"
    required_input_fields = ['cache_data', 'optimization_config']
    required_output_fields = ['cache_strategy', 'hit_rate_improvement']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cache optimizer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cache optimizer: {data}"
