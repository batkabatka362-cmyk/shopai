...
class UseTaxEngine(BaseEngine):
    engine_name = "use_tax"
    required_input_fields = ['purchase_data', 'tax_config']
    required_output_fields = ['use_tax_calc', 'accrual_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} use tax", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} use tax: {data}"
