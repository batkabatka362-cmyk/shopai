...
class ModelFairnessEngine(BaseEngine):
    engine_name = "model_fairness"
    required_input_fields = ['model_data', 'fairness_config']
    required_output_fields = ['fairness_metrics', 'bias_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} model fairness", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} model fairness: {data}"
