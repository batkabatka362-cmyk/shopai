"""Product Description Engine — memory reader.

Reads past product description records from memory storage.
Used to inform current generation with historical context.
"""
...
def read_past_descriptions(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past product description records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_descriptions(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past description records."""
    if not records:
        return {
            "avg_seo_score": 0.0,
    ...
    bullet_counts = [r.get("bullet_count", 0) for r in records]

    n = len(records)
    return {
        "avg_seo_score": round(sum(seo_scores) / n, 1) if n else 0.0,
        "avg_bullet_count": round(sum(bullet_counts) / n, 1) if n else 0.0,
        "total_runs": n,
    }
