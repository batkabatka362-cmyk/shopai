...
class BurnRateEngine(BaseEngine):
    engine_name = "burn_rate"
    required_input_fields = ['financial_data', 'burn_config']
    required_output_fields = ['burn_analysis', 'runway_estimate']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} burn rate", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} burn rate: {data}"
