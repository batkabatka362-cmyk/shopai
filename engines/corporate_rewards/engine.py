...
class CorporateRewardsEngine(BaseEngine):
    engine_name = "corporate_rewards"
    required_input_fields = ['program_data', 'reward_config']
    required_output_fields = ['reward_catalog', 'redemption_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} corporate rewards", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} corporate rewards: {data}"
