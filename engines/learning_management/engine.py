...
class LearningManagementEngine(BaseEngine):
    engine_name = "learning_management"
    required_input_fields = ['learning_data', 'lms_config']
    required_output_fields = ['learning_paths', 'completion_tracking']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} learning management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} learning management: {data}"
