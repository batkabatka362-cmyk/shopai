...
class ServiceMeshEngine(BaseEngine):
    engine_name = "service_mesh"
    required_input_fields = ['service_data', 'mesh_config']
    required_output_fields = ['mesh_topology', 'routing_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} service mesh", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} service mesh: {data}"
