...
class ProductLiabilityEngine(BaseEngine):
    engine_name = "product_liability"
    required_input_fields = ['product_data', 'liability_config']
    required_output_fields = ['liability_assessment', 'insurance_needs']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} product liability", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} product liability: {data}"
