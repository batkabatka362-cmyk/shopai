...
class TrialBalanceEngine(BaseEngine):
    engine_name = "trial_balance"
    required_input_fields = ['gl_data', 'tb_config']
    required_output_fields = ['trial_balance', 'out_of_balance_items']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} trial balance", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} trial balance: {data}"
