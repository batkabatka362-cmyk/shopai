...
class TrainingContentEngine(BaseEngine):
    engine_name = "training_content"
    required_input_fields = ['product_data', 'training_config']
    required_output_fields = ['training_modules', 'assessment_questions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} training content", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} training content: {data}"
