...
class RedirectManagementEngine(BaseEngine):
    engine_name = "redirect_management"
    required_input_fields = ['url_data', 'redirect_config']
    required_output_fields = ['redirect_rules', 'chain_detection']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} redirect management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} redirect management: {data}"
