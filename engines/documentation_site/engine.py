...
class DocumentationSiteEngine(BaseEngine):
    engine_name = "documentation_site"
    required_input_fields = ['doc_data', 'site_config']
    required_output_fields = ['site_structure', 'search_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} documentation site", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} documentation site: {data}"
