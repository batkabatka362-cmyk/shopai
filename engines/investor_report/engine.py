...
class InvestorReportEngine(BaseEngine):
    engine_name = "investor_report"
    required_input_fields = ['financial_data', 'report_config']
    required_output_fields = ['report_content', 'kpi_summary']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} investor report", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} investor report: {data}"
