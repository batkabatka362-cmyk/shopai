...
class RobotsTxtEngine(BaseEngine):
    engine_name = "robots_txt"
    required_input_fields = ['site_data', 'robots_config']
    required_output_fields = ['robots_content', 'crawl_directives']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} robots txt", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} robots txt: {data}"
