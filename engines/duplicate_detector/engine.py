...
class DuplicateDetectorEngine(BaseEngine):
    engine_name = "duplicate_detector"
    required_input_fields = ['record_data', 'detection_config']
    required_output_fields = ['duplicate_pairs', 'similarity_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} duplicate detector", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} duplicate detector: {data}"
