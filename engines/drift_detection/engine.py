...
class DriftDetectionEngine(BaseEngine):
    engine_name = "drift_detection"
    required_input_fields = ['prediction_data', 'baseline_data']
    required_output_fields = ['drift_scores', 'alert_triggers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} drift detection", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} drift detection: {data}"
