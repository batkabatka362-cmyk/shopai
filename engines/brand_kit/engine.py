...
class BrandKitEngine(BaseEngine):
    engine_name = "brand_kit"
    required_input_fields = ['brand_data', 'kit_config']
    required_output_fields = ['brand_kit', 'download_package']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} brand kit", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} brand kit: {data}"
