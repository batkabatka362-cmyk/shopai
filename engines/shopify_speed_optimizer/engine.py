...
class ShopifySpeedOptimizerEngine(BaseEngine):
    engine_name = "shopify_speed_optimizer"
    required_input_fields = ['speed_data', 'optimization_config']
    required_output_fields = ['speed_report', 'fix_priorities']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shopify speed optimizer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shopify speed optimizer: {data}"
