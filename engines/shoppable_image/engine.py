...
class ShoppableImageEngine(BaseEngine):
    engine_name = "shoppable_image"
    required_input_fields = ['image_data', 'shopping_config']
    required_output_fields = ['image_setup', 'product_pins']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shoppable image", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shoppable image: {data}"
