...
class PropensityModelEngine(BaseEngine):
    engine_name = "propensity_model"
    required_input_fields = ['customer_data', 'model_config']
    required_output_fields = ['propensity_scores', 'segment_propensities']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} propensity model", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} propensity model: {data}"
