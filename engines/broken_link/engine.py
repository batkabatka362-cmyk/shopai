...
class BrokenLinkEngine(BaseEngine):
    engine_name = "broken_link"
    required_input_fields = ['site_data', 'scan_config']
    required_output_fields = ['broken_links', 'fix_suggestions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} broken link", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} broken link: {data}"
