...
class SmokeTestingEngine(BaseEngine):
    engine_name = "smoke_testing"
    required_input_fields = ['build_data', 'smoke_config']
    required_output_fields = ['smoke_results', 'go_nogo']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} smoke testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} smoke testing: {data}"
