...
class RailFreightEngine(BaseEngine):
    engine_name = "rail_freight"
    required_input_fields = ['shipment_data', 'freight_config']
    required_output_fields = ['rail_quotes', 'terminal_options']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} rail freight", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} rail freight: {data}"
