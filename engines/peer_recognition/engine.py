...
class PeerRecognitionEngine(BaseEngine):
    engine_name = "peer_recognition"
    required_input_fields = ['recognition_data', 'peer_config']
    required_output_fields = ['recognition_feed', 'badge_awards']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} peer recognition", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} peer recognition: {data}"
