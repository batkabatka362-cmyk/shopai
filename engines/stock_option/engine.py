...
class StockOptionEngine(BaseEngine):
    engine_name = "stock_option"
    required_input_fields = ['option_data', 'management_config']
    required_output_fields = ['option_report', 'exercise_planning']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} stock option", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} stock option: {data}"
