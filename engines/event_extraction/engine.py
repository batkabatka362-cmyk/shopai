...
class EventExtractionEngine(BaseEngine):
    engine_name = "event_extraction"
    required_input_fields = ['text_data', 'event_config']
    required_output_fields = ['events', 'temporal_ordering']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} event extraction", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} event extraction: {data}"
