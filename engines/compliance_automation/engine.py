...
class ComplianceAutomationEngine(BaseEngine):
    engine_name = "compliance_automation"
    required_input_fields = ['compliance_data', 'automation_config']
    required_output_fields = ['compliance_status', 'automated_checks']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} compliance automation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} compliance automation: {data}"
