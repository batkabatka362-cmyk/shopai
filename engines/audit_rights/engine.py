...
class AuditRightsEngine(BaseEngine):
    engine_name = "audit_rights"
    required_input_fields = ['contract_data', 'audit_config']
    required_output_fields = ['audit_provisions', 'frequency_terms']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} audit rights", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} audit rights: {data}"
