...
class GoogleTagManagerEngine(BaseEngine):
    engine_name = "google_tag_manager"
    required_input_fields = ['container_data', 'integration_config']
    required_output_fields = ['gtm_setup', 'tag_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} google tag manager", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} google tag manager: {data}"
