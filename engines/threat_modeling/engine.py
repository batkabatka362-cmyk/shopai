...
class ThreatModelingEngine(BaseEngine):
    engine_name = "threat_modeling"
    required_input_fields = ['system_architecture', 'threat_config']
    required_output_fields = ['threat_map', 'mitigation_priorities']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} threat modeling", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} threat modeling: {data}"
