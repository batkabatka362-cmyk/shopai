...
class GoodwillTestEngine(BaseEngine):
    engine_name = "goodwill_test"
    required_input_fields = ['cgu_data', 'test_config']
    required_output_fields = ['goodwill_assessment', 'fair_value_estimate']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} goodwill test", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} goodwill test: {data}"
