...
class TrendPredictionAiEngine(BaseEngine):
    engine_name = "trend_prediction_ai"
    required_input_fields = ['trend_data', 'signal_config']
    required_output_fields = ['trend_forecast', 'emerging_trends']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} trend prediction ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} trend prediction ai: {data}"
