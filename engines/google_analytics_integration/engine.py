"""
GoogleAnalyticsIntegration Engine
...
class GoogleAnalyticsIntegrationEngine(BaseEngine):
    engine_name = "google_analytics_integration"
    required_input_fields = ['property_data', 'integration_config']
    required_output_fields = ['ga_setup', 'event_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} google analytics integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} google analytics integration: {data}"
