...
class CartDrawerEngine(BaseEngine):
    engine_name = "cart_drawer"
    required_input_fields = ['cart_data', 'drawer_config']
    required_output_fields = ['drawer_layout', 'upsell_placement']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cart drawer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cart drawer: {data}"
