...
class GreenCertificationEngine(BaseEngine):
    engine_name = "green_certification"
    required_input_fields = ['product_data', 'certification_config']
    required_output_fields = ['certification_plan', 'audit_preparation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} green certification", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} green certification: {data}"
