...
class LboModelEngine(BaseEngine):
    engine_name = "lbo_model"
    required_input_fields = ['target_data', 'lbo_config']
    required_output_fields = ['returns_analysis', 'debt_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} lbo model", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} lbo model: {data}"
