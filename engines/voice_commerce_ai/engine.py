...
class VoiceCommerceAiEngine(BaseEngine):
    engine_name = "voice_commerce_ai"
    required_input_fields = ['voice_data', 'catalog_data']
    required_output_fields = ['voice_response', 'product_matches']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} voice commerce ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} voice commerce ai: {data}"
