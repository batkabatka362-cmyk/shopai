"""Affiliate Engine — memory reader.

Reads past affiliate program records from memory storage.
...
def read_past_affiliate_runs(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past affiliate run records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_affiliate_runs(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past affiliate records."""
    if not records:
        return {
            "avg_program_roi": 0.0,
            "avg_total_commission": 0.0,
            "total_runs": 0,
        }

    rois = [r.get("program_roi", 0.0) for r in records]
    commissions = [r.get("total_commission", 0.0) for r in records]

    n = len(records)
    return {
        "avg_program_roi": round(sum(rois) / n, 2) if n else 0.0,
        "avg_total_commission": round(sum(commissions) / n, 2) if n else 0.0,
        "total_runs": n,
    }
