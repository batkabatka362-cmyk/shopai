...
class JourneyMapperAiEngine(BaseEngine):
    engine_name = "journey_mapper_ai"
    required_input_fields = ['touchpoint_data', 'behavior_data']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} journey mapper ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} journey mapper ai: {data}"
