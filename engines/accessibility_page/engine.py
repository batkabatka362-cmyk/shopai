...
class AccessibilityPageEngine(BaseEngine):
    engine_name = "accessibility_page"
    required_input_fields = ['accessibility_data', 'page_config']
    required_output_fields = ['statement_content', 'compliance_status']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} accessibility page", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} accessibility page: {data}"
