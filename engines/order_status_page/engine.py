...
class OrderStatusPageEngine(BaseEngine):
    engine_name = "order_status_page"
    required_input_fields = ['order_data', 'page_config']
    required_output_fields = ['status_display', 'tracking_widget']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} order status page", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} order status page: {data}"
