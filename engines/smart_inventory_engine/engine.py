...
class SmartInventoryEngineEngine(BaseEngine):
    engine_name = "smart_inventory_engine"
    required_input_fields = ['inventory_data', 'demand_data']
    required_output_fields = ['inventory_actions', 'optimization_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} smart inventory engine", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} smart inventory engine: {data}"
