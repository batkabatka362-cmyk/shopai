"""
WarrantyPolicyGenerator Engine
...
class WarrantyPolicyGeneratorEngine(BaseEngine):
    engine_name = "warranty_policy_generator"
    required_input_fields = ['product_data', 'policy_config']
    required_output_fields = ['warranty_policy', 'claim_procedure']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} warranty policy generator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} warranty policy generator: {data}"
