"""Subscription Engine — memory reader.

Reads past subscription analysis records from memory storage.
...
def read_past_subscription_runs(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past subscription run records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_subscription_runs(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past subscription records."""
    if not records:
        return {
            "avg_mrr": 0.0,
            "avg_churn_risks": 0.0,
    ...
    mrrs = [r.get("mrr", 0.0) for r in records]
    churn_counts = [r.get("churn_risk_count", 0) for r in records]

    n = len(records)
    return {
        "avg_mrr": round(sum(mrrs) / n, 2) if n else 0.0,
        "avg_churn_risks": round(sum(churn_counts) / n, 1) if n else 0.0,
        "total_runs": n,
    }
