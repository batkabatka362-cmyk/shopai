...
class AccountsPayableEngine(BaseEngine):
    engine_name = "accounts_payable"
    required_input_fields = ['ap_data', 'management_config']
    required_output_fields = ['ap_aging', 'payment_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} accounts payable", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} accounts payable: {data}"
