...
class WaterFootprintEngine(BaseEngine):
    engine_name = "water_footprint"
    required_input_fields = ['water_data', 'footprint_config']
    required_output_fields = ['footprint_report', 'reduction_targets']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} water footprint", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} water footprint: {data}"
