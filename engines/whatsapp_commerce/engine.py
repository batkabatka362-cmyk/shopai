...
class WhatsappCommerceEngine(BaseEngine):
    engine_name = "whatsapp_commerce"
    required_input_fields = ['catalog_data', 'whatsapp_config']
    required_output_fields = ['catalog_sync', 'message_templates']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} whatsapp commerce", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} whatsapp commerce: {data}"
