...
class ArrForecastingEngine(BaseEngine):
    engine_name = "arr_forecasting"
    required_input_fields = ['revenue_data', 'forecast_config']
    required_output_fields = ['arr_forecast', 'scenario_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} arr forecasting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} arr forecasting: {data}"
