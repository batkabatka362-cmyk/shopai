...
class PromptTestingEngine(BaseEngine):
    engine_name = "prompt_testing"
    required_input_fields = ['prompt_variants', 'test_config']
    required_output_fields = ['test_results', 'best_prompt']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} prompt testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} prompt testing: {data}"
