...
class WafManagementEngine(BaseEngine):
    engine_name = "waf_management"
    required_input_fields = ['traffic_data', 'waf_config']
    required_output_fields = ['waf_rules', 'threat_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} waf management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} waf management: {data}"
