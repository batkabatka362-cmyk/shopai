...
class RiskDecisionEngine(BaseEngine):
    engine_name = "risk_decision"
    required_input_fields = ['risk_data', 'threshold_config']
    required_output_fields = ['risk_decision', 'mitigation_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} risk decision", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} risk decision: {data}"
