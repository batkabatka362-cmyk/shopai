...
class PerformanceStandardEngine(BaseEngine):
    engine_name = "performance_standard"
    required_input_fields = ['service_data', 'standard_config']
    required_output_fields = ['performance_metrics', 'measurement_method']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} performance standard", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} performance standard: {data}"
