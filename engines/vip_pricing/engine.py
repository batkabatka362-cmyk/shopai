...
class VipPricingEngine(BaseEngine):
    engine_name = "vip_pricing"
    required_input_fields = ['customer_data', 'vip_config']
    required_output_fields = ['vip_prices', 'tier_benefits']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} vip pricing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} vip pricing: {data}"
