"""Competitor Monitor Engine — memory reader.

Reads past competitor monitoring records from memory storage.
Used to inform current analysis with historical competitor data.
"""
...
def read_past_monitoring(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past competitor monitoring records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_monitoring(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past competitor monitoring records."""
    if not records:
        return {
            "avg_price_changes": 0,
            "avg_new_products": 0,
    ...
    changes = [r.get("price_changes_count", 0) for r in records]
    new_prods = [r.get("new_products_count", 0) for r in records]
    alert_counts = [r.get("alerts_count", 0) for r in records]

    n = len(records)
    return {
        "avg_price_changes": round(sum(changes) / n) if n else 0,
        "avg_new_products": round(sum(new_prods) / n) if n else 0,
        "avg_alerts": round(sum(alert_counts) / n) if n else 0,
        "total_runs": n,
    }
