...
class AiDescriptionWriterEngine(BaseEngine):
    engine_name = "ai_description_writer"
    required_input_fields = ['product_data', 'seo_config']
    required_output_fields = ['descriptions', 'seo_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ai description writer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ai description writer: {data}"
