...
class FeedbackSystemEngine(BaseEngine):
    engine_name = "feedback_system"
    required_input_fields = ['feedback_config', 'collection_channels']
    required_output_fields = ['feedback_flow', 'analysis_pipeline']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} feedback system", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} feedback system: {data}"
