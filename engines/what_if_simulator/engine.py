...
class WhatIfSimulatorEngine(BaseEngine):
    engine_name = "what_if_simulator"
    required_input_fields = ['scenario_data', 'model_config']
    required_output_fields = ['simulation_results', 'sensitivity_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} what if simulator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} what if simulator: {data}"
