...
class AutoPriceAdjusterEngine(BaseEngine):
    engine_name = "auto_price_adjuster"
    required_input_fields = ['product_data', 'rule_config']
    required_output_fields = ['price_changes', 'impact_forecast']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} auto price adjuster", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} auto price adjuster: {data}"
