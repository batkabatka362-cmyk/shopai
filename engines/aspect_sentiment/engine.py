...
class AspectSentimentEngine(BaseEngine):
    engine_name = "aspect_sentiment"
    required_input_fields = ['review_data', 'aspect_config']
    required_output_fields = ['aspect_sentiments', 'summary_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} aspect sentiment", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} aspect sentiment: {data}"
