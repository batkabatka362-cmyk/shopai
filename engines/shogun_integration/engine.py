...
class ShogunIntegrationEngine(BaseEngine):
    engine_name = "shogun_integration"
    required_input_fields = ['page_data', 'integration_config']
    required_output_fields = ['shogun_sync', 'component_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shogun integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shogun integration: {data}"
