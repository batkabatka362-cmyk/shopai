...
class MarketOpportunityAiEngine(BaseEngine):
    engine_name = "market_opportunity_ai"
    required_input_fields = ['market_data', 'trend_data']
    required_output_fields = ['opportunities', 'scoring']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} market opportunity ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} market opportunity ai: {data}"
