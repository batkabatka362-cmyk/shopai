...
class ForceMajeureEngine(BaseEngine):
    engine_name = "force_majeure"
    required_input_fields = ['event_data', 'fm_config']
    required_output_fields = ['fm_notice', 'obligation_suspension']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} force majeure", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} force majeure: {data}"
