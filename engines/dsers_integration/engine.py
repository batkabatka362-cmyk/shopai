...
class DsersIntegrationEngine(BaseEngine):
    engine_name = "dsers_integration"
    required_input_fields = ['product_data', 'integration_config']
    required_output_fields = ['dsers_sync', 'aliexpress_mapping']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dsers integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dsers integration: {data}"
