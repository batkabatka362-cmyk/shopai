...
class SellerSupportEngine(BaseEngine):
    engine_name = "seller_support"
    required_input_fields = ['support_request', 'support_config']
    required_output_fields = ['support_response', 'escalation_path']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} seller support", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} seller support: {data}"
