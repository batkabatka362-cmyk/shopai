...
class GrantWritingEngine(BaseEngine):
    engine_name = "grant_writing"
    required_input_fields = ['project_data', 'grant_config']
    required_output_fields = ['grant_proposal', 'budget_narrative']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} grant writing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} grant writing: {data}"
