...
class AutoEmailSenderEngine(BaseEngine):
    engine_name = "auto_email_sender"
    required_input_fields = ['trigger_data', 'template_data']
    required_output_fields = ['email_queue', 'send_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} auto email sender", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} auto email sender: {data}"
