...
class FuzzTestingEngine(BaseEngine):
    engine_name = "fuzz_testing"
    required_input_fields = ['input_spec', 'fuzz_config']
    required_output_fields = ['crash_report', 'edge_cases']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} fuzz testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} fuzz testing: {data}"
