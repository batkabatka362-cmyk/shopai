...
class FragileHandlingEngine(BaseEngine):
    engine_name = "fragile_handling"
    required_input_fields = ['product_data', 'handling_config']
    required_output_fields = ['handling_instructions', 'packaging_spec']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} fragile handling", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} fragile handling: {data}"
