...
class ReservationSystemEngine(BaseEngine):
    engine_name = "reservation_system"
    required_input_fields = ['inventory_data', 'reservation_config']
    required_output_fields = ['reservation_flow', 'capacity_management']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} reservation system", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} reservation system: {data}"
