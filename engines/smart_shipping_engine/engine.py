...
class SmartShippingEngineEngine(BaseEngine):
    engine_name = "smart_shipping_engine"
    required_input_fields = ['order_data', 'carrier_data']
    required_output_fields = ['shipping_actions', 'cost_optimization']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} smart shipping engine", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} smart shipping engine: {data}"
