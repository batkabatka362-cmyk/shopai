...
class ConjointAnalysisEngine(BaseEngine):
    engine_name = "conjoint_analysis"
    required_input_fields = ['attribute_data', 'respondent_data']
    required_output_fields = ['utility_scores', 'preference_shares']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} conjoint analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} conjoint analysis: {data}"
