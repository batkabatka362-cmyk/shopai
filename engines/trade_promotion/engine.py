...
class TradePromotionEngine(BaseEngine):
    engine_name = "trade_promotion"
    required_input_fields = ['trade_data', 'promotion_config']
    required_output_fields = ['trade_plan', 'roi_forecast']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} trade promotion", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} trade promotion: {data}"
