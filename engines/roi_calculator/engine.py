...
class RoiCalculatorEngine(BaseEngine):
    engine_name = "roi_calculator"
    required_input_fields = ['investment_data', 'return_data']
    required_output_fields = ['roi_metrics', 'payback_period']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} roi calculator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} roi calculator: {data}"
