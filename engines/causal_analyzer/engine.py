...
class CausalAnalyzerEngine(BaseEngine):
    engine_name = "causal_analyzer"
    required_input_fields = ['data', 'causal_config']
    required_output_fields = ['causal_relationships', 'intervention_effects']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} causal analyzer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} causal analyzer: {data}"
