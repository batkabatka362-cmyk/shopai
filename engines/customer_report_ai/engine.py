...
class CustomerReportAiEngine(BaseEngine):
    engine_name = "customer_report_ai"
    required_input_fields = ['customer_data', 'report_config']
    required_output_fields = ['customer_report', 'segment_analysis']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} customer report ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} customer report ai: {data}"
