...
class InternalSearchEngine(BaseEngine):
    engine_name = "internal_search"
    required_input_fields = ['search_logs', 'optimization_config']
    required_output_fields = ['search_improvements', 'zero_result_fixes']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} internal search", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} internal search: {data}"
