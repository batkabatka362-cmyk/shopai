...
class VideoSeoEngine(BaseEngine):
    engine_name = "video_seo"
    required_input_fields = ['video_data', 'seo_config']
    required_output_fields = ['video_schema', 'optimization_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} video seo", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} video seo: {data}"
