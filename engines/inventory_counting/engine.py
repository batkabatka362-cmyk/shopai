...
class InventoryCountingEngine(BaseEngine):
    engine_name = "inventory_counting"
    required_input_fields = ['inventory_data', 'count_config']
    required_output_fields = ['count_results', 'variance_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} inventory counting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} inventory counting: {data}"
