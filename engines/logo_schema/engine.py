...
class LogoSchemaEngine(BaseEngine):
    engine_name = "logo_schema"
    required_input_fields = ['brand_data', 'schema_config']
    required_output_fields = ['logo_markup', 'image_specs']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} logo schema", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} logo schema: {data}"
