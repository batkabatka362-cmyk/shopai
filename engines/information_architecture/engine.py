"""
InformationArchitecture Engine
...
class InformationArchitectureEngine(BaseEngine):
    engine_name = "information_architecture"
    required_input_fields = ['content_data', 'ia_config']
    required_output_fields = ['site_map', 'card_sort_results']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} information architecture", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} information architecture: {data}"
