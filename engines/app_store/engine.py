...
class AppStoreEngine(BaseEngine):
    engine_name = "app_store"
    required_input_fields = ['app_data', 'store_config']
    required_output_fields = ['store_setup', 'submission_guidelines']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} app store", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} app store: {data}"
