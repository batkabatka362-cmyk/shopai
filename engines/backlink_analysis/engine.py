...
class BacklinkAnalysisEngine(BaseEngine):
    engine_name = "backlink_analysis"
    required_input_fields = ['backlink_data', 'analysis_config']
    required_output_fields = ['backlink_profile', 'quality_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} backlink analysis", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} backlink analysis: {data}"
