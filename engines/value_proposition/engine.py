...
class ValuePropositionEngine(BaseEngine):
    engine_name = "value_proposition"
    required_input_fields = ['product_data', 'customer_needs']
    required_output_fields = ['value_props', 'messaging_framework']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} value proposition", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} value proposition: {data}"
