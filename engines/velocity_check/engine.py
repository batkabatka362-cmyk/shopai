...
class VelocityCheckEngine(BaseEngine):
    engine_name = "velocity_check"
    required_input_fields = ['transaction_data', 'velocity_config']
    required_output_fields = ['velocity_scores', 'throttle_actions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} velocity check", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} velocity check: {data}"
