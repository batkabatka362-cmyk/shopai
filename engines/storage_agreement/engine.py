...
class StorageAgreementEngine(BaseEngine):
    engine_name = "storage_agreement"
    required_input_fields = ['warehouse_data', 'storage_config']
    required_output_fields = ['agreement_draft', 'rate_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} storage agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} storage agreement: {data}"
