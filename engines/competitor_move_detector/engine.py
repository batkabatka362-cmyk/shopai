...
class CompetitorMoveDetectorEngine(BaseEngine):
    engine_name = "competitor_move_detector"
    required_input_fields = ['competitor_data', 'alert_config']
    required_output_fields = ['move_alerts', 'counter_strategies']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} competitor move detector", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} competitor move detector: {data}"
