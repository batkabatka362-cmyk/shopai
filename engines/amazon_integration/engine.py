...
class AmazonIntegrationEngine(BaseEngine):
    engine_name = "amazon_integration"
    required_input_fields = ['product_data', 'amazon_config']
    required_output_fields = ['amazon_listings', 'sync_status']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} amazon integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} amazon integration: {data}"
