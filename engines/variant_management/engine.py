...
class VariantManagementEngine(BaseEngine):
    engine_name = "variant_management"
    required_input_fields = ['product_data', 'variant_config']
    required_output_fields = ['variant_matrix', 'sku_generation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} variant management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} variant management: {data}"
