...
class IntegrationAgreementEngine(BaseEngine):
    engine_name = "integration_agreement"
    required_input_fields = ['integration_data', 'agreement_config']
    required_output_fields = ['integration_terms', 'technical_specs']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} integration agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} integration agreement: {data}"
