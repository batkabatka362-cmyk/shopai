...
class BusinessInsuranceEngine(BaseEngine):
    engine_name = "business_insurance"
    required_input_fields = ['business_data', 'insurance_config']
    required_output_fields = ['coverage_options', 'premium_comparison']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} business insurance", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} business insurance: {data}"
