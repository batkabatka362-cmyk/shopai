...
class VatValidationEngine(BaseEngine):
    engine_name = "vat_validation"
    required_input_fields = ['vat_number', 'validation_config']
    required_output_fields = ['validation_result', 'company_info']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} vat validation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} vat validation: {data}"
