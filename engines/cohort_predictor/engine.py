...
class CohortPredictorEngine(BaseEngine):
    engine_name = "cohort_predictor"
    required_input_fields = ['cohort_data', 'behavior_data']
    required_output_fields = ['cohort_forecast', 'retention_curves']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cohort predictor", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cohort predictor: {data}"
