...
class ConfiguratorEngine(BaseEngine):
    engine_name = "configurator"
    required_input_fields = ['product_options', 'config_rules']
    required_output_fields = ['configurator_setup', 'pricing_logic']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} configurator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} configurator: {data}"
