...
class Customer360DashboardEngine(BaseEngine):
    engine_name = "customer_360_dashboard"
    required_input_fields = ['customer_data', 'dashboard_config']
    required_output_fields = ['dashboard_data', 'kpi_summary']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} customer 360 dashboard", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} customer 360 dashboard: {data}"
