...
class AcceptanceClauseEngine(BaseEngine):
    engine_name = "acceptance_clause"
    required_input_fields = ['product_data', 'acceptance_config']
    required_output_fields = ['acceptance_criteria', 'rejection_procedure']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} acceptance clause", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} acceptance clause: {data}"
