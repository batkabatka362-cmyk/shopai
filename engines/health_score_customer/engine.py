...
class HealthScoreCustomerEngine(BaseEngine):
    engine_name = "health_score_customer"
    required_input_fields = ['customer_data', 'health_config']
    required_output_fields = ['health_scores', 'risk_indicators']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} health score customer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} health score customer: {data}"
