...
class GoalAlignmentEngine(BaseEngine):
    ...
    required_input_fields = ['goal_data', 'alignment_config']
    required_output_fields = ['alignment_map', 'cascade_view']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} goal alignment", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} goal alignment: {data}"
