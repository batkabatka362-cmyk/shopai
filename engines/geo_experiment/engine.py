...
class GeoExperimentEngine(BaseEngine):
    engine_name = "geo_experiment"
    required_input_fields = ['geo_data', 'experiment_config']
    required_output_fields = ['geo_lift', 'market_comparison']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} geo experiment", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} geo experiment: {data}"
