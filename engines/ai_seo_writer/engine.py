...
class AiSeoWriterEngine(BaseEngine):
    engine_name = "ai_seo_writer"
    required_input_fields = ['keyword_data', 'content_config']
    required_output_fields = ['seo_content', 'meta_tags']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ai seo writer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ai seo writer: {data}"
