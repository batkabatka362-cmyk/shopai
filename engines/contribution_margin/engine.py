...
class ContributionMarginEngine(BaseEngine):
    engine_name = "contribution_margin"
    required_input_fields = ['product_data', 'cost_data']
    required_output_fields = ['contribution_analysis', 'product_ranking']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} contribution margin", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} contribution margin: {data}"
