...
class PartialShipmentEngine(BaseEngine):
    engine_name = "partial_shipment"
    required_input_fields = ['order_data', 'partial_config']
    required_output_fields = ['shipment_plan', 'backorder_handling']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} partial shipment", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} partial shipment: {data}"
