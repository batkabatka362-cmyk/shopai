...
class PersonalizationAiEngine(BaseEngine):
    engine_name = "personalization_ai"
    required_input_fields = ['visitor_data', 'behavior_data']
    required_output_fields = ['personalized_experience', 'recommendation_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} personalization ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} personalization ai: {data}"
