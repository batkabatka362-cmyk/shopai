...
class AgeVerificationEngine(BaseEngine):
    engine_name = "age_verification"
    required_input_fields = ['customer_data', 'product_restrictions']
    required_output_fields = ['verification_result', 'restriction_actions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} age verification", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} age verification: {data}"
