...
class CopywritingEngine(BaseEngine):
    engine_name = "copywriting"
    required_input_fields = ['brief_data', 'tone_config']
    required_output_fields = ['copy_variants', 'headline_options']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} copywriting", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} copywriting: {data}"
