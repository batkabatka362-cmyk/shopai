...
class PledgeAgreementEngine(BaseEngine):
    engine_name = "pledge_agreement"
    required_input_fields = ['asset_data', 'pledge_config']
    required_output_fields = ['pledge_draft', 'enforcement_rights']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} pledge agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} pledge agreement: {data}"
