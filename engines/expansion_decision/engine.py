...
class ExpansionDecisionEngine(BaseEngine):
    engine_name = "expansion_decision"
    required_input_fields = ['market_data', 'capacity_data']
    required_output_fields = ['expansion_recommendation', 'resource_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} expansion decision", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} expansion decision: {data}"
