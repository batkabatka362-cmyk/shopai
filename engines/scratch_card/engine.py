...
class ScratchCardEngine(BaseEngine):
    engine_name = "scratch_card"
    required_input_fields = ['prize_data', 'card_config']
    required_output_fields = ['card_setup', 'redemption_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} scratch card", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} scratch card: {data}"
