...
class CrossDockEngine(BaseEngine):
    engine_name = "cross_dock"
    required_input_fields = ['shipment_data', 'dock_config']
    required_output_fields = ['dock_plan', 'timing_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cross dock", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cross dock: {data}"
