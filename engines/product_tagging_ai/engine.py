"""
ProductTaggingAi Engine — Product Tagging Ai
...
class ProductTaggingAiEngine(BaseEngine):
    engine_name = "product_tagging_ai"
    required_input_fields = ['product_data', 'taxonomy']
    required_output_fields = ['auto_tags', 'confidence_scores']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} product tagging ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} product tagging ai: {data}"
