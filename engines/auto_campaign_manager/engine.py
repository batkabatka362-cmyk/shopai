...
class AutoCampaignManagerEngine(BaseEngine):
    engine_name = "auto_campaign_manager"
    required_input_fields = ['campaign_data', 'budget_data']
    required_output_fields = ['campaign_schedule', 'performance_targets']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} auto campaign manager", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} auto campaign manager: {data}"
