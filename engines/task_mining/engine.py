...
class TaskMiningEngine(BaseEngine):
    engine_name = "task_mining"
    required_input_fields = ['activity_data', 'mining_config']
    required_output_fields = ['task_patterns', 'automation_candidates']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} task mining", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} task mining: {data}"
