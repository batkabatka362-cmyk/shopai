...
class DueDiligenceEngine(BaseEngine):
    engine_name = "due_diligence"
    required_input_fields = ['company_data', 'diligence_config']
    required_output_fields = ['diligence_report', 'risk_findings']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} due diligence", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} due diligence: {data}"
