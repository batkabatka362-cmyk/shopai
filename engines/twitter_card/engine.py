...
class TwitterCardEngine(BaseEngine):
    engine_name = "twitter_card"
    required_input_fields = ['page_data', 'card_config']
    required_output_fields = ['card_tags', 'preview_data']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} twitter card", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} twitter card: {data}"
