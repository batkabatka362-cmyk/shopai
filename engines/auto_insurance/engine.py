...
class AutoInsuranceEngine(BaseEngine):
    engine_name = "auto_insurance"
    required_input_fields = ['employee_data', 'auto_config']
    required_output_fields = ['group_rate', 'enrollment_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} auto insurance", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} auto insurance: {data}"
