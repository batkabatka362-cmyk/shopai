...
class SoundDesignEngine(BaseEngine):
    engine_name = "sound_design"
    required_input_fields = ['brand_data', 'sound_config']
    required_output_fields = ['sound_brief', 'audio_guidelines']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} sound design", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} sound design: {data}"
