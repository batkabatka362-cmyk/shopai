...
class AiEmailWriterEngine(BaseEngine):
    engine_name = "ai_email_writer"
    required_input_fields = ['campaign_data', 'audience_data']
    required_output_fields = ['email_drafts', 'subject_lines']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ai email writer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ai email writer: {data}"
