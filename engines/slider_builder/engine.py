...
class SliderBuilderEngine(BaseEngine):
    engine_name = "slider_builder"
    required_input_fields = ['slide_data', 'slider_config']
    required_output_fields = ['slider_setup', 'animation_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} slider builder", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} slider builder: {data}"
