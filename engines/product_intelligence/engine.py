...
class ProductIntelligenceEngine(BaseEngine):
    engine_name = "product_intelligence"
    required_input_fields = ['product_data', 'market_data']
    required_output_fields = ['product_score', 'lifecycle_stage']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} product intelligence", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} product intelligence: {data}"
