...
class ResponsiveDesignEngine(BaseEngine):
    engine_name = "responsive_design"
    required_input_fields = ['page_data', 'responsive_config']
    required_output_fields = ['breakpoint_specs', 'layout_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} responsive design", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} responsive design: {data}"
