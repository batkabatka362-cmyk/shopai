...
class PrivacyPageEngine(BaseEngine):
    engine_name = "privacy_page"
    required_input_fields = ['policy_data', 'page_config']
    required_output_fields = ['policy_content', 'consent_manager']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} privacy page", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} privacy page: {data}"
