...
class ShippingPageEngine(BaseEngine):
    engine_name = "shipping_page"
    required_input_fields = ['shipping_data', 'page_config']
    required_output_fields = ['shipping_content', 'rate_calculator']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shipping page", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shipping page: {data}"
