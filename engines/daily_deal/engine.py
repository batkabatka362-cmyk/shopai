...
class DailyDealEngine(BaseEngine):
    engine_name = "daily_deal"
    required_input_fields = ['product_pool', 'deal_config']
    required_output_fields = ['daily_selection', 'promotion_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} daily deal", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} daily deal: {data}"
