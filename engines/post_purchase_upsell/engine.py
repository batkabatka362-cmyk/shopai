...
class PostPurchaseUpsellEngine(BaseEngine):
    engine_name = "post_purchase_upsell"
    required_input_fields = ['order_data', 'upsell_config']
    required_output_fields = ['upsell_offer', 'one_click_add']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} post purchase upsell", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} post purchase upsell: {data}"
