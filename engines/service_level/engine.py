...
class ServiceLevelEngine(BaseEngine):
    engine_name = "service_level"
    required_input_fields = ['performance_data', 'sla_config']
    required_output_fields = ['sla_report', 'breach_prevention']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} service level", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} service level: {data}"
