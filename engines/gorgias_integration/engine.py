...
class GorgiasIntegrationEngine(BaseEngine):
    engine_name = "gorgias_integration"
    required_input_fields = ['support_data', 'integration_config']
    required_output_fields = ['gorgias_sync', 'macro_config']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} gorgias integration", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} gorgias integration: {data}"
