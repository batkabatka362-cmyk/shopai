...
class CampaignAttributionEngine(BaseEngine):
    engine_name = "campaign_attribution"
    required_input_fields = ['conversion_data', 'attribution_config']
    required_output_fields = ['attribution_report', 'channel_credit']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} campaign attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} campaign attribution: {data}"
