...
class JointVentureEngine(BaseEngine):
    engine_name = "joint_venture"
    required_input_fields = ['partner_data', 'venture_config']
    required_output_fields = ['venture_plan', 'profit_sharing']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} joint venture", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} joint venture: {data}"
