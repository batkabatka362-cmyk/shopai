...
class ProductRecallEngine(BaseEngine):
    engine_name = "product_recall"
    required_input_fields = ['recall_data', 'customer_orders']
    required_output_fields = ['recall_plan', 'notification_list']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} product recall", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} product recall: {data}"
