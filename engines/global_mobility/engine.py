...
class GlobalMobilityEngine(BaseEngine):
    engine_name = "global_mobility"
    required_input_fields = ['assignment_data', 'mobility_config']
    required_output_fields = ['mobility_package', 'tax_equalization']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} global mobility", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} global mobility: {data}"
