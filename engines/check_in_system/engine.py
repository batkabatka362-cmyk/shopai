...
class CheckInSystemEngine(BaseEngine):
    engine_name = "check_in_system"
    required_input_fields = ['attendee_data', 'checkin_config']
    required_output_fields = ['checkin_flow', 'badge_printing']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} check in system", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} check in system: {data}"
