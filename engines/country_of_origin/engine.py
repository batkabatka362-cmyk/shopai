...
class CountryOfOriginEngine(BaseEngine):
    engine_name = "country_of_origin"
    required_input_fields = ['product_data', 'origin_config']
    required_output_fields = ['origin_labeling', 'regulatory_requirements']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} country of origin", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} country of origin: {data}"
