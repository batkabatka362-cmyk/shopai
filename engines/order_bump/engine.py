...
class OrderBumpEngine(BaseEngine):
    engine_name = "order_bump"
    required_input_fields = ['cart_data', 'bump_config']
    required_output_fields = ['bump_offer', 'placement_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} order bump", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} order bump: {data}"
