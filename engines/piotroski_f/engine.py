...
class PiotroskiFEngine(BaseEngine):
    engine_name = "piotroski_f"
    required_input_fields = ['financial_data', 'f_config']
    required_output_fields = ['f_score', 'fundamental_assessment']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} piotroski f", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} piotroski f: {data}"
