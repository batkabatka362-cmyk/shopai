...
class NdaGenerationEngine(BaseEngine):
    engine_name = "nda_generation"
    required_input_fields = ['party_data', 'nda_config']
    required_output_fields = ['nda_document', 'terms_summary']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} nda generation", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} nda generation: {data}"
