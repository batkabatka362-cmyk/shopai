...
class HeadlessCheckoutEngine(BaseEngine):
    engine_name = "headless_checkout"
    required_input_fields = ['checkout_data', 'headless_config']
    required_output_fields = ['checkout_api', 'payment_flow']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} headless checkout", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} headless checkout: {data}"
