"""
CrossPlatformAttribution Engine
...
class CrossPlatformAttributionEngine(BaseEngine):
    engine_name = "cross_platform_attribution"
    required_input_fields = ['platform_data', 'attribution_config']
    required_output_fields = ['unified_attribution', 'platform_contribution']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cross platform attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cross platform attribution: {data}"
