...
class OfficeSpaceEngine(BaseEngine):
    engine_name = "office_space"
    required_input_fields = ['space_data', 'planning_config']
    required_output_fields = ['space_plan', 'utilization_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} office space", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} office space: {data}"
