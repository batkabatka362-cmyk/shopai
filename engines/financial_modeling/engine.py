...
class FinancialModelingEngine(BaseEngine):
    engine_name = "financial_modeling"
    required_input_fields = ['financial_data', 'model_config']
    required_output_fields = ['financial_model', 'projections']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} financial modeling", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} financial modeling: {data}"
