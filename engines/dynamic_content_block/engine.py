...
class DynamicContentBlockEngine(BaseEngine):
    engine_name = "dynamic_content_block"
    required_input_fields = ['content_data', 'block_config']
    required_output_fields = ['content_variants', 'targeting_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dynamic content block", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dynamic content block: {data}"
