...
class CommissionSplitEngine(BaseEngine):
    engine_name = "commission_split"
    required_input_fields = ['transaction_data', 'split_config']
    required_output_fields = ['split_calculations', 'payout_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} commission split", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} commission split: {data}"
