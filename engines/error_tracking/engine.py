...
class ErrorTrackingEngine(BaseEngine):
    engine_name = "error_tracking"
    required_input_fields = ['error_data', 'tracking_config']
    required_output_fields = ['error_report', 'resolution_priorities']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} error tracking", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} error tracking: {data}"
