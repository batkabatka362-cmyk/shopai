...
class DdosProtectionEngine(BaseEngine):
    engine_name = "ddos_protection"
    required_input_fields = ['traffic_data', 'protection_config']
    required_output_fields = ['protection_plan', 'mitigation_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} ddos protection", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} ddos protection: {data}"
