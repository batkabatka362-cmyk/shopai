...
class SaasAgreementEngine(BaseEngine):
    engine_name = "saas_agreement"
    required_input_fields = ['saas_data', 'agreement_config']
    required_output_fields = ['agreement_terms', 'usage_limits']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} saas agreement", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} saas agreement: {data}"
