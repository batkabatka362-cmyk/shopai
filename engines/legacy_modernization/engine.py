...
class LegacyModernizationEngine(BaseEngine):
    engine_name = "legacy_modernization"
    required_input_fields = ['legacy_code', 'modernization_config']
    required_output_fields = ['modernization_plan', 'refactored_code']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} legacy modernization", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} legacy modernization: {data}"
