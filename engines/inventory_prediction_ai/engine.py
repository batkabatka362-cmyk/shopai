...
class InventoryPredictionAiEngine(BaseEngine):
    engine_name = "inventory_prediction_ai"
    required_input_fields = ['inventory_data', 'demand_data']
    required_output_fields = ['stock_forecast', 'reorder_alerts']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} inventory prediction ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} inventory prediction ai: {data}"
