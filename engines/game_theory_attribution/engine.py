...
class GameTheoryAttributionEngine(BaseEngine):
    engine_name = "game_theory_attribution"
    required_input_fields = ['touchpoint_data', 'game_config']
    required_output_fields = ['coalition_values', 'fair_allocation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} game theory attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} game theory attribution: {data}"
