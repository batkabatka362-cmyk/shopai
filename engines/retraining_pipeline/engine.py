...
class RetrainingPipelineEngine(BaseEngine):
    engine_name = "retraining_pipeline"
    required_input_fields = ['trigger_data', 'pipeline_config']
    required_output_fields = ['retrain_plan', 'schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} retraining pipeline", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} retraining pipeline: {data}"
