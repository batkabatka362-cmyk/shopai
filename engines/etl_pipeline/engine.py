...
class EtlPipelineEngine(BaseEngine):
    engine_name = "etl_pipeline"
    required_input_fields = ['source_config', 'transform_rules']
    required_output_fields = ['pipeline_config', 'schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} etl pipeline", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} etl pipeline: {data}"
