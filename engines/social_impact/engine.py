...
class SocialImpactEngine(BaseEngine):
    engine_name = "social_impact"
    required_input_fields = ['program_data', 'impact_config']
    required_output_fields = ['impact_report', 'sdg_alignment']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} social impact", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} social impact: {data}"
