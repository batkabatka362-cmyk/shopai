"""Browse Recovery Engine — memory reader.

Reads past browse recovery records from memory storage.
Used to inform current recovery analysis with historical context.
"""
...
def read_past_recoveries(
    limit: int = 10,
) -> dict[str, Any]:
    """Read past browse recovery records.
...
def find_similar_past_run(input_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    result = read_past_recoveries(limit=50)
...
def _compute_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute summary statistics over past browse recovery records."""
    if not records:
        return {
            "avg_recoverable": 0,
            "avg_estimated_revenue": 0.0,
            "avg_targets_count": 0,
            "total_runs": 0,
        }

    recoverables = [r.get("total_recoverable", 0) for r in records]
    revenues = [r.get("estimated_revenue", 0.0) for r in records]
    targets = [r.get("targets_count", 0) for r in records]

    n = len(records)
    return {
        "avg_recoverable": round(sum(recoverables) / n) if n else 0,
        "avg_estimated_revenue": round(sum(revenues) / n, 2) if n else 0.0,
        "avg_targets_count": round(sum(targets) / n) if n else 0,
        "total_runs": n,
    }
