...
class BacklogManagementEngine(BaseEngine):
    engine_name = "backlog_management"
    required_input_fields = ['ticket_data', 'management_config']
    required_output_fields = ['backlog_report', 'priority_queue']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} backlog management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} backlog management: {data}"
