...
class DatasetSchemaEngine(BaseEngine):
    engine_name = "dataset_schema"
    required_input_fields = ['dataset_data', 'schema_config']
    required_output_fields = ['dataset_markup', 'distribution_info']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dataset schema", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dataset schema: {data}"
