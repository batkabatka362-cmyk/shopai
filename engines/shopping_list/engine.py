...
class ShoppingListEngine(BaseEngine):
    engine_name = "shopping_list"
    required_input_fields = ['list_data', 'product_catalog']
    required_output_fields = ['enriched_list', 'purchase_links']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} shopping list", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} shopping list: {data}"
