...
class ConvertibleNoteEngine(BaseEngine):
    engine_name = "convertible_note"
    required_input_fields = ['note_data', 'conversion_config']
    required_output_fields = ['conversion_analysis', 'dilution_impact']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} convertible note", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} convertible note: {data}"
