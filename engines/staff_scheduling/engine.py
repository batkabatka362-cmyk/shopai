...
class StaffSchedulingEngine(BaseEngine):
    engine_name = "staff_scheduling"
    required_input_fields = ['demand_data', 'staff_config']
    required_output_fields = ['schedule', 'coverage_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} staff scheduling", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} staff scheduling: {data}"
