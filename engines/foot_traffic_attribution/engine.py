...
class FootTrafficAttributionEngine(BaseEngine):
    engine_name = "foot_traffic_attribution"
    required_input_fields = ['visit_data', 'attribution_config']
    required_output_fields = ['attribution_report', 'store_visit_lift']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} foot traffic attribution", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} foot traffic attribution: {data}"
