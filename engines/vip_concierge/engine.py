...
class VipConciergeEngine(BaseEngine):
    engine_name = "vip_concierge"
    required_input_fields = ['vip_data', 'concierge_config']
    required_output_fields = ['concierge_plan', 'priority_queue']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} vip concierge", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} vip concierge: {data}"
