"""Feedback Collection Engine — memory writer."""
...
def write_feedback_result(sentiment_summary: dict[str, Any], insights_count: int) -> dict[str, Any]:
    try:
        record_id = uuid.uuid4().hex[:12]
        timestamp = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
        record = {
            "record_id": record_id, "timestamp": timestamp,
            "avg_rating": sentiment_summary.get("avg_rating", 0),
