...
class NetZeroEngine(BaseEngine):
    engine_name = "net_zero"
    required_input_fields = ['emission_data', 'net_zero_config']
    required_output_fields = ['roadmap', 'milestone_tracking']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} net zero", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} net zero: {data}"
