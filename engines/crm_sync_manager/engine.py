...
class CrmSyncManagerEngine(BaseEngine):
    engine_name = "crm_sync_manager"
    required_input_fields = ['crm_data', 'sync_config']
    required_output_fields = ['sync_plan', 'field_mapping']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} crm sync manager", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} crm sync manager: {data}"
