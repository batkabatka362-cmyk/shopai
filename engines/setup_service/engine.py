...
class SetupServiceEngine(BaseEngine):
    engine_name = "setup_service"
    required_input_fields = ['product_data', 'setup_config']
    required_output_fields = ['setup_options', 'scheduling']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} setup service", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} setup service: {data}"
