...
class DeliveryExperienceEngine(BaseEngine):
    engine_name = "delivery_experience"
    required_input_fields = ['delivery_data', 'experience_config']
    required_output_fields = ['experience_score', 'improvement_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} delivery experience", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} delivery experience: {data}"
