...
class GreenLogisticsEngine(BaseEngine):
    engine_name = "green_logistics"
    required_input_fields = ['logistics_data', 'green_config']
    required_output_fields = ['green_plan', 'emission_reduction']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} green logistics", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} green logistics: {data}"
