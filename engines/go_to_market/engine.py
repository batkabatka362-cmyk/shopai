...
class GoToMarketEngine(BaseEngine):
    engine_name = "go_to_market"
    required_input_fields = ['product_data', 'market_data']
    required_output_fields = ['gtm_plan', 'launch_timeline']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} go to market", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} go to market: {data}"
