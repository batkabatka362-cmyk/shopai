...
class HandwrittenNoteEngine(BaseEngine):
    engine_name = "handwritten_note"
    required_input_fields = ['order_data', 'note_config']
    required_output_fields = ['note_content', 'personalization']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} handwritten note", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} handwritten note: {data}"
