...
class DependencyManagementEngine(BaseEngine):
    ...
    required_input_fields = ['dependency_data', 'update_config']
    required_output_fields = ['update_plan', 'vulnerability_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} dependency management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} dependency management: {data}"
