...
class TerritoryManagementEngine(BaseEngine):
    engine_name = "territory_management"
    required_input_fields = ['sales_data', 'territory_config']
    required_output_fields = ['territory_map', 'assignment_plan']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} territory management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} territory management: {data}"
