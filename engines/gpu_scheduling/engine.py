...
class GpuSchedulingEngine(BaseEngine):
    engine_name = "gpu_scheduling"
    required_input_fields = ['job_queue', 'gpu_config']
    required_output_fields = ['schedule', 'utilization_forecast']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} gpu scheduling", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} gpu scheduling: {data}"
