...
class EntitySeoEngine(BaseEngine):
    engine_name = "entity_seo"
    required_input_fields = ['entity_data', 'seo_config']
    required_output_fields = ['entity_optimization', 'knowledge_panel']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} entity seo", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} entity seo: {data}"
