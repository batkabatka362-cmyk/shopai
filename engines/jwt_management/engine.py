...
class JwtManagementEngine(BaseEngine):
    engine_name = "jwt_management"
    required_input_fields = ['jwt_config', 'key_data']
    required_output_fields = ['jwt_setup', 'rotation_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} jwt management", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} jwt management: {data}"
