...
class CanonicalUrlEngine(BaseEngine):
    engine_name = "canonical_url"
    required_input_fields = ['page_data', 'canonical_config']
    required_output_fields = ['canonical_urls', 'duplicate_resolution']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} canonical url", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} canonical url: {data}"
