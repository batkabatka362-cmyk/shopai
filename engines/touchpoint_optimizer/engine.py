...
class TouchpointOptimizerEngine(BaseEngine):
    engine_name = "touchpoint_optimizer"
    required_input_fields = ['touchpoint_data', 'conversion_data']
    required_output_fields = ['touchpoint_improvements', 'priority_list']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} touchpoint optimizer", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} touchpoint optimizer: {data}"
