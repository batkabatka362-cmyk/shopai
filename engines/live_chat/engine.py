...
class LiveChatEngine(BaseEngine):
    engine_name = "live_chat"
    required_input_fields = ['chat_data', 'routing_config']
    required_output_fields = ['chat_response', 'agent_assignment']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} live chat", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} live chat: {data}"
