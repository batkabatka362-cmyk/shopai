...
class DeliveryZoneEngine(BaseEngine):
    engine_name = "delivery_zone"
    required_input_fields = ['geo_data', 'zone_config']
    required_output_fields = ['zone_definitions', 'rate_structure']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} delivery zone", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} delivery zone: {data}"
