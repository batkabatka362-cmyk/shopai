...
class DeferredCompEngine(BaseEngine):
    engine_name = "deferred_comp"
    required_input_fields = ['comp_data', 'deferral_config']
    required_output_fields = ['deferral_plan', 'payout_schedule']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} deferred comp", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} deferred comp: {data}"
