...
class DemandGenCampaignEngine(BaseEngine):
    engine_name = "demand_gen_campaign"
    required_input_fields = ['campaign_data', 'demandgen_config']
    required_output_fields = ['campaign_setup', 'audience_signal']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} demand gen campaign", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} demand gen campaign: {data}"
