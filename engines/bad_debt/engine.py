...
class BadDebtEngine(BaseEngine):
    engine_name = "bad_debt"
    required_input_fields = ['ar_data', 'writeoff_config']
    required_output_fields = ['writeoff_candidates', 'provision_calc']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} bad debt", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} bad debt: {data}"
