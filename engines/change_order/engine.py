...
class ChangeOrderEngine(BaseEngine):
    engine_name = "change_order"
    required_input_fields = ['change_data', 'order_config']
    required_output_fields = ['change_order_draft', 'impact_assessment']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} change order", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} change order: {data}"
