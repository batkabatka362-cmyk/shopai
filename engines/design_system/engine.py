...
class DesignSystemEngine(BaseEngine):
    engine_name = "design_system"
    required_input_fields = ['brand_data', 'system_config']
    required_output_fields = ['component_library', 'design_tokens']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} design system", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} design system: {data}"
