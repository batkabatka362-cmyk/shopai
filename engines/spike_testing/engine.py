...
class SpikeTestingEngine(BaseEngine):
    engine_name = "spike_testing"
    required_input_fields = ['system_data', 'spike_config']
    required_output_fields = ['spike_results', 'recovery_time']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} spike testing", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} spike testing: {data}"
