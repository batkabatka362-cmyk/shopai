...
class CancellationFlowEngine(BaseEngine):
    engine_name = "cancellation_flow"
    required_input_fields = ['subscription_data', 'cancellation_config']
    required_output_fields = ['cancellation_flow', 'save_offers']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cancellation flow", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cancellation flow: {data}"
