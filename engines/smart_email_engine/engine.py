...
class SmartEmailEngineEngine(BaseEngine):
    engine_name = "smart_email_engine"
    required_input_fields = ['email_data', 'segment_data']
    required_output_fields = ['email_actions', 'send_optimization']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} smart email engine", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} smart email engine: {data}"
