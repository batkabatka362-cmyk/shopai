...
class MobileRankEngine(BaseEngine):
    engine_name = "mobile_rank"
    required_input_fields = ['keyword_data', 'mobile_config']
    required_output_fields = ['mobile_rankings', 'desktop_comparison']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} mobile rank", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} mobile rank: {data}"
