...
class SafetyComplianceEngine(BaseEngine):
    engine_name = "safety_compliance"
    required_input_fields = ['product_data', 'safety_standards']
    required_output_fields = ['compliance_status', 'certification_needs']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} safety compliance", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} safety compliance: {data}"
