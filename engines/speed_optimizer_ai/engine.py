...
class SpeedOptimizerAiEngine(BaseEngine):
    engine_name = "speed_optimizer_ai"
    required_input_fields = ['performance_data', 'page_data']
    required_output_fields = ['speed_fixes', 'performance_forecast']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} speed optimizer ai", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} speed optimizer ai: {data}"
