...
class PlanogramEngine(BaseEngine):
    engine_name = "planogram"
    required_input_fields = ['product_data', 'shelf_config']
    required_output_fields = ['shelf_layout', 'space_allocation']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} planogram", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} planogram: {data}"
