...
class OrderPromisingEngine(BaseEngine):
    engine_name = "order_promising"
    required_input_fields = ['order_data', 'fulfillment_config']
    required_output_fields = ['promise_date', 'fulfillment_source']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} order promising", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} order promising: {data}"
