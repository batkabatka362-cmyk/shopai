...
class ColorPaletteGeneratorEngine(BaseEngine):
    engine_name = "color_palette_generator"
    required_input_fields = ['brand_data', 'palette_config']
    required_output_fields = ['color_palette', 'usage_guidelines']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} color palette generator", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} color palette generator: {data}"
