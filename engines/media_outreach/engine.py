...
class MediaOutreachEngine(BaseEngine):
    engine_name = "media_outreach"
    required_input_fields = ['pitch_data', 'media_list']
    required_output_fields = ['outreach_plan', 'pitch_templates']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} media outreach", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} media outreach: {data}"
