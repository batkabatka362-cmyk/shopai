...
class RealTimeFraudEngine(BaseEngine):
    engine_name = "real_time_fraud"
    required_input_fields = ['transaction_data', 'risk_config']
    required_output_fields = ['risk_scores', 'block_decisions']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} real time fraud", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} real time fraud: {data}"
