...
class SmartPricingEngineEngine(BaseEngine):
    engine_name = "smart_pricing_engine"
    required_input_fields = ['product_data', 'market_data']
    required_output_fields = ['smart_prices', 'adjustment_rules']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} smart pricing engine", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} smart pricing engine: {data}"
