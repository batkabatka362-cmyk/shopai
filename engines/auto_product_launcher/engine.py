...
class AutoProductLauncherEngine(BaseEngine):
    engine_name = "auto_product_launcher"
    required_input_fields = ['product_data', 'launch_config']
    required_output_fields = ['launch_plan', 'timeline']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} auto product launcher", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} auto product launcher: {data}"
