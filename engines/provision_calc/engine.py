...
class ProvisionCalcEngine(BaseEngine):
    engine_name = "provision_calc"
    required_input_fields = ['financial_data', 'provision_config']
    required_output_fields = ['provision_amount', 'methodology_report']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} provision calc", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} provision calc: {data}"
