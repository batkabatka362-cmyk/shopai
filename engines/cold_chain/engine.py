...
class ColdChainEngine(BaseEngine):
    engine_name = "cold_chain"
    required_input_fields = ['shipment_data', 'temperature_config']
    required_output_fields = ['monitoring_plan', 'compliance_checks']
    ...
    def define_steps(self) -> None:
        for step, role, req in [("analyze","analyzer",True),("execute","worker",True),("enhance","creative",False),("validate","validator",True)]:
            self.flow.add_step(EngineStep(name=step, model_role=role, description=f"{step} cold chain", required=req, stop_on_reject=req))
    ...
    def _build_prompt(self, step: str, data: dict[str, Any]) -> str:
        return f"{step.upper()} cold chain: {data}"
