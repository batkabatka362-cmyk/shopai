"""CrmBaseAdapter — shared base for CRM adapters.

Concrete adapters (HubSpot today; Salesforce / Pipedrive later)
inherit this class and override vendor-specific hooks.

Capabilities:
  * CRM_SEARCH_CONTACTS  — find contacts by email / name / prop
  * CRM_UPSERT_CONTACT   — idempotent create-or-update of a contact
  * CRM_LIST_DEALS       — list open / recent deals
  * CRM_CREATE_DEAL      — open a new deal / opportunity
"""
...
logger = get_logger("adapters.crm")


class CrmBaseAdapter(BaseAdapter):
    """Abstract base for CRM adapters."""

    category = AdapterCategory.CRM
    capabilities = {
        Capability.CRM_SEARCH_CONTACTS,
        Capability.CRM_UPSERT_CONTACT,
        Capability.CRM_LIST_DEALS,
    ...
    def _execute(
        ...
        dispatch = {
            Capability.CRM_SEARCH_CONTACTS: self._do_search_contacts,
            Capability.CRM_UPSERT_CONTACT: self._do_upsert_contact,
            Capability.CRM_LIST_DEALS: self._do_list_deals,
            Capability.CRM_CREATE_DEAL: self._do_create_deal,
        }
