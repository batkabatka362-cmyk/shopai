"""Product-reviews adapter bootstrap.

Single entry point that instantiates every reviews adapter and
registers it with ``AdapterRegistry``. Mirrors the LLM /
Email / Shopify / search / shipping / payment / image / sms /
scraper / translation / image_cdn bootstraps so the controller
can call all of them at startup with the same idempotency
guarantees.
...
Judge.me requires ``JUDGEME_API_TOKEN``. The adapter still
registers when the token is unset; the smart router silently
skips it via ``is_configured()`` so the brain transparently
picks a different reviews vendor (or surfaces
``NoAdapterAvailable`` when no adapter satisfies the
capability).
"""
...
logger = get_logger("adapters.reviews.bootstrap")


_REVIEWS_ADAPTER_CLASSES = (
    JudgeMeAdapter,
)
...
def register_all(
    ...
    """Instantiate every reviews adapter and register it.
    ...
    logger.info(
        "Reviews adapters registered: %d total, %d configured",
