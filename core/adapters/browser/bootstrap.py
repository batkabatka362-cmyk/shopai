"""Browser adapter bootstrap.

Single entry point that instantiates every browser adapter and
registers it with ``AdapterRegistry``.

Usage::

    from core.adapters.browser.bootstrap import register_all
    register_all()

Playwright requires ``playwright install chromium`` to download
the browser binary. The adapter still registers when Playwright
is not installed; the smart router skips it via
``is_configured()``.
"""
from __future__ import annotations

from utils.logger import get_logger

from ..registry import AdapterRegistry, get_registry
from .playwright import PlaywrightAdapter

logger = get_logger("adapters.browser.bootstrap")


_BROWSER_ADAPTER_CLASSES = (
    PlaywrightAdapter,
)


def register_all(
    registry: AdapterRegistry | None = None,
) -> dict[str, bool]:
    """Instantiate every browser adapter and register it.

    Returns a ``{adapter_name: is_configured}`` map.
    """
    reg = registry or get_registry()
    status: dict[str, bool] = {}

    for cls in _BROWSER_ADAPTER_CLASSES:
        try:
            adapter = cls()
        except Exception as exc:  # noqa: BLE001
            logger.warning(
                "failed to instantiate %s: %s", cls.__name__, exc,
            )
            continue

        try:
            reg.register(adapter, replace=True)
            status[adapter.name] = adapter.is_configured()
        except Exception as exc:  # noqa: BLE001
            logger.warning(
                "failed to register %s: %s", adapter.name, exc,
            )

    configured = sum(1 for v in status.values() if v)
    logger.info(
        "Browser adapters registered: %d total, %d configured",
        len(status), configured,
    )
    return status
