"""SubscriptionBaseAdapter — shared base for subscription adapters.

Concrete adapters (ReCharge today; Bold, Seal later) inherit this
class and override vendor-specific hooks.

Capabilities:
  * SUBSCRIPTION_MANAGE — CRUD operations on subscriptions
"""
...
logger = get_logger("adapters.subscription")


class SubscriptionBaseAdapter(BaseAdapter):
    """Abstract base for subscription management adapters."""

    category = AdapterCategory.SUBSCRIPTION
    capabilities = {Capability.SUBSCRIPTION_MANAGE}
    ...
    def _execute(
        ...
        if capability != Capability.SUBSCRIPTION_MANAGE:
            raise AdapterValidationError(
                self.name, f"unsupported capability: {capability.value}",
            )
        action = params.get("action", "list")
        dispatch = {
            "list": self._list_subscriptions,
            "get": self._get_subscription,
            "create": self._create_subscription,
            "update": self._update_subscription,
            "cancel": self._cancel_subscription,
        }
        handler = dispatch.get(action)
        if handler is None:
            raise AdapterValidationError(
                self.name, f"unknown action: {action}",
            )
        ...
        *,
        ...
        try:
            if method.upper() == "GET":
                ...
                response = _requests.delete(url, headers=hdrs, timeout=self.timeout)
            else:
