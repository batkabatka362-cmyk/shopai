...
class RevenueTracker:
    ...
    def get_roi_summary(self) -> dict[str, Any]:
        ...
        total_revenue = sum(e.get("revenue", 0) for e in with_revenue)
        total_cost = sum(e.get("cost", 0) for e in with_revenue)
        total_orders = sum(e.get("orders", 0) for e in with_revenue)
        ...
        for e in with_revenue:
            ...
            if t not in by_type:
                by_type[t] = {"revenue": 0, "cost": 0, "count": 0}
            by_type[t]["revenue"] += e.get("revenue", 0)
            by_type[t]["cost"] += e.get("cost", 0)
            by_type[t]["count"] += 1
    ...
    def get_product_performance(self) -> list[dict[str, Any]]:
        ...
        for e in entries:
            ...
            if p not in products:
                products[p] = {"product": p, "revenue": 0, "cost": 0, "actions": 0}
            products[p]["revenue"] += e.get("revenue", 0)
            products[p]["cost"] += e.get("cost", 0)
    ...
    def _top_product(entries: list[dict]) -> dict[str, Any] | None:
        ...
        for e in entries:
            p = e.get("product", "unknown")
            products[p] = products.get(p, 0) + e.get("revenue", 0)
        if not products:
            return None
