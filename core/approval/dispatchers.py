...
# ── warranty → SHOPIFY_ADD_TAGS ─────────────────────────────────


@register_dispatcher("tag_warranty_risk")
def _tag_warranty_risk_dispatch(
    params: dict[str, Any],
) -> tuple[bool, dict[str, Any]]:
    """Replay warranty's per-product warranty-risk tag.

    Enqueue carries ``{product_id, tag, claim_count,
    risk_score}``. The adapter side wants
    ``{id, tags: [tag]}`` — same shape as catalog_apply_tags
    (claim_count / risk_score are kept in approval-side
