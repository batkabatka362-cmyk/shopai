...
# ── product_validation → SHOPIFY_ADD_TAGS ───────────────────────


@register_dispatcher("tag_validation_failed")
def _tag_validation_failed_dispatch(
    params: dict[str, Any],
) -> tuple[bool, dict[str, Any]]:
    """Replay product_validation's per-product failed tag.

    Enqueue carries ``{product_id, tag, risk_level,
    recommendation}``. The adapter side wants
    ``{id, tags: [tag]}`` — same shape as catalog_apply_tags
    (risk_level / recommendation are kept in approval-side
