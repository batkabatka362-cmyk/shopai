...
# ── wishlist → SHOPIFY_ADD_TAGS ─────────────────────────────────


@register_dispatcher("tag_wishlisted_hot")
def _tag_wishlisted_hot_dispatch(
    params: dict[str, Any],
) -> tuple[bool, dict[str, Any]]:
    """Replay wishlist's per-product hot-wishlist tag write.

    Enqueue carries ``{product_id, tag, title, wishlist_count}``.
    The adapter side wants ``{id, tags: [tag]}`` — same shape
    as catalog_apply_tags (title and wishlist_count are kept
    in approval-side params for operator context but aren't
    part of the mutation). SHOPIFY_ADD_TAGS is additive —
