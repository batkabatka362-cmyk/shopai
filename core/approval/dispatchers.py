...
# ── price_elasticity → SHOPIFY_ADD_TAGS ─────────────────────────


@register_dispatcher("tag_price_elasticity")
def _tag_price_elasticity_dispatch(
    params: dict[str, Any],
) -> tuple[bool, dict[str, Any]]:
    """Replay price_elasticity's per-product elasticity tag.

    Enqueue carries ``{product_id, tag, bucket, coefficient,
    optimal_price}``. The adapter side wants
    ``{id, tags: [tag]}`` — same shape as catalog_apply_tags
    (bucket / coefficient / optimal_price are kept in
