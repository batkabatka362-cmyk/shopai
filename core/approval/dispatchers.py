...
# ── nps_engine → SHOPIFY_TAG_CUSTOMER ───────────────────────────


@register_dispatcher("tag_nps_customer")
def _tag_nps_customer_dispatch(
    params: dict[str, Any],
) -> tuple[bool, dict[str, Any]]:
    """Replay nps_engine's per-customer bucket tag write.

    Enqueue carries ``{customer_id, tag, score, bucket}``. The
    adapter side wants ``{id, tags: [tag]}`` — same shape as
    apply_segment_tag (score / bucket are kept in approval-side
