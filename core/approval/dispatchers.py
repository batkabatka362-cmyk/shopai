...
# ── profitability_calculator → SHOPIFY_ADD_TAGS ─────────────────


@register_dispatcher("tag_profitability_margin")
def _tag_profitability_margin_dispatch(
    params: dict[str, Any],
) -> tuple[bool, dict[str, Any]]:
    """Replay profitability_calculator's per-product margin tag.

    Enqueue carries ``{product_id, tag, bucket, net_margin,
    roi, revenue}``. The adapter side wants
    ``{id, tags: [tag]}`` — same shape as catalog_apply_tags
    (bucket / net_margin / roi / revenue are kept in
    approval-side params for operator context but aren't part
    of the mutation). SHOPIFY_ADD_TAGS is additive — existing
    tags preserved.
    """
