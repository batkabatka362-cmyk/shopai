...
@register_dispatcher("tag_at_risk_customer")
def _tag_at_risk_customer_dispatch(
    params: dict[str, Any],
) -> tuple[bool, dict[str, Any]]:
    """Replay churn_prediction's at-risk tag write.

    Enqueue carries ``{customer_id, tag, risk_level,
    churn_probability}``. The adapter wants
    ``{id, tags: [tag]}`` -- same translation
    ``apply_segment_tag`` does for the segmentation engine.
    The risk metadata (``risk_level``, ``churn_probability``)
    is kept in approval-side params for operator context but
    isn't part of the Shopify mutation.
    """
    customer_id = str(params.get("customer_id", "")).strip()
    tag = str(params.get("tag", "")).strip()
    if not customer_id or not tag:
