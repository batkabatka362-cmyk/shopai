...
# ── product_risk → SHOPIFY_ADD_TAGS ─────────────────────────────


@register_dispatcher("tag_product_risk")
def _tag_product_risk_dispatch(
    params: dict[str, Any],
) -> tuple[bool, dict[str, Any]]:
    """Replay product_risk's per-product risk-level tag.

    Enqueue carries ``{product_id, tag, risk_level, overall}``.
    The adapter side wants ``{id, tags: [tag]}`` — same shape
    as catalog_apply_tags (risk_level / overall are kept in
